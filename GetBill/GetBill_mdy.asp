...
BillReturnDate = Request("BillReturnDate")
sys_title=0

for i=len(trim(Request("BillStartNumber"))) to 1 step -1
...
startTail = Mid(BillStartNumber,sys_title+1,len(BillStartNumber)-len(Sno))

Select Case tag
	Case "NEW" :
	  sql = "select /*+ INDEX(getbilldetail GETBILLDETAIL_PK) */ BILLNO from getbilldetail where " & _
	        "(billno >= '" & BILLSTARTNUMBER & "' AND billno <= '" & BILLENDNUMBER & "')" 
    ...
    if not Rs.eof and isBillIn=0 then
        ...
        RsTemp.Close
	      ...
	      sql = "Insert into GetBillBase (GETBILLSN,DISPATCHMEMBERID,GETBILLDATE,GETBILLMEMBERID,BILLSTARTNUMBER,BILLENDNUMBER,COUNTERFOIRETURN,RecordDate,RecordMemberID,note,BillIn) " & _
	            "values (" & GETBILLSN & "," & Session("User_ID") & "," & funGetDate(gOutDT(GetBillDate),1) &"," & GetBillMemberID & ",'" & BILLSTARTNUMBER & "','" & BILLENDNUMBER & _
	            "'," & COUNTERFOIRETURN & ",sysdate," & Session("User_ID") & ",'" & Note & "'," & isBillIn & ")"  
       ...
       if isBillIn = 0 then    
          For i=0 To BillCount
             ...
             BillNo = Sno&(Right("00000000000" & CDbl(Tno)+(i), 9-len(Sno)))
             sqlTemp = "Insert into GetBillDetail (GetBillSN,BillNo,BillStateID) Values (" & GETBILLSN & ",'" & BillNo & "'," & 463 & ")"
	...
	Case "UPD":
	  sql = "Update GetBillBase Set GetBillDate=" & funGetDate(gOutDT(GetBillDate),1)  & ",GetBillMemberID=" & GetBillMemberID & _
	        ",BillStartNumber='" & BillStartNumber & "',BillEndNumber='" & BillEndNumber & "'" & _
	        ",COUNTERFOIRETURN=" & COUNTERFOIRETURN & ",note='" & note & "', BillReturnDate=" & funGetDate(gOutDT(BillReturnDate),1)  & " Where GETBILLSN=" & GETBILLSN
	  Conn.Execute(sql)
	  If sys_City = "台東縣" then
		  If trim(COUNTERFOIRETURN) = 1 Then

			strSQL="select '("&gInitDT(now)&"_'||(getbillseq.NextVal)||')' getbillseq from Dual"
	...
	Case "CHANGE":
		sql = "select BILLNO from getbilldetail  Where GETBILLSN=" & GETBILLSN &" and SUBSTR(BillNo,1,"&len(Sno)&")='"&Sno&"' and SUBSTR(BillNo,"&len(Sno)+1&") between '"&Tno&"' and '"&Tno2&"'"
		...
		if not Rs.eof then
			...
			"'," & COUNTERFOIRETURN & ",sysdate," & Session("User_ID") & ",'" & Note & "',0)"
			'to_date('" & GetBillDate & "','yyyy/mm/dd')
			Conn.BeginTrans
			Conn.Execute(sql)
		...
		' 	入庫的紀錄不用新增GetBillDetail 避免後續領單使用新增會重覆
			...
			if trim(BillStartNumber)=trim(tempEndNum) then
				strSQL = "delete from GetBillBase Where GETBILLSN=" & GETBILLSN
				Conn.Execute(strSQL)
			else
				strSQL = "Update GetBillBase Set BillEndNumber='" & tempEndNum & "' Where GETBILLSN=" & GETBILLSN
				Conn.Execute(strSQL)
			end if

			strSQL="delete from GetBillDetail where GETBILLSN=" & GETBILLSN & " and SUBSTR(BillNo,1,"&len(Sno)&")='"&Sno&"' and SUBSTR(BillNo,"&len(Sno)+1&") between '"&Tno&"' and '"&Tno2&"'"
			Conn.Execute(strSQL)

			For i=int(Tno) To int(Tno2)
				BillNo = Trim(Sno)&right("000000000"&trim(int(i)),len(Tno))
				sqlTemp = "Insert into GetBillDetail (GetBillSN,BillNo,BillStateID) Values (" & new_GETBILLSN & ",'" & BillNo & "'," & 463 & ")"
  ...
  Case "DEL":
    sql1 = "Delete From GetBillDetail Where GetBillSN=" & GetBillSN
    sql2 = "Delete From GetBillBase Where GetBillSN=" & GetBillSN
    Conn.BeginTrans
       Conn.Execute(sql1)
       Conn.Execute(sql2)
    if err.number = 0 then
...
%>
				<form name=GetBillAdd method=post action=GetBillAdd.asp>
			  ...
			  <Script Language=JavaScript>window.GetBillAdd.submit();</Script>	
<%
   elseif tag="UPD" then
%>
				<form name=GetBillUpdate method=post action=GetBillUpdate.asp>
				...
				<Script Language=JavaScript>window.GetBillUpdate.submit();</Script>
<%      
   elseif tag="CHANGE" then
%>
				<form name=GetBillChange method=post action=GetBillChange.asp>
					<input type=hidden name=tag value="upd">
					<input type=hidden name=GETBILLSN value="<%=request("GETBILLSN")%>">
					<input type=hidden name=CounterfoiReturn value="<%=CounterfoiReturn%>">
					<input type=hidden name=BillCount value="<%=Request("BillCount")%>">
				...
				<Script Language=JavaScript>window.GetBillChange.submit();</Script>
<%      
   end if
end if
...
IF( Session("Msg") = "") THEN
	if tag="NEW" then
		response.write "<script>alert('儲存完成!!');location='GetBillAdd.asp';</script>"
	else
