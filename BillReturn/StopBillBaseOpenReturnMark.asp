...
if trim(request("DB_Selt"))="Selt" then
	...
	for i=0 to Ubound(Sys_BillNo)
		if trim(Sys_BillNo(i))<>"" and trim(Sys_BackDate(i))<>"" then
			...
			strSQL="Update StopBillMailHistory set OpenGovDate="&funGetDate(gOutDT(request("Sys_opengovDate")),0)& ",OpenGovStationID='"&request("Sys_Station")&"',OpenGovResonID='"&trim(Sys_BackCause(i))&"',MailReturnDate="&funGetDate(gOutDT(Sys_BackDate(i)),0)&",OpenGovRecordMemberID="&Session("User_ID")&",OpenGovReCordDate="&funGetDate((Sys_Now),1)&",UserMarkMemberID="&Session("User_ID")&",UserMarkDate="&funGetDate((Sys_Now),1)&",ReturnResonID='"&trim(Sys_BackCause(i))&"',UserMarkResonID='"&trim(Sys_BackCause(i))&"',UserMarkReturnDate="&funGetDate(gOutDT(Sys_BackDate(i)),0)&",mailStation=null where BillNo='"&trim(Ucase(Sys_BillNo(i)))&"'"
			conn.execute(strSQL)
...
<table width="100%" border="0" bgcolor="#ffffff">
	<tr>
		<td height="27" bgcolor="#FFCC33" class="pagetitle"><strong>催繳單退-公示送達</strong><br>
			...
			<table width="100%" height="100%"  border="0" cellpadding="5" cellspacing="1" bgcolor="#FFFFFF">
				<tr>
					<td nowrap>
						...
						&nbsp;&nbsp;
						<b><font color="red">公&nbsp;告&nbsp;日&nbsp;期</font>&nbsp;
						<input name="Sys_opengovDate" type="text" class="btn1" size="10" maxlength="11" onkeyup="value=value.replace(/[^\d]/g,'')">
...
function insertRow(isTable){
	for(i=0;i<=29;i++){
		...
		txtArea.innerHTML =cnt_num+"<Span ID='UrlJpg'>催繳單號</span>&nbsp;<input type=text name='item' size=16 class='btn1' onkeydown='keyFunction("+cunt+");'>&nbsp;&nbsp;原因<select name='Sys_BackCause' class='btn1' onkeydown='keyBackCause("+cunt+");'><%=seltarr%></select>&nbsp;&nbsp;單退日期<input type=text name='Sys_BackDate' size=10 class='btn1' onkeyup='funkeyChk(this);' onkeydown='keyBackDate("+cunt+");'>&nbsp;&nbsp;大宗掛號碼<input type=text name='mailNumber' size=10 class='btn1' onkeydown='keyMailNumber("+cunt+");'>&nbsp;&nbsp;<%=CarName%><input type=text name='CarNo' size=10 class='btn1' readOnly><input type='Hidden' name='Sys_JpgFile' value=''>";
	}
}
...
function keyFunction(itemcnt) {
	...
	if (event.keyCode==13||event.keyCode==9||myForm.item[itemcnt-1].length>=16) {
		...
		if (myForm.item[itemcnt-1].value!=''){
			...
			runServerScript("StopchkBillBaseOpenReturnMark.asp?BillNo="+myForm.item[itemcnt-1].value);
		}
	}
}
	...
	fp="f:\\ScannerImport" 
		...
		For Each fil In fic
			if UCase(fso.GetExtensionName(fil.Name)) ="JPG" Or UCase(fso.GetExtensionName(fil.Name)) ="JPEG" then
				...
				tb_BillBase="select CarNo,BillUnitID from BillBase where ImagefilenameB='"&trim(tmpBillno)&"' and RecordStateID=0"
