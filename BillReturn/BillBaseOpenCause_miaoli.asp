...
'jafe------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		...
		If fso.FolderExists(fp) Then
			...
			If trim(Sys_BillNo(0))<>"" Then
				...
				cf_FileName = server.mappath("\traffic\BillReturn\Upaddress")&"\P"&Session("User_ID")&".bat"
			...
			'if fso.fileexists(fp)=true then 
				...
				WShShell.Run server.mappath("\traffic\BillReturn\Upaddress")&"\P"&Session("User_ID")&".bat",1,true
...
<table width="100%" border="0" bgcolor="#ffffff">
	...
	<tr>
		<td height="26" bgcolor="#FFCC33">單退紀錄列表<b>&nbsp;<%
