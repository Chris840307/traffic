...
tb_BillBase="select Sn,BillNo,CarNo,BillUnitID from BillBase where BillNo='"&trim(Ucase(request("BillNo")))&"' and RecordStateID=0"
...
tb_BillMailHistory="select BillSn,BillNo,MailReturnDate,UserMarkReturnDate from BillMailHistory where BillNo='"&trim(Ucase(request("BillNo")))&"'"


strSQL = "select d.DCIReturnStation,b.DCIStationName,b.StationID,c.UnitID,c.UnitName,c.UnitTypeID,d.CarNo,d.Owner,e.MailReturnDate,e.UserMarkReturnDate from ("&tb_BillBase&") a,Station b,UnitInfo c,("&tb_BillbaseDCIReturn&") d,("&tb_BillMailHistory&") e where a.BillNo=d.BillNo and a.CarNo=d.CarNo and d.DCIReturnStation=b.DCIStationID(+) and a.BillUnitID=c.UnitID(+) and a.BillNo=e.BillNo(+) and a.Sn=e.BillSn(+)"
...
if Not rscnt.eof then
	...
	if(err=='0'){
		...
		if(City=='彰化縣' || City=='澎湖縣' || City=='基隆市' || City=='南投縣' || City=='台中市'){
			myForm.Sys_BackDate[myForm.chkcnt.value-1].value='<%=trim(gInitDT(rscnt("UserMarkReturnDate")))%>';
		}
		if(myForm.chkcnt.value<myForm.item.length){
			if(City=='基隆市'||City=='彰化縣'||City=='高雄市'||City=='台東縣'){
				myForm.item[myForm.chkcnt.value].focus();
			}else if(City=='南投縣'){
				myForm.mailStation[myForm.chkcnt.value-1].focus();
			}else{
