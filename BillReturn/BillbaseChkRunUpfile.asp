...
for each formName in upload.file ''列出所有上傳的檔案
	...
	if TypeFlag = 1 then
		...
		strSQL="Select * From [Sheet1$] order by 標示單號碼"
		Set Rs=ConnEx.Execute(strSQL)
		sys_err="":cmt=0
		Sys_now=funGetDate(now,1)
		While not rs.eof
			Sys_BillNo="":Sys_illegalDate="":Sys_CarNo="":Sys_Rule1="":Sys_IllegalAddress=""
			Sys_IllegalAddressID="null":Sys_UnitID="":Sys_BillMemID1="":sys_chname=""

			If not ifnull(rs("車牌號碼"))>0 Then Sys_CarNo=trim(rs("車牌號碼"))

			strSQL="select count(1) cmt from (select chname,unitid from memberdata where Loginid='"&trim(rs("舉發員警代碼"))&"' and AccountStateID=0 and RecordStateID=0) a,(select UnitID,UnitName from UnitInfo) b where a.unitid=b.unitid"

			set rscnt=conn.execute(strSQL)

			If cdbl(rscnt("cmt"))=1 Then
				
				strSQL="select a.memberid,a.chname,b.UnitID,b.UnitName from (select chname,memberid,unitid from memberdata where Loginid='"&trim(rs("舉發員警代碼"))&"' and AccountStateID=0 and RecordStateID=0) a,(select UnitID,UnitName from UnitInfo) b where a.unitid=b.unitid"
			...
			If trim(Sys_CarNo)<>"" and trim(sys_BillMemID1)<>"" Then

				strSQL="select streetid from street where address like '%"&trim(rs("違規地點"))&"%' order by streetID"
				set rsstret=conn.execute(strSQL)

				If not rsstret.eof Then
					Sys_IllegalAddressID="'"&trim(rsstret("streetid"))&"'"
				else
					Sys_IllegalAddressID="null"
				End if

				rsstret.close
				

				strSQL="select count(1) cmt from BillRunCarAccept where BillNo='"&trim(rs("標示單號碼"))&"' and recordstateid=0"
		
				set rsnt=conn.execute(strSQL)

				If cdbl(rsnt("cmt"))=0 Then
				
					strSQL="insert into BillRunCarAccept(Billno,Carno,IllegalDate,Rule1,IllegalAddressID,IllegalAddress,BillUnitID,BillMemID1,Acceptdate,RecordStateID,RecordDate) values('"&trim(rs("標示單號碼"))&"','"&trim(rs("車牌號碼"))&"',"&funGetDate(gOutDT(rs("違規日期")),1)&",'"&trim(rs("違反條款代碼"))&"',"&Sys_IllegalAddressID&",'"&trim(rs("違規地點"))&"','"&trim(Sys_UnitID)&"','"&trim(Sys_BillMemID1)&"',"&funGetDate(date,0)&",0,"&Sys_now&")"

					conn.execute(strSQL)
				
				else

					strSQL="Update BillRunCarAccept set CarNo='"&trim(rs("車牌號碼"))&"',BillUnitID='"&Sys_UnitID&"',IllegalDate="&funGetDate(gOutDT(rs("違規日期")),1)&",AcceptDate="&funGetDate(date,0)&",Rule1='"&trim(rs("違反條款代碼"))&"',IllegalAddressID="&Sys_IllegalAddressID&",IllegalAddress='"&trim(rs("違規地點"))&"',BILLMEMID1="&trim(Sys_BillMemID1)&",RECORDDATE="&Sys_now&" where billno='"&trim(rs("標示單號碼"))&"' and recordstateid=0"
...
'				Response.write "</script>"
'				cmt=cmt+1
			elseif not ifnull(rs("標示單號碼")) then
