...
if trim(request("DB_Selt"))="Selt" then
	...
	For i = 0 to Ubound(Sys_CarNo)
		If not ifnull(Sys_CarNo(i)) Then
			...
			If not ifnull(RecordDate1(i)) Then
				...
				strSQL="Update BillRunCarAccept set BatchNumber='"&Sys_BatChNumber&"',BillNo='"&trim(Sys_BillNo(i))&"',CARNO='"&trim(Sys_CarNo(i))&"',CARSIMPLEID="&ChkNum(Sys_CarSimpleID(i))&",Note='"&trim(Sys_Note(i))&"',BackDate=null,RecordStateID=0,AcceptDate="&funGetDate(gOutDT(Sys_AcceptDate(i)),0)&" where RecordDate1=to_date('"&sysdate&"','YYYY/MM/DD/HH24/MI/SS') and BatchNumber='"&Sys_Old_BatchNumber&"'"
			...
			If not ifnull(Sys_chkBackBillBase(i)) Then
				strSQL="Update BillRunCarAccept set RecordStateID=-1,BackDate="&funGetDate(gOutDT(Sys_BackDate(i)),0)&" where RecordDate1=to_date('"&sysdate&"','YYYY/MM/DD/HH24/MI/SS') and BatchNumber='"&Sys_BatChNumber&"'"
			...
			If not ifnull(Sys_BillBaseDel(i)) Then
				strSQL="delete BillRunCarAccept where RecordDate1=to_date('"&sysdate&"','YYYY/MM/DD/HH24/MI/SS') and BatchNumber='"&Sys_BatChNumber&"'"
	...
	If ifnull(RecordDate1(0)) Then Response.Write "location='BillBaseRunCheckAccept_TaiChungCity.asp';"
	Response.write "</script>"
end if
%>
						...
						<table border="0">
							<tr>
								<td>
									...
									<input type="button" name="cancel" value="清除" onClick="location='BillBaseRunCheckAccept_TaiChungCity.asp'">
										...
										<tr>
											<td colspan="5">
												...
												While not rs.eof
													...
													Response.Write "<input type=""button"" name=""btnAcc"" class=""btn3"" style=""width:50px;height:25px;font-size:16px;"" value=""詳細"" onclick=""funAcceptLoad('"&rs("BatchNumber")&"');"">&nbsp;"

													Response.Write "<input type=""button"" name=""btnDec"" class=""btn3"" style=""width:50px;height:25px;font-size:16px;"" value=""清冊"" onclick=""funPrintDec('"&rs("BatchNumber")&"');"""
								...
								<br>
								<input class="btn1" type="checkbox" name="chk_CarSimpleID" value="1">
								略過車種檢查
...
function insertRow(isTable){
	for(i=0;i<=49;i++){
		...
		"車號&nbsp;&nbsp;<input type=text name='CarNo' style='ime-mode:disabled' size=8 class='btn1' onkeyup='UpperCase(this);' onkeydown='keyCarNo("+cunt+");'>" +		
		"&nbsp;&nbsp;" +		
		"<span style='color:#ff0000;'>*</span>" +
...
function keyCarNo(itemcnt){
	if (event.keyCode==13||event.keyCode==9){
		...
		}else if(myForm.chk_CarSimpleID.checked==false){
...
function funSelt(){
	...
	for(i=0;i<myForm.CarNo.length;i++){
		if(myForm.CarNo[i].value!=''){
			...
			if(myForm.chk_CarSimpleID.checked==false&&myForm.CarSimpleID[i].value==''){
				err=1;
...
function funBackList(){
	...
	if((upForm.Sys_BackDate1.value!=''&&upForm.Sys_BackDate2.value!='')||(upForm.Sys_AcceptDate1.value!=''&&upForm.Sys_AcceptDate2.value!='')||myForm.Search_BatChNumber.value!=''||upForm.Search_CarNo.value!=''||upForm.Search_Unit.value!=''){
