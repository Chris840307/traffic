...
'jafe------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		...
		If fso.FolderExists(fp) Then
			...
			If trim(Sys_BillNo(0))<>"" Then
				...
				cf_FileName = server.mappath("\traffic\BillReturn\Upaddress")&"\T"&Session("User_ID")&".bat"
			...
			set fic=fod.Files
				...
				WShShell.Run server.mappath("\traffic\BillReturn\Upaddress")&"\T"&Session("User_ID")&".bat",1,true
...
<table width="100%" border="0" bgcolor="#ffffff">
	...
	<tr>
		<td height="26" bgcolor="#FFCC33">寄存送達註記列表<img src="space.gif" width="12" height="8"> <b>期滿日,掛號碼,郵局 為 非必填 欄位</b>. 其餘為必填欄位<br><%=titleStr%>
			 <br><%
...
function runChk(itemcnt) {
	if (myForm.item[itemcnt-1].value!=''){
		return runServerScript("chkReturnMark_two.asp?BillNo="+myForm.item[itemcnt-1].value+"&itemcnt="+itemcnt);
	}
