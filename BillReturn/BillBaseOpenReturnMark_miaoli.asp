...
'jafe----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		...
		If fso.FolderExists(fp) Then
			...
			If trim(Sys_BillNo(0))<>"" Then
				...
				cf_FileName = server.mappath("\traffic\BillReturn\Upaddress")&"\B"&Session("User_ID")&".bat"
			...
			'if fso.fileexists(fp)=true then 
				...
				WShShell.Run server.mappath("\traffic\BillReturn\Upaddress")&"\B"&Session("User_ID")&".bat",1,true
				...
				<tr>
					<td nowrap>
						...
						if sys_City="苗栗縣" then
							...
							Response.Write "<input type=""button"" name=""btnOK"" value=""匯入地址資料"" onclick=""funAddressSelt();"">"
	...
	<tr>
		<td height="26" bgcolor="#FFCC33">單退紀錄列表 <b> <%=titleStr%>&nbsp;<%
				...
				if sys_City="苗栗縣" then
					...
					Response.Write "<input type=""button"" name=""btnOK"" value=""文字檔匯入"" onclick=""funExportTxt();"">"
				end if
...
function insertRow(isTable){
	...
	for(i=0;i<=cnt;i++){
		var Rindex = isTable.rows.length;
...
function funExportTxt(){
	newWin("","ExportTxt",700,550,50,10,"yes","yes","yes","no");
	UrlStr="BillBaseOpenReturnMark_miaoli_BillNo_SendStyle.asp";
	myForm.action=UrlStr;
	myForm.target="ExportTxt";
	myForm.submit();
	myForm.action="";
	myForm.target="";
}
...
function runChk(itemcnt) {
	if (myForm.item[itemcnt-1].value!=''){
		return runServerScript("chkBillBaseOpenReturnMark.asp?BillNo="+myForm.item[itemcnt-1].value+"&itemcnt="+itemcnt);
	}
