...
If sys_City<>"台中縣" Then
	...
	if trim(Sys_BillTypeID)="1" then
		DelphiASPObj.GenBillPrintBarCode PBillSN(i+PrintSum),Sys_BillNo,Sys_Rule1,Sys_CarNo,Sys_MailNumber,"220073",Sys_DriverHomeZip,Sys_DealLineDate(0)&Sys_DealLineDate(1)&Sys_DealLineDate(2),Sys_StationID,"台北市交通事件裁決所",0,Sum_Level,0,True,False,date
		'response.write "DelphiASPObj.GenBillPrintBarCode"& PBillSN(i+PrintSum)&","&Sys_BillNo&","&Sys_Rule1&","&Sys_CarNo&","&Sys_MailNumber&",220073,"&Sys_DriverHomeZip&","&Sys_DealLineDate(0)&Sys_DealLineDate(1)&Sys_DealLineDate(2)&","&Sys_StationID&",台北市交通事件裁決所,0,"&Sum_Level&",0,True,False,"&Sys_MailDate
		'response.end
	else
		DelphiASPObj.GenBillPrintBarCode PBillSN(i+PrintSum),Sys_BillNo,Sys_Rule1,Sys_CarNo,Sys_MailNumber,"220073",Sys_OwnerZip,Sys_DealLineDate(0)&Sys_DealLineDate(1)&Sys_DealLineDate(2),Sys_StationID,"台北市交通事件裁決所",0,Sum_Level,0,True,False,date
		'response.write "DelphiASPObj.GenBillPrintBarCode "& PBillSN(i+PrintSum)&","&Sys_BillNo&","&Sys_Rule1&","&Sys_CarNo&","&Sys_MailNumber&",220073,"&Sys_OwnerZip&","&Sys_DealLineDate(0)&Sys_DealLineDate(1)&Sys_DealLineDate(2)&","&Sys_StationID&",台北市交通事件裁決所,0,"&Sum_Level&",0,True,False,"&Sys_MailDate
		'response.end
	end if
end if
...
<table width="650" border="0" cellspacing=0 cellpadding=0>
	<tr><th rowspan=2 valign="bottom" align="center" width="80%">
			...
			<strong><span class="style4">　　<%=thenPasserCity&replace(sysunit,trim(thenPasserCity),"")%>送達證書</span></strong>
		</th>
		<th valign="bottom" align="left" class="style1" nowrap><%
			If sys_City="台南縣" then
				If trim(Sys_LoginID)="A139" or trim(Sys_LoginID)="A141" or trim(Sys_LoginID)="A007" Then
			...
			elseif sys_City="基隆市" then
				response.Write "郵寄日期："&right("00"&Sys_StoreAndSendFinalMailDate(0),3)&Sys_StoreAndSendFinalMailDate(1)&Sys_StoreAndSendFinalMailDate(2)
			else
...
<table width="645" border="1" cellspacing=0 cellpadding=0>
  <tr>
	...
	if Sys_BillTypeID="1" and trim(Sys_DriverHomeAddress)<>"" then
		response.write Sys_Driver&"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;車號："&Sys_CarNo
		response.write "<br>"&Sys_DriverHomeZip&"&nbsp;&nbsp;"&Sys_DriverZipName&Sys_DriverHomeAddress
	else
		response.write Sys_Owner&"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;車號："&Sys_CarNo
		response.write "<br>"&Sys_OwnerZip&"&nbsp;&nbsp;"&Sys_OwnerZipName&Sys_OwnerAddress
	end if%>&nbsp;</span></td>
  </tr>
  <tr>
    <td colspan="2" align="center"><span class="style2">文　　　　　　　　　號</span></td>
    <td colspan="3" nowrap><span class="style2"><%=theBillNumber%>交字第<img src=<%="""..\BarCodeImage\"&Sys_BillNo_BarCode&".jpg"""%>>號</span></td>
  </tr>
  <tr>
...
<span class="style1"><strong>※請繳回<%=thenPasserCity&Sys_UnitName%>　應到案處所：<%=Sys_STATIONNAME%> 　操作人員：<%=Sys_BillFillerMemberID%><br>本證書送回地址：<%=Sys_UnitAddress%></strong>
<%
if sys_City<>"台南縣" then
