...
<%
	...
	strSQL="select a.*,b.UnitName,c.chname,c.LoginID from (select * from BillStopCarAccept where billunitid='"&trim(Request("UnitID"))&"' and AcceptDate="&funGetDate(gOutDT(Request("AcceptDate")),0)&"  and recorddate="&funGetDate(Request("RecordDate"),1)&" and recordstateid=0) a,UnitInfo b,memberdata c where a.BillUnitID=b.UnitID and a.BillMemID1=c.MemberID order by a.billno"
	...
	While not rsmem.eof
		...
		Response.Write "myForm.DriverName["&cmt&"].value='"&trim(rsmem("Driver"))&"';"

		Response.Write "myForm.Fastener1["&cmt&"].value='"&trim(rsmem("FastenerTypeID1"))&"';"
