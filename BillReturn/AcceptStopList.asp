...
<%
	...
	BasSQL="select a.*,b.UnitName,c.chname BillUnitName,c.LoginID,decode(a.RecordStateID,'0','正常','退件') RecordType,d.ChName AcceptName1,e.Chname AcceptName2,f.content FastenerTypeName from (select * from BillStopCarAccept where billunitid='"&trim(Request("DB_BillUnitID"))&"' and AcceptDate="&funGetDate(gOutDT(Request("DB_AcceptDate")),0)&"  and recorddate="&funGetDate(Request("DB_RecordDate"),1)&") a,UnitInfo b,memberdata c,memberdata d,memberdata e,(select ID,Content from DCICode where typeid=6) f where a.BillUnitID=b.UnitID and a.BillMemID1=c.MemberID(+) and a.RecordMemberID1=d.MemberID(+) and a.RecordMemberID2=e.MemberID(+) and a.FastenerTypeID1=f.ID(+) order by a.RecordDate"
...
<table border="0" cellpadding="0" cellspacing="0">
	...
	<table width="640" border="1" cellspacing="0" cellpadding="0">
		...
		for i=1 to 50
			...
			Response.Write "<td class=""style2"" nowrap>"
			Response.Write trim(rsfound("FastenerTypeName"))
			Response.Write "&nbsp;</td>"
