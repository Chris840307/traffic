...
if trim(request("DB_Selt"))="Selt" then
	...
	Sys_BackDate=Split(request("StoreAndSendSendDate"),",")
	...
	for i=0 to Ubound(Sys_BillNo)
		if trim(Sys_BillNo(i))<>"" then
			...
			if trim(Sys_ChkSend(i))="是" then
				...
				strSQL="Update BillMailHistory set StoreAndSendSendDate="&funGetDate(gOutDT(Sys_BackDate(i)),0)&" where BillNo='"&trim(Ucase(Sys_BillNo(i)))&"'"
				conn.execute(strSQL)
...
<table width="100%" border="0" bgcolor="#ffffff">
	<tr>
		<td height="27" bgcolor="#FFCC33" class="pagetitle"><strong>公示送達證書 戶籍地址補正 </strong>
		<img src="../Image/dot.gif"></img>
		<a href="../storechkAddrss.doc" target="_blank" >單退二次郵寄簡要說明.doc</a>
		<br><bR>
		透過此功能可修正公示送達郵記之正確地址。 自動抓取監理站民眾領牌登錄之戶籍地址部份則不適用此功能<br>
	...
	if (event.keyCode==13||event.keyCode==9) {
		if (chkBillNo(itemcnt)){
			if (myForm.item[itemcnt-1].value!=''){
				...
				runServerScript("chkOpenAndSendChkAdder.asp?BillNo="+myForm.item[itemcnt-1].value);
			}
		}else{
			alert("單號長度必須為9碼!!");
		}
...
function funDriverZip(itemcnt) {
	...
	if(myForm.OwnerZip[itemcnt-1].value.length<4){
		if (event.keyCode==13||event.keyCode==9) {
			...
			myForm.chkcnt.value=itemcnt;
			runServerScript("chkOpenAndSendZipChkDriverAddr.asp?DriverZip="+myForm.DriverZip[itemcnt-1].value);
		}
	}else{
		alert('郵地區號錯誤!!');
	}
...
function funOwnerZip(itemcnt) {
	...
	if(myForm.OwnerZip[itemcnt-1].value.length<4){
		if (event.keyCode==13||event.keyCode==9) {
			...
			myForm.chkcnt.value=itemcnt;
			runServerScript("chkOpenAndSendZipChkOwnerAddr.asp?OwnerZip="+myForm.OwnerZip[itemcnt-1].value);
		}
	}else{
		alert('郵地區號錯誤!!');
	}
...
function funDriverAddress(itemcnt) {
	if (event.keyCode==13||event.keyCode==9) {
		myForm.chkcnt.value=itemcnt;
		runServerScript("chkOpenAndSendZipChkDriverAddr.asp?ZipName="+myForm.DriverAddress[itemcnt-1].value);
	}
}
...
function funOwnerAddress(itemcnt) {
	if (event.keyCode==13||event.keyCode==9) {
		myForm.chkcnt.value=itemcnt;
		runServerScript("chkOpenAndSendZipChkOwnerAddr.asp?ZipName="+myForm.OwnerAddress[itemcnt-1].value);
	}
}
