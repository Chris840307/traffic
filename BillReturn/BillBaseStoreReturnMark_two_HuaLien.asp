		...
		'if fso.fileexists(fp)=true then 
			for i=0 to Ubound(Sys_JpgFile)
				...
				'	response.write Sys_JpgFile(i)
					FileDirAndName="/img/scan/寄存/" & fDir & Sys_BillNo(i) & "_" & replace(Sys_JpgFile(i),Sys_BillNo(i) & "_","")
...
<table width="100%" border="0" bgcolor="#ffffff">
	<tr>
		<td height="27" bgcolor="#FFCC33" class="pagetitle"><strong>寄存送達註記</strong><br>
	...
	<tr>
		<td height="26" bgcolor="#FFCC33">寄存送達註記列表&nbsp;<%
		...
		<img src="space.gif" width="12" height="8"> 
		<b>期滿日,掛號碼,郵局 為 非必填 欄位</b>. 其餘為必填欄位
		<br><%=titleStr%></td>
	</tr>
	<tr>
...
function insertRow(isTable){
	...
	for(i=0;i<=cnt;i++){
		...
		txtArea.innerHTML =cnt_num+".&nbsp;<Span ID='UrlJpg'>單號</span>&nbsp;<input type=text name='item' size=6 class='btn1' onkeydown='keyFunction("+cunt+");'>&nbsp;&nbsp;<%=Sys_BackCause%>&nbsp;&nbsp;單退日<input type=text name='Sys_BackDate' size=3 class='btn1' onkeyup='chknumber(this);' onkeydown='keyBackDate("+cunt+");' maxlength='7'>&nbsp;&nbsp;送達日<input type=text name='EffectDate' size=3 class='btn1' onkeyup='chknumber(this);' onkeydown='keyEffectDate("+cunt+");' maxlength='7'>&nbsp;&nbsp;期滿日<input type=text name='MailDate' size=3 class='btn1' onkeydown='keyMailDate("+cunt+");' maxlength='7'>&nbsp;&nbsp;掛號碼<input type=text name='mailNumber' size=3 class='btn1' onkeydown='keyMailNumber("+cunt+");' maxlength='20'>&nbsp;&nbsp;<%=CarName%><input type=text name='CarNo' size=4 class='btn1' readOnly>&nbsp;&nbsp;郵局<input type=text name='mailStation' size=4 class='btn1' maxlength='45'><input type='Hidden' name='Sys_JpgFile' value=''>";
	}
}
...
'固定位置
    ...
    For Each fil In fic
		if UCase(fso.GetExtensionName(fil.Name)) ="JPG" Or UCase(fso.GetExtensionName(fil.Name)) ="JPEG" then
			...
			tb_BillBase="select Sn,BillNo,CarNo,BillUnitID from BillBase where BillNo='"&trim(Ucase(Sys_tmpBillNo(0)))&"' and RecordStateID=0"

			tb_BillbaseDCIReturn="select BillNo,CarNo,Owner,DCIReturnStation from BillbaseDCIReturn where ExChangeTypeID='W' and BillNo='"&trim(Ucase(Sys_tmpBillNo(0)))&"'"

			tb_BillMailHistory="select BillSn,BillNo,MailReturnDate,UserMarkReturnDate from BillMailHistory where BillNo='"&trim(Ucase(Sys_tmpBillNo(0)))&"'"


			strSQL = "select d.DCIReturnStation,b.DCIStationName,b.StationID,c.UnitID,c.UnitName,d.CarNo,d.Owner,e.MailReturnDate,e.UserMarkReturnDate from ("&tb_BillBase&") a,Station b,UnitInfo c,("&tb_BillbaseDCIReturn&") d,("&tb_BillMailHistory&") e where a.BillNo=d.BillNo and a.CarNo=d.CarNo and d.DCIReturnStation=b.DCIStationID(+) and a.BillUnitID=c.UnitID(+) and a.BillNo=e.BillNo(+) and a.Sn=e.BillSn(+)"
