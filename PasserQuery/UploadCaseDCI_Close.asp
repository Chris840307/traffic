...
if request("CaseCloseDate")<>"" and request("CaseCloseDate1")<>""then
	CaseCloseDate1=gOutDT(request("CaseCloseDate"))&" 0:0:0"
	CaseCloseDate2=gOutDT(request("CaseCloseDate1"))&" 23:59:59"

	strwhere=strwhere&" and sn in(select billsn from passerpay where CaseCloseDate between TO_DATE('"&CaseCloseDate1&"','YYYY/MM/DD/HH24/MI/SS') and TO_DATE('"&CaseCloseDate2&"','YYYY/MM/DD/HH24/MI/SS'))"
...
if request("Sys_RecordUnit")<>"" then
	...
	strwhere=strwhere&" and a.sn in(select billsn from passerpay where recordmemberid in (select MemberID from MemberData where UnitID in ('"&trim(request("Sys_RecordUnit"))&"')))"
end If 

if request("Sys_RecordMemberID")<>"" then

	strwhere=strwhere&" and a.sn in(select billsn from passerpay where recordmemberid="&request("Sys_RecordMemberID")&")"
end If 
...
if trim(request("kinds"))="BillToDCILog" then
	...
	strSQL="select count(1) cnt " & _
		"from PasserBase a where CARSIMPLEID=8 and billstatus = 9 " & _
		"and (select count(1) cnt from PasserDCILog where ReturnMarkType=9 and sn=a.dcilogsn)=0 " & _
	"and (select count(1) cnt from PasserDCILog where ExchangetypeID='W' and dcireturnstatusid in(select dcireturn from dcireturnstatus where dciactionid='W' and dcireturnstatus=1) and sn=a.dcilogsn) > 0 " & _
	...
	If chkcnt > 0 Then
		...
		if not rsSN.eof then
			theBatchTime=(year(now)-1911)&"B"&trim(rsSN("SN"))
		end if
		rsSN.close
		set rsSN=nothing	
	...
	"and (select count(1) cnt from PasserDCILog where ExchangetypeID='W' and dcireturnstatusid in(select dcireturn from dcireturnstatus where dciactionid='W' and dcireturnstatus=1) and sn=a.dcilogsn) > 0 " & _
		...
		While not rs.eof
			...
			strInsCaseIn="insert into PASSERDCILOG(" & _
				...
				",sysdate,"&Session("User_ID")&",sysdate,9,'B','"&theBatchTime&"'" &_
				",(" &_
			...
			strInsCaseIn="insert into PasserBaseDciReturn(" & _
				...
				"values(" & DCISN & ","&rs("sn")&",'"&rs("billno")&"','"&rs("carno")&"','B')" 
		...
		Response.Write "alert(""上傳監理所結案資料，批號："&theBatchTime&""");"
		Response.write "</script>"
...
<table width="100%" border="0">
		...
		<td bgcolor="#CCCCCC">
			<table width="100%" border="0" cellpadding="5" cellspacing="1" bgcolor="#FFFFFF">
				<tr>
					<td>
						結案日期
						<input name="CaseCloseDate" type="text" value="<%
							if isempty(request("DB_Selt")) Then
								...
								RecordDateTmp=trim(request("CaseCloseDate"))
							end if
						...
						~
						<input name="CaseCloseDate1" type="text" value="<%
							if trim(request("DB_Selt"))="" then
								RecordDate1Tmp=ginitdt(now)
							else
								RecordDate1Tmp=trim(request("CaseCloseDate1"))
							end if
							response.write RecordDate1Tmp
						%>" size="8" maxlength="7" class="btn1" onKeyup="chknumber(this);">
						<input type="button" name="datestr" value="..." onclick="OpenWindow('CaseCloseDate1');">
						 &nbsp;　&nbsp;
						...
						處理繳款人員
						<input type="text" name="chekChMemID" size="2" onkeyup="ListItem('Sys_RecordUnit','Sys_RecordMemberID');" class="btn1">
						...
						<input type="button" name="cancel" value="清除" onClick="location='UploadCaseDCI_Close.asp';"> 
	...
	<tr>
		<td height="35" bgcolor="#FFDD77" align="center">
			...
			<input type="button" name="Submit4242" value="結案上傳" onclick="BillToDCILog();">
...
<script language="javascript">
	...
	function funSelt(){
		...
		if(myForm.CaseCloseDate.value==""||myForm.CaseCloseDate1.value==""){
			error=error+1;
		...
		if(myForm.CaseCloseDate.value!=""){
			if(!dateCheck(myForm.CaseCloseDate.value)){
				error=error+1;
				errorString=errorString+"\n"+error+"：建檔日期輸入不正確!!";
			}else if( myForm.CaseCloseDate.value.substr(0,1)=="9" && myForm.CaseCloseDate.value.length==7 ){
				error=error+1;
				errorString=errorString+"\n"+error+"：建檔日期輸入不正確!!";
			}else if( myForm.CaseCloseDate.value.substr(0,1)=="1" && myForm.CaseCloseDate.value.length==6 ){
				error=error+1;
				errorString=errorString+"\n"+error+"：建檔日期輸入不正確!!";
			}
		}

		if(myForm.CaseCloseDate1.value!=""){
			if(!dateCheck(myForm.CaseCloseDate1.value)){
				error=error+1;
				errorString=errorString+"\n"+error+"：建檔日期輸入不正確!!";
			}else if( myForm.CaseCloseDate1.value.substr(0,1)=="9" && myForm.CaseCloseDate1.value.length==7 ){
				error=error+1;
				errorString=errorString+"\n"+error+"：建檔日期輸入不正確!!";
			}else if( myForm.CaseCloseDate1.value.substr(0,1)=="1" && myForm.CaseCloseDate1.value.length==6 ){
				error=error+1;
				errorString=errorString+"\n"+error+"：建檔日期輸入不正確!!";
			}
		}
	...
	function BillToDCILog(){
		...
		if (myForm.DB_Selt.value==""){
			...
			if (myForm.Sys_RecordMemberID.value==""){
				if(confirm('您選擇將所有建檔人的舉發單結案資料，是否確定要上傳？')){
					myForm.kinds.value="BillToDCILog";
					myForm.submit();
				}
			}else{
				if(confirm('確定要將結案資料上傳到監理所？')){
					myForm.kinds.value="BillToDCILog";
					myForm.submit();
				}
			}
