...
.style15 {font-family:"標楷體"; font-size: 9px;}
-->
</style>
</head>
...
Sys_RuleVer="":Sys_MailDate="":tmp_MailDate=""
...
if Not rs.eof then 
	...
	tmp_MailDate=split(gArrDT(trim(rs("BillFillDate"))),"-")
...
Sys_MailNumber=0:Sys_MAILCHKNUMBER=0:chk_mailnumber=0
...
if Not rsSend.eof then
	...
	Sys_MAILCHKNUMBER=Sys_MailNumber&" 700 013 17"
	chk_mailnumber=Sys_MailNumber&"70001317"
end If 
rsSend.close

DelphiASPObj.GenBillPrintBarCode PBillSN(i),Sys_BillNo,Sys_Rule1,Sys_CarNo,Sys_MailNumber,"220073",Sys_OwnerZip,right(Sys_DealLineDate(0),2)&Sys_DealLineDate(1)&Sys_DealLineDate(2),Sys_StationID,"台北市交通事件裁決所",0,Sum_Level,0,True,False,Sys_MailDate,700,013,17


DelphiASPObj.GenSendStoreBillno chk_mailnumber,128,60,160,0
rsbil.close
...
<div id="Layer44" class="style2" style="position:absolute; left:160px; top:0px; height:12px; z-index:36"><B><%="臺南市政府警察局"&SysUnit&"送達證書　　　　請繳回："&SysAddress%></B></div>

<div id="Layer01" class="style3" style="position:absolute; left:130px; top:15px; z-index:3"><%
...
<div id="Layer4" class="style5" style="position:absolute; width:200px; left:450px; top:15px; z-index:1"><%
	Response.Write "<table border=""1"" cellspacing=0 cellpadding=0>"
	Response.Write "<tr><td height=""30"" align=""center"">"
	Response.Write "台南十三支局"
	Response.Write "<br>&nbsp;" & tmp_MailDate(0)&tmp_MailDate(1)&tmp_MailDate(2) & "&nbsp;</td></tr>"
...
<div id="Layer04" class="style3" style="position:absolute; left:130px; top:280px; z-index:1"><b><%
	response.write funcCheckFont(Sys_Owner,16,1)
	
	%>　台啟</b>
</div>

<div id="Layer06" class="style3" style="position:absolute; left:110px; top:300px; width:370px; z-index:5"><b><%
	'--------------------------------------如果是抓 戶籍補正的資料-----------------------------------------------------------------------------------------------------------
...
<div id="Layer45" class="style3" style="position:absolute; left:130px; top:320px; height:12px; z-index:3"><b><%="臺南市政府警察局"&SysUnit%></b></div>

<div id="Layer05" class="style3" style="position:absolute; left:450px; top:282px; z-index:3">
	<%If Sys_MailNumber <> 0 Then%>
		　<b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;第<%=Sys_MailNumber%>號</b><br>
		　<img src=<%="""../BarCodeImage/"&chk_mailnumber&".jpg"""%>><br>
	    　<b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<%=Sys_MAILCHKNUMBER%></b>	
	<%End if %>
</div>

<div id="Layer05" class="style5" style="position:absolute; left:635px; top:358px; z-index:3"><%
	response.write "臺南郵局許可證<br>"
	Response.Write "臺南字第&nbsp;260號"
    %>
</div>

<div id="Layer05" class="style3" style="position:absolute; left:235px; top:345px; z-index:2"><%
	Response.Write "<img src=""../BarCodeImage/"&Sys_BillNo&"_4.jpg"">"%>
</div>
...
if trim(Sys_ImageFileNameA)<>"" then
	...
	Response.Write "<div id=""Layer09"" style=""position:absolute; left:35px; top:475px; z-index:5"">"
...
if trim(Sys_ImageFileNameB)<>"" then
	...
	Response.Write "<div id=""Layer10"" style=""position:absolute; left:425px; top:475px; z-index:1"">"
...
<div id="Layer4" style="position:absolute; left:175px; top:823px; width:202px; height:36px; z-index:5">v</div>
...
<div id="Layer12" style="position:absolute; left:115px; top:915px; width:250px; height:11px; z-index:6"><span class="style7">逕行舉發　</span></div>

<div id="Layer13" style="position:absolute; left:260px; top:910px; width:28px; height:11px; z-index:3"><%'=Sys_Sex%></div>
<div id="Layer14" class="style15" style="position:absolute; left:355px; top:915px; width:400px; height:10px; z-index:4"><%
	'if showBarCode then response.write "*本單可至郵局或委託代收之超商繳納" left:360px; top:910px;	
	Response.Write "＊受通知人收受通知單時應到案日期不足30日或已逾應到案日期者，得於送達生效日後30日內到案。"
%></div>

<div id="Layer15" style="position:absolute; left:260px; top:920px; width:100px; height:10px; z-index:8"><font size=2><%'if trim(Sys_DriverBirth(0))<>"" then response.write Sys_DriverBirth(0)&"年"&right("0"&Sys_DriverBirth(1),2)&"月"&right("0"&Sys_DriverBirth(2),2)&"日"%></font></div>
...
<div id="Layer22" style="position:absolute; left:120px; top:1005px; width:300px; height:13px; z-index:15"><%
...
<div id="Layer27" style="position:absolute; left:395px; top:1005px; width:350px; height:31px; z-index:20"><span class="style3"><%
...
<div id="Layer29" style="position:absolute; left:125px; top:1060px; width:300px; height:11px; z-index:22"><%
	Response.Write Sys_DealLineDate(0)&"年"&Sys_DealLineDate(1)&"月"&Sys_DealLineDate(2)&"日"
%></div>
<div id="Layer32" class="style5" style="position:absolute; left:410px; top:1060px; width:400px; height:49px; z-index:29"><%
	chkRule1=0
	If left(trim(Sys_Rule1),2) = 35 Then
	...
	chkRule2=0
	If left(trim(Sys_Rule2),2) = 35 Then
	...
	if len(trim(Sys_Rule1))>chkRule1 then response.write "之"&right(trim(Sys_Rule1),1)
	'response.write ""
	...
	if trim(Sys_Rule2)<>"0" and not isnull(Sys_Rule2) then
		...
		if len(trim(Sys_Rule2))>chkRule2 then response.write "之"&right(trim(Sys_Rule2),1)
...
<div id="Layer33" style="position:absolute; left:420px; top:1115px; width:200px; height:80px; z-index:28"><span class="style7"><%
...
<div id="Layer35" class="style1" style="position:absolute; left:400px; top:1175px; width:130px; height:49px; z-index:29"><%

	'If chk_UnitTypeID = "7000" or chk_UnitTypeID = "7100" or chk_UnitTypeID = "7200" or chk_UnitTypeID = "7300" or chk_UnitTypeID = "J01" Then
...
%></div>
<div id="Layer36" class="style14" style="position:absolute; left:600px; top:1180px; width:140px; height:43px; z-index:30"><%
	'If chk_UnitTypeID = "7000" or chk_UnitTypeID = "7100" or chk_UnitTypeID = "7200" or chk_UnitTypeID = "7300" or chk_UnitTypeID = "J01" Then
		response.write "<table style=""border-color:#ff0000;border-style:solid;"" border=""1"" cellspacing=0 cellpadding=0>"
		response.write "<tr><td style=""border-color:#ff0000;border-style:solid;"" width=100 height=25 align=""center"">"&Sys_BillFillerJobName&"&nbsp;"&Sys_ChName&"</td></tr>"
		response.write "</table>"
	'End if 
%></div>
