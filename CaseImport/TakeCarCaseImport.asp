...
<script language="javascript">
  function InsertData()
  {
   if (myForm.T1.value!="")
   {
    myForm.action="TakeCarCaseImport.asp?ImportFileName="+myForm.T1.value+"&DB_Selt=DB_Insert";
    myForm.submit();
   }
   else
   {
  ...
  function BrowsFile()
  {
    myForm.action="TakeCarCaseImport.asp?ImportFileName="+myForm.attach1.value;
    //myForm.T1.value=myForm.attach1.value;
	//myForm.action="TakeCarCaseImport.asp";
    myForm.submit();
  }
  function Clear()
  {
...
memName=Session("Ch_Name")
      ...
      '讀ini檔
			  ...
			  '抓出停管業務員警名字
            	...
            	'因為花蓮拖釣匯出格式跟台中市的格式相同. 所以欲設該數值為台中市
              tCity="台中市"
...
<%
                ...
                %>
                  <script language="javascript">
	    		    if (confirm("該檔案已匯入過\n\n匯入日期:<%=ImportDate%>\n匯入檔名:<%=FileName%>\n匯入人員:<%=ImportMem%>\n\n  是否繼續匯入"))
	    		    {
                      myForm.action="TakeCarCaseImport.asp?ImportFileName="+myForm.T1.value+"&DB_Selt=DB_StillInsert";
                      myForm.submit();
	    		    }
	    		  </script>
                <%
