...
memName=Session("Ch_Name")
	...
	'response.write strMem
	   ...
	   '
			 ...
			 if tCity="新竹市" Then 
				  response.write "&nbsp;填單人員&nbsp;:<input type=""radio"" value=""J220017575"" name=""loginid"""
...
'-------------------------------------------------------------------------------------------------------------------------------------------------------------------
			...
			'新增每筆記錄---------------------------------------------------------------------------------------------------
                           ...
                           theIllegalDate=funGetDate(gOutDT(Replace(tempValue(3),"/",""))&" "&tempValue(4),1)

						If DateDiff("d","2015/07/01",gOutDT(Replace(tempValue(3),"/","")))>-1 Then
							Rule="5630001"
						Else
							Rule="5620001"
						End If
						   ...
						   BillFillDate=funGetDate(date(),0)
							...
							CarSimpleID=chkCarNoFormat(trim(tempValue(10)))
							if CarSimpleID="" then CarSimpleID="1"
							If Len(trim(tempValue(10)))=5 Then CarSimpleID="3"
       ...
       '-----------------------------------------------------------------------------------------------------------------------------------------------------
			...
			elseif tCity="新竹市" Then 
				strMem="select ChName,UnitID,MemberID from MemberData where loginid ='"&request("loginid")&"' and RecordStateID=0 and AccountStateID=0 "
                         	set rsMem=conn.execute(strMem)
                        	if not rsMem.eof then
                        		MemberName=trim(rsMem("ChName"))
                        		UnitID=trim(rsMem("UnitID"))
								MemberID=trim(rsMem("MemberID"))
								RecordMemID=BillfillerID
...
set rsMatch=conn.execute(strSQL)	
						if rsMatch.eof and MemberID<>"" then 
							tmpMemo=""
							if tCity="高雄縣" Then 
								tmpMemo="催繳日期:"&trim(tempValue(6))&" 催繳單號:"&trim(tempValue(1))
							elseif tCity="新竹市" Then 
								tmpMemo="(經催繳未於"&trim(tempValue(7))&"繳費期限補繳)"
							End If
						...
						'smith end
													strInsert="insert into BillBase(SN,BillTypeID,BillNo,UseTool,Insurance,CarNo,CarSimpleID,IllegalDate" & _
																...
																" values(BillBase_seq.nextval,'2','"&tempValue(0)&"',0,0" &_
																",'"&tempValue(10)&"',"&CarSimpleID&_						          
																","&theIllegalDate&",null" &_
																",'"&trim(tempValue(8))&"','"&Rule&"'" &_
																","&Level1&"" &_
																",null,null,null" &_
																",null,null,null" &_
																",'"&UnitID&"','"&MemberID&"','"&MemberName&"'" &_
																",'"&MemberID&"','"&MemberName&"'" &_
																","&BillFillDate&","&BillFillDate2&",'0',0,"&funGetDate(Sys_Now,1)&",'"&RecordMemID&"'" &_
																",'"&tmpMemo&"','"&Version&"'" &_
																",null" &_
																",0,'0','"&trim(tempValue(1))&"','1')"
																'response.write strInsert
																conn.execute strInsert		    
										Corr= Corr+1    
												i=i+1			        
