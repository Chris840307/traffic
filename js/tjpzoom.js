...
function TJPhandlemouse(evt,obj) {
 ...
 if(evt.type == 'mousemove') {
  ...
  if(navigator.appVersion.indexOf('Mac') == -1 || navigator.appVersion.indexOf('MSIE') == -1) { //hi Mac IE
	x=obj.parentNode;
	//var divcnt=x.getElementsByTagName('div').length-1;
	//while(x.getElementsByTagName('div').length>0){
		x.removeChild(x.getElementsByTagName('div')[0]);
		x.removeChild(x.getElementsByTagName('div')[0]);
		//divcnt=divcnt-1;
	//}
   // x=document.getElementById("newTJPstage");
   //x.parentNode.removeChild(x)
	//y=document.getElementById("newTJPwin");
   //y.parentNode.removeChild(y)
   //x.removeChild(x.getElementById('newTJPstage')[0]);
  // x.removeChild(x.getElementById('newTJPwin')[0]);
  }
 }
}
