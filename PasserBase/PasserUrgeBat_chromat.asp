...
if request("DB_Selt")="Save" then
	...
	Session("Sys_UnitGroup")=trim(request("Sys_UnitGroup"))
	...
	for i=0 to Ubound(BillSN)
		...
		if rsRuge.eof then
			...
			if trim(request("Sys_JudeNo"))<>"" then
				strUpd="update PasserUrge set OpenGovNumber='"&trim(theJudeNo)&"',UrgeDate=TO_DATE('"&theJudeDate&"','YYYY/MM/DD') where BillSN="&trim(rs("Sn"))&" and BillNo='"&trim(rs("BillNo"))&"'"
			else
				strUpd="update PasserUrge set UrgeDate=TO_DATE('"&theJudeDate&"','YYYY/MM/DD') where BillSN="&trim(rs("Sn"))&" and BillNo='"&trim(rs("BillNo"))&"'"
			end if
			conn.execute(strUpd)
		end if
		rsRuge.close
...
<table width="100%" border="0" bgcolor="#ffffff">
	<tr>
		<td height="27" bgcolor="#FFCC33" class="pagetitle">催告批次套印</td>
	</tr>
	<tr>
			...
			<table width="100%" height="100%"  border="0" cellpadding="5" cellspacing="1" bgcolor="#FFFFFF">
				<tr>
					<td nowrap>
						...
						&nbsp;&nbsp;&nbsp;&nbsp;
						組別：
						<input name="Sys_UnitGroup" type="text" class="btn1" size="10" maxlength="12" value="<%
							if trim(request("Sys_UnitGroup"))<>"" then
								response.write trim(request("Sys_UnitGroup"))
							else
								response.write trim(Session("Sys_UnitGroup"))
							end if
						%>">
						<br>
						...
						催繳日期&nbsp;<input name="Sys_JudeDate" type="text" class="btn1" size="10" maxlength="11" onkeyup="value=value.replace(/[^\d]/g,'')" value="<%
							if Sys_SendBillSN<>"" and instr(Sys_SendBillSN,",")=0 then
								strSQL="select UrgeDate from PasserUrge where BillSN="&Sys_SendBillSN
								set rspass=conn.execute(strSQL)
								if Not rspass.eof then
									response.write gInitDT(rspass("UrgeDate"))
								else
				...
				<tr>
					<td>
						<input type="button" name="btnSelt" value="催繳通知書套印" onclick="funPasserUrgeSelt();">
						<input type="button" name="btnSelt" value="送達證書套印" onclick="funPasserDeliverSelt();">
						<input name="Submit433222" type="button" class="style3" value=" 關 閉 " onclick="self.close();">
						&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="UrgePrint.doc"><font size=5 color="blue">催繳單套印格式說明</font></a>
					</td>
				</tr>
			</table>
		</td>
