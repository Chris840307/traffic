...
strSQL="select SendDate,Driver,illegalYear,count(1) cnt from (select sn,Driver,DriverID,to_char(Illegaldate,'YYYY')-1911 illegalYear,(select to_char(max(SendDate),'YYYY')-1911 SendDate from PasserSendDetail where BillSN=PasserBase.SN and BillSN in(select Billsn from (select BillSn,count(1) cnt from PasserSendDetail group by BillSN) where cnt > 1)) SendDate,(select to_char(max(PetitionDate),'YYYY')-1911 PetitionDate from PasserCreditor where Billsn=PasserBase.SN and BillSN in(select BillSN from(select BillSN,count(1) cnt from PasserCreditor group by BillSN) where cnt>1)) PetitionDate from PasserBase where Exists(select 'Y' from "&BasSQL&" where SN=PasserBase.SN)) where SendDate is not null and PetitionDate is not null group by SendDate,Driver,illegalYear order by Driver,SendDate,illegalYear"
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0">
		<tr>
			<td class="style1" align="center">
				...
				<%
					Response.Write  (Year(date)-1911)&"年"&Month(date)
					Response.Write "月份『再移送』債權憑證明細統計表"%>
			</td>
		</tr>
		<tr>
			<td>
...
fname=year(now)&fMnoth&fDay&"_再移送債權憑證明細統計表"
