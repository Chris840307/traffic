...
'檢查是否可進入本系統
	...
	strSQL="select BillUnitID,UnitName,ForFeit1,count(*) cnt from (" &_
				"select BillUnitID,(nvl(ForFeit1,0)+nvl(ForFeit2,0)) ForFeit1," &_				
					"(select UnitName from Unitinfo where UnitID=a.BillUnitID) UnitName" &_
				" from PasserBase a where a.RecordStateID=0" &_
				" and Exists(select 'Y' from "&BasSQL&" where SN=a.SN)" &_
				" and billstatus<>9" &_
			") tempTable group by BillUnitID,UnitName,ForFeit1 order by BillUnitID"
...
<table width="700" border="0">
	<tr><td>
		<table width="100%" border="0" cellpadding="4" cellspacing="1">
			<tr>
				<td align="center"><strong>未繳費統計表</strong></td>
			</tr>
			<tr>
		...
		<td>
			<table width="100%" border="1" cellpadding="4" cellspacing="1">
				...
				while Not rsfound.eof
					...
					if trim(rsfound("ForFeit1"))<>"" then
						cntNt=Cint(rsfound("ForFeit1"))
						sumNT=sumNT+cdbl(rsfound("ForFeit1"))*cdbl(rsfound("cnt"))
					end if
					response.write "<tr>"
				...
				response.write "<td align=""left"">"&sumNT&"</td></tr>"
				rsfound.close
				set rsfound=nothing
				%>
			</table>
...
fname=year(now)&fMnoth&fDay&"_未繳費統計表.xls"
