...
if DB_Selt="Selt" then
	...
	elseif request("Sys_type")="7" then
		strwhere=strwhere&" and (select count(1) cnt from PasserSendDetail where billsn=passerbase.sn)=1 " & _
		" and exists(select 'Y' from PasserSendDetail where TRUNC(sysdate-SENDDATE)>1097 and billsn=passerbase.sn) " & _
		" and not Exists(select 'N' from PasserCreditor where PETITIONDATE is not null and BillSn=PasserBase.SN)"
	...
	elseif request("Sys_DetailType")="5" then
		strwhere=strwhere&" and TRUNC(SYSDATE-DEALLINEDATE) > 184 and Not Exists(select 'N' from PasserJude where billsn=passerBase.SN)"

	elseif request("Sys_DetailType")="6" then
		strwhere=strwhere&" and Exists(select 'Y' from PasserJude where TRUNC(SYSDATE-JudeDate) > 184 and billsn=passerBase.SN)" & _
		" and Not Exists(select 'N' from PasserSend where billsn=passerBase.SN)"

	elseif request("Sys_DetailType")="7" then
	...
	strSQL="select SN,BillNo,IllegalDate,(select UnitName from UnitInfo where UnitID=PasserBase.BillUnitID) UnitName," & _
			...
			" and recordstateid=0 and billstatus <> 9" & strwhere & " order by IllegalDate,UnitName,BillNo"
	...
	While Not rs1.Eof
		...
		cntobj.Add rs1("illegal_year")&"_7",cdbl(rs1("Send3Year"))
		...
		<td bgcolor="#E0E0E0">
			<table width="100%" height="100%" border="1" cellpadding="1" cellspacing="1">
				<tr align="center">
					...
					response.write "<tr bgcolor='#FFFFFF' align='center'>"
					response.write "<td bgcolor=""#FFCC33"" align='right'>移送逾三年無債權</td>"
					For i = maxyear to minyear step -1
						
						response.write "<td align='left' onclick=""funSetQry('" & i & "','7');"">"&cntobj.Item(i & "_7")&"</td>"
				...
				<tr>
					...
					<td>
						<select name="Sys_DetailType" class="btn1">
							...
							<option value="7"<%If Request("Sys_DetailType") = "7" Then Response.Write " selected"%>>移送逾三年無債權</option>
						</select>
					</td>
					<td>
