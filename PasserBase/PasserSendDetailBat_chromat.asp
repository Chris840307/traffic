...
strSQL = "select Count(*) as cnt from PasserSend where BillSN="&trim(PBillSN(i))&" and BillNo='"&trim(PBillNo(i))&"'"
		...
		If ifnull(MakeSureDate) Then
			...
			strSQL="select count(1) cnt from PasserJude where Billsn="&PBillSN(i)&" and JudeDate < TO_DATE('2012/09/01 00:00:00','YYYY/MM/DD/HH24/MI/SS')"
			
			set rsPassercnt=conn.execute(strSQL)
			passerCnt=cdbl(rsPassercnt("cnt"))
			rsPassercnt.close

			strSQL="select * from PasserSendArrived where ArriveType=0 and PasserSN="&PBillSN(i)
			...
			If Not rsSendArr.eof Then
				...
				If passerCnt>0 Then
					MakeSureDate=DateAdd("d",20,rsSendArr("ArrivedDate"))
					LimitDate=DateAdd("d",20,rsSendArr("ArrivedDate"))
				End if

				strSQL="select ArrivedDate from PasserSendArrived where ArriveType=1 and PasserSN="&PBillSN(i)

				set rsUrgeArr=conn.execute(strSQL)

				If Not rsUrgeArr.eof Then
					LimitDate=DateAdd("d",15,rsUrgeArr("ArrivedDate"))
				end if
				rsUrgeArr.close
				

				If sys_City="雲林縣" Then
					LimitDate=DateAdd("d",40,rsSendArr("ArrivedDate"))
		 		End If 
				...
				If passerCnt>0 Then
					...
					LimitDate=DateAdd("d",20,rsjude("JudeDate"))
				End if
