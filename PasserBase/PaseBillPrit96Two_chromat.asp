...
End if
%>
<div id="Layer1" class="style6" style="position:absolute; left:550px; top:40px; z-index:1"><%

strSQL = "select SendNumber from PasserSendDetail where BillSN="&trim(BillSN(i))&" and sn=(select min(sn) from PasserSendDetail where BillSN="&trim(BillSN(i))&")"

set rsp=conn.execute(strSQL)
If Not rsp.eof Then Response.Write rsp("SendNumber")

If sys_cmt(i) >1 Then
	lencmt=len(mid(rsp("SendNumber"),instrrev(rsp("SendNumber"),"0")))
	For y = 2 to sys_cmt(i)
		tmp_SendNumber=(cdbl(mid(rsp("SendNumber"),instrrev(rsp("SendNumber"),"0")))+(y-1))
		Response.Write ","&right("0000"&tmp_SendNumber,lencmt)
	Next	
End if
rsp.close
%></div>
...
<div id="Layer3" class="style6" style="position:absolute; left:595px; top:130px; z-index:1"><%
strSQL = "select OpenGovNumber from PasserSendDetail where BillSN="&trim(BillSN(i))&" and sn=(select max(sn) from PasserSendDetail where BillSN="&trim(BillSN(i))&")"
set rsp=conn.execute(strSQL)
If Not rsp.eof Then Response.Write rsp("OpenGovNumber")
rsp.close
%></div>
...
<div id="Layer13" class="style6" style="position:absolute; left:580px; top:630px; z-index:1"><%=Sys_Total(i)%></div>
