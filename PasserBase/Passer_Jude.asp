...
'	ForFeit0=0
'	ForFeit0=cdbl(Request("ForFeit_1"))+cdbl(Request("ForFeit_2"))
...
'	conn.execute(strSQL)
'
'	If cdbl(Request("ForFeit_1")) >0 Then
'		UpForiet="update passerbase set forfeit1="&cdbl(Request("ForFeit_1"))&" where sn="&trim(request("PBillSN"))
'
'		conn.execute UpForiet
'	end If 
'
'	If cdbl(Request("ForFeit_2")) >0 Then
'		UpForiet="update passerbase set forfeit2="&cdbl(Request("ForFeit_2"))&" where rule2 is not null and sn="&trim(request("PBillSN"))
'
'		conn.execute UpForiet
'	end If 
...
<%
	...
	if not rsState.eof then
		...
		theBigUnitBossName=trim(rsState("BigUnitBossName"))
		theSubUnitSecBossName=trim(rsState("SubUnitSecBossName"))
...
<%
		...
		if trim(rsSql("Rule1"))<>"" then
			strRule1="select * from Law where ItemID='"&trim(rsSql("Rule1"))&"' and VERSION='"&RuleVer&"'"
		...
		if trim(rsSql("Rule2"))<>"" then
			strRule1="select * from Law where ItemID='"&trim(rsSql("Rule2"))&"' and VERSION='"&RuleVer&"'"
			set rsRule1=conn.execute(strRule1)
			if not rsRule1.eof then
				L1ForFeit=L1ForFeit+cint(trim(rsRule1("Level1")))
				if trim(rsRule1("Level2")="" or isnull(rsRule1("Level2"))) then
					L2ForFeit=L2ForFeit+cint(trim(rsRule1("Level1")))
				else
					L2ForFeit=L2ForFeit+cint(trim(rsRule1("Level2")))
				end if
				L3ForFeit=L3ForFeit+cint(trim(rsRule1("Level3")))
				L4ForFeit=L4ForFeit+cint(trim(rsRule1("Level4")))
			end if
			rsRule1.close
			set rsRule1=nothing
		end if
          ...
          <input name="ForFeit" class="btn1" type="text" size="12" maxlength="12" value="<%
			If not ifnull(UForFeit) Then

				Response.Write UForFeit
			else 
				sum_ForFeit=0
				...
				If not rspay.eof Then
					...
					if trim(rsPay("PaySum"))<>"" and not isnull(rsPay("PaySum")) then
						...
						sum_ForFeit=cdbl(rsPay("PaySum"))
					end if
				End if 
				rsPay.close

				If sys_City = "基隆市" Then
					response.write L1ForFeit-sum_ForFeit
      ...
      <tr>
		...
		if not ifnull(UPunishmentMainBody) then
			...
			ForFeit_1=0:ForFeit_2=0
			If trim(rsSql("forfeit1"))<>"" Then ForFeit_1=cdbl(rsSql("forfeit1"))
			If trim(rsSql("forfeit2"))<>"" Then ForFeit_2=cdbl(rsSql("forfeit2"))

		else
			Sys_ForFeit1=0:ForFeit_1=0:ForFeit_2=0
			...
			if trim(rsSql("Rule1"))<>"" and not isnull(rsSql("Rule1")) then
				...
				if not rsRule1.eof then
					...
					if datediff("d",trim(rsSql("DealLineDate")),now)=<0 then
						...
						ForFeit_1=cdbl(rsRule1("Level1"))

					elseif datediff("d",trim(rsSql("DealLineDate")),now)>0 and datediff("d",trim(rsSql("DealLineDate")),now)<=15 then

						ForFeit_1=cdbl(rsRule1("Level2"))

					elseif datediff("d",trim(rsSql("DealLineDate")),now)>15 and datediff("d",trim(rsSql("DealLineDate")),now)<=30 then

						ForFeit_1=cdbl(rsRule1("Level3"))

					elseif datediff("d",trim(rsSql("DealLineDate")),now)>30 then

						ForFeit_1=cdbl(rsRule1("Level4"))
					end If 

					If sys_City = "基隆市" Then ForFeit_1=trim(rsRule1("Level1"))
				...
				if trim(rsSql("Rule2"))<>"" and not isnull(rsSql("Rule2")) then
					...
					if not rsRule1.eof then
						...
						if datediff("d",trim(rsSql("DealLineDate")),now)=<0 then
							...
							ForFeit_2=cdbl(rsRule1("Level1"))

						elseif datediff("d",trim(rsSql("DealLineDate")),now)>0 and datediff("d",trim(rsSql("DealLineDate")),now)<=15 then

							ForFeit_2=cdbl(rsRule1("Level2"))

						elseif datediff("d",trim(rsSql("DealLineDate")),now)>15 and datediff("d",trim(rsSql("DealLineDate")),now)<=30 then

							ForFeit_2=cdbl(rsRule1("Level3"))

						elseif datediff("d",trim(rsSql("DealLineDate")),now)>30 then

							ForFeit_2=cdbl(rsRule1("Level4"))
						end If 				

						If sys_City = "基隆市" Then ForFeit_2=trim(rsRule1("Level1"))
					end if
					rsRule1.close
					set rsRule1=nothing
				end If 
				
				Sys_ForFeit1=cdbl(ForFeit_1)+cdbl(ForFeit_2)
					
				If sys_City="台南市" or sys_City="台南縣" Then 
					response.Write("罰鍰新臺幣"&to_Money(Sys_ForFeit1-sum_ForFeit)&"元整.(限於接到裁決書之翌日起30日內繳納。逾期不繳納者，依法移送強制執行)。")
				else
					response.write "一、罰鍰新臺幣"&to_Money(Sys_ForFeit1-sum_ForFeit)&"元整。"
				end if
				
				If sys_City="澎湖縣" or sys_City="彰化縣" Then response.Write("(限於接到裁決書之翌日起20日內繳納。)")
				...
				If sys_City <> "台中市" and sys_City<>"台南市" and sys_City<>"台南縣" Then
      ...
      <tr>
		...
		if BillState="Update" then
			...
			response.write "受處分人於上開違規時間、地點，因違反道路交通管理處罰條例，經本分局"&replace(BillUnitName,DB_UnitName,"")&"當場製單舉發，未依通知日期到案者，依「道路交通管理處罰條例」裁決，如主文。"
      ...
      <tr>
		...
		if trim(rsSql("Rule3"))<>"" and not isnull(rsSql("Rule3")) then
			...
			strRule1="select * from Law where ItemID='"&trim(rsSql("Rule3"))&"' and VERSION='"&RuleVer&"'"
			set rsRule1=conn.execute(strRule1)
			if not rsRule1.eof then
				response.write cint(trim(rsRule1("Level1")))
				if trim(rsRule1("Level2")="" or isnull(rsRule1("Level2"))) then
		...
		if trim(rsSql("Rule4"))<>"" and not isnull(rsSql("Rule4")) then
			...
			strRule1="select * from Law where ItemID='"&trim(rsSql("Rule4"))&"' and VERSION='"&RuleVer&"'"
			set rsRule1=conn.execute(strRule1)
			if not rsRule1.eof then
				response.write cint(trim(rsRule1("Level1")))
				if trim(rsRule1("Level2")="" or isnull(rsRule1("Level2"))) then
		...
		<%if sys_City="彰化縣" or sys_City="基隆市" then%>
			...
			<td width="19%">
				<%
					...
					" where BillSN="&trim(request("PBillSN"))&" and PkeySN="&trim(request("PBillSN"))&_
					" and ImgKindID='1' and ImgTypeID='1' order by ImageFileName"
					...
					While Not rsimg.eof
						...
						If not ifnull(rsimg("ImageFileName")) Then
							...
							Response.Write "<br><br><a href=""./PasserImage/"&trim(rsimg("Imagefilename"))&""" target=""_blank"">裁決掃描檔"&cnt&"</a>"
  ...
  <tr>
    <td height="35" bgcolor="#1BF5FF"><p align="center" class="style1">
		...
		<input name="btnprint" type="button" value="列印裁決書word"  onclick='PrintReports2();'>
		...
		<input type="hidden" value="" name="Jude_BillSN">			
		<input type="hidden" value="" name="Sys_PasserJude">	
		<input type="hidden" value="" name="Sys_PasserJude_Label">
		<input type="hidden" value="" name="strPath">
		<input type="hidden" value="<%=ForFeit_1%>" name="ForFeit_1">
		<input type="hidden" value="<%=ForFeit_2%>" name="ForFeit_2">
		</td>
  </tr>
  <tr>
    <td><p>&nbsp;</p></td></tr>
...
function PrintReports(){
	if(myForm.BillEof.value=='0'){
		myForm.Sys_PasserJude.value="1";
		myForm.Sys_PasserJude_Label.value="";
...
function PrintReports_Label(){
	if(myForm.BillEof.value=='0'){
		...
		myForm.Sys_PasserJude.value="0";
		myForm.Sys_PasserJude_Label.value="1";
		myForm.action="PasserJudeBatList.asp";
		myForm.target="JudeBat";
		myForm.submit();
		myForm.action="";
...
function funMap(PBillSN){
	
	newWin("","JudeImg",700,550,50,10,"yes","yes","yes","no");

	myForm.Jude_BillSN.value=PBillSN;

	myForm.action="Passer_Jude_upload.asp";
	myForm.target="JudeImg";
	myForm.submit();
	myForm.action="";
	myForm.target="";
