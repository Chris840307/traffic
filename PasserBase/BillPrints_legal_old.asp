...
<table width="645" height="393" border="0">
  ...
  <tr>
    ...
    <td align="center">&nbsp;</td>
    <td align="right"><p>&nbsp;</p>
    <p class="style7"><%=Sys_StationID%></p></td>
  </tr>
  <tr>
...
<div id="Layer19" style="position:absolute; left:60px; top:<%=965+1300*i%>px; width:117px; height:20px; z-index:12"><span class="style7"><%=Sys_BillNo%></span></div>
<div id="Layer32" style="position:absolute; left:60px; top:<%=980+1300*i%>px; width:400px; height:49px; z-index:29"><span class="style7"><%response.write "第"&left(trim(Sys_Rule1),2)&"條"
			if len(trim(Sys_Rule1))>7 then response.write "之"&right(trim(Sys_Rule1),1)
				response.write Mid(trim(Sys_Rule1),3,2)&"項"&Mid(trim(Sys_Rule1),5,2)&"款規定"
