...
if not rsState.eof then
	...
	str_JudeDate=rsState("JudeDate")
	UJudeDate=split(gArrDT(rsState("JudeDate")),"-")
	UpayDate=split(gArrDT(DateAdd("d", 30, rsState("JudeDate"))),"-")
...
chk_UnitLevelID=""
strSQL="select UnitLevelID from UnitInfo where UnitID in(select UnitTypeID from UnitInfo where UnitID='"&rsSql("memberstation")&"')"
...
if Not rsUnit.eof then 
	If sys_City="高雄市" or sys_City="宜蘭縣" or sys_City="嘉義市" or sys_City="南投縣" then
...
<table width="635" border="0" cellpadding="0" cellspacing="0">
  <tr>
    <td height="0" colspan="2" align="center"><%="<img src=""./Snap/"&rsSql("memberstation")&"_01.jpg"" width=400>"%></td>
  </tr>
  ...
  <tr>
    ...
    <td><div align="right" class="style22"><%
		...
		If sys_City="澎湖縣" Then
			Response.Write trim(BillPageUnit)&"裁字第"&UOpenGovNumber&"號"
		else
			Response.Write trim(BillPageUnit)&"交裁字第"&UOpenGovNumber&"號"
		End if		
	%></div></td>
  </tr>
</table>
<table width="665" height="90%" border="1" cellspacing=0 cellpadding=0>
  ...
  <tr>
    ...
    <td colspan="5"><span class="style22"><%=trim(rsSql("DriverZip"))&replace(trim(rsSql("DriverAddress"))&" ","台","臺")%></span>&nbsp;</td>
  </tr>
  <tr>
    <td height="25" nowrap><span class="style22">車號</span></td>
    <td colspan="3">&nbsp;</td>
    ...
    <td colspan="3"><span class="style22">
      <%
		...
		if trim(rsSql("DealLineDate"))<>"" then
			...
			DealDate=DateDiff("d",rsSql("DealLineDate"),str_JudeDate)
  ...
  <tr>
    ...
    <td colspan="5"><span class="style23">
		<%=UPunishmentMainBody%>（限於<%=UpayDate(0)%>年<%=cdbl(UpayDate(1))%>月<%=cdbl(UpayDate(2))%>日前繳納）
		<br>
		二、其他：依違反道路交通管理事件統一裁罰基準及處理細則（逾期<%=DealDate%>日）
	</span></td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td colspan="5"><span class="style22"><%=thenPasserCity&theUnitName&"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;劃撥帳號："&theBankAccount%>
	<br>地址：<%=theUnitAddress%>　
	<br>電話：<%=theContactTel%></span></td>
  </tr>
  <tr>
    <td height="69" nowrap><span class="style22">機關首長&nbsp;</span></td>
	<td colspan="5" height="44" nowrap><span class="style22">
	&nbsp;&nbsp;
	<%="<img src=""./Snap/"&rsSql("memberstation")&"_02.jpg"" width=100>"%>
	</span></td>
  </tr>
  <tr>
    <td height="200"><div align="center"><span class="style22">附<br>
    記</span></div></td>
    <td colspan="5" valign="top"><span class="style23">
	一、受處分人不服本裁決者，應以原處分機關(<%=thenPasserCity%><%=theUnitName%>)<br>
	&nbsp;&nbsp;&nbsp;&nbsp;為被告，向原告住所地、居所地、所在地、違規行為地、或原處分機關<br>
	&nbsp;&nbsp;&nbsp;&nbsp;所在地之地方法院行政訴訟庭提起訴訟；其中撤銷訴訟之提起，應於裁<br>
	&nbsp;&nbsp;&nbsp;&nbsp;決書送達後30日之不變期間內為之。<br>

    二、罰鍰逾期不繳納者，即移送法務部行政執行署所屬行政執行處執行。
