...
strSQL="Select * from Apconfigure where ID=40"
set apName=conn.execute(strSQL)
titleName=apName("Value")
apName.close
strSQL="Select * from Apconfigure where ID=31"
set apName=conn.execute(strSQL)
cityName=apName("Value")
apName.close
%>
<html>
<head>
...
.style4 {font-family: "標楷體"; font-size: 24px; }
-->
</style>
</head>

<body>
<table width="333" border="1" align="right" cellspacing=0 cellpadding=0>
...
<table width="100%" border="1" cellspacing=0 cellpadding=0>
	<tr>
		<td colspan="5" rowspan="2" align="center" valign="middle" class="style4"><%=titleName%>行政執行案件移送書</td>
		<td width="85" height="34"><span class="style1">發文日期</span></td>
		<td width="211"><span class="style1"><%=PrintDate(0)%>年<%=PrintDate(1)%>月<%=PrintDate(2)%>日</span></td>
	</tr>
	<tr>
		<td height="39"><span class="style1">發文字號</span></td>
	  <td><span class="style1"><%=rsfound("OpenGovNumber")%>號</span></td>
	</tr>
	<tr>
		<td height="41" colspan="2" align="center"><span class="style1">姓　名　或　名　稱</span></td>
		<td width="99" align="center"><span class="style1">出生年月日</span></td>
		<td width="75" align="center"><span class="style1">性別</span></td>
		<td width="131" align="center"><span class="style2">身分證統一編號或<br>
	    營利事業統一編號</span></td>
		<td colspan="2" align="center"><span class="style1">住址或事務所、營業所地址及郵遞區號</span></td>
	</tr>
	<tr>
	  <td width="138" height="56" align="center"><span class="style1">
      義　務　人</span></td>
		<td width="218"><span class="style1"><%=rsfound("Driver")%></span></td>
		...
		<td><span class="style1"><%=trim(rsfound("DriverZip"))&trim(rsfound("DriverAddress"))%></span></td>
	</tr>
	<tr>
	...
	<tr>
		...
		<%
			if trim(rsfound("Rule1"))<>"" then
				response.write "違反道路交通管理處罰條例第"&left(trim(rsfound("Rule1")),2)
				if len(trim(rsfound("Rule1")))>7 then response.write "之"&right(trim(rsfound("Rule1")),1)
				response.write "<br>"
				response.write "　　　　　　"&Mid(trim(rsfound("Rule1")),3,1)&"項"&Mid(trim(rsfound("Rule1")),4,2)&"款規定"
				response.write "<br>"
			end if	
			...
			response.write "違反道路交通管理事件"&IllegalDate(0)&"年"&IllegalDate(1)&"月"&IllegalDate(2)&"日<BR>"
			response.write "裁文字第"&rsfound("JudeOGN")&"裁決書"
			%>
	    </span></td>
		<td height="51"><span class="style1">
	    <%
			'if trim(rsfound("MakeSureDate"))<>"" then
			response.write "■確定日期<br>"
			response.write "□限繳日期"
			'end if%>
		  <br>
        <%
	...
	<tr>
		<td height="53"><span class="style1">繳納金額</span></td>
		<td colspan="2"><span class="style1">新台幣<%=rsfound("ForFeit")%>元</span></td>
		<td colspan="2"><span class="style1"><%=rsfound("IllegalAddress")%></span></td>
	</tr>
	<tr>
...
'			end if%>
	...
	<tr>
		<td align="center"><span class="style1">附　　件</span></td>
...
'			end if%>
	...
	<tr>
		<td height="90" colspan="7"><span class="style4">　　　　此致<br>
		法務部行政執行署<%=cityName%>行政執行處　　　　<%=titleName%>長<br>
		　　　　　　　　　　　　　　　　　　　　　　　　　分局長　　　　代為決行</span></td>
	</tr>
</table>
</body>
