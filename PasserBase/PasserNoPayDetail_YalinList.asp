...
strSQL="select UitName,UnitOrder,IllegalDate_year,sum(sroCnt) sroCnt,sum(Forfeit) Forfeit,sum(CloseCnt) CloseCnt,sum(CloseForFeit) CloseForFeit," & _
		"sum(MarkCnt) MarkCnt,sum(MarkForfeit) MarkForfeit,sum(notSendCnt) notSendCnt,sum(notSendForFeit) notSendForFeit," & _
		"sum(SendCnt) SendCnt,sum(SendForFeit) SendForFeit" & _
		" from (" & _
		...
		"(case when billstatus=9 and (select count(1) cnt_a from passerpay where CaseCloseDate<=to_date('"&sysdate2&"','YYYY/MM/DD') and billsn=pb.sn)>0" & _
		" then 1 else 0 end) CloseCnt," & _
		"(case when billstatus=9 and (select count(1) cnt_a from passerpay where CaseCloseDate<=to_date('"&sysdate2&"','YYYY/MM/DD') and billsn=pb.sn)>0" & _
		" then (select nvl(sum(payAmount),0) payAmount from passerpay where billsn=pb.sn) else 0 end) CloseForFeit," & _
		"(case when billstatus=9 and (select count(1) cnt_b from passerpay where billsn=pb.sn) = 0 then 1 else 0 end) MarkCnt," & _
		"(case when billstatus=9 and (select count(1) cnt_b from passerpay where billsn=pb.sn) = 0 then (forfeit1+nvl(forfeit2,0)) else 0 end) MarkForfeit," & _
		"(case when billstatus<>9 and DEALLINEDATE >= to_date('"&sysdate2&"','YYYY/MM/DD') then (forfeit1+nvl(forfeit2,0)) else 0 end) normalForFeit," & _
		...
		"(case when (select count(1) cnt from PasserPay where CaseCloseDate<=to_date('"&sysdate2&"','YYYY/MM/DD') and billsn=pb.sn)=0 and (DEALLINEDATE >= to_date('"&sysdate2&"','YYYY/MM/DD')" & _
		" or (select count(1) cnta from passersend where billsn=pb.sn) > 0) then 1 else 0 end) SendCnt," & _
		"(case when (select count(1) cnt from PasserPay where CaseCloseDate<=to_date('"&sysdate2&"','YYYY/MM/DD') and billsn=pb.sn)=0 and (DEALLINEDATE >= to_date('"&sysdate2&"','YYYY/MM/DD')" & _
		" or (select count(1) cnta from passersend where billsn=pb.sn) > 0) then (forfeit1+nvl(forfeit2,0)) else 0 end) SendForFeit" & _
...
Uit_sroCnt=0:Uit_Forfeit=0:Uit_CloseCnt=0:Uit_CloseForFeit=0:Uit_MarkCnt=0:Uit_MarkForfeit=0
Uit_notSendCnt=0:Uit_notSendForFeit=0:Uit_SendCnt=0:Uit_SendForFeit=0

total_sroCnt=0:total_Forfeit=0:total_CloseCnt=0:total_CloseForFeit=0:total_MarkCnt=0:total_MarkForfeit=0
total_notSendCnt=0:total_notSendForFeit=0:total_SendCnt=0:total_SendForFeit=0
...
<table width="100%" border="0">
	<tr><td>
		<table width="100%" border="0" cellpadding="4" cellspacing="1">
			<tr>
				<td align="center"><strong><%=thenPasserCity%>政府暨所屬機關以前年度應收未收行政罰鍰收繳情形明細表</strong></td>
			</tr>
			<tr>
		...
		<td>
			<table width="100%" border="1" cellpadding="4" cellspacing="0">
				<tr>
					...
					<td class="font3" align="center" rowspan=2 colspan=2 nowrap>期初應收未收罰鍰</td>
					<td class="font3" align="center" rowspan=2 colspan=2 nowrap>已收繳數</td>
					<td class="font3" align="center" rowspan=2 colspan=2 nowrap>註銷數</td>
					<td class="font3" align="center" colspan=4 nowrap>未收繳數</td>
					<td class="font3" align="center" rowspan=2 colspan=2 nowrap>期未餘額</td>
					<td class="font3" align="center" colspan=4 nowrap>執行率</td>
				</tr>
				<tr>
					<td class="font3" align="center" colspan=2 nowrap>未移送</td>
					<td class="font3" align="center" colspan=2 nowrap>其他待收繳</td>
				...
				For i = 1 to 10
					...
					Uit_MarkCnt=Uit_MarkCnt+cdbl(rs("MarkCnt"))
					Uit_MarkForfeit=Uit_MarkForfeit+cdbl(rs("MarkForfeit"))
					...
					total_MarkCnt=total_MarkCnt+cdbl(rs("MarkCnt"))
					total_MarkForfeit=total_MarkForfeit+cdbl(rs("MarkForfeit"))
					...
					Response.Write "<td class=""font3"">"
					Response.Write rs("MarkCnt")
					Response.Write "</td>"

					Response.Write "<td class=""font3"">"
					Response.Write rs("MarkForfeit")
					Response.Write "</td>"
					...
					Response.Write "<td class=""font3"">"
					Response.Write cdbl(rs("sroCnt"))-cdbl(rs("CloseCnt"))-cdbl(rs("MarkCnt"))
					Response.Write "</td>"

					Response.Write "<td class=""font3"">"
					Response.Write cdbl(rs("Forfeit"))-cdbl(rs("CloseForFeit"))-cdbl(rs("MarkForfeit"))
					Response.Write "</td>"

					Response.Write "<td class=""font3"">"
					Response.Write fix((cdbl(rs("CloseCnt"))+cdbl(rs("MarkCnt")))/cdbl(rs("sroCnt"))*100)&"%"
					Response.Write "</td>"

					Response.Write "<td class=""font3"">"
					Response.Write fix((cdbl(rs("CloseForFeit"))+cdbl(rs("MarkForfeit")))/cdbl(rs("Forfeit"))*100)&"%"
					Response.Write "</td>"

					Response.Write "<td class=""font3"">"
					Response.Write fix((cdbl(rs("CloseCnt"))+cdbl(rs("MarkCnt"))+cdbl(rs("SendCnt")))/cdbl(rs("sroCnt"))*100)&"%"
					Response.Write "</td>"

					Response.Write "<td class=""font3"">"
					Response.Write fix((cdbl(rs("CloseForFeit"))+cdbl(rs("MarkForfeit"))+cdbl(rs("SendForFeit")))/cdbl(rs("Forfeit"))*100)&"%"
					Response.Write "</td>"
				...
				If not rs.eof Then
					If chkUit <> rs("UitName") Then
						...
						Response.Write "<td class=""font3"">"
						Response.Write Uit_MarkCnt
						Response.Write "</td>"

						Response.Write "<td class=""font3"">"
						Response.Write Uit_MarkForfeit
						Response.Write "</td>"
						...
						Response.Write "<td class=""font3"">"
						Response.Write cdbl(Uit_sroCnt)-cdbl(Uit_CloseCnt)-cdbl(Uit_MarkCnt)
						Response.Write "</td>"

						Response.Write "<td class=""font3"">"
						Response.Write cdbl(Uit_Forfeit)-cdbl(Uit_CloseForFeit)-cdbl(Uit_MarkForfeit)
						Response.Write "</td>"

						Response.Write "<td class=""font3"">"
						Response.Write fix((cdbl(Uit_CloseCnt)+cdbl(Uit_MarkCnt))/cdbl(Uit_sroCnt)*100)&"%"
						Response.Write "</td>"

						Response.Write "<td class=""font3"">"
						Response.Write fix((cdbl(Uit_CloseForFeit)+cdbl(Uit_MarkForfeit))/cdbl(Uit_Forfeit)*100)&"%"
						Response.Write "</td>"

						Response.Write "<td class=""font3"">"
						Response.Write fix((cdbl(Uit_CloseCnt)+cdbl(Uit_MarkCnt)+cdbl(Uit_SendCnt))/cdbl(Uit_sroCnt)*100)&"%"
						Response.Write "</td>"

						Response.Write "<td class=""font3"">"
						Response.Write fix((cdbl(Uit_CloseForFeit)+cdbl(Uit_MarkForfeit)+cdbl(Uit_SendForFeit))/cdbl(Uit_Forfeit)*100)&"%"
						Response.Write "</td>"
					...
					Response.Write "<td class=""font3"">"
					Response.Write total_MarkCnt
					Response.Write "</td>"

					Response.Write "<td class=""font3"">"
					Response.Write total_MarkForfeit
					Response.Write "</td>"
					...
					Response.Write "<td class=""font3"">"
					Response.Write cdbl(total_sroCnt)-cdbl(total_CloseCnt)-cdbl(total_MarkCnt)
					Response.Write "</td>"

					Response.Write "<td class=""font3"">"
					Response.Write cdbl(total_Forfeit)-cdbl(total_CloseForFeit)-cdbl(total_MarkForfeit)
					Response.Write "</td>"

					Response.Write "<td class=""font3"">"
					Response.Write fix((cdbl(total_CloseCnt)+cdbl(total_MarkCnt))/cdbl(total_sroCnt)*100)&"%"
					Response.Write "</td>"

					Response.Write "<td class=""font3"">"
					Response.Write fix((cdbl(total_CloseForFeit)+cdbl(total_MarkForfeit))/cdbl(total_Forfeit)*100)&"%"
					Response.Write "</td>"

					Response.Write "<td class=""font3"">"
					Response.Write fix((cdbl(total_CloseCnt)+cdbl(total_MarkCnt)+cdbl(total_SendCnt))/cdbl(total_sroCnt)*100)&"%"
					Response.Write "</td>"

					Response.Write "<td class=""font3"">"
					Response.Write fix((cdbl(total_CloseForFeit)+cdbl(total_MarkForfeit)+cdbl(total_SendForFeit))/cdbl(total_Forfeit)*100)&"%"
					Response.Write "</td>"
					Response.Write "<tr>"
				End if 
			%>
	...
	<tr>
		<td class="font3">
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			1.清理比率=（已收繳數＋註銷數）&nbsp;÷&nbsp;期初應收未收罰鍰<br>
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			2.已移送執行比率=（已收繳數＋註銷數＋其他待收繳)&nbsp;÷&nbsp;期初應收未收罰鍰
		</td>
	</tr>
</table>
<%
