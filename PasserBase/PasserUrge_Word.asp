...
for i=0 to Ubound(BillSN)
	Sys_IllegalDate="":Sys_FORFEIT1=0:Sys_Driver="":Sys_DriverSex="":Sys_DriverID="":Sys_DriverZip="":Sys_DriverAddress=""

	strSQL="select DriverID,Driver,(case when DriverSex=1 then '男' when DriverSex=2 then '女' end) DriverSex,DriverZip,DriverAddress,'1' as affair,FORFEIT1+nvl(FORFEIT2,0) FORFEIT,IllegalDate from PasserBase where SN ="&trim(BillSN(i))

	set rssum=conn.execute(strSQL)

	Sys_IllegalDate=split(gArrDT(rssum("IllegalDate")),"-")
	Sys_FORFEIT1=rssum("FORFEIT")
	Sys_affair=1
	Sys_Driver=rssum("Driver")
	Sys_DriverSex=rssum("DriverSex")
	Sys_DriverID=rssum("DriverID")
	Sys_DriverZip=rssum("DriverZip")
	Sys_DriverAddress=rssum("DriverAddress")
%>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=big5">
<title>違反道路交通管理事件催繳通知書</title>
<style type="text/css">
<!--
.style1 {
	font-size: 16px;
