...
<%
	...
	strSQL="select * from (select rownum as Item,a.* from (Select * from PasserBase where MemberStation in(select UnitTypeID from Unitinfo where UnitID='"&trim(Session("Unit_ID"))&"') and RecorDStateID<>-1 order by RecordDate) a) where BillNo = '"&request("Sys_Billno")&"'"
