...
for each formName in upload.objFile ''列出所有上傳的檔案
	...
	if cdbl(TypeFlag) > 0 and trim(Session("User_ID"))<>"" then
		...
		fname = "PasserSend_"&trim(Session("SendImg_PBillSN"))&"_"&trim(Session("SendImg_SendDetialSN"))&"_1_"&dcnt&cnt_file & "." & ExName
		...
		" values("&trim(Session("SendImg_PBillSN"))&","&trim(Session("SendImg_SendDetialSN"))&",3,1,'"&fname&"',sysdate,"&Session("User_ID")&")"
		conn.execute(strSQL)
