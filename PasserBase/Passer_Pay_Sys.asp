...
Function getpayno()
	...
	strSQL="select substr(payno,1,6)||LPAD(to_char(to_number(substr(payno,7))+1),4,'0') PayNo from passerpayno where UnitID=(select UnitTypeID from Unitinfo where UnitID='"&Session("Unit_ID")&"') and YEAR_VER='111' and recordstateid=0"

	set rsno=conn.execute(strSQL)
	If not rsno.eof Then getpayno=trim(rsno("PayNo"))
	rsno.close

	If Session("UnitLevelID") < 2 and trim(Session("Credit_ID"))<>"A000000000" then
		getpayno=""
	end If 
...
if trim(request("kinds"))="db_insert" then
	...
	if (trim(request("PayAmount"))<>"" and trim(request("PayNo"))<>"") or trim(request("Note"))<>"" then
		...
		strIns="insert into PasserPay(BillSN,BillNo,PayNo,PayTimes,PayTypeID,PayDate,Payer,ForFeit" &_
			",PayAmount,CaseClose,RecordStateID,RecordDate,RecordMemberID,Note,IsLate,MIDDLEMONEY,CreditorSendNo)" &_
			"values("&trim(request("BillSN"))&",'"&trim(request("BillNo"))&"'" &_
			",'"&getpayno()&"',"&MaxTime&","&trim(request("PayTypeID")) &_
			...
			","&PayMIDDLEMONEY&",'"&trim(Request("Sys_CreditorSendNo"))&"'"&_
			")" 
		conn.execute strIns
		
		if trim(request("PayAmount"))>"0" then

			strSQL="update passerpayno set payno=substr(payno,1,6)||LPAD(to_char(to_number(substr(payno,7))+1),4,'0') where UnitID=(select UnitTypeID from Unitinfo where UnitID='"&Session("Unit_ID")&"') and YEAR_VER='111' and recordstateid=0"

			conn.execute(strSQL)
		else

			strIns="Update PasserPay set PayNo=null where BillSN="&trim(request("BillSN"))&" and PayTimes="&MaxTime

			conn.execute strIns
		end if

		if theCaseClose=1 then
			strIns="Update PasserPay set CaseCloseDate=TO_DATE('"&gOutDT(request("CaseCloseDate"))&"','YYYY/MM/DD') where BillSN="&trim(request("BillSN"))&" and PayTimes="&MaxTime
			conn.execute strIns
		else
			strIns="Update PasserPay set CaseClose=0,CaseCloseDate=null where BillSN="&trim(request("BillSN"))&" and PayTimes="&MaxTime
			conn.execute strIns
		end If 
...
<table width="100%" height="100%" border="0">
  ...
  <tr>
    <td height="26"><table width="100%" height="100%"  border="0" cellpadding="5" cellspacing="1" bgcolor="#FFFFFF">
      ...
      <tr>
		...
		<td width="9%" nowrap bgcolor="#FFFF99"><div align="right" class="style3">必要費用<br>（非郵局手續費）</div></td>
        <td>
      ...
      <tr>
        ...
        <td width="19%"><%
			...
			Response.Write "<select name=""IsLate"">"
        ...
        <td>
			<input type="text" name="Sys_CreditorSendNo" value="">
		</td>
      </tr>
      <tr>
        ...
        <td>
			<input type="text" name="PayAmount" value="" maxlength="6" onkeyup="funChkMoney(this);">
			...
			If sys_City = "彰化縣" Then
				...
				set rsjude=conn.execute(strSQL)
				If not rsjude.eof Then
					If trim(rsSql("ReserveYear")) <> "" Then
						
						Response.Write "保留至"&rsSql("ReserveYear")&"年度"
					else
      ...
      <tr>
        ...
        <td nowrap bgcolor="#FFFFff">
			...
			<input type="checkbox" name="CaseClose" value="1" <%
				...
				End if
			%> onclick="funPaycheck('0');">
		</td>
        <td nowrap bgcolor="#FFFF99"><div align="right"><span class="style3">收據字號</span></div></td>
        <td>
          <input name="PayNo" type="text" value="<%=getpayno()%>" size="31" maxlength="50" readonly>
  ...
  <tr>
	...
	While Not rsPayHis.Eof
		...
		If totalMoney = 0 Then checkdata=1
%>		<tr bgcolor="#FFFFFF">
			<td><%=gInitDT(trim(rsPayHis("PayDate")))%></td>
			<td><%
			if trim(rsPayHis("PayTypeID"))="1" then
				response.write "窗口"
			elseif trim(rsPayHis("PayTypeID"))="2" then
				response.write "郵撥"
			else
				response.write "其他代收單位"
			end if
			%></td>
			<td><%
			...
			%></td>
			<td nowrap>
			<input type="button" name="PrintImg" value="列印收據" onclick="funPrintImage('<%=trim(rsPayHis("BillSN"))%>','<%=trim(rsPayHis("PayTimes"))%>');">

			<%if trim(rsPayHis("RecordMemberID"))=trim(Session("User_ID")) then%>
			<input type="button" name="<%=gInitDT(trim(rsPayHis("PayDate")))%>" value="修改" onclick="location='Passer_Pay_Sys_Update.asp?PBillSN=<%=trim(rsPayHis("BillSN"))%>&PTime=<%=trim(rsPayHis("PayTimes"))%>'">

			<input type="button" name="<%=gInitDT(trim(rsPayHis("PayDate")))%>" value="作癈" onclick="funDel('<%=trim(rsPayHis("BillSN"))%>','<%=trim(rsPayHis("PayTimes"))%>');">
			<%end if%>&nbsp;
			</td>
		</tr>
<%	rsPayHis.MoveNext
...
function funChkMoney(Obj){
	...
	if(eval(Obj.value) > 0){
		...
		if(eval(Obj.value) < eval(myForm.totalMoney.value)){
...
function funPaycheck(checkData){ 

	if (myForm.CaseClose.checked&&checkData=='1'){
		
		myForm.subPayCLose.disabled=true;
	}else{
		myForm.subPayCLose.disabled=false;
	}
}
