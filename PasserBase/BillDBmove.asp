...
<%
	...
	strSQL="select * from (select rownum as Item,a.* from (Select * from PasserBase where RecordMemberID in(select MemberID from MemberData where UnitID='"&trim(Session("Unit_ID"))&"' and RecorDStateID<>-1) and RecorDStateID<>-1 order by RecordDate) a) where Item = "&request("DBFile")
