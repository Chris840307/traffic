...
'	Executor.Application = Server.MapPath("\wkhtmltopdf") & "\wkhtmltopdf.exe"  '指定要執行的應用程式路徑
	...
	Provider="Provider=SQLOLEDB;Data Source=10.112.8.30;User Id=traffic;Password=joly902f;"
	...
	For i = 0 to Ubound(BillSN)
		...
		WShShell.Run Server.MapPath("\wkhtmltopdf") & "\wkhtmltopdf.exe --zoom 1.3 http://"&ServerIP&"/traffic/PasserBase/PasserJudeBatList_Pdf_miaoli.asp?Sys_PasserJude=1&Sys_SendBillSN="&BillSN(i)&"&JpgUnitID="&Session("Unit_ID")&" "&Server.MapPath("\img\"&gInitDT(rs("JudeDate"))&"\"&CaseNo&".pdf"),1,true

		'Executor.Parameters = "--zoom 1.3 http://"&ServerIP&"/traffic/PasserBase/PasserJudeBatList_Pdf_miaoli.asp?Sys_PasserJude=1&Sys_SendBillSN="&BillSN(i)&"&JpgUnitID="&Session("Unit_ID")&" "&Server.MapPath("\img\"&gInitDT(rs("JudeDate"))&"\"&CaseNo&".pdf")  '執行應用程式所需的參數                      
