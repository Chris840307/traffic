...
if DB_Selt="Selt" then
	...
	if request("Sys_JudeKind")="A" then
		...
		strwhere=strwhere&" and Not Exists(Select 'N' from PasserSend where BillSN=a.SN) and Exists(Select 'Y' from PasserJude where TRUNC(sysdate-JudeDate) between 31 and 365 and BillSN=a.SN) and BillStatus<>9"

	elseif request("Sys_JudeKind")="B" then

		strwhere=strwhere&" and Not Exists(Select 'N' from PasserSend where BillSN=a.SN) and Exists(Select 'Y' from PasserJude where BillSN=a.SN) and BillStatus<>9"
				...
				<tr>
					<td><table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
						...
						<select Name="Sys_JudeKind" class="btn1">
							<option value="A"<%if trim(request("Sys_JudeKind"))="A" then response.write " selected"%>>裁決30日後未移送</option>
							<option value="B"<%if trim(request("Sys_JudeKind"))="B" then response.write " selected"%>>所有裁決後未移送</option>
		...
		<td bgcolor="#E0E0E0">
			...
			if DB_Display="show" then
				...
				for i=DBcnt+1 to DBcnt+10+request("sys_MoveCnt")
					...
					response.write "<td class=""font10"">"&trim(gInitDT(rsfound("SENDDATE")))&"</td>"
...
%>					<td class="font10" nowrap align="left">
						...
						<input type='button' value='移送' class="btn3" style="width:60px;height:25px;" onclick='window.open("PasserSendDetail.asp?PBillSN=<%=trim(rsfound("SN"))%>","WebPage3","left=0,top=0,location=0,width=1000,height=600,resizable=yes,scrollbars=yes")' <%if trim(rsfound("BILLSTATUS"))="9" and sys_City="宜蘭縣" then Response.Write "disabled"%>>
...
<%
	...
	<tr>
		<td height="35" bgcolor="#FFDD77" align="center" nowrap>
			...
			<input type="button" name="btnExecel" value="批次移送通知" class="btn3" style="width:130px;height:30px;font-size:16px;" onclick="funSendBat();">

			<input type="button" name="btnExecel" value="行政執行移送電子清冊" class="btn3" style="width:190px;height:30px;font-size:16px;" onclick="funCountryBat();">

			<input type="button" name="Submite32" value="移送案件明細表" class="btn3" style="width:140px;height:30px;font-size:16px;" onclick="funPasserSend_Stop();">
