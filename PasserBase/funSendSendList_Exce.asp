...
<%
	...
	"(select Max(Note) Note from PassersEndArrived where ArriveType=0 and ReturnResonID='2' and PasserSN=PasserBase.sn) Note" &_
	" from PasserBase where RecordStateID=0 and Exists(select 'Y' from "&BasSQL&" where sn=PasserBase.sn) and Exists(select 'Y' from PassersEndArrived where ArriveType=0 and ReturnResonID='2' and PasserSN=PasserBase.sn) "
...
<%
	...
	While not rsdb.eof
		...
		If PageContrl Mod 20 = 1 Then
			...
			Response.Write "<center><font size=""3"">舉發違反道路交通事件通知單寄存送達移送清冊</font></center>"
