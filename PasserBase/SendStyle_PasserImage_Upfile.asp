...
for each formName in upload.objFile ''列出所有上傳的檔案
	...
	if cdbl(TypeFlag) > 0 and trim(Session("User_ID"))<>"" then
		...
		fname = "PasserBase_"&trim(Session("Map_SN"))&"_"&cnt_file & "." & ExName
		file.SaveAs Server.mappath(UpFilePath&fname)   ''儲存檔案

		strSQL="delete PasserImage where Imagefilename='"&fname&"'"
		conn.execute(strSQL)

		strSQL="insert into PasserImage(billsn,ImageFileName,RecordDate,RecordMemberID) "&_
		" values("&trim(Session("Map_SN"))&",'"&fname&"',sysdate,"&Session("User_ID")&")"
		conn.execute(strSQL)
