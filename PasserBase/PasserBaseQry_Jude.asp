...
if DB_Selt="Selt" then
	...
	if request("Sys_JudeKind")="A" then
		...
		strwhere=strwhere&" and TRUNC(sysdate-DeallineDate)>=90 and Not Exists(Select 'N' from PasserJude where BillSN=a.SN) and BillStatus<>9"

	elseif request("Sys_JudeKind")="B" then

		strwhere=strwhere&" and Not Exists(Select 'N' from PasserJude where BillSN=a.SN) and BillStatus<>9"
				...
				<tr>
					<td><table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
						...
						<select Name="Sys_JudeKind" class="btn1">
							<option value="A"<%if trim(request("Sys_JudeKind"))="A" then response.write " selected"%>>逾期三個月未裁決</option>
							<option value="B"<%if trim(request("Sys_JudeKind"))="B" then response.write " selected"%>>所有未裁決</option>
...
%>					<td class="font10" nowrap align="left">
						...
						<input type='button' value='裁決' class="btn3" style="width:60px;height:25px;" onclick='window.open("Passer_Jude.asp?PBillSN=<%=trim(rsfound("SN"))%>","WebPage1","left=0,top=0,location=0,width=950,height=700,status=yes,resizable=yes,scrollbars=yes")' <%if trim(rsfound("BILLSTATUS"))="9" and sys_City="宜蘭縣" then Response.Write "disabled"%>>	
...
<%
	...
	<tr>
		<td height="35" bgcolor="#FFDD77" align="center" nowrap>
			...
			<input type="button" name="btnExecel" value="批次裁決通知" class="btn3" style="width:120px;height:30px;font-size:16px;" onclick="funJudeBat();">

			<input type="button" name="btnExecel" value="裁決清冊" class="btn3" style="width:80px;height:30px;font-size:16px;" onclick="funJudeListExecel()">
	...
	<tr>		
		<td height="35" bgcolor="#FFFFFF" align="center" nowrap>
			...
			<input type="button" name="btnExecel" value="裁決清冊" class="btn3" style="width:80px;height:30px;font-size:14px;" onclick="funJudeListExecel()">
