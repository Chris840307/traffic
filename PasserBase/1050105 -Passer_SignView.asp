...
if not rsState.eof Then
    ...
    If DateDiff("d",CDate(date),trim(rsState("DealLineDate")))>-1 Then 
	  Sys_ForFeit1=trim(rsState("Level1"))
    Else
      Sys_ForFeit1=trim(rsState("Level2"))
    End If 
...
<table border="1" cellpadding="0" cellspacing="0" style="border-collapse: collapse" bordercolor="#111111" width="599" id="AutoNumber1" height="85%">
  ...
  <tr>
    <td width="686" height="535" colspan="4"><span align="center"><font face="標楷體" size="4">受處份人<%=Sys_Driver%>於　<%=Sys_Date(0)%>年　<%=cdbl(Sys_Date(1))%>月　<%=cdbl(Sys_Date(2))%>日　<%=Sys_tHH%>時　<%=cdbl(Sys_tMI)%>分，在<%=replace(trim(Sys_IllegalAddress),"台","臺")%> </font></span>
    <p><font face="標楷體">
    <br><font size="4">&nbsp;　
...
<br>	　　<%If Sys_rule1="8210101" Then
 ...
 <table border="0" cellspacing="0" width="640" cellpadding="0" bordercolor="#111111" style="border-collapse: collapse" height="62"> 
    ...
    <font face="標楷體" size="4">依道路交通管理處罰條例第　<%=Sys_rule1_1%> 條第　<%=cdbl(Sys_rule1_2)%>　項第 <%=cdbl(Sys_rule1_3)%>　款<%
	if Sys_Rule2<>"" then Response.Write "與第"&Sys_rule2_1&"條第"&Sys_rule2_2&"項第"&Sys_rule2_3&"款"
	%>規定裁決。</font></td>
  </tr>
  <tr>
...
<%=BillPageUnit%><%=UrgeNo%><%=UOpenGovNumber%>號</font></td>
    ...
    <td width="178" height="19" ><font face="標楷體" size="4">　<%=UJudeDate(0)%>年<%=cdbl(UJudeDate(1))%>月<%=cdbl(UJudeDate(2))%>日</font></td>
</table>    
</td>
  </tr>
  <tr>
