...
if DB_Selt="Selt" then
	...
	if request("Sys_type")="1" then
		strwhere=strwhere&" and Exists(select 'Y' from PasserSendArrived where TRUNC(sysdate-ArrivedDate)>30 and ImageFileName is null and PASSERSN=passerBase.sn)"

	elseif request("Sys_type")="2" then
		strwhere=strwhere&" and Exists(select 'Y' from PasserJude where TRUNC(sysdate-JudeDate)>30 and billsn=passerBase.sn) and not exists(select 'N' from PasserImage where BillSN=passerBase.sn and PkeySN=passerBase.sn and ImgKindID='1' and ImgTypeID='1')"

	elseif request("Sys_type")="3" then
		strwhere=strwhere&" and Exists(select 'Y' from PasserSendDetail where TRUNC(sysdate-SendDate)>7 and not exists(select 'N' from PasserImage where BillSN=passerBase.sn and PkeySN=PasserSendDetail.sn and ImgKindID='3' and ImgTypeID='1') and billsn=passerBase.sn)"

	elseif request("Sys_type")="4" then
		strwhere=strwhere&" and Exists(select 'Y' from PasserCreditor where PetitionDate is not null and TRUNC(sysdate-PetitionDate)>90 and imagefilename is null and imagefilename2 is null and imagefilename3 is null and imagefilename4 is null and billsn=passerBase.sn)"
	...
	strSQL="select SN,BillNo,IllegalDate,(select UnitName from UnitInfo where UnitID=PasserBase.BillUnitID) UnitName," & _
			...
			"(select min(ArrivedDate) from PasserSendArrived where ReturnResonID=0 and PASSERSN=passerBase.sn) ArrivedDate " & _
			"from passerbase where MemberStation=(select UnitID from UnitInfo where UnitName='"&trim(Request("Sys_UnitName"))&"')" & _
			" and recordstateid=0 and to_number(to_char(illegaldate,'YYYY')) between to_number(to_char(sysdate,'YYYY'))-10 and to_number(to_char(sysdate,'YYYY')) and billstatus <> 9 " & strwhere & " order by UnitName,BillNo"
	...
	strSQL="select to_number(to_char(illegaldate,'YYYY'))-1911 illegal_year," & _
			...
			"from passerbase where MemberStation=(select UnitID from UnitInfo where UnitName='"&trim(Request("Sys_UnitName"))&"') " & _
			"and recordstateid=0 and to_number(to_char(illegaldate,'YYYY')) between to_number(to_char(sysdate,'YYYY'))-10 and to_number(to_char(sysdate,'YYYY')) and billstatus <> 9 group by (to_number(to_char(illegaldate,'YYYY'))-1911) order by illegal_year DESC"
	...
	While Not rs1.Eof
		...
		cntobj.Add rs1("illegal_year")&"_1",cdbl(rs1("ArrivedCnt"))
		cntobj.Add rs1("illegal_year")&"_2",cdbl(rs1("JudeCnt"))
		cntobj.Add rs1("illegal_year")&"_3",cdbl(rs1("SendCnt"))
		cntobj.Add rs1("illegal_year")&"_4",cdbl(rs1("CreditCnt"))
...
<table width="100%" height="100%" border="0">
	<tr>
		<td bgcolor="#FFCC33" height="33"><%=Request("Sys_UnitName")%>上傳狀況明細表</td>
	</tr>
	<tr>
		<td bgcolor="#E0E0E0">
			<table width="100%" height="100%" border="1" cellpadding="1" cellspacing="1">
				<tr align="center">
					...
					response.write "<td bgcolor=""#FFCC33"" align='right'>送達未上傳</td>"
					For i = maxyear to minyear step -1
					...
					response.write "<tr bgcolor='#FFFFFF' align='center'>"
					response.write "<td bgcolor=""#FFCC33"" align='right'>裁決未上傳</td>"
					For i = maxyear to minyear step -1
					...
					response.write "<tr bgcolor='#FFFFFF' align='center'>"
					response.write "<td bgcolor=""#FFCC33"" align='right'>移送未上傳</td>"
					For i = maxyear to minyear step -1
					...
					response.write "<tr bgcolor='#FFFFFF' align='center'>"
					response.write "<td bgcolor=""#FFCC33"" align='right'>債權未上傳</td>"
					For i = maxyear to minyear step -1
			...
			<table width="100%" height="100%" border="1" cellpadding="1" cellspacing="1">
				<tr bgcolor="#EBFBE3" align="center">
					...
					while Not rs.eof 
						...
						If not ifnull(rs("Forfeit2")) and rs("Forfeit2")>0 Then Response.Write "/"&rs("Forfeit2")
						If not ifnull(rs("Forfeit3")) and rs("Forfeit3")>0 Then Response.Write "/"&rs("Forfeit3")
						Response.Write "</td>"

						response.write "<td align='left'>"&gInitDT(trim(rs("ArrivedDate")))&"&nbsp;</td>"
