...
if DB_Selt="Selt" then
	...
	if request("Sys_SendKind")="A" then
		...
		strwhere=strwhere&" and not exists(select 'N' from PasserCreditor where billsn=a.SN and PetitionDate is not null) and BillStatus<>9"

	elseif request("Sys_SendKind")="B" then

		strwhere=strwhere&" and Exists(select 'Y' from PasserSendDetail where not exists(select 'N' from PasserCreditor where SendDetailSN=PasserSendDetail.SN and PetitionDate is not null) and billsn=a.sn) and BillStatus<>9"
...
if strwhere="" then
	...
	strwhere=strwhere&" and Exists(Select 'Y' from PasserSend where billsn=a.sn and SendDate between TO_DATE('"&(year(now)-2)&"/01/01 00:00:00','YYYY/MM/DD/HH24/MI/SS') and TO_DATE('"&(year(now)-1)&"/12/31 00:00:00','YYYY/MM/DD/HH24/MI/SS')) and not exists(select 'N' from PasserCreditor where billsn=a.SN and PetitionDate is not null) and BillStatus<>9"
				...
				<tr>
					<td><table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
						...
						<select Name="Sys_SendKind" class="btn1">
							<option value="A"<%if trim(request("Sys_SendKind"))="A" then response.write " selected"%>>首次移送未取得債權</option>
							<option value="B"<%if trim(request("Sys_SendKind"))="B" then response.write " selected"%>>再次移送未取得債權</option>
							<option value="C"<%if trim(request("Sys_SendKind"))="C" then response.write " selected"%>>全部</option>
						...
						<tr>
							...
							<td colspan=2>								
								...
								<input type="button" name="btnCls" value="清除" class="btn3" style="width:60px;height:25px;" onClick="location='PasserBaseQry_Creditor.asp'">
