...
elseIf not ifnull(rsfound("ArrivedDate")) Then
	...
	If rsfound("ReturnResonID") = "1" Then
		...
		If instr(rsfound("arrivedNote"),"國外") > 0 Then
			MakeSureDate=split(gArrDT(DateAdd("d",80,rsfound("ArrivedDate"))),"-")
			LimitDate=split(gArrDT(DateAdd("d",80,rsfound("ArrivedDate"))),"-")
		End if 
...
If showCreditor=true Then
	
	passerSN=""

	strSQL="select min(sn) sn from PasserSendDetail where BillSN="&trim(BillSN(i))
	set rspay=conn.execute(strSQL)
	If not rspay.eof Then passerSN=cdbl(rspay("sn"))
	rspay.close
...
If sys_City="彰化縣" then
	If sys_City="高雄市" or sys_City="彰化縣" or sys_City="金門縣" or sys_City="苗栗縣" or sys_City="保二總隊第二大隊第一中隊" or sys_City="保二總隊三大隊二中隊" Then
		DelphiASPObj.GenSendStoreBillno imgName,0,57,160,1
	else
		DelphiASPObj.GenSendStoreBillno imgName,0,57,160
	end If 
end If 

%>
<table width="95%" height="1%" border="0" cellspacing=0 cellpadding=0>
<tr><td align="right">

		<table width="220" border="1" cellspacing=0 cellpadding=3>
		  <tr>
			<td width="80" class="style1">移送案號</td>
		...
		<table width="95%" height="90%" border="1" cellspacing=0 cellpadding=1>
		  <tr>
			<td colspan="5" height="40" class="style2">
			
				<%
					If sys_City="彰化縣" then
						Response.Write "<div style=""position:absolute;"">"
						Response.Write "<img src=""../BarCodeImage/"&imgName&".jpg"">"
						Response.Write "</div>"

						Response.Write "<div style=""position:relative;left:135px;"">"
						Response.Write thenPasserCity&replace(DB_UnitName,trim(thenPasserCity),"")&"行政執行案件移送書"
						Response.Write "</div>"
		  ...
		  <tr>
			...
			<td colspan="2"><%
			if Not rsfound.eof then
				If not ifnull(Trim(rsfound("AgentID"))) Then
					If Mid(Trim(rsfound("AgentID")),2,1)="1" Then
						Response.write "男"
					elseif Mid(Trim(rsfound("AgentID")),2,1)="2" Then
						Response.write "女"
					End if
				End if
			end if%>&nbsp;</td>
		  ...
		  <tr>
			<td class="style6">義務發生之原因或<br>裁處罰鍰之法令依據</td>
			<td colspan="2" class="style3"><%
...
'				end if
		...
		<tr>
			...
			<td class="<%
			'宜爛分局
			If Trim(Session("Unit_ID"))="TM00" Then
				response.write "style6"
			Else
			...
			<td rowspan="2" class="style1"><%
				strchk="select UrgeTypeID from PasserUrge where BillSN="&rsfound("BillSN")&" and BillNo='"&rsfound("BillNo")&"'"
	
				set rschk=conn.execute(strchk)
				if not rschk.eof then 
					
					If trim(rschk("UrgeTypeID")) = "0" Then
						response.write "■"
					else
						response.write "□"
					end if
					...
					if trim(rschk("UrgeTypeID"))="1" then
						response.write "■"
					else
						response.write "□"
					end if
					Response.Write "明信片或信函方式催繳<br>"

					if trim(rschk("UrgeTypeID"))="2" then
						response.write "■"
						Response.Write "催繳書<br>"
					else
						response.write "□"
						Response.Write "其他方式（方式為　）<br>"
					end if
				...
				<table border="0" width="100%" cellspacing="0" cellpadding="0">
					...
					<tr>
						...
						If showCreditor Then
							If not ifnull(rsfound("AgentAddress")) Then
