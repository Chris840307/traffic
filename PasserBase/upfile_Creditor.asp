...
for each formName in upload.objFile ''列出所有上傳的檔案
	...
	if cdbl(TypeFlag) > 0 then

		If cnt_file = 1 Then

			fname = "PassersCreditor_"&trim(Session("Map_SN")) & "." & ExName
			...
			strSQL="Update PasserCreditor set Imagefilename='"&fname&"' where SN="&trim(Session("Map_SN"))

		else

			fname = "PassersCreditor_"&trim(Session("Map_SN"))&"_"&cnt_file & "." & ExName
			...
			strSQL="Update PasserCreditor set Imagefilename"&cnt_file&"='"&fname&"' where SN="&trim(Session("Map_SN"))
