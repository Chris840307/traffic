...
strSQL = "select ZipID,ZipName from Zip where ZipName like '"&replace(left(trim(request("DriverAddress")),3),"臺","台")&"%'"
	...
	strUpd="update PasserBase set BillTypeID='"&trim(theBilltype)&"'" &_
		...
		",BillMemID1='"&trim(request("BillMemID1"))&"',BillMem1='"&trim(request("BillMemName11"))&"'" &_
		",BillMemID2='"&trim(request("BillMemID2"))&"',BillMem2='"&trim(request("BillMemName12"))&"'" &_
		",BillMemID3='"&trim(request("BillMemID3"))&"',BillMem3='"&trim(request("BillMemName13"))&"'" &_
		",BillMemID4='"&trim(request("BillMemID4"))&"',BillMem4='"&trim(request("BillMemName14"))&"'" &_
		",BillFillerMemberID='"&trim(request("BillMemID1"))&"',BillFiller='"&trim(request("BillMemName11"))&"'" &_
	...
	<form name="myForm" method="post">  
		<table width='993' border='1' align="left" cellpadding="1">
			...
			if trim(rs1("Billno"))<>"" and not isnull(rs1("Billno")) then
				...
				%>" onkeydown="funTextControl(this);" size="10" maxlength="9" readonly></td>
			...
			if trim(rs1("Driverid"))<>"" and not isnull(rs1("Driverid")) then
				...
				<%If Trim(sys_City)="台南市" then%>
					<input type="text" size="5" value="<%=trim(rs1("DriverZip"))%>" name="DriverZip" onkeydown="funTextControl(this);">
				<%else%>
					<input type="hidden" size="5" value="<%=trim(rs1("DriverZip"))%>" name="DriverZip">
				<%end if%>
			...
			<tr>
				...
				<td bgcolor="#FFFFCC" align="right"><span class="style5">＊</span>應到案處所</td>
				<td>
				...
				<td bgcolor="#FFFFCC" align="right"><span class="style5">＊</span>舉發人臂章1</td>
		  		<td>
					<input type="text" size="4" value="<%
			...
			<tr>
				...
				<td bgcolor="#FFFFCC" align="right" width="14%">舉發人臂章2</td>
				<td width="20%">
					<input type="text" size="4" value="<%
				...
				<td bgcolor="#FFFFCC" align="right" width="13%">舉發人臂章3</td>
				<td width="20%">
					<input type="text" size="4" value="<%
				...
				<td bgcolor="#FFFFCC" align="right" width="13%">舉發人臂章4</td>
				<td width="20%">
					<input type="text" size="4" value="<%
...
<%else%>
MoveTextVar("Billno1,DriverName,DriverBrith||DriverPID,DriverAddress||IllegalDate,IllegalTime||IllegalAddressID,IllegalAddress||Rule1||Rule2||DealLineDate,MemberStation,BillMem1||BillMem2,BillMem3,BillMem4||SignType||BillUnitID||BillFillDate,Note");
<%end if%>
//新增告發單
...
function getBillMemID1(){
	if (event.keyCode==116){	
		...
		if (myForm.BillMem1.value.length > 1){
			...
			runServerScript("getBillPeoPleMemID.asp?MemOrder=1&MemID="+BillMemNum);
...
function getBillMemID2(){
	if (event.keyCode==116){	
		...
		if (myForm.BillMem2.value.length > 1){
			...
			runServerScript("getBillPeoPleMemID.asp?MemOrder=2&MemID="+BillMemNum);
...
function getBillMemID3(){
	if (event.keyCode==116){	
		...
		if (myForm.BillMem3.value.length > 1){
			...
			runServerScript("getBillPeoPleMemID.asp?MemOrder=3&MemID="+BillMemNum);
...
function getBillMemID4(){
	if (event.keyCode==116){	
		...
		if (myForm.BillMem4.value.length > 1){
			...
			runServerScript("getBillPeoPleMemID.asp?MemOrder=4&MemID="+BillMemNum);
