...
if Isnull(rscnt("StoreAndSendMailNumber")) then
	strSQL="Update BillMailHistory set StoreAndSendMailNumber=MailNumber_Sn.NextVal where BillSN="&PBillSN(i)
	conn.execute(strSQL)
end if
rscnt.close
...
rs.close
strSql="select StoreAndSendMailNumber,MailTypeID,MailDate,MAILCHKNUMBER,FirstBarCode from BillMailHistory where BILLSN="&PBillSN(i)
...
if trim(Sys_BillTypeID)="1" then
	...
	DelphiASPObj.GenBillPrintBarCode PBillSN(i),Sys_BillNo,Sys_Rule1,Sys_CarNo,Sys_StoreAndSendMailNumber,"220073",Sys_DriverZip,Sys_DealLineDate(0)&Sys_DealLineDate(1)&Sys_DealLineDate(2),Sys_StationID,"台北市交通事件裁決所",0,Sys_Level1,0,True,False,Sys_MailDate
	'response.write "DelphiASPObj.GenBillPrintBarCode"& PBillSN(i)&","&Sys_BillNo&","&Sys_Rule1&","&Sys_CarNo&","&Sys_StoreAndSendMailNumber&",220073,"&Sys_DriverZip&","&Sys_DealLineDate(0)&Sys_DealLineDate(1)&Sys_DealLineDate(2)&","&Sys_StationID&",台北市交通事件裁決所,0,"&Sys_Level1&",0,True,False,"&Sys_MailDate
	'response.end
else
	Set DelphiASPObj = Server.CreateObject("GenBarCode.BarCodeASP") 
...
err.Clear
%>
<div style="position:absolute; left:10px; top:<%=410+1300*i%>px;">
...
<div id="Layer12" style="position:absolute; left:115px; top:<%=930+1300*i%>px; width:450px; height:11px; z-index:3"><span class="style7"><%if trim(Sys_BillTypeID)="1" then
				response.write Sys_DriverHomeZip&Sys_DriverZipName&Sys_DriverAddress
			elseif trim(Sys_BillTypeID)="2" then
				response.write Sys_OwnerZip&Sys_OwnerZipName&Sys_OwnerAddress
			end if%></span></div>
...
<!--<div id="Layer34" style="position:absolute; left:45px; top:<%=990+1300*i%>px; width:400px; height:30px; z-index:28"><img src=<%="""../BarCodeImage/"&Sys_BillNo&"_5.jpg"""%>></div>-->
<%next%>
</body>
</html>
