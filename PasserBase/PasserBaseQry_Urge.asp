...
if DB_Selt="Selt" then
	...
	if request("Sys_JudeKind")="A" then

		strwhere=strwhere&" and Not Exists(Select 'N' from PasserUrge where BillSN=a.SN) and Exists(Select 'Y' from PasserJude where TRUNC(sysdate-JudeDate) between 31 and 365 and BillSN=a.SN) and BillStatus<>9"

	elseif request("Sys_JudeKind")="B" then

		strwhere=strwhere&" and Not Exists(Select 'N' from PasserUrge where BillSN=a.SN) and Exists(Select 'Y' from PasserJude where BillSN=a.SN) and BillStatus<>9"
			...
			<table width="100%" border="0" cellpadding="5" cellspacing="1" bgcolor="#FFFFFF">
				<tr>
					<td><table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
						...
						<select Name="Sys_JudeKind" class="btn1">
							<option value="A"<%if trim(request("Sys_JudeKind"))="A" then response.write " selected"%>>一年內裁決後未催告</option>
							<option value="B"<%if trim(request("Sys_JudeKind"))="B" then response.write " selected"%>>所有裁決後未催告</option>
							<option value="C"<%if trim(request("Sys_JudeKind"))="C" then response.write " selected"%>>全部</option>
						...
						<tr>
							...
							<td colspan=2>								
								...
								<input type="button" name="btnCls" value="清除" class="btn3" style="width:60px;height:25px;" onClick="location='PasserBaseQry_Urge.asp'">
...
%>					<td class="font10" nowrap align="left">
						...
						<input type='button' value='催告' class="btn3" style="width:60px;height:25px;" onclick='window.open("PasserUrgeDetail.asp?PBillSN=<%=trim(rsfound("SN"))%>","WebPage2","left=0,top=0,location=0,width=1000,height=600,resizable=yes,scrollbars=yes")' <%if trim(rsfound("BILLSTATUS"))="9" and sys_City="宜蘭縣" then Response.Write "disabled"%>>
						
						<input type='button' value='送達' class="btn3" style="width:60px;height:25px;" onclick='window.open("PasserSendArrived.asp?BillSN=<%=trim(rsfound("SN"))%>","WebPage4","left=0,top=0,location=0,width=900,height=575,resizable=yes,scrollbars=yes")'>
...
<%
	...
	<tr>
		<td height="35" bgcolor="#FFDD77" align="center" nowrap>
			...
			<input type="button" name="btnExecel" value="郵局大宗函件" class="btn3" style="width:120px;height:30px;font-size:16px;" onclick="funPasserMailMoney();">
			
			<input type="button" name="btnExecel" value="批次催繳通知" class="btn3" style="width:120px;height:30px;font-size:16px;" onclick="funUrgeBat();">

			<input type="button" name="btnExecel" value="催告清冊" class="btn3" style="width:120px;height:30px;font-size:16px;" onclick="funSendExecel()">
