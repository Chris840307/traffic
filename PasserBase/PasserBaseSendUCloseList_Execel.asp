...
fname=year(now)&fMnoth&fDay&"_催告催繳逾期未到案裁決清冊.xls"
...
<table width="700" border="1">
	<tr><td>
		<table width="100%" border="0" cellpadding="4" cellspacing="1">
			<tr>
				<td align="center"><strong>催告催繳逾期未到案裁決清冊</strong></td>
			</tr>
			<tr>
	...
	<tr>
		<td>
			<table width="100%" border="1" cellpadding="4" cellspacing="1">
				...
				filecnt=0:sumFile=0:tmpDriverID="":sumNT=0:cntNt=0
				
				if Not rsfound.eof then
					tmpDriverID=trim(rsfound("DriverID"))
					if Not isnull(rsfound("ForFEIT1")) then sumNT=Cint(rsfound("ForFEIT1"))
				end if
				while Not rsfound.eof
					...
					response.write "<td>"&trim(rsfound("ForFEIT1"))&"</td>"
					response.write "</tr>"
					filecnt=filecnt+1
					rsfound.MoveNext
					if Not rsfound.eof then
						if trim(rsfound("DriverID"))=trim(tmpDriverID) then
							if Not isnull(rsfound("ForFEIT1")) then
								sumNT=sumNT+Cint(rsfound("ForFEIT1"))
							end if
				...
				response.write "</table>"
				response.write "</td></tr>"
				response.write "<tr><td><table border=0><td align=""right"" colspan=""0"">&nbsp;</td>"
				response.write "<td align=""right"">小計：</td>"
				response.write "<td align=""right"">"&filecnt&"筆"&"</td>"
				response.write "<td align=""right"">"&sumNT&"</td></tr></table></td></tr>"
				cntNt=cntNt+sumNT
