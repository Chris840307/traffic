...
hid_BillTypeID=Request("hid_BillTypeID")

If ifnull(hid_BillTypeID) Then hid_BillTypeID=1

%>
<!--#include virtual="traffic/BillKeyIn/getBillno_people.asp"-->
<%
...
'此張照片無效
if trim(request("kinds"))="Pic_Delete" then
	'strUpdate2="update ProsecutionImageDetail set VERIFYRESULTID=-1 where Operator='"&trim(Session("Credit_ID"))&"' and VERIFYRESULTID=1 and billsn is null"
	strUpdate2="delete from PIDetail where Operator='"&trim(Session("Credit_ID"))&"' and filename='"&request("SelFileName")&"'"
	Conn.execute strUpdate2
	strUpdate2b="delete from PI where OperatorA='" & trim(Session("Credit_ID")) & "' and filename='"&request("SelFileName")&"'"
	Conn.execute strUpdate2b
	'response.write strUpdate2
end if
...
strSQL = "select ZipID,ZipName from Zip where ZipName like '"&replace(left(trim(request("DriverAddress")),3),"臺","台")&"%'"
	...
	End if	
	strUpd="update PasserBase set BillNo='"&UCase(trim(request("Billno1")))&"',CarNo='"&UCase(trim(request("Sys_CarNo")))&"',IllegalDate="&theIllegalDate&",CARSIMPLEID=null,Insurance=null,IllegalSpeed=null,RuleSpeed=null"&_
		...
		If not ifnull(request("Sys_CarNo")) Then
			If chkrule1 >=69 and chkrule2>=69 Then
				...
				strUpd="update PasserBase set CARSIMPLEID=8,ProjectID='traffic009' where SN="&trim(request("PBillSN"))
				conn.execute strUpd

				If not ifnull(request("Rule1")) Then

					strSQL="select nvl(min(level1),0) forfeit from law where version=2 and itemid='"&trim(request("Rule1"))&"' and CARSIMPLEID='F'"

					set rs=conn.execute(strSQL)
					If not rs.eof Then
						If cdbl(rs("forfeit")) > 0 Then

							strUpd="update PasserBase set forfeit1="&rs("forfeit")&" where SN="&trim(request("PBillSN"))
				...
				If not ifnull(request("Rule2")) Then
					...
					strSQL="select nvl(min(level1),0) forfeit from law where version=2 and itemid='"&trim(request("Rule2"))&"' and CARSIMPLEID='F'"

					set rs=conn.execute(strSQL)
					If not rs.eof Then
						If cdbl(rs("forfeit")) > 0 Then

							strUpd="update PasserBase set forfeit2="&rs("forfeit")&" where SN="&trim(request("PBillSN"))
				...
				If not ifnull(request("Rule3")) Then

					strSQL="select nvl(min(level1),0) forfeit from law where version=2 and itemid='"&trim(request("Rule3"))&"' and CARSIMPLEID='F'"

					set rs=conn.execute(strSQL)
					If not rs.eof Then
						If cdbl(rs("forfeit")) > 0 Then

							strUpd="update PasserBase set forfeit3="&rs("forfeit")&" where SN="&trim(request("PBillSN"))
...
strSQL = "select ZipID,ZipName from Zip where ZipName like '"&replace(left(trim(request("DriverAddress")),3),"臺","台")&"%'"
		...
		'查流水號
		strSN="select passerbase_seq.nextval as SN from Dual"
		...
		If not ifnull(request("Billno1")) Then

			Sys_Billno=UCase(trim(request("Billno1")))

		elseIf hid_BillTypeID = 2 Then

			Sys_Billno=UCase(funcBillToDCICaseIn())
		...
		strInsert="insert into PasserBase(SN,BillTypeID,BillNo,CarNo,IllegalDate" & _
				...
				",BillFillerMemberID,BillFiller,IllegalSpeed,RuleSpeed" &_
				",BillFillDate,DealLineDate,BillStatus,RecordStateID,RecordDate,RecordMemberID" &_
				",Note,RuleVer,IsLecture,DoubleCheckStatus"&ColAdd&")" &_
				" values("&theSN&", '"&trim(hid_BillTypeID)&"','"&Sys_Billno&"','"&UCase(trim(request("Sys_CarNo")))&"'" &_
				...
				",'"&trim(request("MemberStation"))&"','"&trim(request("BillUnitID"))&"'" &_
				...
				If trim(request("ProjectID"))="traffic009" then

					strUpd="update PasserBase set CARSIMPLEID=8" &_
					" where SN="&theSN
	...
	'查流水號
		...
		If hid_BillTypeID <> 2 Then
			...
			strUpd="update PasserBase set" &_
				" DriverID='"&UCase(trim(request("DriverPID")))&"',DriverBirth="&theDriverBirth &_
				",Driver='"&trim(request("DriverName"))&"',DriverAddress='"&trim(Sys_DriverAddress)&"'" &_
				",DriverZip='"&trim(zipid)&"',DriverSex='"&trim(request("DriverSex"))&"'" &_
				",SignType='"&UCase(trim(request("SignType")))&"',Insurance="&theInsurance &_
				" where SN="&theSN
			conn.execute strUpd
		...
		If not ifnull(request("Sys_CarNo")) Then
			If chkrule1 >=69 and chkrule2>=69 Then
				...
				strUpd="update PasserBase set CARSIMPLEID=8,ProjectID='traffic009' where SN="&theSN
				conn.execute strUpd
				...
				If not ifnull(request("Rule1")) Then
					...
					If not rs.eof Then
						If cdbl(rs("forfeit")) > 0 Then
							...
							strUpd="update PasserBase set forfeit1="&rs("forfeit")&" where SN="&theSN
				...
				If not ifnull(request("Rule2")) Then
					...
					If not rs.eof Then
						If cdbl(rs("forfeit")) > 0 Then
							...
							strUpd="update PasserBase set forfeit2="&rs("forfeit")&" where SN="&theSN
				...
				If not ifnull(request("Rule3")) Then
					...
					If not rs.eof Then
						If cdbl(rs("forfeit")) > 0 Then
							...
							strUpd="update PasserBase set forfeit3="&rs("forfeit")&" where SN="&theSN
	...
	'影像存檔passerillegalimage*************************
		'台中市要填告發單號
		if sys_City="台中市" Then
			If Trim(request("ReportNo"))<>"" Then
				strReportNo="insert into PasserReportNo(BillSN,ReportNo)" &_
		...
		theImageFileNameB=trim(request("gImageFileNameB"))
		theImageFileNameC=trim(request("gImageFileNameC"))

		strBillImage="Insert Into PasserILLEGALIMAGE(BillSn,BillNo,ImageFileNameA,ImageFileNameB,ImageFileNameC," &_
		"IISImagePath) " &_
		"values("&theSN&",'"&UCase(trim(request("Billno1")))&"','"&trim(theImageFileName)&"'" &_
		",'"&trim(theImageFileNameB)&"','"&trim(theImageFileNameC)&"','"&trim(theImagePathName)&"')"

		strUpdate2="delete from PIDetail where FileName='"&request("SelFileName")&"' and Operator='" & trim(request("SelOperator")) & "'"
		Conn.execute strUpdate2
		strUpdate2b="delete from PI where FileName='"&request("SelFileName")&"' and OperatorA='" & trim(request("SelOperator")) & "'"
		Conn.execute strUpdate2b

		if trim(request("gFileName1"))<>"" then
			strdel1="delete from PIDetail where FileName='"&trim(request("gFileName1"))&"' and Operator='" & trim(request("gOperator1")) & "'"
			Conn.execute strdel1
			
			strdel1b="delete from PI where FileName='"&trim(request("gFileName1"))&"' and OperatorA='" & trim(request("gOperator1")) & "'"
			Conn.execute strdel1b
		end if 

		if trim(request("gFileName2"))<>"" then
			strdel1="delete from PIDetail where FileName='"&trim(request("gFileName2"))&"' and Operator='" & trim(request("gOperator2")) & "'"
			Conn.execute strdel1
			
			strdel1b="delete from PI where FileName='"&trim(request("gFileName2"))&"' and OperatorA='" & trim(request("gOperator2")) & "'"
			Conn.execute strdel1b
		end if 
		...
		<table width='1000' border='1' align="center" cellpadding="1">
			<tr bgcolor="#1BF5FF">
				<td colspan="6" nowrap>
					...
					%>
					<br>
					<input type="button" value="上傳照片" style="width:120px;" onclick='window.open("http://10.116.99.233/IllegalImageUpload/UploadillegalImage.aspx?CityName=TC&TypeID=8&UserID=<%=Trim(Session("Credit_ID"))%>","UploadillegalImage","left=0,top=0,location=0,width=770,height=555,resizable=yes,scrollbars=yes,status=yes")'>
					<input type="button" value="重整畫面" style="width:120px;" onclick='location.href=location.href;'>
					<input type="button" name="Submit2CF32" onClick="funAllNotKeyInVerifyResult();" value="清除未建檔照片(共 <%
					...
					%> 筆)" >
					&nbsp; 
					<a href="微電車逕舉相片建檔系統使用手冊.docx" target="_blank" class="style2">操作手冊(請按滑鼠右鍵另存目標) </a>	
					<%End if 
					%>
				</td>
			</tr>
...
'if trim(request("Filename1"))<>"" then
	if sys_City="台中市" then
		strSQLP1="select b.CarNo,b.CarSimpleID,a.ProsecutionTime,a.ProsecutionTypeID,a.SiteCode,a.FileName,a.DirectoryName,a.FIXEQUIPTYPE,a.VideoFileName,a.IMAGEFILENAMEA,a.IMAGEFILENAMEB,a .IMAGEFILENAMEC,a.Location,b.LawItemID,b.SN,a.LIMITSPEED,a.OVERSPEED,a.OperatorA,b.MemberID,b.Note from PI a, PIDetail b where a.FILENAME = b.FILENAME and a.OperatorA=b.Operator and b.Operator='"&trim(Session("Credit_ID"))&"' and FixEquipType in (1,2,5,8,10) and VERIFYRESULTID<>-1 and BillSn is null and (a.RejectCode<>'262' or a.RejectCode is null)" 
	else
		strSQLP1="select b.CarNo,b.CarSimpleID,a.ProsecutionTime,a.ProsecutionTypeID,a.SiteCode,a.FileName,a.DirectoryName,a.FIXEQUIPTYPE,a.VideoFileName,a.IMAGEFILENAMEA,a.IMAGEFILENAMEB,a .IMAGEFILENAMEC,a.Location,b.LawItemID,b.SN,a.LIMITSPEED,a.OVERSPEED,a.OperatorA,b.MemberID,b.Note from PI a, PIDetail b where a.FILENAME = b.FILENAME and a.OperatorA=b.Operator and b.Operator='"&trim(Session("Credit_ID"))&"' and FixEquipType in (1,2,5,8,10) and VERIFYRESULTID<>-1 and BillSn is null and (a.RejectCode<>'262' or a.RejectCode is null) and a.FileName='" & trim(request("Filename1")) & "'" 
	end if 
	set rsP1=conn.execute(strSQLP1)
	If Not rsP1.eof Then 
		sImgWebPath = ""
		sImgWebPath=toImageDir(rsP1("ProsecutionTime"))

		firstIMAGEFILENAMEA=trim(rsP1("IMAGEFILENAMEA"))
		piIMAGEPATHNAMEA = replace(sImgWebPath & replace(trim(rsP1("DirectoryName")),"\","/") & "/" ,"//","/")

		sFIXEQUIPTYPE=""
		if trim(rsP1("FIXEQUIPTYPE"))="1" then
			sFIXEQUIPTYPE="Type1"
		elseif trim(rsP1("FIXEQUIPTYPE"))="2" then
			sFIXEQUIPTYPE="Type2"
		elseif trim(rsP1("FIXEQUIPTYPE"))="3" then
			sFIXEQUIPTYPE="Type3"
		elseif trim(rsP1("FIXEQUIPTYPE"))="5" then
			sFIXEQUIPTYPE="Type5"
		end if
		
		if sFIXEQUIPTYPE="Type3" then
			RealFileName=right(rsP1("FileName").value,4)
			WebPicPathTmp=left(rsP1("FileName").value,14)
		end if

		if trim(rsP1("FixEquipType"))="8" Then
			If HowCatchPicture="0" then
				if len(trim(rsP1("IMAGEFILENAMEA")))>14 then
					Type_IMAGEFILENAMEA=right(trim(rsP1("IMAGEFILENAMEA")),len(trim(rsP1("IMAGEFILENAMEA")))-14)
				else
					Type_IMAGEFILENAMEA=trim(rsP1("IMAGEFILENAMEA"))
				end If
			Else
				Type_IMAGEFILENAMEA=trim(rsP1("IMAGEFILENAMEA"))
			End If 
		...
		If sys_City="彰化縣" Or sys_City="金門縣" Then
			...
			If sys_City="金門縣" Then
				FileLocation="F:\image\finish"&trim(rsP1("DirectoryName"))
			ElseIf sys_City="彰化縣" Then 
				FileLocation="E:\image\finish"&trim(rsP1("DirectoryName"))
			End If 
		...
		if HowCatchPicture="0" then
			...
			bPicWebPath=replace(replace(sImgWebPath & trim(rsP1("DirectoryName")),"\","/") & "/" & Type_IMAGEFILENAMEA,"//","/")
		end if
%>
		<input type="hidden" name="SelFileName" value="<%=trim(rsP1("FileName"))%>">
		<input type="hidden" name="SelSN" value="<%=trim(rsP1("SN"))%>">
		<input type="hidden" name="SelOperator" value="<%=trim(rsP1("OperatorA"))%>">
<%
	end if
	rsP1.close
	set rsP1=nothing 
...
<%if trim(request("Filename2"))<>"" then
	strSQLP2="select b.CarNo,b.CarSimpleID,a.ProsecutionTime,a.ProsecutionTypeID,a.SiteCode,a.FileName,a.DirectoryName,a.FIXEQUIPTYPE,a.VideoFileName,a.IMAGEFILENAMEA,a.IMAGEFILENAMEB,a .IMAGEFILENAMEC,a.Location,b.LawItemID,b.SN,a.LIMITSPEED,a.OVERSPEED,a.OperatorA,b.MemberID,b.Note from PI a, PIDetail b where a.FILENAME = b.FILENAME and a.OperatorA=b.Operator and b.Operator='"&trim(Session("Credit_ID"))&"' and FixEquipType in (1,2,5,8,10) and VERIFYRESULTID<>-1 and BillSn is null and (a.RejectCode<>'262' or a.RejectCode is null) and a.FileName='" & trim(request("Filename2")) & "'" 
	set rsP2=conn.execute(strSQLP2)
...
<%if trim(request("Filename3"))<>"" then
	strSQLP3="select b.CarNo,b.CarSimpleID,a.ProsecutionTime,a.ProsecutionTypeID,a.SiteCode,a.FileName,a.DirectoryName,a.FIXEQUIPTYPE,a.VideoFileName,a.IMAGEFILENAMEA,a.IMAGEFILENAMEB,a .IMAGEFILENAMEC,a.Location,b.LawItemID,b.SN,a.LIMITSPEED,a.OVERSPEED,a.OperatorA,b.MemberID,b.Note from PI a, PIDetail b where a.FILENAME = b.FILENAME and a.OperatorA=b.Operator and b.Operator='"&trim(Session("Credit_ID"))&"' and FixEquipType in (1,2,5,8,10) and VERIFYRESULTID<>-1 and BillSn is null and (a.RejectCode<>'262' or a.RejectCode is null) and a.FileName='" & trim(request("Filename3")) & "'" 
	set rsP3=conn.execute(strSQLP3)
	If Not rsP3.eof Then 
		fileName2=trim(rsP3("FileName"))
		PicName2=trim(rsP3("IMAGEFILENAMEA"))
		Operator2=trim(rsP3("OperatorA"))
		if trim(rsP3("FixEquipType"))="8" Then
			If HowCatchPicture="0" then
				if len(trim(rsP3("IMAGEFILENAMEA")))>14 then
					PicName2img=right(trim(rsP3("IMAGEFILENAMEA")),len(trim(rsP3("IMAGEFILENAMEA")))-14)
				else
					PicName2img=trim(rsP3("IMAGEFILENAMEA"))
				end If
			Else
				PicName2img=trim(rsP3("IMAGEFILENAMEA"))
			End if
		...
		If sys_City="彰化縣" Or sys_City="金門縣" Then
			If sys_City="金門縣" Then
				FileLocation="F:\image\finish"&trim(rsP3("DirectoryName"))
			ElseIf sys_City="彰化縣" Then 
				FileLocation="E:\image\finish"&trim(rsP3("DirectoryName"))
			End If 
			...
			set fso3=nothing
		End If 
		PicName2imgPath=replace(sImgWebPath & replace(trim(rsP3("DirectoryName")),"\","/") & "/" & trim(PicName2img),"//","/")
	end if
	rsP3.close
	set rsP3=nothing 
	...
	if sPicWebPath<>"" then %>
		<img src="<%=sPicWebPath&"?nowTime="&now%>" border=1 id="SmallImg2" width="<%
		If sys_City="苗栗縣" Then
			response.write "300"
		else
			response.write "230"
...
<%end if%>
			...
			If sys_City="彰化縣" Then
				response.write Type_IMAGEFILENAMEA
			Else
				response.write firstIMAGEFILENAMEA
			End If 
		...
		<%End If %>
			...
			<% else %>
				<input type="hidden" value="A123456789" name="DriverPID">
				<input type="hidden" value="999" name="DriverName">
			<% end If %>
			...
			<% else %>
				<input type="hidden" value="950101" name="DriverBrith">
				<input type="hidden" value="1" name="DriverSEX">
				<input type="hidden" value="242" name="DriverZip">
			...
			<% If hid_BillTypeID <> 2 Then %>
				<tr>
					...
					While Not rsItem.Eof
						...
						Response.Write "<input type=""checkbox"" class=""btn5"" value="""&trim(rsItem("ID"))&"_"&trim(rsItem("Content"))&""" name=""Fastener1"">"&trim(rsItem("Content"))&"&nbsp;"
			...
			<% else %>
				<input type="hidden" value="" name="Fastener1">
			...
			<tr>
		  	<td bgcolor="#EBE5FF" align="right"><span class="style4">＊</span>舉發單位</td>
				...
				<span id="Layer001" style="font-size:16px; background-color:#CCFFFF;"></span>
					...
					<input type="radio" name="StreetType" value="traffic009" onClick="myForm.ProjectID.value=this.value;ProjectF5();">
					微型電動二輪車
				</td>
			...
			<tr>
			  <td bgcolor="#1BF5FF" align="center" colspan="6">
				...
				if trim(bPicWebPath)="" then
					response.write "disabled"
				end if 
					%> class="btn1">
					<img src="/image/space.gif" width="29" height="8">
					<input type="hidden" name="Submit1343" onClick="ChangUrl();" value="清 除 F4" class="btn1">
					<img src="/image/space.gif" width="29" height="8">
					<input type="hidden" name="Submit5322" onClick="funcOpenBillQry();" value="查 詢 <%=F6StrName%>" class="btn1">
					<img src="/image/space.gif" width="29" height="8">
					...
					<img src="/image/space.gif" width="29" height="8">
					<input type="button" name="Submit4232" onClick="funPic_Delete();" value="本張照片無效" class="btn1">
					<br>
					<input type="hidden" name="Submit5322" onClick="funDBfisrt();" value="第一筆" class="btn1">
					<input type="hidden" name="Submit5322" onClick="funDBupmove();" value="上一筆 PgUp" class="btn1">
					<input type="hidden" name="Submit5322" onClick="funDBdownmove();" value="下一筆 PgDn" class="btn1">
					<input type="hidden" name="Submit5322" onClick="funDBlast();" value="最後一筆" class="btn1">
					<input type="hidden" name="Submit1343" onClick="ChangUrl();" value="新增一筆" class="btn1">
                </span>
				<!-- 違規人性別 -->
				<input type="hidden" value="" name="DBFile">				
				<input type="hidden" value="<%=hid_BillTypeID%>" name="hid_BillTypeID">
				<input type="hidden" value="<%=DBsume%>" name="DBCunt">
				<input type="hidden" value="" name="PBillSN">
				<input type="hidden" value="" name="Sys_BilltypeID">
...
if(myForm.hid_BillTypeID.value==2){
	MoveTextVar("Billno1,Sys_CarNo||IllegalDate,IllegalTime||IllegalAddressID,IllegalAddress||Rule1||Rule2||RuleSpeed,IllegalSpeed||DealLineDate,MemberStation,BillMem1||BillMem2,BillMem3,BillMem4||BillUnitID||BillFillDate,Note");
	...
	MoveTextVar("Billno1,Sys_CarNo,Insurance,ReportNo||DriverPID,DriverSEX,DriverName||DriverBrith,DriverZip,DriverAddress||IllegalDate,IllegalTime||IllegalAddressID,IllegalZip,IllegalAddress||Rule1||Rule2||RuleSpeed,IllegalSpeed||DealLineDate,MemberStation,BillMem1||BillMem2,BillMem3,BillMem4||SignType||BillUnitID||BillFillDate,Note");
}else if(sys_City=="高雄市"){

	MoveTextVar("Billno1,Sys_CarNo,Insurance||DriverPID,DriverSEX,DriverName||DriverBrith,DriverZip,DriverAddress||IllegalDate,IllegalTime||IllegalAddressID,IllegalZip,IllegalAddress||Rule1||Rule2||RuleSpeed,IllegalSpeed||DealLineDate,MemberStation,BillMem1||BillMem2,BillMem3,BillMem4||SignType||BillUnitID||BillFillDate,Note");

}else{

	MoveTextVar("Sys_BatChNumber||Billno1,Sys_CarNo,Insurance||DriverPID,DriverSEX,DriverName||DriverBrith,DriverZip,DriverAddress||IllegalDate,IllegalTime||IllegalAddressID,IllegalAddress||Rule1||Rule2||RuleSpeed,IllegalSpeed||DealLineDate,MemberStation,BillMem1||BillMem2,BillMem3,BillMem4||SignType||BillUnitID||BillFillDate,Note");
...
function funDBupmove(){
	if(myForm.DBFile.value==''){
		...
		runServerScript("BillDBmove.asp?DBFile="+myForm.DBFile.value+"&BillTypeID="+myForm.hid_BillTypeID.value);
...
function InsertBillVase(){
	...
	if (myForm.Billno1.value==""&&myForm.hid_BillTypeID.value!='2'){
		error=error+1;
	...
	if (myForm.ProjectID.value==""){
		if(sys_City=="彰化縣"){
			...
			if (myForm.Rule1.value.substr(0,2)>72 && myForm.Rule1.value.substr(0,2)<77){
				error=error+1;
				errorString=error+"：需要輸入車種。";
			}else if (myForm.Rule2.value.substr(0,2)>72 && myForm.Rule2.value.substr(0,2)<77){
				error=error+1;
				errorString=error+"：需要輸入車種。";
			}
		}

		if(sys_City=="花蓮縣" || sys_City=="台南市" || sys_City=="高雄市"){
	...
	if (myForm.Sys_CarNo.value==""&&myForm.hid_BillTypeID.value=='2'){
		error=error+1;
		errorString=error+"：請輸入車號。";
	}
<%if sys_City="台中市" then %>
	if (myForm.ReportNo.value!=""){
		if (myForm.ReportNo.value.length<9){
			error=error+1;
			errorString=errorString+"\n"+error+"：告示單號不可少於9碼。";
		}
	}
...
<%end if%>
	...
	if (error==0){
		...
		runServerScript("checkPasserBase.asp?DriverID="+myForm.DriverPID.value+"&illegaldate="+myForm.IllegalDate.value+"&IllegalTime="+myForm.IllegalTime.value+"&rule1="+myForm.Rule1.value+"&rule2="+myForm.Rule2.value+"&Sys_CarNo="+myForm.Sys_CarNo.value+"&BillTypeID="+myForm.hid_BillTypeID.value+"&PBillSN="+myForm.PBillSN.value+"&nowtime="+dt);
...
function SpeedChk(){
	...
	if(RuleSpeed.value!='' && IllegalSpeed.value!=''){
		...
		if(Number(RuleSpeed.value) >= Number(IllegalSpeed.value)){
			...
			if(IllegalSpeed.value <= 35){

				obj.value='72000041';
			}else if(IllegalSpeed.value <= 45){
			...
			if(Number(IllegalSpeed.value)>=100){
...
function getDealLineDate(){
	...
	if (BFillDateTemp.length >= 6){
		...
		DLineDate=DateAdd("d",45,BFillDate);
...
function funCarchk(){
	if(myForm.Sys_CarNo.value!=''){
		myForm.Sys_CarNo.value=myForm.Sys_CarNo.value.toUpperCase();

		if(myForm.Sys_CarNo.value.search("-")>0||myForm.Sys_CarNo.value.length!=7){

			alert("格式錯誤！只能輸入微電車車牌！");
			myForm.Sys_CarNo.value='';
			LayerCarSimple.innerHTML='';

		}else if(myForm.Sys_CarNo.value.substr(0,1).charCodeAt()<65 || myForm.Sys_CarNo.value.substr(0,1).charCodeAt()>90){

			alert("格式錯誤！只能輸入微電車車牌！");
			myForm.Sys_CarNo.value='';
			LayerCarSimple.innerHTML='';

		}else if(myForm.Sys_CarNo.value.substr(1,1).charCodeAt()<65 || myForm.Sys_CarNo.value.substr(1,1).charCodeAt()>90){
		...
		}else if(isNaN(myForm.Sys_CarNo.value.substr(2,5))){
			...
			LayerCarSimple.innerHTML='微型電動二輪車';
		}
...
function funTextControl(obj){
	...
	if (obj.name=="MemberStation"&&event.keyCode==<%=F5str%>){	
		window.open("Query_Unit.asp?SType=S","WebPage1","left=0,top=0,location=0,width=800,height=660,resizable=yes,scrollbars=yes");
	}
}
function ChangUrl(){
	if(myForm.hid_BillTypeID.value==2){
		location='BillKeyIn_People.asp?hid_BillTypeID=2';
	}else{
...
function funPic_Delete(){
	if(confirm('確定要將此張影像設為無效？')){
		myForm.kinds.value="Pic_Delete";
		myForm.submit();
	}
