...
If Trim(request("ReportKeyInType"))="1" Then
	Session("BillKeyIn_ReportKeyInType")="1"
else
	Session("BillKeyIn_ReportKeyInType")="0"
End If 
'============================================
	'要到ApConfigure抓法條版本
...
If chkIsSpeedTooOver=0 And checkReportCaseFlag=0 And chkIsRule56Flag=0 and chkIllegalAddress53Flag=0 and chkIllegalAddressID53Flag=0 And chkReKeyInBill=0 And chkReKeyInImgBill=0 then
	'違規日期
	theIllegalDate=""
	if trim(request("IllegalDate"))<>"" then
		theIllegalDate=funGetDate(gOutDT(request("IllegalDate") ) &" "&left(trim(request("IllegalTime")),2)&":"&right(trim(request("IllegalTime")),2),1)
	...
	strInsert="insert into BillBaseTmp(SN,BillTypeID,BillNo,CarNo,CarSimpleID,CarAddID,IllegalDate" & _
				...
				",Note,EquipmentID,RuleVer,DriverSex,ImageFileName"&ColAdd&",JurgeDay,CheckFlag,ReportCaseNo,ReportCreditID)" &_
				...
				""&valueAdd&"," & theJurgeDay & ",'0','"&Trim(request("ReportCaseNo"))&"','"&Trim(request("ReportCreditID"))&"')"
				conn.execute strInsert  
	
	'保留第一張照片
	If Trim(request("KeepImage"))="1" Then
		PIFileNameTemp=""
		...
		If Not rsPI1.eof Then
			...
			End If 
			PIFileNameTemp=Trim(rsPI1("FileName"))&"_"&Right("00"&hour(now),2)&Right("00"&minute(now),2)&Right("00"&second(now),2)
			strPIadd="insert into PI(FileName,DIRECTORYNAME,FIXEQUIPTYPE,SITECODE,PROSECUTIONTIME" &_
				...
				") values('"&PIFileNameTemp&"','"&Trim(rsPI1("DIRECTORYNAME"))&"'" &_
		...
		If Not rsPID1.eof Then	
			...
			strPIDadd="insert into PiDetail(FILENAME,SN,CARNO,REALCARNO,CARSIMPLEID,LAWITEMID" &_
				",LPRRESULTID,VERIFYRESULTID,MEMBERID,NOTE,CARADDID,BILLSN,OPERATOR)" &_
				" values('"&PIFileNameTemp&"',(select nvl(max(SN),0)+1 from PiDetail),'"&Trim(rsPID1("CARNO"))&"'" &_
				",'',"&sysCARSIMPLEID&",'"&Trim(rsPID1("LAWITEMID"))&"'" &_
				",'"&Trim(rsPID1("LPRRESULTID"))&"',0,"&theRecordMemberID&",'"&Trim(rsPID1("NOTE"))&"'" &_
				","&sysCARADDID&",null,'"&Trim(rsPID1("OPERATOR"))&"'" &_
				")"
			'response.write strPIDadd
...
%>' border='1' align="left" cellpadding="0">
	...
	End If 
		...
		<%end if%>
			<div id="LayerWTF" style="position:absolute; width:auto; height:32px; z-index:0;  border: 1px none #FFFFCC#FFFFFF; color: #FF0000; background-color: #FFFFFF; font-weight: bold;float:right;">
		...
		<table border='0' cellpadding="0">
			<tr>
			<td>
				<iframe src="/traffic/33/countb.asp?PicFileName=<%=Replace(bPicWebPath,"/","(@)")%>" width="690px" height="480px" frameborder="0" scrolling="auto"></iframe>
			</td>
			<td valign="top">
				<input type="checkbox" name="KeepImage" style="width:30px;height:30px;" value="1" <%
				if instr(trim(request("KeepImage")),"1")>0 then
					response.write "checked"
				end if
				%>><font style="font-size:10pt;color:#FF0000;">建檔後相片保留</font>
				<br> <br>

				<font style="font-size:10pt;">放大鏡:<br>Shift+左鍵=放大<br>Ctrl+左鍵=縮小</font>
	...
	<%If sys_City="台中市"  then%>
		<input type="button" name="uploadb3" value='上傳相片(限Ie11)' onclick='window.open("http://10.116.99.233/IllegalImageUpload/UploadillegalImage.aspx?CityName=TC&TypeID=8&UserID=<%=Trim(Session("Credit_ID"))%>","UploadillegalImage","left=0,top=0,location=0,width=770,height=555,resizable=yes,scrollbars=yes")' style="font-size: 9pt; width: 115px; height: 28px">
	<%End If %>
	<%if not rs1.eof then%>
		...
		<div align="left">
		<%If sys_City<>"嘉義縣" And sys_City<>"彰化縣" And sys_City<>"台中市" then%>
		影像存放位置
		<%End If %>
		<span class="style4">
		...
		<%End If %>
		<%
		If ((sys_City="宜蘭縣" And Trim(Session("Unit_ID"))="TQ00") Or sys_City="高雄市" Or Trim(Session("Credit_ID"))="A000000000") And sys_City<>"嘉義縣" And sys_City<>"台中市" Then
		%>
	...
	<%if not rs1.eof then%>
		...
		<div id="Layer5g7" style="position:absolute; width:34px; height:32px; z-index:0;  border: 1px none #000000; color: #FF0000; background-color: #CCFFFF; font-weight: bold;">
		<input type="checkbox" name="SelectImage" style="width:30px;height:30px;" value="1" onclick="ChangeImageCount()" <%
		...
		<%else%>
		<div id="Layer5g7" style="position:absolute; width:32px; height:32px; z-index:0;  border: 1px none #000000; color: #FF0000; background-color: #CCFFFF; font-weight: bold;">
	...
	<%if not rs1.eof then%>
		...
		%>
		<%if sPicWebPath<>"" then%>
		<div id="Layer5g4" style="position:absolute; width:34px; height:32px; z-index:0;  border: 1px none #000000; color: #FF0000; background-color: #CCFFFF; font-weight: bold;">
...
<%If sys_City="台中市" then%>
				...
				<td colspan="3" >
					<input type="text" size="13" name="ReportNo" onkeydown="funTextControl(this);" style=ime-mode:disabled maxlength="11" onBlur="getAcceptData();" onkeyup="this.value=this.value.toUpperCase();" value="<%
				getReportNo=Trim(request("ReportNo"))
				'getReportNo="1047a142a00045"
				NewReportNo=""
			If getReportNo<>"" Then
				If IsNumeric(Right(getReportNo,8)) Then
					NewReportNo=Left(getReportNo,Len(getReportNo)-8)
					NewReportNo=NewReportNo&Right("0000000"&(CDbl(Right(getReportNo,8))+1),8)
				ElseIf IsNumeric(Right(getReportNo,7)) Then
					NewReportNo=Left(getReportNo,Len(getReportNo)-7)
					NewReportNo=NewReportNo&Right("0000000"&(CDbl(Right(getReportNo,7))+1),7)
				ElseIf IsNumeric(Right(getReportNo,6)) Then
					NewReportNo=Left(getReportNo,Len(getReportNo)-6)
					NewReportNo=NewReportNo&Right("0000000"&(CDbl(Right(getReportNo,6))+1),6)
				ElseIf IsNumeric(Right(getReportNo,5)) Then
					NewReportNo=Left(getReportNo,Len(getReportNo)-5)
					NewReportNo=NewReportNo&Right("0000000"&(CDbl(Right(getReportNo,5))+1),5)
				ElseIf IsNumeric(Right(getReportNo,4)) Then
					NewReportNo=Left(getReportNo,Len(getReportNo)-4)
					NewReportNo=NewReportNo&Right("0000000"&(CDbl(Right(getReportNo,5))+1),4)
				End If 
				response.write NewReportNo
			End If 
					%>">
				</td>
<%End If %>
		...
		<%end if%>
			...
			<%end if%>
				...
				<td bgcolor="#FFFFCC" height="30"><div align="right"><span class="style4">民眾檢舉日期</span></div></td>
		  		<td >
				...
				<td bgcolor="#FFFFCC" height="30"><div align="right"><span class="style4">民眾檢舉案號</span></div></td>
		  		<td >
					<input type="text" name="ReportCaseNo" value="" style=ime-mode:disabled onkeydown="funTextControl(this);" style="width:100px;" onblur="this.value=this.value.toUpperCase()">
				</td>
				<td bgcolor="#FFFFCC" height="30"><div align="right"><span class="style4">檢舉人證號</span></div></td>
		  		<td >
					<input type="text" name="ReportCreditID" value="" style=ime-mode:disabled onkeydown="funTextControl(this);" style="width:100px;" onblur="this.value=this.value.toUpperCase()">
				</td>
			</tr>
		...
		<%if sys_City="宜蘭縣" Or sys_City="彰化縣" then%>
			<tr>
				...
				<td colspan="9">
                	<input type="Text" size="40" value="" name="Note" style=ime-mode:active>
				</td>
			</tr>
		<%End If%>
		</table>
	...
	<%end if%>
		...
		if rs1.eof then
			...
			<input type="button" name="Submit1343" onClick="location='BillKeyIn_Image_Fix_TC.asp<%
		...
		if rs1.eof then
			...
			<input type="button" name="btnExecel1" value="匯出違規舉發移送資料(含相片)" style="font-size: 9pt;width:200px; height: 27px" onclick="funSendData();">
...
<%end if%>
			<input type="button" name="SubmitBack2" onClick="location='BillKeyIn_Image_Fix_Back_TC.asp?PageType=First'" value="<< 第一筆 Home" style="font-size: 9pt; width: 100px; height: 27px">
			
			<input type="button" name="SubmitBack" onClick="location='BillKeyIn_Image_Fix_Back_TC.asp?PageType=Back'" value="< 上一筆 PgUp" style="font-size: 9pt; width: 100px; height: 27px">
		<%If sys_City="高雄市" then%>
			&nbsp; &nbsp; 
	...
	<tr>
	<td colspan="2">
	<iframe src="MemberBillKeyInDataList.asp" width="100%" height="480px" frameborder="0" scrolling="auto"></iframe>
...
MoveTextVar("CarNo,CarSimpleID,IllegalDate,IllegalTime||IllegalAddressID,IllegalAddress,Rule1||Rule2,BillMem1||BillUnitID,JurgeDay,BillFillDate,CarAddID,ProjectID");
<%elseif sys_City="台中市" then%>
...
%>
		...
		event.keyCode=0;   
		event.returnValue=false;  
		location='BillKeyIn_Image_Fix_TC.asp<%
			If Session("BillKeyIn_ReportKeyInType")="1" Then
				response.write "?ReportKeyInType=1"
			End If 
			%>'
...
function funSendData(){
	window.open("ReportCase_SendData_MemKey.asp","ReportCase_SendData","left=0,top=0,location=0,width=800,height=660,resizable=yes,scrollbars=yes,status=yes,menubar=yes");
