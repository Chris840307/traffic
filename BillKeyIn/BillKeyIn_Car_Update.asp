...
dcicheckWflag="0"
strDciChk="select sn from dcilog where billsn="&trim(request("BillSN"))&" and exchangetypeid='W'"
...
<%if sys_City="高雄市" Or sys_City=ApconfigureCityName then%>
	...
	<%end if%>
			<tr>
				...
				%>
					<br>
				<span class="style17"><strong>(已入案成功七天內的舉發單必須刪除後，再重新建檔入案，入案七天後的舉發單，須請監理站修改資料，以避免與監理站資料不同)</strong></span>
                </span>
				<!-- 告發類別 -->
...
<%elseif trim(rs1("Rule3"))<>"" and not isnull(rs1("Rule3")) then%>
	...
	<%elseif sys_City="台中市" then%>
	MoveTextVar("Billno1,Insurance||DriverPID,DriverBrith||CarNo,CarSimpleID||IllegalDate,IllegalTime||IllegalAddressID,IllegalAddress,IllegalZip||Rule1||Rule2||Rule3||RuleSpeed,IllegalSpeed||DealLineDate,MemberStation,BillMem1||BillMem2,BillMem3,BillMem4||Fastener1,Fastener2,Fastener3||BillUnitID,SignType,BillFillDate||CarAddID,ProjectID||Note,TrafficAccidentNo,TrafficAccidentType");
	<%elseif sys_City="澎湖縣" then%>
...
<%end if%>
	...
	*/
	...
	if (myForm.Rule1.value==""){
		...
		error=error+1;
		errorString=errorString+"\n"+error+"：違規法條一輸入錯誤。";
	}else if (myForm.Rule1.value.substr(0,2)>68 && myForm.Rule1.value.substr(0,2)!="92"){
		error=error+1;
		errorString=errorString+"\n"+error+"：違規法條一輸入錯誤。";
	}
	...
	if (myForm.Rule2.value!=""){
		if (TDLawErrorLog2==1){
			error=error+1;
			errorString=errorString+"\n"+error+"：違規法條二輸入錯誤。";
		}else if (myForm.Rule2.value.substr(0,2)>68 && myForm.Rule2.value.substr(0,2)!="92"){
			error=error+1;
			errorString=errorString+"\n"+error+"：違規法條二輸入錯誤。";
		}
	}
...
<%end if%>
<%if sys_City="台中市" then%>
	//myForm.Billno1.value.substr(0,2)!="GI" && myForm.Billno1.value.substr(0,2)!="GK" && myForm.Billno1.value.substr(0,2)!="GL" && myForm.Billno1.value.substr(0,2)!="GJ" && myForm.Billno1.value.substr(0,3)!="GPE" && myForm.Billno1.value.substr(0,3)!="GQE" && myForm.Billno1.value.substr(0,3)!="GRE" && myForm.Billno1.value.substr(0,2)!="GM" && myForm.Billno1.value.substr(0,2)!="GN" && myForm.Billno1.value.substr(0,2)!="GS" && myForm.Billno1.value.substr(0,2)!="GT" && myForm.Billno1.value.substr(0,2)!="GU"
	if (myForm.Billno1.value.substr(0,1)!="G"){
		error=error+1;
		errorString=errorString+"\n"+error+"：請確認單號開頭碼是否正確。";
	}
<%elseif sys_City="苗栗縣" then%>
	...
	if (myForm.BillUnitID.value!=""){
		if (myForm.BillUnitID.value.substr(0,2)=="03" && myForm.BillUnitID.value!="03B6"){
			if (myForm.Billno1.value.substr(0,2)!="F1" && myForm.Billno1.value.substr(0,2)!="F2" && myForm.Billno1.value.substr(0,2)!="FY"){
				error=error+1;
				errorString=errorString+"\n"+error+"：交通隊開頭碼應為F1、FY,請確認單號開頭碼是否正確。";
			}
		}
		if (myForm.BillUnitID.value=="03B6"){
			if (myForm.Billno1.value.substr(0,2)!="F2" && myForm.Billno1.value.substr(0,2)!="F0"){
				error=error+1;
				errorString=errorString+"\n"+error+"：保安隊開頭碼應為F2、F0,請確認單號開頭碼是否正確。";
			}
		}
...
<%if dcicheckWflag="1" then%>
	if (myForm.Note.value==""){
		error=error+1;
		errorString=errorString+"\n"+error+"：本案件已經入案，如要修改請在備註欄位填寫修改原因。";
	}
<%end if %>
<%if sys_City="台東縣" then%>
...
function getChkCarSimpleIDandRule(){
	...
	runServerScript("getChkCarSimpleIDandRule.asp?CarID="+NewCarNo+"&IllDate="+NewIllDate+"&IllTime="+NewIllTime+"&IllRule1="+NewIllRule1+"&IllRule2="+NewIllRule2+"&CarSimpleID="+NewCarSimpleID+"&nowTime=<%=now%>");
}
...
function CheckBillNoExist(){
	...
	if (myForm.Billno1.value!=""){
		...
		<%if sys_City="台中市" then%>
			runServerScript("getCheckBillNoExist.asp?BillNo="+BillNum+"&IsCheckGet=No&nowTime=<%=now%>");
		<%else%>
