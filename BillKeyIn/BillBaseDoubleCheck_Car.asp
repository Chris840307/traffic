...
<body leftmargin="0" topmargin="0" marginwidth="0" marginheight="0">
	<form name="myForm" method="post">  
		<table width='760' border='1' align="left" cellpadding="1">
			...
			<tr>
			<%
			if not isnull(rs1("SignType")) and not isnull(rs2("SignType")) then
				if trim(rs1("SignType"))<>trim(rs2("SignType")) then
					FontType1="<font color='#FF0000'><strong>"
					FontType2="</strong></font>"
				else
					FontType1="<font color='#000000'>"
			...
			%>
				<td bgcolor="#FFFFCC" align="right">簽收狀況</td>
				<td><%=FontType1%>
				<%'(A:簽收U:拒收) 
				...
				%>
				<%=FontType2%></td>
				<td bgcolor="#EAFDE1" align="right">簽收狀況</td>
				<td><%=FontType1%>
				<%
				if trim(rs2("SignType"))<>"" and not isnull(rs2("SignType")) then
					if trim(rs2("SignType"))="A" then
						response.write "簽收"
					elseif trim(rs2("SignType"))="U" then
						response.write "拒簽"
					end if
				else
					response.write "&nbsp;"
			...
			if FastenerStr1<>"" and FastenerStr2<>"" then
				...
				for FA2=0 to ubound(FastenerArray2)
					for FA1=0 to ubound(FastenerArray1)
						if FastenerArray2(FA2)=FastenerArray1(FA1) then
							if FastenerStrTmp2="" then
								FastenerStrTmp2=FastenerArray1(FA1)
							else
								FastenerStrTmp2=FastenerStrTmp2&","&FastenerArray1(FA1)
							end if
							exit for
						end if
					next
				next
				if FastenerStr1<>FastenerStrTmp or FastenerStr2<>FastenerStrTmp2 then
					FontType1="<font color='#FF0000'><strong>"
					FontType2="</strong></font>"
				else
					FontType1="<font color='#000000'>"
