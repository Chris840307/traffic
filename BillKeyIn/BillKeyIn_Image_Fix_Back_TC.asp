...
if trim(request("kinds"))="DB_insert" Then
	...
	chkReKeyInImgBill=0
	If sys_City="彰化縣" Then ' Or sys_City="台中市"
		If (left(trim(request("Rule1")),2)="56" And Len(trim(request("Rule1")))=7) Or (left(trim(request("Rule1")),2)="55" And Len(trim(request("Rule1")))=7) then
			...
			" and ((Rule1 like '55%' and length(Rule1)=7) or (Rule2 like '55%' and length(Rule2)=7) or (Rule1 like '56%' and length(Rule1)=7) or (Rule2 like '56%' and length(Rule2)=7)) and Recordstateid=0 " &_
			" and sn<>"&Trim(request("BillSN"))
		...
		Else
			strChk="select count(*) as cnt from BillbaseTmp where CarNo='"&Trim(request("CarNo"))&"'" & _
			" and IllegalDate="& theIllegalDate & _
			" and Rule1='"&Trim(request("Rule1"))&"' and Recordstateid=0 " &_
			" and sn<>"&Trim(request("BillSN"))
...
If chkIsSpeedTooOver=0 And checkReportCaseFlag=0 And chkReKeyInImgBill=0 then
	'違規日期
	theIllegalDate=""
	if trim(request("IllegalDate"))<>"" then
		theIllegalDate=funGetDate(gOutDT(request("IllegalDate") ) &" "&left(trim(request("IllegalTime")),2)&":"&right(trim(request("IllegalTime")),2),1)
	...
	End if	
	thebillstatus=""
	If Trim(request("UpdBillsn"))="" then
		thebillstatus="1"
	else
		thebillstatus="3"
	end if 
		...
		""&ColAdd &_
		",billstatus='"&thebillstatus&"',checkflag='0'" &_
	...
	if sys_City="台中市" Or sys_City="連江縣" Then
		strDelR="delete from BILLREPORTNOTemp where BillSN="&trim(request("BillSN"))
	...
	if sys_City="台中市" Then
		...
		strIns2="Insert into ReportCaseCheckRecord(Sn,ReportSN,BillTempSN,CheckFlag,RecordMemberID" &_
			",RecordDate,Note)" &_
			" values((select nvl(max(Sn),0)+1 from ReportCaseCheckRecord),"&trim(request("BillSN")) &_
			",'"&Trim(request("BillSN"))&"','A',"&Trim(session("User_ID"))&"" &_
			",sysdate,''" &_
			")"
	...
	If sys_City="高雄市" Then
		If Trim(request("ReportCaseNo"))<>Trim(request("OldReportCaseNo")) Then
			If Trim(request("OldReportCaseNo"))<>"" Then
				strKRO="Update BillBaseTmp set Carno='"&UCase(trim(request("CarNo")))&"',BillStatus='1',BillSn=null where ReportCaseNo='"&Trim(request("OldReportCaseNo"))&"'"
				'response.write strKRO
				conn.execute strKRO
			End If 
			If Trim(request("ReportCaseNo"))<>"" Then
				strKR="Update BillBaseTmp set Carno='"&UCase(trim(request("CarNo")))&"',BillStatus='8',BillSn="&trim(request("BillSN"))&" where ReportCaseNo='"&Trim(request("ReportCaseNo"))&"'"
...
if trim(request("kinds"))="DelCaseAndSaveImg" then
	...
	if not rsImgUpdate1.eof Then
		...
		strPIDA="insert into PIDetail(FILENAME,SN,CARSIMPLEID" &_
			...
			" values('"&Replace(Replace(Trim(rsImgUpdate1("ImageFileNameA")),".JPG",""),".jpg","")&"',(select nvl(max(SN),0)+1 from PIDetail)" &_
	...
	strDelBill="Update BillBaseTmp set billstatus='6',RecordStateID=-1,DelMemberID='"&Session("User_ID")&"'" &_
		" where SN="&trim(request("BillSN"))
...
if trim(request("kinds"))="VerifyResultNull" then
	...
	elseif sys_City="台中市" then
		If Trim(request("UpdBillsn"))<>"" Then
			strDelBill="Update BillBaseTmp set billstatus='8',checkflag='0',DelMemberID='"&Session("User_ID")&"'" &_
				" where SN="&trim(request("BillSN"))
			conn.execute strDelBill
		else
			strDelBill="Update BillBaseTmp set billstatus='7',RecordStateID=-1,DelMemberID='"&Session("User_ID")&"'" &_
	...
	if sys_City="台中市" Then
		...
		strIns2="Insert into ReportCaseCheckRecord(Sn,ReportSN,BillTempSN,CheckFlag,RecordMemberID" &_
			",RecordDate,Note)" &_
			" values((select nvl(max(Sn),0)+1 from ReportCaseCheckRecord),"&trim(request("BillSN")) &_
			",'"&Trim(request("BillSN"))&"','B',"&Trim(session("User_ID"))&"" &_
			",sysdate,''" &_
			")"
		conn.execute strIns2
		End if
...
if HowCatchPicture="xxx" then '刪除下載沖洗照片
	...
	'日期資料夾名稱
	TodayFolder=""
	strFileDate="select RecordDate from BillBaseTmp where sn="&trim(request("BillSN"))
	...
	If sys_City="彰化縣" Or sys_City="台中市" Then
	...
	elseif trim(request("BillSn"))<>"" Then
		sqlPage=" and a.sn="&trim(request("BillSn"))&""
	end If
If sys_City="彰化縣" Or sys_City="台中市" Then
	If Trim(request("UpdBillsn"))="" then
	...
	Else
	'strSql="select * from (select a.*,b.FixEquipType from BillBaseTmp a,ProsecutionImage b,ProsecutionImageDetail c where a.BillTypeID='2' and a.RecordStateID=0 and a.sn="&Trim(request("UpdBillsn"))&" and a.SN=c.BillSN and c.FileName=b.FileName and b.OperatorA=c.Operator and b.FixEquipType in (1,2,5,8,10) "&sqlPage&") where rownum<=1"
	strSql="select * from (select a.*,'8' as FixEquipType from BillBaseTmp a where a.BillTypeID='2' and a.RecordStateID=0 and a.sn="&Trim(request("UpdBillsn"))&" ) where rownum<=1"
	End If 
	...
	if rs1.eof Then
		If Session("BillKeyIn_ReportKeyInType")="1" Then
			if trim(request("PageType"))="Next" then
				Response.Redirect "BillKeyIn_Image_Fix_TC.asp?SessionFlag=1&ReportKeyInType=1"
			elseif trim(request("PageType"))="Back" then
				Response.Redirect "BillKeyIn_Image_Fix_TC.asp?SessionFlag=1&ReportKeyInType=1"
			elseif trim(request("PageType"))="First" then
				Response.Redirect "BillKeyIn_Image_Fix_TC.asp?SessionFlag=1&ReportKeyInType=1"
		...
		Else
			if trim(request("PageType"))="Next" then
				Response.Redirect "BillKeyIn_Image_Fix_TC.asp?SessionFlag=1"
			elseif trim(request("PageType"))="Back" then
				Response.Redirect "BillKeyIn_Image_Fix_TC.asp?SessionFlag=1"
			elseif trim(request("PageType"))="First" then
				Response.Redirect "BillKeyIn_Image_Fix_TC.asp?SessionFlag=1"
...
Else
	strSql="select * from (select a.*,b.FixEquipType from BillBaseTmp a,ProsecutionImage b,ProsecutionImageDetail c where a.BillTypeID='2' and a.BillStatus in ('0') and a.RecordStateID=0 and a.RecordMemberID="&theRecordMemberID&" and a.SN=c.BillSN and c.FileName=b.FileName and b.OperatorA=c.Operator and b.FixEquipType in (1,2,5,8,10) "&sqlPage&") where rownum<=1"
	set rs1=conn.execute(strSql)
	If Not rs1.Bof Then rs1.MoveFirst 
	if rs1.eof then
		if trim(request("PageType"))="Next" then
...
%>' border='1' align="left" cellpadding="0">
	...
	%>
		...
		%> id="imgSource" src="<%=bPicWebPath%>?nowtime=<%=now%>" >
		<div id="Layer5g8" style="position:absolute; width:34px; height:32px; z-index:0;  border: 1px none #000000; color: #FF0000;  font-weight: bold;">
		<input type="button" value="編輯圖片" style="width:60px;font-size:10pt;" onclick="EditImage('<%=Replace(bPicWebPath,"/","(@)")%>');">
		</div>
		<%end if%>
	<%else%>
	...
	<%if not rs1.eof then%>
		...
		%>">
			<img id=BigImg style='position:relative' src="<%=bPicWebPath%>?nowtime=<%=now%>">
		</div>
		<br />
	<%end if%>
	<%
	'if sys_City="高雄市" Or sys_City="彰化縣" Or sys_City=ApconfigureCityName then
		if not rs1.eof then%>
		<!-- 影像小圖C -->
		<%
		sPicWebPath2=""
		...
		<img src="<%=sPicWebPath2%>?nowtime=<%=now%>" border=1 <%
		If sys_City="苗栗縣" Then
			response.write "width=""300"" height=""200"""
		else
			response.write "width=""200"""
		end if
		%> id="SmallImg2" ondblclick="OpenPic('<%=sPicWebPath2%>?nowtime=<%=now%>')">
		<div id="Layer5g7" style="position:absolute; width:34px; height:32px; z-index:0;  border: 1px none #000000; color: #FF0000;  font-weight: bold;">
		<input type="button" value="編輯圖片" style="width:60px;font-size:10pt;" onclick="EditImage('<%=Replace(sPicWebPath2,"/","(@)")%>');">
		</div>
		<%end if%>
	<%
		end if
	...
	<%if not rs1.eof then%>
		...
		%>
		<%if sPicWebPath<>"" then%>
		<img src="<%=sPicWebPath%>?nowtime=<%=now%>" border=1 <%
		If sys_City="苗栗縣" Then
			response.write "width=""300"" height=""200"""
		else
			response.write "width=""200"""
		end if
		%> id="SmallImg"  ondblclick="OpenPic('<%=sPicWebPath%>?nowtime=<%=now%>')">
		<div id="Layer5g8" style="position:absolute; width:34px; height:32px; z-index:0;  border: 1px none #000000; color: #FF0000;  font-weight: bold;">
		<input type="button" value="編輯圖片" style="width:60px;font-size:10pt;" onclick="EditImage('<%=Replace(sPicWebPath,"/","(@)")%>');">
		</div>
		<%end if%>
...
%>
				...
				<td bgcolor="#FFFFCC"><div align="right">標示單號</div></td>
				<td colspan="3">
					<input type="text" size="13" name="ReportNo" onkeydown="funTextControl(this);" style=ime-mode:disabled maxlength="11"  onkeyup="this.value=this.value.toUpperCase();" value="<%
		...
		<%if sys_City<>"苗栗縣" then%>
				...
				%>" name="IllegalAddressID" onblur="funGetSpeedRule()" onKeyUp="getillStreet();" style=ime-mode:disabled onkeydown="funTextControl(this);">
					<img src="../Image/BillkeyInButton.jpg" onclick='window.open("Query_Street.asp","WebPage3","left=0,top=0,location=0,width=700,height=555,resizable=yes,scrollbars=yes")' <%
					If sys_City="彰化縣" Then
						response.write "width='16px'"
					Else
						response.write " width='15' height='15'"
					End If 
					%>>
		...
		<%end if%>
					...
					<img src="../Image/BillkeyInButton.jpg" onclick='window.open("Query_Law.asp?LawOrder=1&RuleVer=<%=trim(rs1("RuleVer"))%>","WebPage1","left=0,top=0,location=0,width=900,height=555,resizable=yes,scrollbars=yes")'  <%
					If sys_City="彰化縣" Then
						response.write "width='16px'"
					Else
						response.write " width='15' height='15'"
		...
		<%End If %>
			...
			<%if sys_City<>"苗栗縣" then%>
				...
				%>" name="Rule2" onKeyUp="getRuleData2();" onkeydown="funTextControl(this);">
					<img src="../Image/BillkeyInButton.jpg"  onclick='window.open("Query_Law.asp?LawOrder=2&RuleVer=<%=trim(rs1("RuleVer"))%>","WebPage_Law","left=0,top=0,location=0,width=900,height=555,resizable=yes,scrollbars=yes")' <%
					If sys_City="彰化縣" Then
						response.write "width='16px'"
					Else
						response.write " width='15' height='15'"
			...
			<%end if%>
				...
				<td bgcolor="#FFFFCC"><div align="right"><span class="style4">民眾檢舉日期</span></div></td>
				<td >
				...
				<td bgcolor="#FFFFCC"><div align="right"><span class="style4">民眾檢舉案號</span></td>
				<td >
				...
				<td bgcolor="#FFFFCC"><div align="right"><span class="style4">檢舉人證號</span></td>
				<td >
			...
			<tr>
				...
				%>" onKeyUp="getBillMemID1();" style=ime-mode:disabled onkeydown="funTextControl(this);">
					<img src="../Image/BillkeyInButton.jpg"  onclick='window.open("Query_MemID.asp?MemOrder=1","WebPage1","left=0,top=0,location=0,width=800,height=555,resizable=yes,scrollbars=yes")' <%
					If sys_City="彰化縣" Then
						response.write "width='16px'"
					Else
						response.write " width='15' height='15'"
				...
				%>" onKeyUp="getUnit();" style=ime-mode:disabled onkeydown="funTextControl(this);">
					<img src="../Image/BillkeyInButton.jpg" onclick='window.open("Query_Unit.asp?SType=U","WebPage2","left=0,top=0,location=0,width=800,height=575,resizable=yes,scrollbars=yes")'  <%
					If sys_City="彰化縣" Then
						response.write "width='16px'"
					Else
						response.write " width='15' height='15'"
		...
		<%End If %>
				...
				%>" name="ProjectID" style=ime-mode:disabled onkeyup="ProjectF5()" onkeydown="funTextControl(this);">
					<img src="../Image/BillkeyInButton.jpg" onClick='window.open("Query_Project.asp","WebPage1","left=0,top=0,location=0,width=700,height=555,resizable=yes,scrollbars=yes")'  <%
					If sys_City="彰化縣" Then
						response.write "width='16px'"
					Else
						response.write " width='15' height='15'"
			...
			<%if sys_City<>"宜蘭縣" And sys_City<>"彰化縣" then%>	
				<!-- 備註 -->
	...
	<tr bgcolor="#FFCC33">
		<td height="28" colspan="2" align="center">
					...
					<input type="hidden" name="Submit5322QQ" onClick="funcOpenBillQry()" value="查 詢 <%
				response.write "F6"
			...
			%>" style="font-size: 9pt; width: 60px; height: 27px" >
                ...
                <%If Trim(request("UpdBillsn"))<>"" Then%>
					&nbsp; &nbsp; 
					<input type="button" name="Submitpic2932" onClick="funPictureList('<%=trim(request("UpdBillsn"))%>');" value="相片列表" <%
					if rs1.eof then
				...
				<%If Trim(request("UpdBillsn"))="" Then%>
					...
					<input type="button" name="SubmitBack" onClick="location='BillKeyIn_Image_Fix_Back_TC.asp?PageType=First'" value="<< 第一筆 Home" style="font-size: 9pt; width: 90px; height: 27px">
					<input type="button" name="SubmitBack" onClick="location='BillKeyIn_Image_Fix_Back_TC.asp?PageType=Back'" value="< 上一筆 PgUp" style="font-size: 9pt; width: 90px; height: 27px">
					...
					<input type="button" name="SubmitNext" onClick="location='BillKeyIn_Image_Fix_Back_TC.asp?PageType=Next'" value="下一筆 PgDn >" style="font-size: 9pt; width: 90px; height: 27px">
					<input type="button" name="SubmitBack" onClick="location='BillKeyIn_Image_Fix_Back_TC.asp?PageType=Last'" value="最後一筆 End >>" style="font-size: 9pt; width: 90px; height: 27px">
				<%End if%>
				<!-- 逕舉類別 -->
...
if sys_City="台中市" then
	If Trim(request("UpdBillsn"))<>"" Then
		StrRea="select * from (" &_
			"select * from ReportCaseCheckRecord where CheckFlag in ('2','6') and BillTempSN="&Trim(request("UpdBillsn"))&" order by Recorddate desc" &_
			") where rownum<=1"
...
<%elseif sys_City="台中市" then%>
MoveTextVar("CarNo,CarSimpleID,IllegalDate,IllegalTime,ReportNo||IllegalAddressID,IllegalAddress,IllegalZip,Rule1,IllegalSpeed,RuleSpeed||Rule2,JurgeDay,ReportCaseNo,ReportCreditID||BillMem1,BillUnitID,BillFillDate,CarAddID,ProjectID");
<%else%>
...
function funPrintCaseList_Report(){
	UrlStr="PrintCaseDataList_MemKey.asp?CallType=1";
...
<%end if %>
	...
	*/
	...
	//	funcOpenBillQry();
		...
		event.keyCode=0;   
		location='BillKeyIn_Image_Fix_Back_TC.asp?PageType=Back'
	}else if (event.keyCode==34){ //下一筆PageDn
		event.keyCode=0;   
		event.returnValue=false; 
		location='BillKeyIn_Image_Fix_Back_TC.asp?PageType=Next'
	}else if (event.keyCode==36){ //第一筆Home
		event.keyCode=0;   
		location='BillKeyIn_Image_Fix_Back_TC.asp?PageType=First'
	}else if (event.keyCode==35){ //最後一筆End
		event.keyCode=0;   
		event.returnValue=false; 
		location='BillKeyIn_Image_Fix_Back_TC.asp?PageType=Last'
	}
}
...
<%if sys_City="台中市" then%>
	window.open("http://10.116.99.233/traffic/33/countb.asp?PicFileName="+strUrl,"WebPage1","left=0,top=0,location=0,width=800,height=660,resizable=yes,scrollbars=yes,status=yes");
<%else%>
	window.open("/traffic/33/countb.asp?PicFileName="+strUrl,"WebPage1","left=0,top=0,location=0,width=800,height=660,resizable=yes,scrollbars=yes,status=yes");
<%end if %>
}
