...
<%
	...
	rsCity.close
	set rsCity=nothing
	If trim(Session("Credit_ID"))="A000000000" or trim(Session("Credit_ID"))="ADMIN" Then
		strSQL="select * from (select rownum as Item,a.* from (Select * from PasserBase where RecordDate between to_date('"&DateAdd("d",-1024,date)&" 00:00:00','YYYY/MM/DD/HH24/MI/SS') and to_date('"&date&" 23:59:59','YYYY/MM/DD/HH24/MI/SS') and BillTypeID="&Request("BillTypeID")&" and RecorDStateID<>-1 order by RecordDate) a) where Item = "&request("DBFile")
	else
		strSQL="select * from (select rownum as Item,a.* from (Select * from PasserBase where RecordMemberID in(select MemberID from MemberData where UnitID='"&trim(Session("Unit_ID"))&"' and RecorDStateID<>-1) and RecordDate between to_date('"&DateAdd("d",-30,date)&" 00:00:00','YYYY/MM/DD/HH24/MI/SS') and to_date('"&date&" 23:59:59','YYYY/MM/DD/HH24/MI/SS') and BillTypeID="&Request("BillTypeID")&" and RecorDStateID<>-1 order by RecordDate) a) where Item = "&request("DBFile")
	End if 
	...
	if Not rs.eof then
		...
		if sys_City="高雄市" or sys_City="台中市" then
			if trim(rs("IllegalZip"))<>"" and not isnull(rs("IllegalZip")) then
				response.write "myForm.IllegalZip.value='"&trim(rs("IllegalZip"))&"';"
				strZip="select * from zip where zipid='"&trim(rs("IllegalZip"))&"'"
				set rsZip=conn.execute(strZip)
				if not rsZip.eof then
					response.write "LayerIllZip.innerHTML='"&trim(rsZip("ZipName"))&"';"
				else
					response.write "LayerIllZip.innerHTML='';"
				end if
				rsZip.close
				set rsZip=nothing 
			else
				response.write "myForm.IllegalZip.value='';"
				response.write "LayerIllZip.innerHTML='';"
			end if 
		end if
		...
		response.write "myForm.FromNote.value='"&trim(rs("FromNote"))&"';"
		response.write "myForm.FromNoteId.value='"&trim(rs("FromNoteId"))&"';"
	end if
%>
