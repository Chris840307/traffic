...
if HowCatchPicture="xxx" then '刪除下載沖洗照片
	...
	strSqlCnt="select count(*) as cnt from BillBase a,ProsecutionImage b,ProsecutionImageDetail c where a.BillTypeID='2'" &_
		...
		" and a.SN=c.BillSN and c.FileName=b.FileName" &_
		" and b.FixEquipType in (1,2,5,10)"
	...
	strSql="select a.* from BillBase a,ProsecutionImage b,ProsecutionImageDetail c where a.BillTypeID='2' and a.BillStatus in ('0') and a.RecordStateID=0 and a.RecordMemberID="&theRecordMemberID&" and a.SN=c.BillSN and c.FileName=b.FileName and b.FixEquipType in (1,2,5,10) "&sqlPage
	set rs1=conn.execute(strSql)

	if rs1.eof then
		if trim(request("PageType"))="Next" then
			Response.Redirect "BillKeyIn_Image_Fix.asp?SessionFlag=1"
		elseif trim(request("PageType"))="Back" then
			Response.Redirect "BillKeyIn_Image_Fix.asp?SessionFlag=1"
		elseif trim(request("PageType"))="First" then
			Response.Redirect "BillKeyIn_Image_Fix.asp?SessionFlag=1"
...
<table width='1000' border='1' align="left" cellpadding="0">
	...
	<%if not rs1.eof then%>
		...
		<%end if%>
		<%
		strPro="select * from ProsecutionImage a,ProsecutionImageDetail b where a.FileName=b.FileName and b.BillSN="&trim(rs1("SN"))
	...
	<tr>
		...
		<table width='100%' border='1' align="left" cellpadding="0">
			...
			<tr>
				...
				%>" size="6" maxlength="7" style=ime-mode:disabled onkeydown="funTextControl(this);" onblur="this.value=this.value.replace(/[^\d]/g,'');">
				</td>
	...
	<tr bgcolor="#FFCC33">
		<td height="28" colspan="2" align="center">
					...
					<input type="button" name="Submit5322" onClick="funcOpenBillQry()" value="查 詢 F6" >
                    ...
                    <input type="button" name="Submit4232" onClick="funPrintCaseList_Report();" value="建檔清冊 F10">
					
					<img src="/image/space.gif" width="20" height="8">
					<input type="button" name="SubmitBack" onClick="location='BillKeyIn_Image_Fix_Back.asp?PageType=First'" value="<< 第一筆 Home" style="font-size: 9pt; width: 100px; height: 27px">
					...
					<input type="button" name="SubmitNext" onClick="location='BillKeyIn_Image_Fix_Back.asp?PageType=Next'" value="下一筆 PgDn >" style="font-size: 9pt; width: 100px; height: 27px">
					<input type="button" name="SubmitBack" onClick="location='BillKeyIn_Image_Fix_Back.asp?PageType=Last'" value="最後一筆 End >>" style="font-size: 9pt; width: 100px; height: 27px">
				<!-- 逕舉類別 -->
...
function KeyDown(){ 
	...
	*/
		...
		event.keyCode=0;   
		event.returnValue=false; 
		location='BillKeyIn_Image_Fix_Back.asp?PageType=Next'
		...
		event.keyCode=0;   
		event.returnValue=false; 
		location='BillKeyIn_Image_Fix_Back.asp?PageType=Last'
	}
}
