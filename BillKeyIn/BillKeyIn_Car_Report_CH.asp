...
if trim(request("kinds"))="DB_insert" Then
	...
	if trim(request("Billno1"))<>"" then
		...
		strchkno="select BillNo from BillBaseTmp where BillNo='"&trim(request("Billno1"))&"' and RecordStateID=0 and (checkflag<>'2' or checkflag is null)"
		set rschkno=conn.execute(strchkno)
		if not rschkno.eof then
			chkIsExistBillNumFlag=1
		end if
...
'			Set rsChk=Nothing 
	...
	if chkIsExistBillNumFlag=0 And chkIsSpeedTooOver=0 And chkReportNoIsExist=0 And chkIsRule56Flag=0 and chkIllegalAddress53Flag=0 and chkIllegalAddressID53Flag=0 And chkIsRule5620002Flag_TC=0 And chkIsSpeedRuleFlag_TC=0 And chkReKeyInBill=0 And chkIsDoubleFlag_TC=0 then
...
'			End If 
		...
		strInsert="insert into BillBaseTmp(SN,BillTypeID,BillNo,CarNo,CarSimpleID,CarAddID,IllegalDate" & _
			...
			",Note,EquipmentID,RuleVer,DriverSex,TrafficAccidentType,ImageFileName"&ColAdd&",JurgeDay,CheckFlag)" &_
...
if trim(request("Tmp_Order"))="" then
	...
	strSqlCnt="select count(*) as cnt from BillBaseTmp where BillTypeID='2' and BillStatus in ('1') and RecordStateID=0 and RecordMemberID="&theRecordMemberID&" and ImageFileName is null"
...
If Trim(request("ReportCaseSn"))="" Then
'抓上一筆的資料
	strSql="select * from (select * from BillBaseTmp" &_
...
'		end If
		...
		if (trim(rs1("JurgeDay"))<>"" and not isnull(rs1("JurgeDay"))) Then
			bReportCaseJurgeDay=trim(ginitdt(rs1("JurgeDay")))
		end if
...
<%if sys_City="嘉義縣" or sys_City="台東縣" or sys_City="高雄縣" or sys_City="嘉義市" or sys_City="宜蘭縣" or sys_City="台南市" then%>
...
<%elseif sys_City="台中市" then%>
				<td bgcolor="#EBE5FF" width="13%"><div align="right"><span class="style4">＊</span>告示單號</div></td>
				<td >
					<input type="text" size="10" name="ReportNo" onkeydown="funTextControl(this);" style=ime-mode:disabled maxlength="11" onBlur="getAcceptData();">
				</td>
...
<%if sys_City<>"雲林縣" then%>
			  <td bgcolor="#EBE5FF" width="13%"><div align="right"><span class="style4">＊</span>違規車號</div></td>
				...
				<font color="#ff000" size="2">1汽車 / 2拖車/ 3重機/ 4輕機/ 5動力機械/ 6 臨時車牌</font>
				&nbsp;
...
<%else%>
				...
				<div id="Layer170" style="position:absolute; width:245px; height:24px; z-index:0; layer-background-color: #11FFFF; border: 1px none #000000; color: #FF0000;">
				<span class="style7">1汽車 / 2拖車/ 3重機<br>/ 4輕機/ 5動力機械/ 6 臨時車牌</span>
				</div>
			...
			<%End if%>
				...
				<td colspan="3">
					...
					Else
						response.write "29"
					End If 
					%>" value="<%=bIllegalAddress%>" name="IllegalAddress" style=ime-mode:active onkeyup="AutoGetIllStreet();" onblur="funGetSpeedRule()" onkeydown="funTextControl(this);" <%
					if sys_City="台南市" Then Response.Write " onfocus=""autoKeyEnd();"""
					%> >
					<input type="checkbox" name="chkHighRoad" value="1" <%if trim(request("chkHighRoad"))="1" Or bHighSpeedRoad="1" then response.write "checked"%> onclick="setIllegalRule()" <%if sys_City="南投縣" then response.write "disabled"%>><span class="style1">快速道路</span>
					<%if sys_City="台中市" then %>
						<br>區號
						<input type="text" class="btn5" size="3" value="<%=bIllZip%>" name="IllegalZip" onKeyUp="getIllZip();" onkeydown="funTextControl(this);" maxlength="3">
						<Input type="hidden" name="OldIllegalZip" value="<%=Trim(request("IllegalZip"))%>">
...
<%end if%>
			...
			<tr>
		<%If sys_City="台中市" then%>
				<td bgcolor="#EBE5FF"><div align="right">舉發人姓名</div></td>
		  		<td colspan="5">
		...
		<%End if%>
			...
			<tr>
				...
				<td <%
				if sys_City="嘉義縣" or sys_City="台東縣" or sys_City="高雄縣" or sys_City="雲林縣" or sys_City="台南市" Or sys_City="宜蘭縣" or sys_City="嘉義市" then
					response.write "colspan='5'"
				end if
				%>>
					...
					%></div>
				<%if sys_City="嘉義縣" or sys_City="台東縣" or sys_City="高雄縣" or sys_City="嘉義市" or sys_City="宜蘭縣" or sys_City="雲林縣" or sys_City="台南市" then%>
...
<%if sys_City<>"嘉義縣" and sys_City<>"台東縣" and sys_City<>"高雄縣" and sys_City<>"嘉義市" and sys_City<>"雲林縣" and sys_City<>"宜蘭縣" and sys_City<>"台南市" then%>
				<td bgcolor="#EBE5FF"><div align="right"><span class="style4">＊</span>填單日期</div></td>
				<td colspan="3">
				<input type="text" size="10" value="<%
				if bBillType<>"1" then
...
<%end if%>
		...
		<%If sys_City="宜蘭縣" or sys_City="嘉義市" or sys_City="花蓮縣" then%>
			<tr>
				<td bgcolor="#EBE5FF"><div align="right">身分證號<br><span class="style10">非轉歸責案件勿填</span></div></td>
		  		<td>
				...
				<td bgcolor="#EBE5FF"><div align="right">應到案處所<br><span class="style10">非轉歸責案件勿填</span></div>
...
<%if sys_City="雲林縣" then%>	
				...
				%>" maxlength="7" name="BillFillDate" onkeydown="funTextControl(this);" style=ime-mode:disabled>
				</td>
<%else%>
				<td bgcolor="#EBE5FF" align="right">輔助車種</td>
				<td colspan="3">
...
<%end if%>
			...
			<tr>
				...
				if sys_City="嘉義縣" or sys_City="花蓮縣" or sys_City="高雄縣" then
					...
					<font color="#ff000" size="2"> 1固定桿/ 2雷達三腳架/ 3相機/<%
				If sys_City="台南市" Then
					response.write " 4車載攝影機/"
...
<%if (sys_City<>"高雄市" and sys_City<>ApconfigureCityName) or trim(request("BillReCover"))<>"1" then%>
	...
	If Trim(request("ReportCaseSn"))="" then%>
					...
					<input type="button" name="Submit1343" onClick="location='BillKeyIn_Car_Report_CH.asp'" value="清 除 F4" class="btn1">
	<%end if%>
<%end if%>
					<img src="/image/space.gif" width="29" height="8">
...
<%if (sys_City<>"高雄市" and sys_City=ApconfigureCityName) or trim(request("BillReCover"))<>"1" then%>
	...
	<%'高雄檢舉系統不要出現按鈕
	If Trim(request("ReportCaseSn"))="" then%>
				<input type="button" name="SubmitBack2" onClick="location='BillKeyIn_Report_Back_CH.asp?PageType=First'" value="<< 第一筆 Home" class="btn1">
				<img src="/image/space.gif" width="29" height="8">
				<input type="button" name="SubmitBack" onClick="location='BillKeyIn_Report_Back_CH.asp?PageType=Back'" value="< 上一筆 PgUp" class="btn1">
	<%End if%>
...
<%end if%>
				...
				<!-- 應到案處所 -->
		<%If sys_City<>"嘉義市" And sys_City<>"宜蘭縣" And sys_City<>"花蓮縣" then%>
...
MoveTextVar("Billno1||CarNo,CarSimpleID||IllegalDate,IllegalTime||IllegalAddressID,IllegalAddress||RuleSpeed,IllegalSpeed||Rule1||Rule2||BillMem1,BillMem2||BillMem3,BillMem4||BillUnitID,BillFillDate,JurgeDay||ProjectID,CarAddID||UseTool,Note,Fastener1,Fastener2");
...
MoveTextVar("Billno1,BillFillDate||CarNo,CarSimpleID||IllegalDate,IllegalTime||RuleSpeed,IllegalSpeed||IllegalAddressID,IllegalZip,IllegalAddress||Rule1||Rule2||BillMem1,BillMem2||BillMem3,BillMem4||BillUnitID,JurgeDay||ProjectID,CarAddID||UseTool,Note,Fastener1,Fastener2");
<%elseif sys_City="雲林縣" then %>
MoveTextVar("Billno1,CarNo||CarSimpleID,CarAddID||IllegalDate,IllegalTime||RuleSpeed,IllegalSpeed||IllegalAddressID,IllegalAddress||Rule1||Rule2||BillMem1,BillMem2||BillMem3,BillMem4||BillUnitID,JurgeDay||ProjectID,BillFillDate||UseTool,Note,Fastener1,Fastener2");
...
<%end if%>
	...
	<%if sys_City="宜蘭縣" Or sys_City="基隆市" Or sys_City="台南市" Or sys_City="台東縣" Or sys_City="雲林縣" then%>
...
function InsertLaw(){
	...
	TDLaw2.innerHTML="<input type='text' size='10' value='' name='Rule3' onKeyUp='getRuleData3();' onchange='DelSpace1();'  onkeydown='funTextControl(this);'> <img src='../Image/BillkeyInButton.jpg' width='25' height='23' onclick='OpenQueryLaw3()' alt='查詢法條'> <div id='Layer3' style='position:absolute ; width:589px; height:30px; z-index:0; background-color: #CCFFFF; layer-background-color: #CCFFFF; border: 1px none #000000;'></div><input type='hidden' name='ForFeit3' value=''>";

	if (myForm.ReportChk.checked==true){
	<%if sys_City="彰化縣" then %>
	MoveTextVar("Billno1||CarNo,CarSimpleID||IllegalDate,IllegalTime||IllegalAddressID,IllegalAddress||RuleSpeed,IllegalSpeed||Rule1||Rule2||Rule3||DealLineDate||BillMem1,BillMem2||BillMem3,BillMem4||BillUnitID,BillFillDate,JurgeDay||ProjectID,CarAddID||UseTool,Note,Fastener1,Fastener2");
	<%elseif sys_City="嘉義縣" or sys_City="台東縣" or sys_City="高雄縣" then %>
	...
	<%elseif sys_City="台南市" then %>
	MoveTextVar("Billno1,BillFillDate||CarNo,CarSimpleID||IllegalDate,IllegalTime||RuleSpeed,IllegalSpeed||IllegalAddressID,IllegalZip,IllegalAddress||Rule1||Rule2||Rule3||DealLineDate||BillMem1,BillMem2||BillMem3,BillMem4||BillUnitID,JurgeDay||ProjectID,CarAddID||UseTool,Note,Fastener1,Fastener2");
	<%elseif sys_City="雲林縣" then %>
	MoveTextVar("Billno1,CarNo||CarSimpleID,CarAddID||IllegalDate,IllegalTime||RuleSpeed,IllegalSpeed||IllegalAddressID,IllegalAddress||Rule1||Rule2||Rule3||DealLineDate||BillMem1,BillMem2||BillMem3,BillMem4||BillUnitID,JurgeDay||ProjectID,BillFillDate||UseTool,Note,Fastener1,Fastener2");
	<%elseif sys_City="高雄市" then %>
	...
	<%if sys_City="彰化縣" then %>
		MoveTextVar("Billno1||CarNo,CarSimpleID||IllegalDate,IllegalTime||IllegalAddressID,IllegalAddress||RuleSpeed,IllegalSpeed||Rule1||Rule2||Rule3||BillMem1,BillMem2||BillMem3,BillMem4||BillUnitID,BillFillDate,JurgeDay||ProjectID,CarAddID||UseTool,Note,Fastener1,Fastener2");
	<%elseif sys_City="嘉義縣" or sys_City="台東縣" or sys_City="高雄縣" then %>
	...
	<%elseif sys_City="台南市" then %>
		MoveTextVar("Billno1,BillFillDate||CarNo,CarSimpleID||IllegalDate,IllegalTime||RuleSpeed,IllegalSpeed||IllegalAddressID,IllegalZip,IllegalAddress||Rule1||Rule2||Rule3||BillMem1,BillMem2||BillMem3,BillMem4||BillUnitID,JurgeDay||ProjectID,CarAddID||UseTool,Note,Fastener1,Fastener2");
	<%elseif sys_City="雲林縣" then %>
		MoveTextVar("Billno1,CarNo||CarSimpleID,CarAddID||IllegalDate,IllegalTime||RuleSpeed,IllegalSpeed||IllegalAddressID,IllegalAddress||Rule1||Rule2||Rule3||BillMem1,BillMem2||BillMem3,BillMem4||BillUnitID,JurgeDay||ProjectID,BillFillDate||UseTool,Note,Fastener1,Fastener2");
	<%elseif sys_City="高雄市" then %>
...
<%if sys_City<>"南投縣" and sys_City<>"台中縣" and sys_City<>"雲林縣" and sys_City<>"彰化縣" and sys_City<>"嘉義縣" and sys_City<>"宜蘭縣" and sys_City<>"台東縣" and sys_City<>"高雄縣" and sys_City<>"台南市" and sys_City<>"嘉義市" and sys_City<>"高雄市" and sys_City<>ApconfigureCityName then %>
...
function getFastener1(){
	...
	if (myForm.Fastener1.value.length == 1){
		...
		runServerScript("getFastener.asp?FastenerOrder=1&FastenerID="+FastenerNum+"&nowTime=<%=now%>");
...
function getFastener2(){
	...
	if (myForm.Fastener2.value.length == 1){
		...
		runServerScript("getFastener.asp?FastenerOrder=2&FastenerID="+FastenerNum+"&nowTime=<%=now%>");
...
<%if (trim(Session("UnitLevelID"))<>"2" and sys_City="台中縣") or (sys_City<>"台中縣" and sys_City<>"高雄市") then%>
	<%if sys_City<>"南投縣" and sys_City<>"屏東縣" and sys_City<>"台中縣" and sys_City<>"台中市" then%>
		<%if sys_City="台中縣" or sys_City="彰化縣" or sys_City="宜蘭縣" or sys_City="台南市" or sys_City="台東縣" or sys_City="嘉義市" or sys_City="嘉義縣" or sys_City="雲林縣" or sys_City="基隆市" or sys_City="保二總隊四大隊二中隊" then%>
			getDealDateValue="45";
...
function funcReportChk(){
	...
	if (myForm.ReportChk.checked==true){
		...
		%>>否";
	<%if sys_City="彰化縣" then %>
	MoveTextVar("Billno1||CarNo,CarSimpleID||IllegalDate,IllegalTime||IllegalAddressID,IllegalAddress||RuleSpeed,IllegalSpeed||Rule1||Rule2||DealLineDate||BillMem1,BillMem2||BillMem3,BillMem4||BillUnitID,BillFillDate,JurgeDay||ProjectID,CarAddID||UseTool,Note,Fastener1,Fastener2");
	<%elseif sys_City="嘉義縣" or sys_City="台東縣" or sys_City="高雄縣" then %>
	...
	<%elseif sys_City="台南市" then %>
	MoveTextVar("Billno1,BillFillDate||CarNo,CarSimpleID||IllegalDate,IllegalTime||RuleSpeed,IllegalSpeed||IllegalAddressID,IllegalZip,IllegalAddress||Rule1||Rule2||DealLineDate||BillMem1,BillMem2||BillMem3,BillMem4||BillUnitID,JurgeDay||ProjectID,CarAddID||UseTool,Note,Fastener1,Fastener2");
	<%elseif sys_City="雲林縣" then %>
	MoveTextVar("Billno1,CarNo||CarSimpleID,CarAddID||IllegalDate,IllegalTime||RuleSpeed,IllegalSpeed||IllegalAddressID,IllegalAddress||Rule1||Rule2||DealLineDate||BillMem1,BillMem2||BillMem3,BillMem4||BillUnitID,JurgeDay||ProjectID,BillFillDate||UseTool,Note,Fastener1,Fastener2");
	<%elseif sys_City="高雄市" then %>
...
<%end if%>
		...
		event.keyCode=0;   
		event.returnValue=false; 
		location='BillKeyIn_Car_Report_CH.asp'
...
<%end if%>
		...
		event.keyCode=0;   
		location='BillKeyIn_Report_Back_CH.asp?PageType=Back'
	}else if (event.keyCode==36){ //第一筆Home
		event.keyCode=0;   
		location='BillKeyIn_Report_Back_CH.asp?PageType=First'
	}
}
...
function funGetSpeedRule(){
	...
	}else if (reqIllgealAressID=="RA743" || reqIllgealAressID=="RA744")
	{
		if (myForm.IllegalAddressID.value!="RA743" && myForm.IllegalAddressID.value!="RA744")
		{
...
<%if sys_City="台中市" then%>
	function getAcceptData(){
		...
		event.keyCode=13;
		runServerScript("getAcceptData.asp?AcceptNo="+AcceptNo+"&nowTime=<%=now%>");
	...
	if (myForm.ReportChk.checked==false)
			{
				if (myForm.AcceptBatchNumberChk.checked==true)
				{
					...
					{
						runServerScript("getChkRunCarAccept.asp?AcceptBatchNumber="+myForm.AcceptBatchNumber.value+"&CarNo="+myForm.CarNo.value+"&CarSimpleID="+myForm.CarSimpleID.value+"&nowTime=<%=now%>");
					}	
				}
			}
}
...
function getDriverZip(obj,objName){
	if(obj.value!=''&&obj.value.length>2){
		...
		runServerScript("getZipNameForCar.asp?ZipID="+obj.value+"&getZipName="+objName+"&getIllegalAddress="+myForm.IllegalAddress.value+"&nowTime=<%=now%>");
	...
	function getBillMemData(){
		if (event.keyCode==<%
			...
			if (myForm.BillMemNameQry.value.length > 1){
				...
				runServerScript("getBillMemData.asp?MemName="+BillMemNum+"&nowTime=<%=now%>");
