...
'==========================
	'BillBase
	strBill1="select * from BillBase where BillTypeID='2' and CarNo='"&theCarNo&"' order by BillFillDate desc"
...
<%	end if
	...
	strBill2="select * from BillBaseTmp where BillTypeID='2' and CarNo='"&theCarNo&"' order by BillFillDate desc"
...
<body leftmargin="0" topmargin="0" marginwidth="0" marginheight="0">
	<form name="myForm" method="post">  
		<table width='760' border='1' align="left" cellpadding="1">
			...
			%>
				<td bgcolor="#FFFFCC" align="right">違規車號</td>
				<td><%=FontType1%>
				<%
				...
				%>
				<%=FontType2%></td>
				<td bgcolor="#EAFDE1" align="right">違規車號</td>
				<td><%=FontType1%>
				<%
			...
			<tr>
			<%
			if not isnull(rs1("UseTool")) and not isnull(rs2("UseTool")) then
				if trim(rs1("UseTool"))<>trim(rs2("UseTool")) then
					FontType1="<font color='#FF0000'><strong>"
					FontType2="</strong></font>"
				else
					FontType1="<font color='#000000'>"
			...
			%>
				<td bgcolor="#FFFFCC" align="right">採証工具</td>
				<td><%=FontType1%>
				<%
				...
				%>
				<%=FontType2%></td>
				<td bgcolor="#EAFDE1" align="right">採証工具</td>
				<td><%=FontType1%>
				<%
				if trim(rs2("UseTool"))<>"" and not isnull(rs2("UseTool")) then
					if trim(rs2("UseTool"))="1" then
						response.write "固定桿"
					elseif trim(rs2("UseTool"))="2" then
						response.write "雷達測速[三腳架]"
					elseif trim(rs2("UseTool"))="3" then
						response.write "儀器[相機]"
					end if
				else
					response.write "無"
			...
			<tr>
			<%
			if not isnull(rs1("EquipmentID")) and not isnull(rs2("EquipmentID")) then
				if trim(rs1("EquipmentID"))<>trim(rs2("EquipmentID")) then
					FontType1="<font color='#FF0000'><strong>"
					FontType2="</strong></font>"
				else
					FontType1="<font color='#000000'>"
			...
			%>
				<td bgcolor="#FFFFCC" align="right">固定桿編號</td>
				<td><%=FontType1%>
				<%
				...
				%>
				<%=FontType2%></td>
				<td bgcolor="#EAFDE1" align="right">固定桿編號</td>
				<td><%=FontType1%>
				<%
				if trim(rs2("EquipmentID"))<>"" and not isnull(rs2("EquipmentID")) then
					response.write trim(rs2("EquipmentID"))
				else
					response.write "&nbsp;"
				end if
				%>
			...
			%>
				...
				%>
				<%=FontType2%></td>
				<td bgcolor="#EAFDE1" align="right">輔助車種</td>
				<td><%=FontType1%>
				<%
			...
			if RuleStr1<>"" and RuleStr2<>"" then
				...
				for RA2=0 to ubound(RuleArray2)
					for RA1=0 to ubound(RuleArray1)
						if RuleArray2(RA2)=RuleArray1(RA1) then
							if RuleStrTmp2="" then
								RuleStrTmp2=RuleArray1(RA1)
							else
								RuleStrTmp2=RuleStrTmp2&","&RuleArray1(RA1)
							end if
							exit for
						end if
					next
				next
				if RuleStr1<>RuleStrTmp or RuleStr2<>RuleStrTmp2 then
					FontType1="<font color='#FF0000'><strong>"
					FontType2="</strong></font>"
				else
					FontType1="<font color='#000000'>"
			...
			%>
				...
				<td><%=FontType1%>
				<%
				if trim(RuleStr1)<>"" and not isnull(RuleStr1) then
					response.write trim(RuleStr1)
				else
					response.write "&nbsp;"
				end if
				%>
				...
				<td><%=FontType1%>
				<%
				if trim(RuleStr2)<>"" and not isnull(RuleStr2) then
					response.write trim(RuleStr2)
				else
					response.write "&nbsp;"
				end if
				%>
			...
			if trim(rs1("BillMemID4"))<>"" and not isnull(rs1("BillMemID4")) then
				if BillMemID1="" then
					BillMemID1=trim(rs1("BillMemID4"))
				else
					BillMemID1=BillMemID1&","&trim(rs1("BillMemID4"))
				end if
			end if
			...
			if trim(rs2("BillMemID4"))<>"" and not isnull(rs2("BillMemID4")) then
				if BillMemID2="" then
					BillMemID2=trim(rs2("BillMemID4"))
				else
					BillMemID2=BillMemID2&","&trim(rs2("BillMemID4"))
				end if
			end if
			...
			if BillMemID1<>"" and BillMemID2<>"" then
				...
				for BA2=0 to ubound(BillMemIDArray2)
					for BA1=0 to ubound(BillMemIDArray1)
						if BillMemIDArray2(BA2)=BillMemIDArray1(BA1) then
							if BillMemIDStrTmp2="" then
								BillMemIDStrTmp2=BillMemIDArray1(BA1)
							else
								BillMemIDStrTmp2=BillMemIDStrTmp2&","&BillMemIDArray1(BA1)
							end if
							exit for
						end if
					next
				next
				if BillMemID1<>BillMemIDStrTmp or BillMemID2<>BillMemIDStrTmp2 then
					FontType1="<font color='#FF0000'><strong>"
					FontType2="</strong></font>"
				else
					FontType1="<font color='#000000'>"
			...
			%>
				...
				if trim(rs1("BillMem4"))<>"" and not isnull(rs1("BillMem4")) then
					if BillMemName1="" then
						BillMemName1=trim(rs1("BillMem4"))
					else
						BillMemName1=BillMemName1&","&trim(rs1("BillMem4"))
					end if
				end if
				if BillMemName1<>"" then
				...
				if trim(rs2("BillMem4"))<>"" and not isnull(rs2("BillMem4")) then
					if BillMemName2="" then
						BillMemName2=trim(rs2("BillMem4"))
					else
						BillMemName2=BillMemName2&","&trim(rs2("BillMem4"))
					end if
				end if
				if BillMemName2<>"" then
...
<%
'DoubleCheckStatus暫時先都寫1
	strUpdate="Update BillBase set DoubleCheckStatus=1 where Sn="&trim(rs1("sn"))
