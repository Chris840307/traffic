...
End If
	If sys_City="苗栗縣" Then
		If chkIsSpeedRuleFlag_TC=1 Then
	%>
	<script language="JavaScript">
		alert("儲存成功,此車號在此違規日已有超速舉發紀錄 ,請再至舉發單資料維護系統確認！！");
	</script>
<%		End if
	End If 
end if
...
if session("BillIgnore_Fix")<>"" then
	...
	strSQL="select * from ( select b.CarNo,b.CarSimpleID,a.ProsecutionTime,a.ProsecutionTypeID,a.SiteCode,a.FileName,a.DirectoryName,a.FIXEQUIPTYPE,a.IMAGEFILENAMEA,a.VideoFileName,a.IMAGEFILENAMEB,a .IMAGEFILENAMEC,a.Location,b.LawItemID,b.SN,a.LIMITSPEED,a.OVERSPEED,a.OperatorA,b.MemberID,b.Note from PI a, PIDetail b where a.FILENAME = b.FILENAME and a.OperatorA=b.Operator and b.Operator='"&trim(Session("Credit_ID"))&"' and FixEquipType in (1,2,5,8,10) and VERIFYRESULTID<>-1 and BillSn is null and (a.RejectCode<>'262' or a.RejectCode is null) "&strIgnorePlus&" order by  FIXEQUIPTYPE desc,FileName,Location,PROSECUTIONTIME desc ) where rownum<=1"
...
if not rs13.eof Then
	if sys_City="苗栗縣" Then	
		if trim(rs13("CarNo"))<>"" and not isnull(rs13("CarNo")) then
			bCarNo=trim(rs13("CarNo"))
		end if
	End If 
	...
	if trim(rs13("IllegalDate"))<>"" and not isnull(rs13("IllegalDate")) then
		...
		bIllegalDate2=trim(rs13("IllegalDate"))
	end if
	If sys_City="高雄市" Then
...
<%end if%>
<table width='1280' border='1' align="left" cellpadding="0">
	<tr>
	...
	<%end if%>
		...
		<tr>
			...
			<td>
				<input type="Checkbox" name="BillCheck" value="1" checked>是 &nbsp;<font color="red"><%
		If bIllegalDate2<>"" And Not IsNull(bIllegalDate2) then
			Temp_IllegalDate="to_date('"&Year(bIllegalDate2)&"/"&month(bIllegalDate2)&"/"&Day(bIllegalDate2) &" "&Hour(bIllegalDate2)&":"&Minute(bIllegalDate2)&":0','YYYY/MM/DD/HH24/MI/SS')"
			strSQLchk="select * from billruncaraccept where CarNo='"&bCarNo&"' and IllegalDate="&Temp_IllegalDate&" and RecordStateID=0"
			Set rschk=conn.execute(strSQLchk)
			If rschk.eof Then
				response.write "＊上一筆案件登記簿無紀錄"
			End If
			rschk.close
			Set rschk=Nothing
		End if
	...
	<%if not rs1.eof then%>
		<tr>
			<td bgcolor="#FFFFCC" width="25%"><div align="right"> <span class="style3">＊</span>車號&nbsp;</div></td>
			<td >
				<table >
				<tr>
				<td>
				<input type="text" size="11" name="CarNo" onBlur="getVIPCar();" value="<%
				...
				<span class="style6">
				<div id="Layer7" style="position:absolute; width:100px; height:24px; z-index:0;  border: 1px none #000000; color: #FF0000; font-weight: bold;"><%
		...
		<tr>
			...
			<td >
				...
				<tr>
				<td>
				<input type="text" maxlength="1" size="5" value="<%
				...
				<td style="vertical-align:text-top;">
				<div id="Layer012" style="position:absolute; width:185px; height:27px; z-index:1; visibility: visible;">
				<font class="style7">&nbsp;1汽車 / 2拖車 / 3重機 / 4輕機<br>&nbsp;/ 5動力機械 / 6臨時車牌</font></div>
				</td>
				</tr>
				</table>
			</td>
		...
		<tr>
			<td bgcolor="#FFFFCC"><div align="right">
				<span class="style3">＊</span>違規時間</div>
			</td>
			<td>
						<!-- 違規日期 -->
			...
			if sys_City<>"苗栗縣" then
				...
				End If
			Else
				if trim(rs1("FixEquipType"))="5" Then
					if trim(rs1("ProsecutionTime"))<>"" and not isnull(rs1("ProsecutionTime")) then 
						response.write gInitDT(rs1("ProsecutionTime"))
					End If
				End If 
			...
			%>" onBlur="getBillFillDate()" style=ime-mode:disabled onkeydown="funTextControl(this);" onkeyup="IllegalDateKeyUP()" >&nbsp;
				...
				%>onBlur="getBillData()"<%
				End If 
				%> style=ime-mode:disabled onkeydown="funTextControl(this);" onKeyUP="IllegalTimeKeyUP()">
			</td>	
		</tr>
		<tr>
			<td bgcolor="#FFFFCC"><div align="right">實際/限制速度</div></td>
			<td >
			實際
				<input type="text" size="5" maxlength="3" name="IllegalSpeed" class='Text1' value="<%
		...
		End if 
			...
			限制
			<input type="text" size="5" name="RuleSpeed" maxlength="3" class='Text1' value="<%
		If left(Trim(bRule1),5)="33101" Or left(Trim(bRule1),5)="43102" Or left(Trim(bRule1),2)="40" Or Trim(bRule1)="" then
		...
		<tr>
			<td bgcolor="#FFFFCC" height="55">
				<div align="right"><span class="style3">＊</span>法條一</div>
			</td>
			<td valign="top">
		...
		<tr>
			<td bgcolor="#FFFFCC">
				<div align="right"><span class="style3">＊</span>違規地點&nbsp;</div>
			</td>
			<td>
...
'				End If 
				...
				<!-- <input type="checkbox" name="getChkIllegalAddress" value="1" <%
				If Trim(request("getChkIllegalAddress"))="1" Then
					response.write "checked"
				End If 
				%>>
				不要帶入登記簿地點 -->
				<br>
				<input type="text" size="37" value="<%
				if trim(rs1("FixEquipType"))="5" Then
					if bIllegalAddress<>"" Then
				...
				<td style="vertical-align:text-top;">
				<div id="Layerert45" style="position:absolute ; width:280px; height:28px; z-index:0;  layer-background-color: #FFFFFF; border: 1px none #000000;"><span class="style1">快速道路</span>&nbsp; &nbsp; &nbsp;
				<input type="button" value="修改登記簿地址" onclick="UpdateAddress();">
				</div>
				</td>
				</tr>
				</table>
		...
		<tr>
			<td bgcolor="#FFFFCC" height="30">
				<div align="right"><span class="style3">＊</span>舉發人一&nbsp;</div>
			</td>
			<td>
				<table >
				<tr>
		...
		<tr>
			<td bgcolor="#FFFFCC" height="30">
				<div align="right">舉發人二</div>
			</td>
			<td>
				<table >
				<tr>
				<td>
				<input type="text" size="9" name="BillMem2" value="<%
		...
		<tr>
			<td bgcolor="#FFFFCC" height="30">
				<div align="right">舉發人三</div>
			</td>
			<td >
				<table >
				<tr>
				<td>
				<input type="text" size="9" name="BillMem3" value="<%
		...
		<tr>
			<td bgcolor="#FFFFCC" height="30" colspan="1">
			<div align="right" >舉發人四</div>
			</td>
			<td >
				<table >
				<tr>
				<td>
				<input type="text" size="9" name="BillMem4" value="<%
		...
		<tr>
			<td bgcolor="#FFFFCC">
			<div align="right"><span class="style3">＊</span>舉發單位</div>
			</td>
			<td >
				<table >
				<tr>
		...
		<tr>
			<td bgcolor="#FFFFCC">
			<div align="right">
			<span class="style3">＊</span>填單日期</div>
			</td>
			<td >
			...
			&nbsp; &nbsp; &nbsp; &nbsp; 民眾檢舉時間
			<input type="text" name="JurgeDay" value="" size="10" maxlength="7" onkeydown="funTextControl(this);" style=ime-mode:disabled onblur="this.value=this.value.replace(/[^\d]/g,'');">
			</td>
		</tr>
		<tr>
			<td bgcolor="#FFFFCC" >
				<div align="right">專案代碼&nbsp;</div>
			</td>
			<td >
				<table >
				<tr>
		...
		<tr>
			...
			<td >
				...
				<tr>
				<td>
				&nbsp;<input type="text" maxlength="2" size="4" value="" name="CarAddID" onBlur="getAddID();" style=ime-mode:disabled onkeydown="funTextControl(this);">
				</td>
				<td style="vertical-align:text-top;">
				<div id="Layer110" style="position:absolute; width:265px; height:27px; z-index:1;  visibility: visible;">
...
<%
		...
		Operator1=""
		Operator2=""
		strSQL2="select * from (select b.CarNo,b.CarSimpleID,a.ProsecutionTime,a.ProsecutionTypeID,a.SiteCode,a.FileName,a.DirectoryName,a.FIXEQUIPTYPE,a.VideoFileName,a.IMAGEFILENAMEA,a.IMAGEFILENAMEB,a .IMAGEFILENAMEC,a.Location,b.LawItemID,b.SN,a.LIMITSPEED,a.OVERSPEED,a.OperatorA,b.MemberID,b.Note from PI a, PIDetail b where a.FILENAME = b.FILENAME and a.OperatorA=b.Operator and b.Operator='"&trim(Session("Credit_ID"))&"' and FixEquipType in (1,2,5,8,10) and VERIFYRESULTID<>-1 and BillSn is null and (a.RejectCode<>'262' or a.RejectCode is null)"&strIgnorePlus&" order by FIXEQUIPTYPE desc,FileName,Location,PROSECUTIONTIME desc ) where rownum<=5"
		...
		<%if not rs1.eof then%>
			...
			<img src="<%=sPicWebPath2%>" border=1 id="SmallImg2" width="300" height="180" >
	...
	<img src="<%=sPicWebPath%>" border=1 id="SmallImg" width="300" height="180" onmousemove="show(this, '<%=bPicWebPath%>')">
	...
	<%end if%>
		...
		if rs1.eof then
			...
			<input type="button" name="Submit1343" onClick="location='BillKeyIn_Image_Fix_ML.asp'" value="清 除 F4" style="font-size: 10pt; width: 70px; height: 27px">
...
<%end if%>
			<input type="button" name="SubmitBack2" onClick="location='BillKeyIn_Image_Fix_Back_ML.asp?PageType=First'" value="<< 第一筆 Home" style="font-size: 9pt; width: 100px; height: 27px">
			
			<input type="button" name="SubmitBack" onClick="location='BillKeyIn_Image_Fix_Back_ML.asp?PageType=Back'" value="< 上一筆 PgUp" style="font-size: 9pt; width: 100px; height: 27px">
...
<%if sys_City="苗栗縣" then%>
MoveTextVar("CarNo,CarSimpleID,IllegalDate,IllegalTime||IllegalSpeed,RuleSpeed,Rule1,Rule2||IllegalAddressID,IllegalAddress,BillMem1||BillMem2,BillMem3,BillMem4||BillUnitID,BillFillDate,JurgeDay,ProjectID,CarAddID");
<%else%>
MoveTextVar("CarNo,CarSimpleID,IllegalDate,IllegalTime||IllegalAddressID,IllegalAddress,Rule1,IllegalSpeed,RuleSpeed||Rule2,BillMem1||BillUnitID,BillFillDate,CarAddID,ProjectID");
<%end if%>
...
function getChkCarIllegalDate(){
	...
	NewIllSpeed=myForm.IllegalSpeed.value;
	NewRuleSpeed=myForm.RuleSpeed.value;
	if (myForm.BillCheck.checked==true){
		NewBillCheck=1;
	}else{
		NewBillCheck=0;
	}
	runServerScript("getChkCarIllegalDate.asp?CarID="+NewCarNo+"&IllDate="+NewIllDate+"&IllTime="+NewIllTime+"&IllRule1="+NewIllRule1+"&IllRule2="+NewIllRule2+"&CarSimpleID="+NewCarSimpleID+"&BillUnitID="+NewBillUnitID+"&IllegalAddress="+NewIllegalAddress+"&BillCheck="+NewBillCheck+"&IllSpeed="+NewIllSpeed+"&RuleSpeed="+NewRuleSpeed);
}
//檢查同車號同法條在同一天違規日期及違規時間前後兩小時內
...
<%end if%>
		...
		if(RuleDetail==7){
			ErrorStringChkCarIllegal='登記簿檢核無此筆違規資料或輸入資料錯誤，請確認是否正確。\n';
		}
...
function getVIPCar(){
	...
	if (myForm.CarNo.value.length >= 1){
		...
		if (CarType==0){
			...
			runServerScript("getVIPCarForKeyIn.asp?CarID="+CarNum+"&BillType=2&BillImg=1");
		}else{
			runServerScript("getVIPCarForKeyIn.asp?CarID="+CarNum+"&BillType=2&BillImg=1");
...
function UpdateAddress(){
	if (myForm.IllegalAddress.value==""){
		...
		UrlStr="UpdateRunAcceptAddress.asp?OldAddress="+myForm.IllegalAddress.value;
		newWin(UrlStr,"UpdateRunAcceptAddressWin2342",680,375,100,50,"yes","yes","yes","no");
	}
}
//審核無效
...
%>
		...
		event.keyCode=0;   
		event.returnValue=false;  
		location='BillKeyIn_Image_Fix_ML.asp'
...
%>
		...
		event.keyCode=0;   
		location='BillKeyIn_Image_Fix_Back_ML.asp?PageType=Back'
<%if sys_City<>"苗栗縣" then %>
	}else if (event.keyCode==36){ //第一筆Home
		event.keyCode=0;   
		location='BillKeyIn_Image_Fix_Back_ML.asp?PageType=First'
<%end if%>
	}
}
...
function getBillData(){
	...
	if ((myForm.IllegalTime.value.length==4) && (myForm.IllegalDate.value.length==6 || myForm.IllegalDate.value.length==7) && (myForm.CarNo.value!="")){
		...
		//window.open("getBillData.asp?CarNo="+myForm.CarNo.value+"&IllegalDate="+myForm.IllegalDate.value+"&IllegalTime="+myForm.IllegalTime.value+"&nowTime=<%=now%>","WebPage1","left=0,top=0,location=0,width=800,height=660,resizable=yes,scrollbars=yes,status=yes");
	}
