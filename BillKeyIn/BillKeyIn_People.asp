...
if trim(request("kinds"))="DB_Update"  then
	...
	If not ifnull(request("PBillSN")) Then
		strSQL="select count(1) cnt from passerJude where billsn="&trim(request("PBillSN"))
		set rsjude=conn.execute(strSQL)
		chkJude=cdbl(rsjude("cnt"))
		rsjude.close
	End if 
...
strSQL = "select ZipID,ZipName from Zip where ZipName like '"&replace(left(trim(request("DriverAddress")),3),"臺","台")&"%'"
	...
	strUpd="update PasserBase set BillNo='"&UCase(trim(request("Billno1")))&"',CarNo='"&UCase(trim(request("Sys_CarNo")))&"',IllegalDate="&theIllegalDate&",CARSIMPLEID=null,Insurance=null,IllegalSpeed=null,RuleSpeed=null"&_
		...
		",Note='"&trim(request("Note"))&"',FromNote='"&trim(request("FromNote"))&"',FromNoteId='"&trim(request("FromNoteId"))&"',IsLecture='"&trim(request("IsLecture"))&"'" &_
		...
		If not ifnull(request("ReportNo")) Then

			strReportNo="delete PasserReportNo where billsn="&trim(request("PBillSN"))
			conn.execute strReportNo

			strReportNo="insert into PasserReportNo(BillSN,ReportNo)" &_
				" values("&trim(request("PBillSN"))&",'"&trim(request("ReportNo"))&"')"
			conn.execute strReportNo
		End If 
		...
		updDciLog="update PasserDCILog set BillNo='"&UCase(trim(request("Billno1")))&"',CarNo='"&UCase(trim(request("Sys_CarNo")))&"' where billsn="&trim(request("PBillSN"))
		
		ConnExecute updDciLog,353
		conn.execute updDciLog
		...
		If trim(request("ProjectID"))="traffic009" then
			...
			" where illegaldate > to_date('2022/11/30 23:59:59','yyyy/mm/dd/hh24/mi/ss') and SN="&trim(request("PBillSN"))

			conn.execute strUpd

		elseIf ifnull(request("ProjectID")) then

			strUpd="update PasserBase set CARSIMPLEID=null" &_
			" where SN="&trim(request("PBillSN"))
...
strSQL = "select ZipID,ZipName from Zip where ZipName like '"&replace(left(trim(request("DriverAddress")),3),"臺","台")&"%'"
		...
		strInsert="insert into PasserBase(SN,BillTypeID,BillNo,CarNo,IllegalDate" & _
				...
				",Note,FromNote,FromNoteId,RuleVer,IsLecture,DoubleCheckStatus"&ColAdd&")" &_
				...
				",'"&trim(request("Note"))&"','"&trim(request("FromNote"))&"','"&trim(request("FromNoteId"))&"','"&theRuleVer&"','"&trim(request("IsLecture"))&"'" &_
	...
	'查流水號
		...
		If hid_BillTypeID <> 2 Then
			...
			If trim(request("ProjectID"))="traffic009" then
				...
				strUpd="update PasserBase set CARSIMPLEID=8" &_
				" where illegaldate > to_date('2022/11/30 23:59:59','yyyy/mm/dd/hh24/mi/ss') and SN="&theSN
		...
		If not ifnull(request("ReportNo")) Then
			...
			strReportNo="delete PasserReportNo where billsn="&theSN
			conn.execute strReportNo
...
strSQL="select NVL(Max(to_number(DoubleCheckStatus)),0) as DoubleCheckStatus from (select DoubleCheckStatus from passerBase where billunitid in (select UnitID from UnitInfo where UnitTypeID=(select UnitTypeid from Unitinfo uit where UnitID='"&trim(Session("Unit_ID"))&"')) and TO_CHAR(RecordDate,'YYYY')=TO_CHAR(sysdate,'YYYY'))"
set rssum=conn.execute(strSQL)
If rssum.eof Then
	FileSeq=1
else
	...
	<form name="myForm" method="post">  
		<table width='1000' border='1' align="left" cellpadding="1">
					...
					<%
						If sys_City = "台中市" Then
							...
							If hid_BillTypeID = 2 Then
								...
								Response.Write "&nbsp;&nbsp;告示單號：&nbsp;"
								Response.Write "<input type=""text"" size=""13"" name=""ReportNo"" onkeydown=""funTextControl(this);"" style=ime-mode:disabled maxlength=""11"" onBlur=""getAcceptData();"" onkeyup=""this.value=this.value.toUpperCase();"">"
							else
								
								response.Write "<input name=""ReportNo"" type=""hidden"" value="""">"
							end If 
				...
				<td>
					<table border="0">
						<td width="150">
							...
							<span  style="font-size: 12px;">居留證/護照/統編</span>
						</td>
			...
			<tr>
				...
				<td>
				<input type="text" class="btn5" size="10" value="<%=request("IllegalDate")%>" maxlength="7" name="IllegalDate" onBlur="getDealLineDate(this);" onkeydown="funTextControl(this);"  onKeyUp="funAutoTextControl(this);" style=ime-mode:disabled>
				</td>
		  	...
		  	<td bgcolor="#FFFFCC" align="right"><span class="style4">＊</span>舉發單位</td>
				...
				<span id="Layer001" style="font-size:16px; background-color:#CCFFFF;"></span>
					...
					<table border="0" cellpadding="0" cellspacing="0">
						...
						<tr><td>
							<input type="radio" name="StreetType" value="traffic010" onClick="myForm.ProjectID.value=this.value;ProjectF5();">
							個人行動器具
						</td><td>
					<%'end if%>
			...
			<tr>
				...
				<td colspan="5">
					<input type="text" class="btn5" size="10" value="<%
						If hid_BillTypeID = 2 Then Response.Write gInitDT(date)
					%>" maxlength="7" name="BillFillDate" onKeyUp="funAutoTextControl(this);" onkeydown="funTextControl(this);" style=ime-mode:disabled>
				</td>
			</tr>
			<tr>
			...
			<tr>
			  <td bgcolor="#1BF5FF" align="center" colspan="6">
				...
				if CheckPermission(235,2)=false then
					...
					<input type="button" name="Submit1343" onClick="ChangUrl();" value="清 除 F4" class="btn1">
					<img src="/image/space.gif" width="29" height="8">
					...
					<input type="button" name="Submit1343" onClick="ChangUrl();" value="新增一筆" class="btn1">
                </span>
				<!-- 違規人性別 -->
...
if(myForm.hid_BillTypeID.value==2){
	MoveTextVar("Billno1,Sys_CarNo||IllegalDate,IllegalTime||IllegalAddressID,IllegalAddress||Rule1||Rule2||RuleSpeed,IllegalSpeed||DealLineDate,MemberStation,BillMem1||BillMem2,BillMem3,BillMem4||BillUnitID||BillFillDate,Note,FromNote,FromNoteId");
	...
	MoveTextVar("Sys_BatChNumber,ReportNo||Billno1,Sys_CarNo,Insurance||DriverPID,DriverSEX,DriverName||DriverBrith,DriverZip,DriverAddress||IllegalDate,IllegalTime||IllegalAddressID,IllegalZip,IllegalAddress||Rule1||Rule2||RuleSpeed,IllegalSpeed||DealLineDate,MemberStation,BillMem1||BillMem2,BillMem3,BillMem4||SignType||BillUnitID||BillFillDate,Note");

}else{

	MoveTextVar("Billno1,Sys_CarNo,Insurance||DriverPID,DriverSEX,DriverName||DriverBrith,DriverZip,DriverAddress||IllegalDate,IllegalTime||IllegalAddressID,IllegalAddress||Rule1||Rule2||RuleSpeed,IllegalSpeed||DealLineDate,MemberStation,BillMem1||BillMem2,BillMem3,BillMem4||SignType||BillUnitID||BillFillDate,Note,FromNote,FromNoteId");
...
function InsertBillVase(){
	...
	if (myForm.DriverName.value==""&&myForm.hid_BillTypeID.value!='2'){
		error=error+1;
		errorString=errorString+"\n"+error+"：請輸入違規人姓名。";
	}/*
	...
	if (myForm.DriverAddress.value==""&&myForm.hid_BillTypeID.value!='2'){
		error=error+1;
		errorString=errorString+"\n"+error+"：請輸入違規人地址。";
	}
	if (myForm.DriverPID.value==""&&myForm.hid_BillTypeID.value!='2'){
		error=error+1;
		errorString=errorString+"\n"+error+"：請輸入違規人身份證號碼。";
	}
	/*
	if (myForm.DriverSEX.value!=""&&myForm.hid_BillTypeID.value!='2'){
	...
	*/
	if(myForm.DriverBrith.value!=""&&myForm.hid_BillTypeID.value!='2'){
...
<%end if%>
	...
	if (myForm.SignType.value==""&&myForm.hid_BillTypeID.value!='2'){
		error=error+1;
		errorString=errorString+"\n"+error+"：請輸入簽收狀況。";
	}
...
//由違規日期帶入應到案日期
function getDealLineDate(obj){
	if(obj.value!=obj.value.replace(/[^\d]/g,'')){
		obj.value=obj.value.replace(/[^\d]/g,'');
	}

	if(obj.name=='IllegalDate'){
	...
	if (myForm.hid_BillTypeID.value!='2'&&myForm.Sys_BilltypeID.value!='2'){
