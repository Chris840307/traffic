...
if trim(request("kinds"))="BillQry" then
	...
	if not rsQry.eof then
		...
		if trim(rsQry("BillTypeID"))="1" then
			...
			Session("BillTime_Stop")=Year(BillTime_StopTmp)&"/"&Month(BillTime_StopTmp)&"/"&day(BillTime_StopTmp)&" "&hour(BillTime_StopTmp)&":"&minute(BillTime_StopTmp)&":"&second(BillTime_StopTmp)

			strSqlCnt="select count(*) as cnt from BillBase where BillTypeID='1' and BillStatus in ('0') and RecordStateID=0 and RecordMemberID="&trim(Session("User_ID"))&" and RecordDate between TO_DATE('"&Year(BillTime_StopTmp)&"/"&Month(BillTime_StopTmp)&"/"&day(BillTime_StopTmp)&" 0:0:0','YYYY/MM/DD/HH24/MI/SS') and TO_DATE('"&date&" 23:59:59','YYYY/MM/DD/HH24/MI/SS') and RecordDate < TO_DATE('"&Session("BillTime_Stop")&"','YYYY/MM/DD/HH24/MI/SS')"
	...
	%>
	<script language="JavaScript">
		opener.location="BillKeyIn_Car_Back.asp?PageType=Back";
		window.close();
	</script>
	<%	'逕舉
	...
	<%
		...
		elseif trim(rsQry("BillTypeID"))="2" and (trim(rsQry("ImageFileName"))<>"" and not isnull(rsQry("ImageFileName"))) then
			...
			Session("BillTime_Image")=year(BillTime_ImageTmp)&":"&Month(BillTime_ImageTmp)&":"&day(BillTime_ImageTmp)&" "&hour(BillTime_ImageTmp)&":"&minute(BillTime_ImageTmp)&":"&second(BillTime_ImageTmp)

			strSqlCnt="select count(*) as cnt from BillBase where BillTypeID='2' and BillStatus in ('0') and RecordStateID=0 and RecordMemberID="&trim(Session("User_ID"))&" and RecordDate < TO_DATE('"&Session("BillTime_Image")&"','YYYY/MM/DD/HH24/MI/SS') and ImageFileName is not null"
			set rsCnt1=conn.execute(strSqlCnt)
				Session("BillOrder_Image")=trim(rsCnt1("cnt"))+1
			rsCnt1.close
			set rsCnt1=nothing
			'response.write Session("BillOrder_Image")
			'response.write strSqlCnt
	'response.end
	%>
	<script language="JavaScript">
<%	if sys_City="高雄縣" then%>
		opener.location="BillKeyIn_Image_fix_Back.asp?PageType=Back";
<%	else%>
		opener.location="BillKeyIn_Image_fix_Back_KSC.asp?PageType=Back";
<%	end if%>
		//window.close();
	</script>
	<%
