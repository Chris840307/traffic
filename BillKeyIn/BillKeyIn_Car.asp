...
if trim(request("kinds"))="DB_insert" Then
	...
	strBillChk="select BillNo from BillBase where BillNo='"&UCase(trim(request("Billno1")))&"' and RecordStateID=0"
	set rsBillChk=conn.execute(strBillChk)
	if Not rsBillChk.eof Then
		chkIsExistBillNumFlag=1
	End If
	rsBillChk.close
	Set rsBillChk=Nothing 

	strBillChk="select BillNo from PasserBase where BillNo='"&UCase(trim(request("Billno1")))&"' and RecordStateID=0"
	set rsBillChk=conn.execute(strBillChk)
	if Not rsBillChk.eof Then
		chkIsExistBillNumFlag=1
	End If
	...
	'刪除失敗或未回傳的不能建
	chkBillBaseDeleteErrorFlag=0
	strDci="select DciReturnStatusID from (select DciReturnStatusID from (select BillNo,ExchangeTypeID,DciReturnStatusID from Dcilog where BillNo='"&UCase(trim(request("Billno1")))&"' and ExchangeTypeID='E' order by ExchangeDate Desc) where Rownum<=1) where (DciReturnStatusID<>'S' or DciReturnStatusID is null)"
...
'		End If 
	End If 
	If chkIsExistBillNumFlag=0 and chkIsRule56Flag=0 and chkIllegalAddress53Flag=0 and chkIllegalAddressID53Flag=0 And chkIsRule5620002Flag_TC=0 And chkBillBaseDeleteErrorFlag=0 then
		'違規日期
		theIllegalDate=""
		...
		If sys_City="花蓮縣x" then
			IllegalAddressTemp=trim(request("CityStreet"))&trim(request("IllegalAddress"))
		Else
			IllegalAddressTemp=trim(request("IllegalAddress"))
		End If
		papercheckValue = trim(request("papercheck"))
If papercheckValue = "1" Then
    papercheckValue = 1
Else
    papercheckValue = 0
End If
		strInsert="insert into BillBase(SN,BillTypeID,BillNo,CarNo,CarSimpleID,CarAddID,IllegalDate" & _
					...
					",Note,FromNote,FromNoteId,papercheck,EquipmentID,RuleVer,DriverSex,TrafficAccidentNo,TrafficAccidentType,SignType"&ColAdd&",isVideo)" &_
					...
					",'"&IllegalAddressTemp&"','"&trim(request("Rule1"))&"',"&theIllegalSpeed &_
					...
					",'"&trim(request("Note"))&"','"&trim(request("FromNote"))&"','"&trim(request("FromNoteId"))&"','"&papercheckValue&"','"&trim(request("IsMail"))&"','"&theRuleVer&"'" &_
					",'"&trim(request("DriverSex"))&"','"&trim(request("TrafficAccidentNo"))&"','"&theTrafficAccidentType&"','"&UserSignType&"'" &_
					""&valueAdd&",'"&Trim(request("IsVideo"))&"'" &_
					")"
...
<%	
	ElseIf chkBillBaseDeleteErrorFlag=1 Then
	%>
	<script language="JavaScript">
		alert("儲存失敗，此單號監理站尚未回傳刪除成功，請至 ' 上傳下載查詢系統 ' 確認刪除後再建檔！！");
	</script>
<%		
	end If
...
<%		
	...
	If sys_City="基隆市" Then
		...
		If UCase(trim(request("Billno1")))<>"" then
			If IsNumeric(Right(trim(request("Billno1")),6)) Then
				If CDbl(Right(trim(request("Billno1")),6))<>0 And CDbl(Right(trim(request("Billno1")),6))<>1 then
					NewCheckBillNO=Left(trim(request("Billno1")),3)&Right("000000"&CDbl(Right(trim(request("Billno1")),6))-1 ,6)
					'response.write NewCheckBillNO
					strChk="select * from BillBaseView where billno='"&NewCheckBillNO&"'"
					Set rsChk=conn.execute(strChk)
					If rsChk.eof Then
%>
	<script language="JavaScript">
		alert("漏號 : 前一張舉發單號：<%=NewCheckBillNO%>未曾建檔，請確認！！");
	</script>
<%		
					End If 
					rsChk.close
		...
		if trim(request("IllegalDate"))<>"" Then
			...
			If Weekday(gOutDT(request("IllegalDate")))=2 Or Weekday(gOutDT(request("IllegalDate")))=1 Then
				OverDay=4
			ElseIf Weekday(gOutDT(request("IllegalDate")))=7 Then
				OverDay=5
			Else
				OverDay=6
			End If 
			'response.write OverDay
			If DateDiff("d",gOutDT(request("IllegalDate")) ,date) > OverDay Then
%>
	<script language="JavaScript">
		alert("攔停案件超過4日,逾時舉發！！");
	</script>
...
<%end if%>
	<form name="myForm" method="post">  
		<table width='985' border='1' align="center" cellpadding="1">
			<tr bgcolor="#1BF5FF">
				<td colspan="6"><strong>攔停資料建檔作業</strong>&nbsp; &nbsp; 日期格式：1150101 &nbsp;時間格式：2300(24小時制)&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; 建檔日期：<%=ginitdt(now)%>&nbsp; &nbsp; <input type="checkbox" name="CaseInByMem" value="1" <%
			'if sys_City="台中市" then
		...
		End If 
			...
			<%elseif sys_City="台中市" then%>
				<td bgcolor="#FFFFCC"><div align="right">批號</div></td>
				<td >
				...
				If Trim(request("AcceptBatchNumber"))<>"" Then
					session("AcceptBatchNumber_Stop")=Trim(request("AcceptBatchNumber"))
					response.write Trim(request("AcceptBatchNumber"))
				ElseIf Trim(session("AcceptBatchNumber_Stop"))<>"" Then
					response.write Trim(session("AcceptBatchNumber_Stop"))
				End If 
			...
			<%else%>
				...
				<td >
					...
					If Trim(request("IsVideo"))="1" Then
						'response.write "checked"
					End If 
					%>>有
					<input type="radio" name="IsVideo" value="0" <%
					If Trim(request("IsVideo"))="0" Then
						'response.write "checked"
					End If 
					%>>無
					&nbsp; &nbsp; 
	...
	<%end if%>
		...
		<%End If %>
			...
			%>">
				...
				<td style="vertical-align:text-top;">
				<div id="Layer111" style="position:absolute; width:235px; height:24px; z-index:0; border: 1px none #000000; color: #FF0000;">
				<span class="style7"> 1汽車/ 2拖車/ 3重機/ 4輕機/ 5動力機械/<%
			If sys_City="苗栗縣" Then
				response.write "<br>6 臨時車牌"
			Else
				response.write " 6臨時車牌/ 7試車牌(車號不須輸入中文字)"
			End if%></span>&nbsp;&nbsp;
				</td>
				</tr>
				</table>
...
<%End if%>
	...
	<%if sys_City="高雄市" Or sys_City=ApconfigureCityName then%>
			...
			<%end if%>>
				...
				<td bgcolor="#FFFFCC" align="right">違規人出生日</td>
				<td <%
		if sys_City<>"嘉義縣" and sys_City<>"新竹市" and sys_City<>"台東縣" and sys_City<>"高雄縣" and sys_City<>"台南縣" and sys_City<>"台南市" and sys_City<>"嘉義市" and sys_City<>"花蓮縣" then 
				response.write "colspan=""3"""
		end if
	...
	<%if sys_City<>"雲林縣" then%>
			...
			if sys_City<>"嘉義縣" and sys_City<>"新竹市" and sys_City<>"台東縣" and sys_City<>"台南市" and sys_City<>"嘉義市" and sys_City<>"苗栗縣" then
			...
			<%End If %>
				...
				<td <%If sys_City<>"台東縣" And sys_City<>"台中市" then%>colspan="3"<%End If %>>

				<table >
				<tr>
				<td>
				<input type="text" size="10" maxlength="4" name="IllegalTime" onBlur="this.value=this.value.replace(/[^\d]/g,'')" onkeydown="funTextControl(this);" style=ime-mode:disabled <%if sys_City="高雄市" Or sys_City=ApconfigureCityName then '打滿跳下格%>
	...
	<%else%>
			...
			%></span>		
				...
				<td>
				<input type="text" size="10" maxlength="7" value="<%
			if sys_City<>"嘉義縣" and sys_City<>"新竹市" and sys_City<>"台東縣" and sys_City<>"高雄縣" and sys_City<>"台南市" and sys_City<>"嘉義市" then
				response.write trim(request("IllegalDate"))
			end if
				%>" name="IllegalDate" onfocus="this.select()" onBlur="getDealLineDate()" onkeydown="funTextControl(this);" style=ime-mode:disabled>
				</td>
				<td bgcolor="#FFFFCC"><div align="right"><span class="style4">＊</span>違規時間</div></td>
				<td>
				<input type="text" size="10" maxlength="4" name="IllegalTime" onBlur="this.value=this.value.replace(/[^\d]/g,'')" onkeydown="funTextControl(this);" style=ime-mode:disabled >
				</td>
	<%end if%>
			</tr>
<%if sys_City="南投縣" then%>
...
<%if sys_City<>"嘉義市" then%>
			<tr>
				...
				<td>
					...
					%>" name="IllegalAddressID" onKeyUp="getillStreet();" onkeydown="funTextControl(this);" style=ime-mode:disabled onblur="getillStreet2();">
					<input type="hidden" name="OldIllegalAddressID" value="<%
			...
			<%	if sys_City="花蓮縣x" Then%>
					...
					response.write request("CityStreet")
			...
			<%	End if%>
					...
					%>" name="IllegalAddress" style=ime-mode:active onkeyup="AutoGetIllStreet();" onkeydown="funTextControl(this);" onblur="funGetSpeedRule()"<%
					if sys_City="台南市" Then Response.Write " onfocus=""autoKeyEnd();"""
					%>>
					<input type="checkbox" name="chkHighRoad" value="1" <%if trim(request("chkHighRoad"))="1" then response.write "checked"%> onclick="setIllegalRule()" <%if sys_City="南投縣" then response.write "disabled"%>><span class="style1">快速道路</span>
					<%if sys_City="台中市" then %>
						<table >
						<tr>
						<td>
...
<%end if%>
			<tr>
				...
				<td colspan="5">
					...
					<td>
					<input type="text" maxlength="9" size="10" value="<%
			if sys_City<>"嘉義縣" and sys_City<>"新竹市" and sys_City<>"澎湖縣" and sys_City<>"台東縣" and sys_City<>"高雄縣" and sys_City<>"台南縣" and sys_City<>"台南市" and sys_City<>"嘉義市" and sys_City<>"苗栗縣" and sys_City<>"台中市" and sys_City<>"南投縣" then
...
<%end if%>
			<tr>
				...
				if sys_City<>"苗栗縣" then
					...
					%>" onBlur="this.value=this.value.replace(/[^\d]/g,'')" onkeydown="funTextControl(this);" style=ime-mode:disabled <%
				if sys_City="高雄市" Or sys_City=ApconfigureCityName then '打滿跳下格%>
			...
			<%	if sys_City="花蓮縣" then%>
					...
					If Trim(request("chkbDealLineDate"))="1" Then
						response.write "checked"
					End if
					%>>45天
			<% End if%>
				...
				<td>
					...
					if sys_City="花蓮縣" or sys_City="南投縣" or sys_City="彰化縣" or sys_City="台中縣" or sys_City="屏東縣" or sys_City="新竹市" or sys_City="高雄縣" or sys_City="嘉義市" or sys_City="高雄市" Or sys_City=ApconfigureCityName then 
				...
				<%if sys_City="台南市" then%>
					<br>
					<font color="red" size="2">法條35條(處乘客以外)只能輸入78</font>
				<%End if%>
				</td>
...
'					end if
			...
			<%If sys_City="台中市" then%>
				<td bgcolor="#FFFFCC" width="13%"><div align="right">舉發人姓名</div></td>
				<td colspan="3">
					...
					<input type="hidden" name="BillMem3" value="">
					...
					<input type="hidden" name="BillMem4" value="">
...
<%end if%>
			...
			<tr>
				...
				if sys_City<>"苗栗縣" then
					response.write trim(request("BillUnitID"))
				end if
					%>" <%
				if sys_City="高雄市" then
					response.write "readonly"
			...
			<%if sys_City<>"嘉義縣" and sys_City<>"新竹市" and sys_City<>"台東縣" and sys_City<>"高雄縣" and sys_City<>"台南縣" and sys_City<>"台南市" and sys_City<>"嘉義市" and sys_City<>"花蓮縣" and sys_City<>"苗栗縣" and sys_City<>"澎湖縣" then %>
				...
				If  sys_City<>"基隆市" then
					response.write trim(request("BillFillDate"))
				End if
				...
				End If 
					%>" onkeydown="funTextControl(this);" style=ime-mode:disabled <%if sys_City="高雄市" Or sys_City=ApconfigureCityName then '打滿跳下格%>
...
<td><input type="text" size="20" value="" name="FromNoteId" onkeydown="funTextControl(this);" style=ime-mode:active></td>
<td bgcolor="#FFFFCC"><div align="right">稽核確認</div></td>
<td><input type="checkbox" name="papercheck" value="1"> <span class="style10" style="color: #FF0000;">打勾允許入案</span></td>
</tr>
<%if sys_City="高雄市" Or sys_City=ApconfigureCityName then%>
			<tr>
				<td bgcolor="#FFFFCC"><div align="right">代保管物1</div></td>
...
<input type="button" name="Submit4232" onClick="funPrintCaseList_Stop2();" value="缺漏號清冊" class="btn1">
				...
				<div id="Layer1f69" style=" width:250px; height:14px; z-index:0; layer-background-color: #11FFFF; border: 1px none #000000; color: #FF0000;">
			...
			<tr>
				...
				<br>
				＊ 35800001酒駕處乘客案件，第二筆後案件，請先將違規時間加(減)一小時儲存，再回到上一筆點選『酒駕處乘客案件修改違規時間』功能修改時間
				<br>
				＊ 如需郵寄列印大宗清冊、送達證書的案件，是否郵寄請勾選『是』
				<br>
...
<%elseif sys_City="台中市" then%>
MoveTextVar("Billno1,Insurance||DriverPID,DriverBrith||CarNo,CarSimpleID||IllegalDate,IllegalTime||IllegalAddressID,IllegalAddress,IllegalZip||Rule1||Rule2||RuleSpeed,IllegalSpeed||DealLineDate,MemberStation,BillMem1||BillMem2,BillMemNameQry||Fastener1,Fastener2,Fastener3||BillUnitID,SignType,BillFillDate||CarAddID,ProjectID||Note,TrafficAccidentNo,TrafficAccidentType");
...
<%end if%>
	...
	if (myForm.IllegalDate.value==""){
		...
		errorString=errorString+"\n"+error+"：違規日期輸入錯誤。";
<%if sys_City="苗栗縣" then%>
	}else if (!ChkIllegalDateML(myForm.IllegalDate.value) && myForm.TrafficAccidentNo.value=="" && myForm.CaseInByMem.checked==false){
		error=error+1;
		errorString=errorString+"\n"+error+"：違規日期已超過二個月期限。";
...
<%end if%>
	...
	if (myForm.BillFillDate.value==""){
		...
		errorString=errorString+"\n"+error+"：填單日期不得比今天晚。";
<%if sys_City="苗栗縣" then%>
	}else if (!ChkIllegalDateML(myForm.BillFillDate.value) && myForm.TrafficAccidentNo.value=="" && myForm.CaseInByMem.checked==false){
		error=error+1;
		errorString=errorString+"\n"+error+"：填單日期已超過二個月。";
	}
<%else%>
...
<%end if %>
	...
	if (myForm.DealLineDate.value==""){
		...
		error=error+1;
		errorString=errorString+"\n"+error+"：應到案日期輸入錯誤。";
	}else if( myForm.DealLineDate.value>="2000101"){
		error=error+1;
		errorString=errorString+"\n"+error+"：應到案日期超過民國200年。";
<%if sys_City="苗栗縣" then%>
	}else if (!ChkIllegalDateML(myForm.DealLineDate.value) && myForm.TrafficAccidentNo.value=="" && myForm.CaseInByMem.checked==false){
		error=error+1;
		errorString=errorString+"\n"+error+"：應到案日期已超過二個月。";
	}
<%else%>
...
<%end if %>
	if (error==0){
		...
		Dyear=parseInt(myForm.DealLineDate.value.substr(0,myForm.DealLineDate.value.length-4))+1911;
		Dmonth=myForm.DealLineDate.value.substr(myForm.DealLineDate.value.length-4,2);
		Dday=myForm.DealLineDate.value.substr(myForm.DealLineDate.value.length-2,2);
		var DDay=new Date(Dyear,Dmonth-1,Dday);

		var OverDate=new Date();
		OverDate=DateAdd("d",30,IllDate);
		if (DDay < OverDate){
			error=error+1;
			errorString=errorString+"\n"+error+"：應到案日小於違規日加30天，請確認是否正確(小於30天無法入案)。";	
		}
	}
...
<%elseif sys_City="苗栗縣" then%>
	...
	if (myForm.BillUnitID.value!=""){
		if (myForm.BillUnitID.value.substr(0,2)=="03" && myForm.BillUnitID.value!="03B6"){
			if (myForm.Billno1.value.substr(0,2)!="F1" && myForm.Billno1.value.substr(0,2)!="F2" && myForm.Billno1.value.substr(0,2)!="FY"){
				error=error+1;
		...
		if (myForm.BillUnitID.value.substr(0,2)=="3N"){
			if (myForm.Billno1.value.substr(0,2)!="F3" && myForm.Billno1.value.substr(0,2)!="F2" && myForm.Billno1.value.substr(0,2)!="F1"){
				error=error+1;
		...
		if (myForm.BillUnitID.value.substr(0,2)=="3R"){
			if (myForm.Billno1.value.substr(0,2)!="F6" && myForm.Billno1.value.substr(0,2)!="F2" && myForm.Billno1.value.substr(0,2)!="F3"){
				error=error+1;
				errorString=errorString+"\n"+error+"：大湖分局開頭碼應為F6,請確認單號開頭碼是否正確。";
			}
		}
		if (myForm.BillUnitID.value.substr(0,2)=="3Q"){
			if (myForm.Billno1.value.substr(0,2)!="F7" && myForm.Billno1.value.substr(0,2)!="F2" && myForm.Billno1.value.substr(0,2)!="F4"){
				error=error+1;
...
<%end if %>
	...
	<%if sys_City="台南市" then%>
		if ((myForm.Rule1.value.substr(0,2))=="35" && (myForm.Rule1.value.substr(0,3))!="357" && (myForm.Rule1.value.substr(0,3))!="358" && (myForm.Rule1.value.substr(0,3))!="359" && myForm.Rule1.value!="351000011" && myForm.Rule1.value!="351000021" && myForm.Rule1.value!="351000031" && myForm.Rule1.value!="352000011" && myForm.Rule1.value!="352000021" && myForm.Rule1.value!="353000011")
		{
			if (myForm.MemberStation.value!="78")
			{
				error=error+1;
				errorString=errorString+"\n"+error+"：酒駕法條，應到案處所必須填入78，輸入其他監理站會被監理站系統剔退。";
			}
		}
	<%end if%>
<%if sys_City="台東縣" then%>
...
<%if sys_City="基隆市" then%>
	if ((myForm.Rule1.value.substr(0,2))=="53" || (myForm.Rule1.value.substr(0,5))=="48102")
	{
		if (myForm.IllegalAddressID.value==""){
			error=error+1;
			errorString=errorString+"\n"+error+"：法條53條、48條1項2款，違規地點只可用違規地點代碼代入，不可自行輸入或修改違規地點。";
		}
	}	
<%end if %>
<%if sys_City="雲林縣" then %>
...
function InsertLaw(){
	...
	<%elseif sys_City="台中市" then%>
	MoveTextVar("Billno1,Insurance||DriverPID,DriverBrith||CarNo,CarSimpleID||IllegalDate,IllegalTime||IllegalAddressID,IllegalAddress,IllegalZip||Rule1||Rule2||Rule3||RuleSpeed,IllegalSpeed||DealLineDate,MemberStation,BillMem1||BillMem2,BillMemNameQry||Fastener1,Fastener2,Fastener3||BillUnitID,SignType,BillFillDate||CarAddID,ProjectID||Note,TrafficAccidentNo,TrafficAccidentType");
	<%elseif sys_City="澎湖縣" then%>
...
<%end if%>
	...
	<%if sys_City="新竹市" Or sys_City="基隆市" Or sys_City="台南市" Or sys_City="台東縣" then%>
	...
	'彰化拒簽收要30天
	%>
		if (myForm.IsMail[0].checked==true && (myForm.SignType.value=="A" || myForm.SignType.value=="5")){
...
<%elseif sys_City="台中市" then%>
	...
	runServerScript("getChkCarIllegalDate.asp?CarID="+NewCarNo+"&IllDate="+NewIllDate+"&IllTime="+NewIllTime+"&IllRule1="+NewIllRule1+"&IllRule2="+NewIllRule2+"&CarSimpleID="+NewCarSimpleID+"&BillUnitID="+NewBillUnitID+"&IllegalAddress="+NewIllegalAddress+"&BillCheck=2&BillNO="+myForm.Billno1.value+"&AcceptBatchNumber="+myForm.AcceptBatchNumber.value+"&AcceptBatchNumberChk="+AcceptBatchNumberChk_Temp+"&nowTime=<%=now%>");
<%else%>
...
function funChkCityStreet(){
	if (myForm.CityStreet.value != "")
	{
		runServerScript("getChkCityStreet.asp?CityStreet="+myForm.CityStreet.value+"&nowTime=<%=now%>");
	}
}
<%end if %>
...
function getVIPCar(){
	...
	if (myForm.CarNo.value.length >= 1 && ((Rule1tmp.substr(0,2))!="32" && (Rule2tmp.substr(0,2))!="32" && (Rule1tmp.substr(0,5))!="12102" && (Rule2tmp.substr(0,5))!="12102" && (Rule1tmp.substr(0,3))!="334" && (Rule2tmp.substr(0,3))!="334")){
		...
		if (CarType==0){
			runServerScript("getVIPCarForKeyIn.asp?CarID="+CarNum+"&BillType=1"+"&nowTime=<%=now%>");
...
function TabFocus(){
	...
	Rule1tmp=myForm.Rule1.value;
	Rule2tmp=myForm.Rule2.value;
<%if sys_City<>"南投縣" and sys_City<>"雲林縣" and sys_City<>"彰化縣" and sys_City<>"嘉義縣" and sys_City<>"新竹市" and sys_City<>"台東縣" and sys_City<>"高雄縣" and sys_City<>"台南縣" and sys_City<>"台南市" and sys_City<>"嘉義市" and sys_City<>"台中縣" and sys_City<>"高雄市" and sys_City<>"苗栗縣" and sys_City<>ApconfigureCityName then %>
...
function getRuleData3(){
	if (myForm.Rule3.value.length > 6){
		...
		var CarSimpleID=myForm.CarSimpleID.value;
		var VerNo=<%=theRuleVer%>;
		runServerScript("getRuleDetail.asp?RuleOrder=3&RuleID="+Rule3Num+"&CarSimpleID="+CarSimpleID+"&RuleVer="+VerNo+"&nowTime=<%=now%>");
		//CallChkLaw3();
...
function getRuleData4(){
	if (myForm.Rule4.value.length > 6){
		...
		var CarSimpleID=myForm.CarSimpleID.value;
		var VerNo=<%=theRuleVer%>;
		runServerScript("getRuleDetail.asp?RuleOrder=4&RuleID="+Rule4Num+"&CarSimpleID="+CarSimpleID+"&RuleVer="+VerNo+"&nowTime=<%=now%>");
		//CallChkLaw4();
...
<%if sys_City="" and session("Unit_ID")<>"JT00" then%>
...
<%end if%>
	...
	{
		if (myForm.chkIllegalZip.checked==false)
		{
...
function getDealLineDate(){
	...
	<%if sys_City="高雄市" then%>
		if (myForm.DriverBrith.value.length>=6 && myForm.IllegalDate.value.length>=6){
			//var x=new Date();
			//var thisYear=x.getFullYear()-1911;
			...
			DLineDate=new Date(Iyear,Imonth-1,Iday);
			BirthYear14=DateAdd2("y",-14,DLineDate);

			if (eval(BirthYear14) < eval(BrithDate)){
				alert("違規人年齡低於14歲!!\n未滿14歲之人違規應處罰其法定代理人或監護人。\n(提示訊息，按確定後可繼續作業)");
			}else if (eval(BirthYear) < eval(BrithDate)){
				alert("違規人年齡低於18歲!!\n(提示訊息，按確定後可繼續作業)");
			}
		}
	<%else%>
...
<%if sys_City<>"嘉義縣" and sys_City<>"新竹市2" and sys_City<>"台東縣" and sys_City<>"高雄縣" and sys_City<>"台南縣" and sys_City<>"台南市" and sys_City<>"嘉義市" and sys_City<>"花蓮縣" and  sys_City<>"基隆市" then %>
	BFillDateTemp=myForm.IllegalDate.value;
	if (BFillDateTemp.length >= 6){
...
<%if sys_City="新竹市" or sys_City="台東縣" then %>
...
function CheckBillNoExist(){
	...
	if (myForm.Billno1.value!=""){
		...
		<%else%>
			runServerScript("getCheckBillNoExist.asp?BillNo="+BillNum+"&nowTime=<%=now%>");
			//window.open("getCheckBillNoExist.asp?BillNo="+BillNum+"&nowTime=<%=now%>","WebPage1","left=20,top=10,location=0,width=500,height=455,resizable=yes,scrollbars=yes");
		<%end if %>
		}else{
	<%if sys_City<>"高雄市" and sys_City<>ApconfigureCityName  then %>
			alert("單號不足九碼！");
			<%if not (trim(Session("Unit_ID"))="6000" and sys_City="雲林縣") then%>
...
function FuncChkPID(){
	...
	if (myForm.DriverPID.value.length == 10){
		...
		<%if sys_City<>"高雄市" and sys_City<>ApconfigureCityName then%>
			alert("身分證輸入錯誤(如為外籍證號則無須理會此訊息)！\n(提示訊息，按確定後可繼續作業)");
			//myForm.DriverPID.focus();
		<%else%>
			if(confirm('身分證輸入錯誤(如為外籍證號則無須理會此訊息)，是否繼續？\n是 請按『確定』，否 請按『取消』')){
