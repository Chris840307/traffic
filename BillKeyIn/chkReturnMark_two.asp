...
strSQL = "select d.DCIRETURNSTATION,b.DCIStationName,b.StationID,c.UnitID,c.UnitName,d.CarNo,d.Owner,e.MailReturnDate from BillBase a,Station b,UnitInfo c,BillbaseDCIReturn d,BillMailHistory e where a.RecordStateID=0 and a.BillNo=d.BillNo and a.CarNo=d.CarNo and d.DCIRETURNSTATION=b.DCIStationID(+) and a.BillUnitID=c.UnitID(+) and a.RecordStateID=0 and a.BillNo=e.BillNo(+) and a.Sn=e.BillSn(+) and d.BillNo='"&trim(Ucase(request("BillNo")))&"'"
...
if Not rscnt.eof then
	...
	if(err=='0'){
		...
		if(City=='彰化縣'){
			...
			myForm.MailDate[myForm.chkcnt.value-1].value='<%=trim(gInitDT(DateAdd("d",90,rscnt("MailReturnDate"))))%>';
		}
