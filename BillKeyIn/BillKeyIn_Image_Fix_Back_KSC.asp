...
if trim(request("kinds"))="DB_insert" Then
	...
	chkReKeyInImgBill=0
	If sys_City="彰化縣" Or (sys_City="金門縣" And Trim(Session("BillKeyIn_ReportKeyInType"))="1") Then
		If (left(trim(request("Rule1")),2)="56" And Len(trim(request("Rule1")))=7) Or (left(trim(request("Rule1")),2)="55" And Len(trim(request("Rule1")))=7) then
			...
			" and ((Rule1 like '55%' and length(Rule1)=7) or (Rule2 like '55%' and length(Rule2)=7) or (Rule1 like '56%' and length(Rule1)=7) or (Rule2 like '56%' and length(Rule2)=7)) and Recordstateid=0  and CheckFlag in ('0','1')" &_
		...
		Else
			strChk="select count(*) as cnt from BillbaseTmp where CarNo='"&Trim(request("CarNo"))&"'" & _
			" and IllegalDate="& theIllegalDate & _
			" and Rule1='"&Trim(request("Rule1"))&"' and Recordstateid=0  and CheckFlag in ('0','1')" &_
			" and sn<>"&Trim(request("BillSN"))
...
If chkIsSpeedTooOver=0 And checkReportCaseFlag=0 And chkReKeyInImgBill=0 then
	...
	If sys_City="彰化縣" Or (sys_City="金門縣" And Trim(Session("BillKeyIn_ReportKeyInType"))="1") Then
		...
		",DriverID='"&UCase(trim(request("DriverPID")))&"'" &_
	...
	Else
		...
		",BillFillerMemberID='"&trim(request("BillMemID1"))&"',BillFiller='"&trim(request("BillMemName1"))&"'" &_
		",BillFillDate="&theBillFillDate&",DealLineDate="&theDealLineDate&_
		",Note='"&trim(request("Note"))&"',FromNote='"&trim(request("FromNote"))&"',FromNoteId='"&trim(request("FromNoteId"))&"',EquipmentID='1'" &_
	...
	if rs1.eof Then
		If Session("BillKeyIn_ReportKeyInType")="1" Then
			if trim(request("PageType"))="Next" then
				Response.Redirect "BillKeyIn_Image_Fix_KSC.asp?SessionFlag=1&ReportKeyInType=1"&test_flag_temp
			elseif trim(request("PageType"))="Back" then
				Response.Redirect "BillKeyIn_Image_Fix_KSC.asp?SessionFlag=1&ReportKeyInType=1"&test_flag_temp
			elseif trim(request("PageType"))="First" then
				Response.Redirect "BillKeyIn_Image_Fix_KSC.asp?SessionFlag=1&ReportKeyInType=1"&test_flag_temp
		...
		Else
			if trim(request("PageType"))="Next" then
				Response.Redirect "BillKeyIn_Image_Fix_KSC.asp?SessionFlag=1"&test_flag_temp
			elseif trim(request("PageType"))="Back" then
				Response.Redirect "BillKeyIn_Image_Fix_KSC.asp?SessionFlag=1"&test_flag_temp
			elseif trim(request("PageType"))="First" then
				Response.Redirect "BillKeyIn_Image_Fix_KSC.asp?SessionFlag=1"&test_flag_temp
...
Else
	If sys_City="嘉義縣" then
		strSql="select * from (select a.*,(select b.FixEquipType from ProsecutionImage b,ProsecutionImageDetail c where c.FileName=b.FileName and b.OperatorA=c.Operator and c.billsn=a.sn AND rownum<=1) as FixEquipType from BillBase a where a.BillTypeID='2' and a.BillStatus in ('0') and a.RecordStateID=0 and a.RecordMemberID="&theRecordMemberID&" and exists (select d.billsn from ProsecutionImageDetail d where d.billsn=a.sn ) "&sqlPage&") where rownum<=1"
	else
...
%>' border='1' align="center" cellpadding="0">
	...
	<%end if%>
	<%
	if sys_City="高雄市" Or sys_City="彰化縣" Or sys_City="金門縣" Or sys_City="屏東縣" Or sys_City="台南市" Or sys_City=ApconfigureCityName then
		if not rs1.eof then%>
		<!-- 影像小圖C -->
		<%
		sPicWebPath2=""
	...
	<%if not rs1.eof then%>
		...
		<%If sys_City<>"彰化縣" and sys_City<>"金門縣" Then%>
	...
	<tr>
		...
		<table width='100%' border='1' align="left" cellpadding="0">
			<tr>
				<td bgcolor="#1BFFFF" width="7%"><div align="right"> <span class="style3">＊</span>車號</div></td>
				<td width="13%">
					<table >
					<tr>
					<td>
				...
				<td bgcolor="#1BFFFF" width="9%"><div align="right"><span class="style3">＊</span>車種&nbsp;</div></td>
				<td width="<%
			If sys_City="高雄市" Then
				response.write "19%"
			else
				...
				End if
					...
					<font class="style7">&nbsp;1汽車 / 2拖車 / 3重機<br>&nbsp;/4輕機/5動力機械/6臨時車牌</font>
					</div>
					</td>
					</tr>
					</table>
				</td>
				<td bgcolor="#1BFFFF"><div align="right"><span class="style3">＊</span>違規時間</div></td>
				<td colspan="5">
				<!-- 違規日期 -->&nbsp;
...
%>
			...
			<tr>
		<%if sys_City<>"苗栗縣" then%>
				<td bgcolor="#1BFFFF"><div align="right"><span class="style3">＊</span>地點</div></td>
				<td colspan="3">
		...
		<%end if%>
				<td bgcolor="#1BFFFF" ><div align="right"><span class="style3">＊</span>法條一</div></td>
		...
		<%end if%>
					...
					%>>
		<%if sys_City<>"苗栗縣" then%>
			<%if sys_City="南投縣" Or sys_City="屏東縣" Or sys_City="花蓮縣" Or sys_City="台南市" then%>
					限制<input type="text" size="2" maxlength="3" class="Text1" name="RuleSpeed" value="<%
					if trim(rs1("RuleSpeed"))<>"" and not isnull(rs1("RuleSpeed")) then
						response.write trim(rs1("RuleSpeed"))
					end if
		...
		<%End If %>
			...
			<%if sys_City="苗栗縣" then%>
				<td bgcolor="#1BFFFF" ><div align="right">法條二</div></td>
			...
			<%end if%>
				<td bgcolor="#1BFFFF"><div align="right"><span class="style3">＊</span>舉發人</div></td>
				<td colspan="<%
				If sys_City="高雄市" or sys_City="台中市" or sys_City="台南市" Then
					response.write "3"
				elseIf sys_City="苗栗縣" Then
					response.write "4"
				Else
...
<%If sys_City="台中市" then%>
				<td bgcolor="#1BFFFF"><div align="right">告示單號</div></td>
				<td >
...
<%If sys_City<>"新竹市" and sys_City<>"苗栗縣" then%>
			<%If sys_City="高雄市" Or sys_City="台南市" then%>
				<td bgcolor="#1BFFFF" height="30"><div align="right" style="font-size: 12px ;">舉發人二</div></td>
				<td >
					<table >
					<tr>
					<td>
...
<%If sys_City="新竹市" or sys_City="苗栗縣" then%>
			<tr>
				<td bgcolor="#1BFFFF"><div align="right">舉發人二</div></td>
				<td colspan="2">
					<table >
					<tr>
					<td>
				...
				<td bgcolor="#1BFFFF"><div align="right">舉發人三</div></td>
				<td colspan="2">
					<table >
					<tr>
					<td>
				...
				<td bgcolor="#1BFFFF"><div align="right">舉發人四</div></td>
				<td colspan="3">
					<table >
					<tr>
					<td>
...
<%End if%>
			<tr>
				<td bgcolor="#1BFFFF" ><div align="right"><span class="style3">＊</span><span class="style4">舉發單位</span></div></td>
				<td colspan="3">
					<table  >
					<tr>
					<td width="75px">
				...
				%>" style="width:50px;ime-mode:disabled;" maxlength="7" onkeydown="funTextControl(this);" onblur="this.value=this.value.replace(/[^\d]/g,'');">
			<%if sys_City="高雄市" then%>
		...
		<%if sys_City<>"苗栗縣" then%>
				<td bgcolor="#1BFFFF" align="right" width="8%" >輔助車種</td>
				<td width="6%">
		...
		<%End If %>
				<td bgcolor="#1BFFFF"><div align="right">專案代碼</div></td>
				<td >
		...
		<%if sys_City="苗栗縣" then%>
				<td bgcolor="#1BFFFF" align="right" width="9%" >輔助車種</td>
				<td >
                 <input type="text" maxlength="2" size="4" value="<%
				if trim(rs1("CarAddID"))<>"" and not isnull(rs1("CarAddID")) then
					response.write trim(rs1("CarAddID"))
		...
		<%if sys_City="新竹市" Or sys_City="彰化縣" Or sys_City="嘉義縣" Or sys_City="屏東縣" Or sys_City="保二總隊三大隊一中隊" then%>
			<tr>
				<td bgcolor="#1BFFFF" align="right" width="8%">備註&nbsp;</td>
				<td colspan="9">
				<input type="Text" size="40" value="<%
					if trim(rs1("Note"))<>"" and not isnull(rs1("Note")) then
						response.write trim(rs1("Note"))
					end if
					%>" name="Note" onkeydown="funTextControl(this);" style=ime-mode:active>
				</td>
			</tr>
		<%End If%>
		<%'if sys_City="台南市" or sys_City="高雄市" or sys_City="基隆市" then%>
			<tr>
			<%if sys_City="台南市" then%>
				<td bgcolor="#1BFFFF" align="right" width="8%">採証工具</td>
				<td colspan="3">
					<table >
					<tr>
					<td>
			...
			<%end if%>
				<td bgcolor="#1BFFFF"><div align="right">身分證號<br><span class="style10">非轉歸責案件勿填</span></div></td>
		  		<td>
				...
				<td bgcolor="#1BFFFF" <%if sys_City="台南市" then%>colspan="2"<%end if%>><div align="right">應到案處所<br><span class="style10">非轉歸責案件勿填</span></div>
...
<tr>
<td bgcolor="#1BFFFF"><div align="right">來源平台</div></td>
<td colspan="2"><input type="text" size="20" value="<%
...
<td bgcolor="#1BFFFF"><div align="right">平台單號/案號</div></td>
<td colspan="7"><input type="text" size="20" value="<%
		...
		<td height="28" colspan="2" align="center">
			...
			%>" style="font-size: 9pt; width: 60px; height: 27px" >
				...
				<%End if%>
					...
					<input type="button" name="SubmitBack" onClick="location='BillKeyIn_Image_Fix_Back_KSC.asp?PageType=First<%=test_flag_temp%>'" value="<< 第一筆 Home" style="font-size: 9pt; width: 90px; height: 27px">
					<input type="button" name="SubmitBack" onClick="location='BillKeyIn_Image_Fix_Back_KSC.asp?PageType=Back<%=test_flag_temp%>'" value="< 上一筆 PgUp" style="font-size: 9pt; width: 90px; height: 27px">
					...
					<input type="button" name="SubmitNext" onClick="location='BillKeyIn_Image_Fix_Back_KSC.asp?PageType=Next<%=test_flag_temp%>'" value="下一筆 PgDn >" style="font-size: 9pt; width: 90px; height: 27px">
					<input type="button" name="SubmitBack" onClick="location='BillKeyIn_Image_Fix_Back_KSC.asp?PageType=Last<%=test_flag_temp%>'" value="最後一筆 End >>" style="font-size: 9pt; width: 90px; height: 27px">
				<!-- 逕舉類別 -->
...
<%elseif sys_City="高雄市" then%>
MoveTextVar("CarNo,CarSimpleID,IllegalDate,IllegalTime||IllegalAddressID,IllegalZip,IllegalAddress,Rule1,RuleSpeed,IllegalSpeed||Rule2,BillMem1,BillMem2||BillUnitID,JurgeDay,ReportCaseNo,BillFillDate,CarAddID,ProjectID||DriverPID,MemberStation");
<%elseif sys_City="台南市" then%>
MoveTextVar("CarNo,CarSimpleID,IllegalDate,IllegalTime||IllegalAddressID,IllegalZip,IllegalAddress,Rule1,RuleSpeed,IllegalSpeed||Rule2,BillMem1,BillMem2||BillUnitID,JurgeDay,BillFillDate,CarAddID,ProjectID||UseTool,DriverPID,MemberStation");
<%elseif sys_City="苗栗縣" then%>
...
<%elseif sys_City="台中市" then%>
MoveTextVar("CarNo,CarSimpleID,IllegalDate,IllegalTime||IllegalAddressID,IllegalAddress,Rule1,IllegalSpeed,RuleSpeed||Rule2,BillMem1,ReportNo||BillUnitID,JurgeDay,BillFillDate,CarAddID,ProjectID||DriverPID,MemberStation");
<%elseif sys_City="基隆市" then%>
...
<%end if %>
	...
	*/
	...
	//	funcOpenBillQry();
		...
		event.keyCode=0;   
		event.returnValue=false; 
		location='BillKeyIn_Image_Fix_Back_KSC.asp?PageType=Next<%=test_flag_temp%>'
		...
		event.keyCode=0;   
		event.returnValue=false; 
		location='BillKeyIn_Image_Fix_Back_KSC.asp?PageType=Last<%=test_flag_temp%>'
	}
}
