...
'if trim(request("kinds"))="img_Upload" then
	...
	rsFile.close
	set rsFile=nothing
	FtpLocation=FtpLocation & "Type8/" & trim(Session("Credit_ID")) & "/" & (year(now)-1911)&Right("00"&month(now),2)&Right("00"&day(now),2) & "/"
	FtpLocationIE8=FtpLocationIE8 &trim(Session("Credit_ID")) & "/" & (year(now)-1911)&Right("00"&month(now),2)&Right("00"&day(now),2) & "/"
