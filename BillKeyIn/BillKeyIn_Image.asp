...
if trim(request("kinds"))="DB_insert" then
	...
	strBillImage="Insert Into BILLILLEGALIMAGE(BillSn,BillNo,ImageFileNameA,ImageFileNameB,ImageFileNameC," &_
		...
		",'"&trim(request("gImageFileNameB"))&"','','"&trim(theImagePathName)&"')"
...
if trim(request("kinds"))="BillIgnore" then
	if trim(request("SelFileName"))<>"" then
		if session("BillIgnore_Image")<>"" then
			session("BillIgnore_Image")=session("BillIgnore_Image")&",'"&request("SelFileName")&"'"
		else
			session("BillIgnore_Image")="'"&request("SelFileName")&"'"
		end if
	end if
end if
if session("BillIgnore_Image")<>"" then
	strIgnorePlus=" and a.FileName not in ("&session("BillIgnore_Image")&")"
	strIgnorePlus2=" and ProsecutionImage.FileName not in ("&session("BillIgnore_Image")&")"
else
	strIgnorePlus=""
	strIgnorePlus2=""
end if
...
if trim(request("getStreetName"))<>"" and trim(request("getStreetName"))<>"all" then
	strSQL="select b.CarNo,b.CarSimpleID,a.ProsecutionTime,a.ProsecutionTypeID,a.SiteCode,a.FileName,a.FIXEQUIPTYPE,a.VideoFileName,a.IMAGEFILENAMEA,a.VideoFileName,a.IMAGEFILENAMEB,a.Location,b.LawItemID,b.SN,a.LIMITSPEED,a.OVERSPEED,a.OperatorA,b.MemberID,b.Note from ProsecutionImage a, ProsecutionImageDetail b where a.FILENAME = b.FILENAME and FixEquipType=3 and VERIFYRESULTID<>-1 and BillSn is null and (a.RejectCode<>'262' or a.RejectCode is null) and a.Location='"&trim(request("getStreetName"))&"'"&strIgnorePlus&" order by FIXEQUIPTYPE desc,Location,PROSECUTIONTypeID,PROSECUTIONTIME desc"
else
	strSQL="select b.CarNo,b.CarSimpleID,a.ProsecutionTime,a.ProsecutionTypeID,a.SiteCode,a.FileName,a.FIXEQUIPTYPE,a.VideoFileName,a.IMAGEFILENAMEA,a.VideoFileName,a.IMAGEFILENAMEB,a.Location,b.LawItemID,b.SN,a.LIMITSPEED,a.OVERSPEED,a.OperatorA,b.MemberID,b.Note from ProsecutionImage a, ProsecutionImageDetail b where a.FILENAME = b.FILENAME and FixEquipType=3 and VERIFYRESULTID<>-1 and BillSn is null and (a.RejectCode<>'262' or a.RejectCode is null)"&strIgnorePlus&" order by FIXEQUIPTYPE desc,Location,PROSECUTIONTypeID,PROSECUTIONTIME desc"
end if
...
strSqlCnt="select count(*) as cnt from BillBase a,ProsecutionImage b,ProsecutionImageDetail c where a.BillTypeID='2' and a.BillStatus in ('0','1') and a.RecordStateID=0 and a.RecordMemberID="&theRecordMemberID&" and a.SN=c.BillSN and c.FileName=b.FileName and b.FixEquipType=3 "
...
<table width='1000' border='1' align="left" cellpadding="0">
	<tr>
		...
		<input type="button" name="uploadb1" value='上傳"員警/類比式固定桿"舉發數位相片' onclick='window.open("../ProsecutionImage/ProsecutionImageUpload.asp","WebPageUpload","left=0,top=0,location=0,width=770,height=555,resizable=yes,scrollbars=yes")' style="font-size: 10pt; width: 235px; height: 28px">
		<br>
		<a href="../ProsecutionImage/FtpSet.htm" target="_blank"><span class="style4">上傳注意事項</span></a>
		<br>
		<br>
		<a href="../ProsecutionImage/ProsecutionImage.asp?FIXEQUIPTYPE=3" target="_blank"><span class="style4">未處理 <%
		...
		strStreetCnt="select count(*) as locationCnt from ProsecutionImage,ProsecutionImageDetail where ProsecutionImage.FILENAME = ProsecutionImageDetail.FILENAME and FixEquipType=3 and VERIFYRESULTID<>-1 and BillSn is null and (RejectCode<>'262' or RejectCode is null)"&StrStreetPlus
		...
		if not rs1.eof then
			...
			sFIXEQUIPTYPE="upload"
			
			RealFileName=right(rs1("FileName").value,4)
			WebPicPathTmp=left(rs1("FileName").value,14)

			bPicWebPath=sImgWebPath & sFIXEQUIPTYPE & "/" & WebPicPathTmp & "/" & RealFileName & ".jpg"
	...
	%>
		<%
			sRealFileName=right(replace(rs1("ImageFileNameB"),".jpg",""),4)
			sWebPicPathTmp=left(replace(rs1("ImageFileNameB"),".jpg",""),14)

			sPicWebPath=sImgWebPath & sFIXEQUIPTYPE & "/" & sWebPicPathTmp & "/" & sRealFileName & ".jpg"
		%>
		<%if sPicWebPath<>"" then%>
	...
	<%if not rs1.eof then%>
		...
		<input type="button" onClick="OpenPic('<%=bPicWebPath%>')" value="檢視原圖一" style="font-size: 10pt; width: 100px; height: 27px">
			...
			if trim(sFIXEQUIPTYPE)="Type1" or trim(sFIXEQUIPTYPE)="Type2" then
				piIMAGEPATHNAMEA = sImgWebPath & trim(sFIXEQUIPTYPE) & "/" & trim(rs1("FileName")) & "/"
				piIMAGEFILENAMEA = trim(rs1("IMAGEFILENAMEA"))
			else
				piIMAGEPATHNAMEA = sImgWebPath & sFIXEQUIPTYPE & "/" & left(trim(rs1("FILENAME")),14) & "/"
				piIMAGEFILENAMEA = right(trim(rs1("IMAGEFILENAMEA")),8)
			end if
			response.write piIMAGEFILENAMEA
			%>">
			<input type="hidden" name="gImagePathNameA" value="<%=piIMAGEPATHNAMEA%>">
		<%if (trim(rs1("IMAGEFILENAMEB"))<>"" and not isnull(rs1("IMAGEFILENAMEB"))) and trim(rs1("ProsecutionTypeID"))="R" then%>
			...
			piIMAGEPATHNAMEB = sImgWebPath & trim(sFIXEQUIPTYPE) & "/" & trim(rs1("FileName")) & "/"
			piIMAGEFILENAMEB = trim(rs1("IMAGEFILENAMEB"))
			response.write piIMAGEFILENAMEB
			%>">
	...
	<tr>
		...
		<table width='100%' border='1' align="left" cellpadding="0">
			<tr>
				...
				<td>
				<input type="text" size="7" name="CarNo" onBlur="getVIPCar();" value="<%
			...
			if trim(Session("SpecUser"))="1" then
				strSC="select count(*) as cnt from SpecCar where CarNo='"&trim(rs1("CarNo"))&"' and recordStateid=0"
				...
				<td colspan="1">
                             &nbsp;
							<!-- 違規日期 -->
					<input type="text" size="6" maxlength="6" name="IllegalDate" class='Text1' value="<%
				...
				<td bgcolor="#FFFFCC" ><div align="right"><span class="style3">＊</span>簡式車種</div></td>
				<td >
				...
				if trim(rs1("CarSimpleID"))<>"" and not isnull(rs1("CarSimpleID")) then
					...
					%>" name="CarSimpleID" onBlur="getRuleAll();" onfocus="Layer012.style.visibility='visible';" style=ime-mode:disabled onkeydown="funTextControl(this);">
				</td>
			</tr>
			<tr>
				...
				<td colspan="3">
					...
					<span class="style5">
					<div id="Layer1" style="position:absolute ; width:260px; height:30px; z-index:0; border: 1px none #000000;"><%
					if trim(rs1("LawItemID"))<>"" and not isnull(rs1("LawItemID")) then
				...
				<td bgcolor="#FFFFCC"><div align="right">違規地點</div></td>
				<td colspan="5">
...
'				end if
			...
			<tr>
				<td bgcolor="#FFFFCC" width="10%"><div align="right"><span class="style3">＊</span>舉發人代碼</div></td>
		  		<td width="12%">
					<input type="text" size="4" name="BillMem1" value="<%
				if trim(rs1("OperatorA"))<>"" then
					strMem1="select LoginID,MemberID,UnitID from MemberData where ChName='"&trim(rs1("OperatorA"))&"'"
					set rsMem1=conn.execute(strMem1)
					if not rsMem1.eof then
						response.write trim(rsMem1("LoginID"))
						BillRecordID=trim(rsMem1("MemberID"))
						BillMemUnitID=trim(rsMem1("UnitID"))
					end if
					rsMem1.close
					set rsMem1=nothing
				else
				...
				%>" onKeyUp="getBillMemID1();" style=ime-mode:disabled onkeydown="funTextControl(this);">
					...
					<span class="style5">
					<div id="Layer12" style="position:absolute ; width:130px; height:30; z-index:0;  border: 1px none #000000;"><%
					if trim(rs1("OperatorA"))<>"" and not isnull(rs1("OperatorA")) then
						response.write trim(rs1("OperatorA"))
					else
				...
				<td width="15%">
					...
					<span class="style5">
					<div id="Layer6" style="position:absolute ; width:140px; height:30px; z-index:0;  border: 1px none #000000; "><%
				...
				<td bgcolor="#FFFFCC" width="8%"><div id="Layer110" style="position:absolute; width:265px; height:27px; z-index:1; background-color: #FFCCCC; visibility: hidden;">
				...
				%>" maxlength="6" name="BillFillDate" style=ime-mode:disabled onkeydown="funTextControl(this);">
				<td bgcolor="#FFFFCC" align="right" width="8%">輔助車種</td>
				<td width="6%">
                 <input type="text" maxlength="2" size="4" value="" name="CarAddID" onBlur="getAddID();" onfocus="Layer110.style.visibility='visible';" style=ime-mode:disabled onkeydown="funTextControl(this);">
				</td>
				...
				<!-- 採証工具 -->
					<input maxlength="1" size="4" value="3" name="UseTool"  onkeyup="getFixID();" type='hidden' style=ime-mode:disabled> 
	...
	<tr bgcolor="#FFCC33">
		<td height="28" colspan="2" align="center">
				...
				if rs1.eof then
					...
					<input type="button" name="Submit1343" onClick="location='BillKeyIn_Image.asp'" value="清 除 F4" style="font-size: 10pt; width: 70px; height: 27px">
				...
				if rs1.eof then
					...
					<input type="button" name="SubmitBack2" onClick="location='BillKeyIn_Image_Back.asp?PageType=First'" value="<< 第一筆 Home" style="font-size: 10pt; width: 100px; height: 27px">
...
function InsertBillVase(){
	...
	if ((myForm.Rule1.value.substr(0,5))=="33101" || (myForm.Rule1.value.substr(0,2))=="40" || (myForm.Rule1.value.substr(0,5))=="43102"){
		IllegalRule=getIllegalRule(myForm.IllegalAddress.value,myForm.RuleSpeed.value,myForm.IllegalSpeed.value,ProsecutionTypeID,myForm.chkHighRoad.checked);
...
function getChkCarIllegalDate(){
	...
	runServerScript("getChkCarIllegalDate.asp?CarID="+NewCarNo+"&IllDate="+NewIllDate+"&IllTime="+NewIllTime+"&IllRule1="+NewIllRule1+"&IllRule2="+NewIllRule2+"&CarSimpleID="+NewCarSimpleID+"&BillUnitID="+NewBillUnitID);
}
//檢查同車號同法條在同一天違規日期及違規時間前後兩小時內
