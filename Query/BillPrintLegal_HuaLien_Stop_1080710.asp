...
<div id="Layer01" class="style3" style="position:absolute; left:135px; top:20px; z-index:3"><B><%
...
<div id="Layer02" class="style3" style="position:absolute; left:135px; top:65px; z-index:2"><%
	response.write Sys_BillNo%>
</div>

<div id="Layer03" class="style3" style="position:absolute; left:155px; top:55px; z-index:1"><%
	Response.Write "<img src=""../BarCodeImage/"&Sys_BillNo&"_4.jpg"">"

	%>
</div>

<div id="Layer03" class="style3" style="position:absolute; left:350px; top:65px; z-index:1"><%
	If Sys_BillFillerMemberID = "3480" Then Response.Write "TG"
	%>
</div>

<div id="Layer44" class="style3" style="position:absolute; left:525px; top:165px; z-index:5">
	<%="<img src=""../BarCodeImage/"&Sys_MailNumber&"97300717"&".jpg"" height=""35"">"%><br>
    　<b><%=Sys_MAILCHKNUMBER%></b>
</div>

<div id="Layer03" class="style3" style="position:absolute; left:390px; top:285px; z-index:1"><%
	Response.Write "<img src=""../image/cutter.jpg""></img>"%>
</div>

<!---------------------------------- 第一段送達證書到這邊------------------------------------->

<div id="Layer04" class="style3" style="position:absolute; left:130px; top:290px; z-index:1"><b><%
	if trim(Sys_BillTypeID)="1" then
...
<div id="Layer06" class="style3" style="position:absolute; left:130px; top:310px; width:350px; z-index:1"><b><%
	'--------------------------------------如果是抓 戶籍補正的資料-----------------------------------------------------------------------------------------------------------
			if trim(Sys_BillTypeID)="1" then
...
<div id="Layer06" class="style3" style="position:absolute; left:130px; top:340px; width:330px; z-index:3">
<%
...
<div id="Layer05" class="style3" style="position:absolute; left:455px; top:297px; z-index:5">
	　<b>第<%=Sys_MailNumber%>號</b><br>
	<%="<img src=""../BarCodeImage/"&Sys_MailNumber&"97300717"&".jpg"">"%><br>
    　<b><%=Sys_MAILCHKNUMBER%></b>
</div>

<div id="Layer07" class="style3" style="position:absolute; left:60px; top:370px; z-index:1"><%
...
<div id="Layer05" class="style3" style="position:absolute; left:230px; top:355px; z-index:1"><%
	Response.Write "<img src=""../BarCodeImage/"&Sys_BillNo&"_4.jpg"">"%>
</div>

<div id="Layer08" class="style3" style="position:absolute; left:550px; top:390px; z-index:1"><%
...
<%
	if Sys_FillerUnitID="A002" then
		Response.Write "<div id=""Layer09"" style=""position:absolute; left:40px; top:480px; z-index:10"">"
		response.write "<img src=""./img/StopCar_HuaLien.jpg"" width=""680"">"
		Response.Write "</DIV>"
		
		Response.Write "<div id=""Layer18"" style=""position:absolute; left:140px; top:525px; width:100px; height:14px; z-index:11"">"&Sys_CarNo&"</div>"

		Response.Write "<div id=""Layer20"" style=""position:absolute; left:360px; top:525px; width:300px; height:17px; z-index:11"">"&funcCheckFont(Sys_Owner,16,1)&"</div>"

		Response.Write "<div id=""Layer32"" style=""position:absolute; left:140px; top:550px; width:400px; height:49px; z-index:11"">"
		response.write left(trim(Sys_Rule1),2)&"條 "
		response.write Mid(trim(Sys_Rule1),3,1)&"項 "&Mid(trim(Sys_Rule1),4,2)&"款"
		Response.Write "</div>"

		Response.Write "<div id=""Layer20"" style=""position:absolute; left:360px; top:550px; width:300px; height:17px; z-index:11"">"&Sys_BillNo&"</div>"

		Response.Write "<div id=""Layer29"" style=""position:absolute; left:590px; top:550px; width:300px; height:11px; z-index:22"">"&Sys_DealLineDate(0)&"年"&Sys_DealLineDate(1)&"月"&Sys_DealLineDate(2)&"日</div>"

	else
		if trim(Sys_ImageFileNameA)<>"" then
			Response.Write "<div id=""Layer09"" style=""position:absolute; left:40px; top:480px; z-index:5"">"
		...
		if trim(Sys_ImageFileNameB)<>"" and (Sys_Rule1 <>"6020303" and Sys_Rule2<>"6020303") then
			Response.Write "<div id=""Layer10"" style=""position:absolute; left:430px; top:480px; z-index:1"">"
...
%>
<%if showBarCode then%>
<div id="Layer1" style="position:absolute; left:50px; top:815px; width:202px; height:36px; z-index:5">Ｖ</div>
<%else%>
<div id="Layer2" style="position:absolute; left:50px; top:850px; width:202px; height:36px; z-index:5">Ｖ</div>
<%end if%>
<%if trim(Sys_BillTypeID)="1" or trim(Sys_DriverID)<>"" then%>
<div id="Layer3" style="position:absolute; left:170px; top:815px; width:202px; height:36px; z-index:5">v</div>
<%else%>
<div id="Layer4" style="position:absolute; left:170px; top:833px; width:202px; height:36px; z-index:5">v</div>
<%end if%>
...
%></div>
<div id="Layer10" style="position:absolute; left:500px; top:865px; width:233px; height:32px; z-index:3"><img src=<%="""../BarCodeImage/"&Sys_BillNo&"_4.jpg"""%>></div>

<div id="Layer12" style="position:absolute; left:120px; top:925px; width:250px; height:11px; z-index:6"><span class="style7">逕行舉發　<%=funcCheckFont(Sys_A_Name,18,1)%><br><%if int(Sys_Rule1)<>4340003 and int(Sys_Rule1)<>5620001 and Sys_BillFillerMemberID <> "3480" then response.write "附採證照片"%>　<%=Sys_CarColor%></span></div>

<div id="Layer14" style="position:absolute; left:370px; top:920px; width:324px; height:10px; z-index:4"><%if showBarCode then response.write "<font color=""red"">*本單可至郵局或委託代收之超商繳納</font>"%></div>

<div id="Layer17" style="position:absolute; left:620px; top:940px; width:99px; height:12px; z-index:10"><%=fastring%></div>
...
<div id="Layer21" style="position:absolute; left:125px; top:990px; width:590px; height:13px; z-index:14"><%=Sys_OwnerZip&" "&funcCheckFont(Sys_OwnerZipName&Sys_OwnerAddress,16,1)%></div>

<div id="Layer22" style="position:absolute; left:120px; top:1010px; width:40px; height:13px; z-index:15"><%=Sys_IllegalDate(0)%></div>
<div id="Layer23" style="position:absolute; left:170px; top:1010px; width:40px; height:17px; z-index:16"><%=Sys_IllegalDate(1)%></div>
<div id="Layer24" style="position:absolute; left:220px; top:1010px; width:40px; height:16px; z-index:17"><%=Sys_IllegalDate(2)%></div>
...
<div id="Layer27" style="position:absolute; left:400px; top:1015px; width:610px; height:31px; z-index:20"><span class="style3"><%
...
%></span></div>
<div id="Layer28" style="position:absolute; left:115px; top:1035px; width:260px; height:15px; z-index:21"><span class="style3"><%
...
%></span></div>
<div id="Layer29" style="position:absolute; left:130px; top:1065px; width:50px; height:11px; z-index:22"><b><font color="red"><%=Sys_DealLineDate(0)%></font></b></div>
<div id="Layer30" style="position:absolute; left:210px; top:1065px; width:35px; height:13px; z-index:23"><b><font color="red"><%=Sys_DealLineDate(1)%></font></b></div>
<div id="Layer31" style="position:absolute; left:290px; top:1065px; width:32px; height:15px; z-index:24"><b><font color="red"><%=Sys_DealLineDate(2)%></font></b></div>

<div id="Layer32" style="position:absolute; left:400px; top:1080px; width:400px; height:49px; z-index:29"><%
...
<div id="Layer34" style="position:absolute; left:395px; top:1110px; width:410px; height:30px; z-index:28"><%
	if showBarCode then	response.write "<img src=""../BarCodeImage/"&Sys_BillNo&"_5.jpg"">"
%></div>
<div id="Layer33" style="position:absolute; left:635px; top:1120px; width:100px; height:40px; z-index:28"><span class="style7"><%=Sys_STATIONNAME&"<br>"&Sys_StationTel%></span></font></div>
<div id="Layer35" style="position:absolute; left:400px; top:1180px; width:130px; height:49px; z-index:29"><%
...
%></div>
<div id="Layer36" style="position:absolute; left:590px; top:1210px; width:140px; height:43px; z-index:30"><%
...
%></div>
<div id="Layer37" style="position:absolute; left:590px; top:1170px; width:200px; height:46px; z-index:31"><%
	If Sys_BillFillerMemberID = "3480" Then
		response.write "<table style=""border-bottom:0; border-top:0 ;border-left:0 ; border-right:0 ;border-color:#ff0000;border-style:solid;"" border=""0"" cellspacing=0 cellpadding=1>"
...
<div id="Layer43" style="position:absolute; left:620px; top:1270px; width:250px; height:12px; z-index:36"><B><font size=4><%=Sys_CarNo%></font></B></div>
