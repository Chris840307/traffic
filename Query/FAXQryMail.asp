		...
		if sys_City = "南投縣" then
			...
			If trim(rsfound("BillTypeID"))="1" Then
				if trim(rsfound("DriverHomeZip"))<>"" and not isnull(rsfound("DriverHomeZip")) then
					...
					GetMailAddress="&nbsp;"&trim(rsfound("DriverHomeZip"))&" "&Replace(ZipName2&trim(rsfound("DriverHomeAddress")),ZipName2&ZipName2,ZipName2)&"&nbsp;"
				else
					GetMailMan="&nbsp;"&funcCheckFont(trim(rsfound("Owner")),15,1)&"&nbsp;"
				GetMailAddress="&nbsp;"&trim(rsfound("OwnerZip"))&" "& Replace(ZipName & trim(rsfound("OwnerAddress")),ZipName&ZipName,ZipName) & "&nbsp;"
				end if
			Else
...
'sys_City="南投縣"
    	...
    	if sys_City="苗栗縣" Then
			mailNumber=Right("000000"&mailNumber,6)&"36400017      "
		End If
				...
				<table border="1" width="485" id="table3" cellspacing="0" cellpadding="0" height="625">
					<tr>
						...
						<%ElseIf sys_City="屏東縣" Then%>
							<%If Session("Unit_ID")="0907" then%>
								<td width="74"  rowspan="2">屏東<br><font size="2">中正路郵局</font></td>
							<%else%>
								<td width="74"  rowspan="2">　　　</td>
							<%End if%>
						<%Else%>
					...
					<tr>
						...
						<%ElseIf sys_City="屏東縣" Then%>
							<%If Session("Unit_ID")="0907" then%>
								<td width="74" ><font size="2">違規罰單<br>行政文書</font></td>
							<%else%>
								<td width="74" height="44">　　　</td>
							<%end if%>
						<%Else%>
					...
					<tr>
						...
						<td width="401" style="font-family:標楷體" colspan="21" height="63">
							<%=replace(funcCheckFont(GetMailMan,14,1)&" ","　"," ")&"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"%>
							<%If sys_City="南投縣" Then  response.write CarNo%>
							<Br>
							<%=replace(funcCheckFont(GetMailAddress,14,1)&" ","　"," ")%>
						</td>
					</tr>
					<tr>
						...
						<%If sys_City="基隆市" Then%>
							...
							<td width="401" colspan="21"><font face="標楷體">　□電話通知　　<span lang=EN-US style='font-family:"Wingdings 2";
								...
								style='mso-char-type:symbol;mso-symbol-font-family:"Wingdings 2"'>R</span></span>傳真　　
								<span lang=EN-US style='font-family:"Wingdings 2";
									mso-ascii-font-family:"Times New Roman";mso-hansi-font-family:"Times New Roman";
									mso-char-type:symbol;mso-symbol-font-family:"Wingdings 2"'><span
						...
						<%elseIf sys_City="屏東縣" Then%>
							<%If Session("Unit_ID")="0907" then%>
							<td width="401" colspan="21"><font face="標楷體">　□電話通知　　□傳真　　
								<span lang=EN-US style='font-family:"Wingdings 2";
									mso-ascii-font-family:"Times New Roman";mso-hansi-font-family:"Times New Roman";
									mso-char-type:symbol;mso-symbol-font-family:"Wingdings 2"'><span
									style='mso-char-type:symbol;mso-symbol-font-family:"Wingdings 2"'>R</span></span>補發回執</font></td>
