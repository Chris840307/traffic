...
<style type="text/css">
<!--
.style1 {font-family:"標楷體"; font-size: 9px}
.style2 {font-family:"標楷體"; font-size: 10px}
.style3 {font-family:"標楷體"; font-size: 18px line-height:9pt;}
.style4 {font-family:"標楷體"; font-size: 12px}
.style7 {font-family:"標楷體"; font-size: 16px}
.style9 {font-family:"標楷體"; font-size: 20px}
.style8 {font-family:"標楷體"; font-size: 22px}
...
Sys_Driver="":Sys_DriverHomeZip="":Sys_DriverHomeAddress=""
...
if Not rsfound.eof then Sys_Driver=trim(replace(rsfound("Owner")," ","*"))
chkaddress=""
If Not ifnull(trim(rsfound("OwnerAddress"))) Then
	If instr(replace(rsfound("OwnerAddress"),"（","("),"(住")>0 or instr(replace(rsfound("OwnerAddress"),"（","("),"(就") Then
		if Not rsfound.eof then Sys_DriverHomeAddress=rsfound("OwnerAddress")
	...
	If ifnull(Sys_DriverHomeAddress) Then
		...
		chkaddress="(戶)"
		if Not rstwo.eof then Sys_DriverHomeAddress=trim(rstwo("DriverHomeAddress"))
		if Not rstwo.eof then Sys_DriverHomeZip=trim(rstwo("DriverHomeZip"))
	End if
	rstwo.close
else
	If ifnull(Sys_DriverHomeAddress) Then
...
if not ifnull(Sys_DriverHomeAddress) then
	Sys_DriverHomeAddress=replace(replace(Sys_DriverHomeAddress,"  "," ")," ","　")
end if
...
If Sys_BillTypeID=2 Then
	If len(trim(rsfound("Owner")))<3 Then errBillNo=errBillNo&rsbil("BillNo")&","&Sys_Driver&"\n"
end if
...
strSql="select a.LoginID,a.ChName,b.UnitName,a.ImageFilename as MemberFileName,b.ImageFilename,b.Tel,b.Address from MemberData a,UnitInfo b where a.UnitID=b.UnitID and MemberID="&Sys_BillFillerMemberID
...
Sys_MailNumber="0"
DelphiASPObj.GenLabelStyleBarCode PBillSN(i),Sys_BillNo,Sys_Rule1,Sys_CarNo,Sys_MailNumber,"220073",Sys_DriverHomeZip,Sys_DealLineDate(0)&Sys_DealLineDate(1)&Sys_DealLineDate(2),Sys_StationID,"台北市交通事件裁決所",0,Sum_Level,0,True,False,Sys_MailDate,400,451,MailKindType
...
<div id="Layer22" style="position:absolute; font-size:16px; left:75px; top:40px; width:240px; height:13px; z-index:15"><span class="style3">
	<%=funcCheckFont(Sys_Driver,25,1)%></span>
</div>

<div id="Layer22" style="position:absolute; font-size:16px; left:75px; top:60px; width:240px; height:13px; z-index:15"><span class="style3"><%
...
<div id="Layer22" style="position:absolute; font-size:16px; left:90px; top:115px; width:500px; height:13px; z-index:20">
	<img src=<%="""../BarCodeImage/"&Sys_BillNo&"_4.jpg"""%>>
</div>

<div id="Layer22" style="position:absolute; font-size:16px; left:150px; top:750px; width:400px; height:13px; z-index:15"><span class="style8">
	<%=funcCheckFont(Sys_Driver,25,1)%></span>
</div>

<div id="Layer22" style="position:absolute; font-size:16px; left:150px; top:790px; width:400px; height:13px; z-index:15"><span class="style8"><%
	Response.Write Sys_DriverHomeZip&"<br>"
...
<div id="Layer22" style="position:absolute; font-size:16px; left:450px; top:900px; width:500px; height:13px; z-index:15">
	<img src=<%="""../BarCodeImage/"&Sys_BillNo&"_4.jpg"""%>>
</div>

<div id="Layer22" style="position:absolute; font-size:16px; left:70px; top:590px; width:500px; height:13px; z-index:15">
	<span class="style9">
		<%=Sys_UnitName%><br>
		<%=Sys_UnitAddress%><br>
		<%=Sys_UnitTel%>
