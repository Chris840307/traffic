...
if trim(Sys_BillTypeID)="1" then
	...
	PBillSN(i),Sys_BillNo,Sys_Rule1,Sys_CarNo,Sys_MailNumber,"220073",Sys_DriverHomeZip,right(Sys_DealLineDate(0),2)&Sys_DealLineDate(1)&Sys_DealLineDate(2),Sys_StationID,"台北市交通事件裁決所",0,Sum_Level,0,True,False,Sys_MailDate,108,184,17
...
<div id="Layer07" class="style3" style="position:absolute; left:60px; top:360px; z-index:5"><%=Sys_BatchNumber&"　"&SysUnit&"　"&"("&cdbl(i+1)&"/"&cdbl(Ubound(PBillSN)+1)&")"%>
</div>
...
<%if showBarCode then%>
<div id="Layer1" style="position:absolute; left:45px; top:805px; width:202px; height:36px; z-index:5">Ｖ</div>
<%else%>
...
<div id="Layer12" style="position:absolute; left:110px; top:915px; width:250px; height:11px; z-index:6"><span class="style7">逕行舉發　<%=Sys_A_Name%><br><%if int(Sys_Rule1)<>5620001 then response.write "附採證照片"%>　<%=Sys_CarColor%></span></div>
