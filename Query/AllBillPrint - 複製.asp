...
'	strSQL="update billbase set billMemID1=5314,billUnitID='07B4' where billno between 'S01346101' and 'S01346150' and billmemid1<>5314"
'
'	conn.execute(strSQL)
'end if
'===================================================================
...
'		end if
		...
		strSQL="select count(*) as cnt from DCILog a,Billbase f where a.BillSN=f.sn "&strwhere&" and ((ExchangeTypeID='E' and DCIReturnStatusID='n') or (ExchangeTypeID='W' and DCIReturnStatusID in ('S','d','e')) or (ExchangeTypeID='N' and (DCIReturnStatusID='n' or DCIReturnStatusID='h')))"
...
'		else
			strCnt="select count(*) as cnt from DCILog a,DCIReturnStatus d,BillBase f where a.BillTypeID='2' and a.ExchangeTypeID=d.DCIActionID(+) and a.DCIReturnStatusID=d.DCIReturn(+) and a.BillSN=f.SN and a.DciErrorCarData in "&KindType&" and a.BillNo not in (select distinct BillNo from BillBaseDCIReturn where EXCHANGETYPEID='W' and Rule4='2607') and d.DCIRETURNSTATUS='1'"&strwhere
'		end if
		set Dbrs=conn.execute(strCnt)
...
<table width="100%" border="0">
	<tr height="30">
		<td bgcolor="#FFCC33"><span class="style3">各式清冊/舉發單列印</span>
		<% 
			...
			<table width="100%" border="0" cellpadding="5" cellspacing="1" bgcolor="#FFFFFF">
				<tr>
					<td nowrap>
						...
						<input name="Sys_BatchNumber" type="text" class="btn1" value="<%=UCase(request("Sys_BatchNumber"))%>" size="35" onkeyup="funShowBillNo()">
						...
						<%if sys_City<>"嘉義縣" then%>
							建檔日期
							<input name="RecordDate" type="text" value="<%=request("RecordDate")%>" size="11" maxlength="6" class="btn1"  onKeyup="value=value.replace(/[^\d]/g,'')">
							<input type="button" name="datestr" value="..." onclick="OpenWindow('RecordDate');">
							~
							<input name="RecordDate1" type="text" value="<%=request("RecordDate1")%>" size="10" maxlength="6" class="btn1"  onKeyup="value=value.replace(/[^\d]/g,'')">
	...
	<tr>
		...
		<br><!-- titan測試用 -->
			...
			<%elseif sys_City="屏東縣" then%>
				...
				<br><input type="button" name="btnprint" value="列印 違規通知單( 98 郵簡含送達證書 使用  ) " onclick="funBillNoPrint(79);">
			...
			<%elseif sys_City="高雄市" then%>
				<br>
				<img src="space.gif" width="8" height="1">
				<input type="button" name="btnprint" value="列印 舉發單封套" onclick="funPasserUrgeHuaLien_DeliverListLabel();">
			<%elseif sys_City="南投縣" then%>
				<br>
				<img src="space.gif" width="8" height="1">
