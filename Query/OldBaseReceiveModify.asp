...
if request("DB_Selt")="Selt" Then
			...
			Set rs = Nothing
			If City="台南市" Then 
			'	strSQL="update traffic3.Fmaster set Recvno='"&receNo&"' where FSEQ='"&request("Billno")&"'" 
			'	conn1.execute(strSQL)

			'	strSQL="update traffic4.Fmaster set Recvno='"&receNo&"' where FSEQ='"&request("Billno")&"'" 
			'	conn1.execute(strSQL)
			else
				strSQL="update Fmaster_S set Recvno='"&receNo&"' where FSEQ='"&request("Billno")&"'" 
				conn1.execute(strSQL)
			End if

			strSQL="delete oldPasserBase  where billno='"&request("Billno")&"'" 
			conn.execute(strSQL)

			strSQL="insert into oldPasserBase(billno,receNo,receMoney,RecordDate,errReason) "
...
'			response.write strsql
			...
			Response.write "</script>"
Else
			strSQL="select receNo,receMoney,RecordDate,errReason from oldPasserBase where BillNo='"&request("Billno")&"'" 
'			response.write strsql
			receNo=""
			receMoney=""
			errReason=""
