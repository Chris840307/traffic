...
sql="select * from FMaster where FSEQ='"&Trim(request("BillNo"))&"'"
	...
	<tr>	
		...
		<tr>			
			...
			if trim(rs1("PCode1"))<>"" and not isnull(rs1("PCode1")) then
				...
				if sys_City<>"台中縣" and sys_City<>"台中市" and sys_City<>"南投縣" and sys_City<>"基隆市" And sys_City<>"澎湖縣" And sys_City<>"中科" and sys_City<> "竹科" Then
	  ...
	  <tr>
		...
		<tr>
			...
			if trim(rs1("RULEF1"))<>"" and not isnull(rs1("RULEF1")) then
				...
				response.write "<br>"&trim(rs1("RULEG1"))&"					"&trim(rs1("FACTG1"))
			else
				response.write "&nbsp;"
			end if
			%></td>
		...
		<tr>
			...
			if trim(rs1("RULEF2"))<>"" and not isnull(rs1("RULEF2")) then
				...
				response.write "<br>"&trim(rs1("RULEG2"))&"					"&trim(rs1("FACTG2"))
			else
				response.write "&nbsp;"
			end if
			%></td>
		...
		<tr>
			...
			if trim(rs1("RULEF3"))<>"" and not isnull(rs1("RULEF3")) then
				...
				response.write "<br>"&trim(rs1("RULEG3"))&"					"&trim(rs1("FACTG3"))
			else
				response.write "&nbsp;"
			end if
			%></td>
		...
		<tr>
			...
			if trim(rs1("RULEF4"))<>"" and not isnull(rs1("RULEF4")) then
				...
				if not rspcode.eof then response.write "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"&rsPCode("RULENAME")
				set rsPcode=nothing
				response.write "<br>"&trim(rs1("RULEG4"))&"					"&trim(rs1("FACTG4"))
			else
				response.write "&nbsp;"
			end if
			%></td>
		...
		<tr>
			<td align="right" bgcolor="#FFFF99"><strong><font color=red>移送日期</font></strong></td>	
			<td align="left" colspan="5"><font color=red><%

				sql="select EnDate from FMASTER_S where FSEQ ='"&request("Billno")&"'"
				set rsPCode=conn1.execute(sql)
			if not rspcode.eof then 
				response.write "&nbsp;"&rsPCode("EnDate")
			else
				response.write "&nbsp;"
			end If
			set rsPcode=nothing
...
<%
		...
		End if
%>
<% sql="select BackDate,MailNo,MailDate,MailSEQNo,SendPrnDate,MailDate2,BackCode,BackDate2,MailNo2,ShowDate,CloseDate,BackCode2,PubType,FStatus,BatChNo,FStatus_P,BatChNo_P,SEQNO,PubDate from FinBack where FSEQ='" & rs1("FSEQ") & "'"
...
%>
	...
	<tr>
			<td colspan="6" bgcolor="#00FFFF" height="32"><b>違規影像</b></td>
	</tr>
	<tr>
		<td colspan="6">
			<table border="0">
				<tr>
					<td>
						<img src="OldBaseImage.asp?FSeq=<%=rs1("FSEQ")%>&filedName=img_1" width="400"> 			

					</td>
					<td>
						<img src="OldBaseImage.asp?FSeq=<%=rs1("FSEQ")%>&filedName=img_2" width="400"> 
					</td>
				</tr>
...
<%
				...
				if sys_City<>"中科" and sys_City<> "竹科" then
					conn1.close
				end if
%>