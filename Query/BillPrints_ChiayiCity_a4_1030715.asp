...
strSql="select BillTypeID,Owner,OwnerZip,OwnerAddress,Driver,DriverID,DriverAddress,DriverZip,ILLEGALADDRESS,IllegalSpeed,RuleSpeed,INSURANCE,RuleVer,Note,BillFillDate from BillBase where SN="&replace(trim(rsbil("BillSN")),"","0")
...
If Sys_BillTypeID = 2 and trim(Sys_DriverID)<>"" Then
	strSQL="update billbasedcireturn set owner=driver,OwnerZip=DriverHomeZip,OwnerAddress=DriverHomeAddress where billno='"&trim(rsbil("BillNo"))&"' and carno='"&trim(rsbil("CarNo"))&"' and exchangetypeid='W' and DriverHomeAddress is not null"
End if 
...
strSql="select a.LoginID,a.ChName,b.UnitID,b.UnitName,b.UnitLevelID,b.UnitTypeID,a.ImageFilename as MemberFileName,b.ImageFilename,b.Tel,c.JobName from MemberData a,UnitInfo b,(select Content JobName,ID from Code where TypeID=4) c where a.UnitID=b.UnitID and a.JobID=c.ID(+) and MemberID="&Sys_BillFillerMemberID
...
<%if trim(Sys_BillTypeID)="1" then%>
	<div id="Layer3" style="position:absolute; left:<%=190+leftpx%>px; top:<%=0+toppx%>px; width:202px; height:36px; z-index:5">Ｖ</div>
<%else%>
...
%></div>
<div id="Layer14" class="style5" style="position:absolute; left:<%=410+leftpx%>px; top:<%=110+toppx%>px; width:324px; height:10px; z-index:4"><%if showBarCode then response.write "*本單可至郵局或委託代收之超商繳納"%></div>
<div id="Layer17" class="style2" style="position:absolute; left:<%=660+leftpx%>px; top:<%=145+toppx%>px; width:99px; height:12px; z-index:10"><%=fastring%></div>
...
<div id="Layer34" class="style7" style="position:absolute; left:<%=430+leftpx%>px; top:<%=335+toppx%>px; width:100px; height:30px; z-index:28"><%=Sys_STATIONNAME%></div>

<div id="Layer33" style="position:absolute; left:<%=515+leftpx%>px; top:<%=335+toppx%>px; width:400px; height:30px; z-index:24"><%
	if showBarCode then	response.write "<img src=""../BarCodeImage/"&Sys_BillNo&"_5.jpg"">"
%></div>

<div id="Layer35" class="style2" style="position:absolute; font-size:16px; left:<%=425+leftpx%>px; top:<%=430+toppx%>px; width:150px; height:49px; z-index:29"><%if sys_City="嘉義市" then response.write "嘉義市政府警察局<br>"&title_unit%></div>
...
<%if showBarCode then%>
	<div id="Layer42" style="position:absolute; left:<%=70+leftpx%>px; top:<%=525+toppx%>px; width:202px; height:36px; z-index:5">Ｖ</div>
<%else%>
	<div id="Layer43" style="position:absolute; left:<%=70+leftpx%>px; top:<%=545+toppx%>px; width:202px; height:36px; z-index:5">Ｖ</div>
<%end if%>

<%if trim(Sys_BillTypeID)="1" then%>
	<div id="Layer44" style="position:absolute; left:<%=190+leftpx%>px; top:<%=530+toppx%>px; width:202px; height:36px; z-index:5">Ｖ</div>
<%else%>
...
%></div>
<div id="Layer55" class="style5" style="position:absolute; left:<%=410+leftpx%>px; top:<%=640+toppx%>px; width:324px; height:10px; z-index:4"><%if showBarCode then response.write "*本單可至郵局或委託代收之超商繳納"%></div>

<div id="Layer58" class="style2" style="position:absolute; left:<%=660+leftpx%>px; top:<%=645+toppx%>px; width:99px; height:12px; z-index:10"><%=fastring%></div>
...
<div id="Layer68" class="style5" style="position:absolute; left:<%=440+leftpx%>px; top:<%=740+toppx%>px; width:280px; height:31px; z-index:20"><%
	if left(trim(Sys_Rule1),2)="40" or (int(Sys_Rule1)>4310200 and int(Sys_Rule1)<4310209) or (int(Sys_Rule1)>3310101 and int(Sys_Rule1)<3310111) then
		if trim(Sys_IllegalSpeed)<>"" and trim(Sys_RuleSpeed)<>"" then
			response.write "限速"&Sys_RuleSpeed&"公里、經測速時速"&Sys_IllegalSpeed&"公里、超速"&Sys_IllegalSpeed-Sys_RuleSpeed&"公里"
			if Sys_IllegalSpeed-Sys_RuleSpeed>100 then
...
<div id="Layer75" class="style7" style="position:absolute; left:<%=430+leftpx%>px; top:<%=870+toppx%>px; width:100px; height:30px; z-index:28"><%=Sys_STATIONNAME%></div>
...
<div id="Layer76" class="style2" style="position:absolute; font-size:16px; left:<%=425+leftpx%>px; top:<%=955+toppx%>px; width:150px; height:49px; z-index:29"><%if sys_City="嘉義市" then response.write "嘉義市政府警察局<br>"&title_unit%></div>
