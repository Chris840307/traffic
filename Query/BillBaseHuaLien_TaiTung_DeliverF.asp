...
<table border="0" cellspacing="0">
     <td>
      ...
      <td class="style4" ><font size="2"><%=wordporss(chstr(thenPasserCity&replace(sysunit,trim(thenPasserCity),"")))%></font></td>
        
			<td valign="top" align="left" width=1>
				<div id=idDiv style="position:absolute; left:45px;">
			...
			<font size="2">
			   <%if Sys_BillTypeID="1" and trim(Sys_Driver)<>"" then
					response.write wordporss(chstr(funcCheckFont(trim(Sys_Driver),12,4)))
				else
					response.write wordporss(chstr(funcCheckFont(trim(Sys_Owner),12,4)))
				end if%>
			</font>
			</td>
			<td  valign="bottom" class="style4"><%
				if Sys_BillTypeID="1" and trim(Sys_DriverHomeAddress)<>"" then
					strtmp=Sys_DriverHomeZip&" "&Sys_DriverZipName&funcCheckFont(Sys_DriverHomeAddress,12,4)
				else
					strtmp=Sys_OwnerZip&" "&Sys_OwnerZipName&funcCheckFont(Sys_OwnerAddress,12,4)
				end if
				response.write wordporss(chstr(trim(strtmp&"")))
			%>
			</td>
    <td  rowspan="2" align="center" valign="bottom" class="style4"> 
...
<td class="style4" width="3%">
    <%=wordporss(chstr(thenPasserCity&replace(sysunit,trim(thenPasserCity),"")))%>
