...
<%If instr(Request("Sys_Batchnumber"),"WT")>0 then%>
	<div id="Layer41" style="position:absolute; left:<%=(70+leftpx)%>px; top:<%=450+toptpx%>px; width:600px; height:36px; z-index:5">
...
<%if showBarCode then%>
<div id="Layer1" style="position:absolute; left:70px; top:535px; width:202px; height:36px; z-index:5">Ｖ</div>
<%else%>
<div id="Layer2" style="position:absolute; left:70px; top:570px; width:202px; height:36px; z-index:5">Ｖ</div>
<%end if%>

<%if trim(Sys_BillTypeID)="1" then%>
<div id="Layer3" style="position:absolute; left:190px; top:535px; width:202px; height:36px; z-index:5">v</div>
<%else%>
<div id="Layer4" style="position:absolute; left:190px; top:550px; width:202px; height:36px; z-index:5">v</div>
<%end if%>
...
<%if trim(Sys_BillTypeID)="1" then%>
	...
	<%end if%>
<%end if%>-->
<div id="Layer9" class="style5" style="position:absolute; left:30px; top:585px; width:202px; height:36px; z-index:5"><%
	if showBarCode then
...
%></div>
<div id="Layer10" class="style5" style="position:absolute; left:515px; top:585px; width:233px; height:32px; z-index:6"><img src=<%="""../BarCodeImage/"&Sys_BillNo&"_4.jpg"""%>></div>

<div id="Layer12" class="style4" class="style5" style="position:absolute; left:130px; top:655px; width:300px; height:11px; z-index:10"><span class="style7"><%
...
<div id="Layer13" class="style5" style="position:absolute; left:280px; top:640px; width:28px; height:11px; z-index:3"><%=Sys_Sex%></div>
<div id="Layer14" class="style5" style="position:absolute; left:400px; top:650px; width:324px; height:10px; z-index:10"><%'if showBarCode then response.write "*本單可至郵局或委託代收之超商繳納"%></div>
<div id="Layer15" class="style5" style="position:absolute; left:280px; top:655px; width:100px; height:10px; z-index:8"><font size=2><%if trim(Sys_DriverBirth(0))<>"" then response.write Sys_DriverBirth(0)&"年"&right("0"&Sys_DriverBirth(1),2)&"月"&right("0"&Sys_DriverBirth(2),2)&"日"%></font></div>
<div id="Layer16" class="style5" style="position:absolute; left:445px; top:655px; width:106px; height:13px; z-index:9"><%=Sys_DriverID%></div>
<div id="Layer17" class="style5" style="position:absolute; left:640px; top:655px; width:99px; height:12px; z-index:10"><%=fastring%></div>
<div id="Layer18" class="style5" style="position:absolute; left:135px; top:715px; width:100px; height:14px; z-index:11"><%=Sys_CarNo%></div>
<div id="Layer19" class="style5" style="position:absolute; left:335px; top:715px; width:117px; height:20px; z-index:12"><%=Sys_DCIRETURNCARTYPE%></div>
<div id="Layer20" class="style5" style="position:absolute; left:575px; top:715px; width:201px; height:17px; z-index:13"><%=funcCheckFont(Sys_Owner,20,1)%></div>
<div id="Layer21" class="style5" style="position:absolute; left:200px; top:740px; width:507px; height:13px; z-index:14"><%=Sys_OwnerZip&" "&funcCheckFont(Sys_OwnerZipName&Sys_OwnerAddress,20,1)%></div>

<div id="Layer22" class="style5" style="position:absolute; left:135px; top:770px; width:40px; height:13px; z-index:15"><%=Sys_IllegalDate(0)%></div>
<div id="Layer23" class="style5" style="position:absolute; left:195px; top:770px; width:40px; height:17px; z-index:16"><%=Sys_IllegalDate(1)%></div>
<div id="Layer24" class="style5" style="position:absolute; left:250px; top:770px; width:40px; height:16px; z-index:17"><%=Sys_IllegalDate(2)%></div>
<div id="Layer25" class="style5" style="position:absolute; left:305px; top:770px; width:40px; height:16px; z-index:18"><%=right("00"&Sys_IllegalDate_h,2)%></div>
<div id="Layer26" class="style5" style="position:absolute; left:365px; top:770px; width:40px; height:13px; z-index:19"><%=right("00"&Sys_IllegalDate_m,2)%></div>
<div id="Layer27" class="style3" style="position:absolute; left:450px; top:770px; width:310px; height:31px; z-index:20"><%
...
%></div>
<div id="Layer28" class="style5" style="position:absolute; left:130px; top:790px; width:250px; height:15px; z-index:21"><span class="style3"><%
	Response.Write Sys_ILLEGALADDRESS
%></span></div>
<div id="Layer29" class="style5" style="position:absolute; left:150px; top:815px; width:34px; height:11px; z-index:22"><%=Sys_DealLineDate(0)%></div>
<div id="Layer30" class="style5" style="position:absolute; left:240px; top:815px; width:35px; height:13px; z-index:23"><%=Sys_DealLineDate(1)%></div>
<div id="Layer31" class="style5" style="position:absolute; left:330px; top:815px; width:32px; height:15px; z-index:24"><%=Sys_DealLineDate(2)%></div>
<div id="Layer32" class="style5" style="position:absolute; left:465px; top:860px; width:400px; height:49px; z-index:29"><span class="style3"><%
...
<div id="Layer34" class="style5" style="position:absolute; left:445px; top:885px; width:400px; height:30px; z-index:28"><%
	if trim(Sys_Rule4)<>"" and instr(Request("Sys_Batchnumber"),"WT")>0 then

		response.write "(已由領車人繳納. 收據字號 "&Sys_Rule4&")"
	elseif showBarCode then	

		response.write "<img src=""../BarCodeImage/"&Sys_BillNo&"_5.jpg"">"
	end If 

%></div>
<div id="Layer33" class="style5" style="position:absolute; left:695px; top:890px; width:70px; height:40px; z-index:28"><span class="style2"><%=Sys_STATIONNAME&"<br>"&Sys_StationTel%></span></div>
<div id="Layer35" style="position:absolute; left:555px; top:940px; width:100px; height:49px; z-index:29"><%
...
<div id="Layer37" style="position:absolute; left:690px; top:935px; width:200px; height:46px; z-index:31"><%
	If ifnull(SysBillMemID2) Then
		if trim(Sys_MemberFilename)<>"" then
			...
			response.write "<table style=""border-color:#ff0000;border-style:solid;"" border=""1"" cellspacing=0 cellpadding=0>"
			response.write "<tr><td style=""border-color:#ff0000;border-style:solid;""  width=""60"" align=""center""><span class=""style9"">"&Sys_BillJobName&"&nbsp;&nbsp;&nbsp;</span><br><span class=""style10"">"&Sys_ChName&"</span></td></tr>"
...
<div id="Layer38" class="style5" style="position:absolute; left:165px; top:990px; width:60px; height:10px; z-index:32"><%=sys_Date(0)%></div>
<div id="Layer39" class="style5" style="position:absolute; left:240px; top:990px; width:60px; height:13px; z-index:33"><%=sys_Date(1)%></div>
<div id="Layer40" class="style5" style="position:absolute; left:325px; top:990px; width:60px; height:11px; z-index:34"><%=sys_Date(2)%></div>
