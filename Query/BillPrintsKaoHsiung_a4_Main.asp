...
If not ifNull(session("billprintuseimage")) Then billprintuseimage=trim(session("billprintuseimage"))

PrintCnt=0:fileCmt=150
If Not ifnull(request("PrintCnt")) Then PrintCnt=cdbl(request("PrintCnt"))+1
PBillSN=split(trim(request("PBillSN")),",")

for i=PrintCnt to Ubound(PBillSN)
...
<div id="Layer20" style="position:absolute; font-size:16px; left:470px; top:155px; width:251px; height:17px; z-index:13"><%=funcCheckFont(Sys_Owner,22,1)%></div>
...
<div id="Layer33" style="position:absolute; font-size:16px; left:510px; top:310px; width:400px; height:30px; z-index:28"><%
...
<div id="Layer42" style="position:absolute; font-size:14px; left:125px; top:580px; width:251px; height:17px; z-index:13"><%=Sys_UnitAddress%></div>

<div id="Layer43" style="position:absolute; font-size:14px; left:125px; top:600px; width:507px; height:13px; z-index:14"><%="高雄縣政府警察局"&SysUnit%></div>

<div id="Layer44" style="position:absolute; font-size:14px; left:125px; top:620px; width:507px; height:13px; z-index:14"><%=Sys_UnitTel%></div>

<div id="Layer45" style="position:absolute; font-size:14px; left:440px; top:580px; width:507px; height:13px; z-index:14">行政文書</div>

<div id="Layer46" style="position:absolute; font-size:14px; left:440px; top:600px; width:507px; height:13px; z-index:14"><%=BillPageUnit%>交字第<%=Sys_BillNo%>號</div>

<div id="Layer47" style="position:absolute; font-size:14px; left:440px; top:640px; width:233px; height:32px; z-index:6"><img src=<%="""../BarCodeImage/"&Sys_BillNo&"_4.jpg"""%>></div>

<div id="Layer48" style="position:absolute; font-size:14px; left:440px; top:690px; width:85px; height:30px; z-index:28"><font size=2><%=Sys_STATIONNAME%></font></div>

<div id="Layer49" style="position:absolute; font-size:14px; left:440px; top:720px; width:400px; height:30px; z-index:28"><%
	if showBarCode then	response.write "<img src=""../BarCodeImage/"&Sys_BillNo&"_5.jpg"">"
%></div>

<div id="Layer50" style="position:absolute; font-size:14px; left:160px; top:770px; width:507px; height:13px; z-index:14"><%=Sys_OwnerZip&"<br>"&funcCheckFont(Sys_OwnerZipName&Sys_OwnerAddress,25,1)%></div>

<div id="Layer51" style="position:absolute; font-size:14px; left:160px; top:810px; width:251px; height:17px; z-index:13"><%=funcCheckFont(Sys_Owner,25,1)%></div>

<div id="Layer52" style="position:absolute; font-size:14px; left:480px; top:810px; width:251px; height:17px; z-index:13">大宗掛號碼</div>

<div id="Layer53" style="position:absolute; font-size:14px; left:520px; top:830px; width:251px; height:17px; z-index:13"><%=Sys_MailNumber%></div>

<div id="Layer54" style="position:absolute; font-size:14px; left:520px; top:850px; z-index:13"><img src=<%="""../BarCodeImage/"&Sys_BillNo&"_2.jpg"""%>></div>
...
<div id="Layer61" style="position:absolute; font-size:20px; left:275px; top:605px; width:251px; height:17px; z-index:13"><%=chstr(Sys_Owner)%></div>

<div id="Layer62" style="position:absolute; font-size:20px; left:225px; top:650px; width:507px; height:13px; z-index:14"><%=Sys_OwnerZip&"　"&chstr(Sys_OwnerZipName&Sys_OwnerAddress)%></div>
...
<div id="Layer75" style="position:absolute; font-size:16px; left:430px; top:840px; width:280px; height:30px; z-index:28"><font size=2><%=Sys_STATIONNAME& " " &Sys_StationTel%></font></div>
...
<div id="Layer77" style="position:absolute; font-size:16px; left:625px; top:910px; width:200px; height:46px; z-index:31"><%
	If trim(billprintuseimage)="1" Then
		response.write "<table style=""border-color:#ff0000;border-style:solid;"" border=""1"" cellspacing=0 cellpadding=0>"
...
<div id="Layer82" style="position:absolute; font-size:16px; left:625px; top:945px; width:160px; height:43px; z-index:30"><%
	If trim(billprintuseimage)="1" Then
		response.write "<table style=""border-color:#ff0000;border-style:solid;"" border=""1"" cellspacing=0 cellpadding=0>"
		response.write "<tr><td style=""border-color:#ff0000;border-style:solid;"" width=100 height=25 align=""center""><span class=""style1"">舉發單專用章<br>隊長&nbsp;林光瀅</span></td></tr>"
		response.write "</table>"
	end if
%></div>
...
if (i>0 and (i mod fileCmt)=0) or i>=Ubound(PBillSN) then
	response.write "<form name=myForm method=""post"">"
	response.write "<input type=""Hidden"" name=""SQLstr"" value="""&request("SQLstr")&""">"
	response.write "<input type=""Hidden"" name=""PBillSN"" value="""&request("PBillSN")&""">"
	response.write "<input type=""Hidden"" name=""PrintCnt"" value="""&i&""">"
	response.write "</from>"%>
</body>
</html>
...
<script language="javascript">
	...
	<%if Ubound(PBillSN)<=fileCmt then%>
		window.focus();
		AutoprintWindow(true,5.08,5.08,5.08,5.08);
	<%elseif i<=fileCmt then%>
		window.focus();
		AutoprintWindow(true,5.08,5.08,5.08,5.08);
		oInterval = setInterval("delaytime(80000)", 1000);
	<%elseif i<Ubound(PBillSN) then%>
		window.focus();
		AutoprintWindow(true,5.08,5.08,5.08,5.08);
		oInterval = setInterval("delaytime(80000)", 1000);
	<%else%>
...
function delaytime(numberMillis){
	var exitTime = nowtime + numberMillis;
	now = new Date();
