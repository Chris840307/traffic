...
<table width='1000' border='1' align="left" cellpadding="0">
	<tr>
		<td width="24%" height="150" valign="top">
	...
	<%
		...
		if sys_City="台東縣" Then
			If Trim(rsfound("Rule1"))="5620001" And Not IsNull(rsfound("ImageFileName")) Then
	...
	%>
		<td rowspan="2" valign="top" height="390">
		<!-- 影像大圖 -->
		<%if bPicWebPath<>"" then%>
		<img src="<%=bPicWebPath%>" border=1 height="390" <%
		if isBig="Y"  then
		%> onmousemove="show(this, '<%=bPicWebPath%>')" onmousedown="show(this, '<%=bPicWebPath%>')"<%
		end if
		%> id="imgSource" src="<%=bPicWebPath%>" >

		<div id="div1" style="position:absolute; overflow:hidden; width:210px; height:90px; left:<%
			...
			<tr>
				...
				if trim(rsfound("Rule1"))<>"" Then
					If Left(trim(rsfound("Rule1")),2)="40" Or Left(trim(rsfound("Rule1")),5)="33101" Or Left(trim(rsfound("Rule1")),5)="43102" Then
						If trim(rsfound("RuleSpeed"))<>"" And trim(rsfound("illegalSpeed"))<>"" Then
							If (CInt(rsfound("illegalSpeed")) - CInt(rsfound("RuleSpeed")))>60 Then
					...
					rsR1.close
					Set rsR1=Nothing
					if sys_City="南投縣" Or sys_City="花蓮縣" Or sys_City="嘉義縣" Then
						If Left(trim(rsfound("Rule1")),2)="40" Or Left(trim(rsfound("Rule1")),5)="33101" Or Left(trim(rsfound("Rule1")),5)="43102" then
							response.write "<br>限速 <strong>"&trim(rsfound("RuleSpeed"))&"</strong> 公里,實速 <strong>"&trim(rsfound("illegalSpeed"))&"</strong> 公里" 
						End If 
					End If
					If Left(trim(rsfound("Rule1")),2)="40" Or Left(trim(rsfound("Rule1")),5)="33101" Or Left(trim(rsfound("Rule1")),5)="43102" Then
						If trim(rsfound("RuleSpeed"))<>"" And trim(rsfound("illegalSpeed"))<>"" Then
