...
if trim(request("kinds"))="CarDataSelect" then
	strwhere=trim(request("SQLstr"))&" and a.CarNo like '%"&trim(request("SelCarNo"))&"%'"
else
	strwhere=trim(request("SQLstr"))
end if
	Session.Contents.Remove("DciPrintCarDataSQLxls")
	Session("DciPrintCarDataSQLxls")=strwhere	
		...
		<td bgcolor="#E0E0E0">
			<table width="1300" border="0" cellpadding="4" cellspacing="1">
				...
				<%end if%>
					...
					<th>駕駛人戶籍地址</th> -->
					<td>違規地點</td>
				<%if sys_City="雲林縣" or sys_City="南投縣" or sys_City="嘉義縣" then %>
					<td>違規日期</td>
				<%end if%>
...
%>						<tr bgcolor="#ffffff" <%lightbarstyle 0 %>>
						...
						<%end if%>
							...
							if (trim(rsfound("A_Name"))<>"" and not isnull(rsfound("A_Name"))) then
								response.write funcCheckFont(rsfound("A_Name"),20,1)
							end if
							%></td>
							<td width="4%"><%
							...
							if (trim(rsfound("OwnerAddress"))<>"" and not isnull(rsfound("OwnerAddress"))) then
								response.write trim(rsfound("OwnerZip"))&funcCheckFont(rsfound("OwnerAddress"),19,1)
							end if
							%><!-- </td> -->
...
'								set rsR1=nothing
					...
					<%if trim(rsfound("RecordStateID"))=0 then	'未刪除
						'建檔人=使用者才能刪或特殊使用者
						if (((trim(rsfound("RecordMemberID"))=trim(session("User_ID")) or trim(Session("SpecUser"))="1") and sys_City<>"雲林縣") or ((trim(rsfound("RecordMemberID"))=trim(session("User_ID")) or Session("Group_ID")="200") and sys_City="雲林縣")) then
							'只有車籍查詢可以修改車號
							if trim(rsfound("BillStatus"))="0" or trim(rsfound("BillStatus"))="1" then
%>
							<input type="button" value="修改" onclick='window.open("../BillkeyIn/BillKeyIn_Car_Report_Update.asp?BillSN=<%=trim(rsfound("SN"))%>","CarNoUpdatePage2","left=0,top=0,location=0,width=800,height=600,resizable=yes,scrollbars=yes")'>		
<%
							end if
							'抓入案日期 及 是否有入案
							CaseInDate=""
...
<%
	...
	<tr>
		<td height="35" bgcolor="#FFDD77">
			<a href="file:///.."></a>
			<a href="file:///......"></a>
			<img src="space.gif" width="85" height="8">
...
<%end if%>
			<img src="space.gif" width="13" height="8">
			<input type="button" name="btnExecel" value="簡式 車籍清冊" onclick="funsimpExecel();">
			<img src="space.gif" width="5" height="8">
...
function newWin(url,win,w,h,l,t,sBar,mBar,res,full,tool){
	var win=window.open(url,win,"width="+w+",height="+h+",left="+l+",top="+t+",scrollbars="+sBar+",menubar="+mBar+",resizable="+res+",fullscreen="+full+",status=yes,toolbar="+tool+"");
	win.focus();
	return win;
}
function funchgExecel(){
	UrlStr="DciPrintCarDataList_Excel.asp?SQLstr=<%=strwhere%>&strDCISQL=<%=strDCISQL%>";
	newWin(UrlStr,"DciPrintWin_xls",790,550,50,10,"yes","yes","yes","no","yes");
}

function funsimpExecel(){
	UrlStr="DciPrintCarDataListSimple_Excel.asp?SQLstr=<%=strwhere%>&strDCISQL=<%=strDCISQL%>";
	newWin(UrlStr,"DciPrintWin_xls",790,550,50,10,"yes","yes","yes","no","yes");
}

function funchgList(){
	UrlStr="DciPrintCarDataList_HL_A3.asp?SQLstr=<%=strwhere%>&strDCISQL=<%=strDCISQL%>";
	newWin(UrlStr,"DciPrintWin_xls",790,550,50,10,"yes","yes","yes","no","yes");
}
