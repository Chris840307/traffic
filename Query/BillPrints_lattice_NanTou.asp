...
if showBarCode then
%>
<div id="Layer1" style="position:absolute; left:10px; top:10px; width:10px; height:20px; font-size:14 z-index:5">Ｖ</div>
<%else%>
<div id="Layer2" style="position:absolute; left:10px; top:45px; width:10px; height:20px; font-size:14 z-index:5">Ｖ</div>
<%end if%>
<%if trim(Sys_BillTypeID)="1" then%>
	<div id="Layer3" style="position:absolute; left:125px; top:15px; width:202px; height:36px; font-size:14 z-index:5">v</div>
<%else%>
	<div id="Layer4" style="position:absolute; left:125px; top:30px; width:202px; height:36px; font-size:14 z-index:5">v</div>
<%end if%>
<!--<div id="Layer5" style="position:absolute; left:185px; top:45px; width:202px; height:36px; font-size:14 z-index:5">Ｖ</div>
<%if trim(Sys_INSURANCE)="0" then%>
	<div id="Layer6" style="position:absolute; left:625px; top:25px; width:202px; height:36px; font-size:14 z-index:5">Ｖ</div>
<%elseif trim(Sys_INSURANCE)="1" then%>
	<div id="Layer7" style="position:absolute; left:625px; top:35px; width:202px; height:36px; font-size:14 z-index:5">Ｖ</div>
<%else%>
	<div id="Layer8" style="position:absolute; left:625px; top:45px; width:202px; height:36px; font-size:14 z-index:5">Ｖ</div>
<%end if%>-->
<div id="Layer9" style="position:absolute; left:0px; top:65px; width:202px; height:36px; font-size:14 z-index:5"><%
	if showBarCode then
...
%></div>
<!--<div id="Layer42" style="position:absolute; left:210px; top:70px; width:202px; height:36px; font-size:14 z-index:5"><%="<font size=1>"&SysUnit&"<br>("&SysUnitTel&")</font>"%></div>-->
<div id="Layer10" style="position:absolute; left:470px; top:60px; width:233px; height:32px; font-size:14 z-index:6"><img src=<%="""../BarCodeImage/"&Sys_BillNo&"_4.jpg"""%>></div>

<!--<div id="Layer11" style="position:absolute; left:485px; top:110px; width:230px; height:12px; font-size:14 z-index:7"><font size=1>　<%=BillPageUnit%>交字第<%=Sys_BillNo%>號</font></div>-->

<div id="Layer12" style="position:absolute; left:70px; top:130px; width:180px; height:11px; font-size:14 z-index:3"><font size=2>逕行舉發　<%=funcCheckFont(Sys_A_Name,20,1)%><br>附採證照片　<%=Sys_CarColor%></font></div>

<div id="Layer13" style="position:absolute; left:240px; top:125px; width:28px; height:11px; font-size:14 z-index:3"><%=Sys_Sex%></div>
<div id="Layer14" style="position:absolute; left:330px; top:125px; width:324px; height:10px; font-size:14 z-index:4"><%if showBarCode then response.write "*本單可至郵局或委託代收之超商繳納"%></div>
<div id="Layer15" style="position:absolute; left:240px; top:150px; width:100px; height:10px; font-size:14 z-index:8"><font size=2><%if trim(Sys_DriverBirth(0))<>"" then response.write Sys_DriverBirth(0)&"年"&right("0"&Sys_DriverBirth(1),2)&"月"&right("0"&Sys_DriverBirth(2),2)&"日"%></font></div>
<div id="Layer16" style="position:absolute; left:400px; top:150px; width:106px; height:13px; font-size:14 z-index:9"><%=Sys_DriverID%></div>
<div id="Layer17" style="position:absolute; left:590px; top:150px; width:99px; height:12px; font-size:14 z-index:10"><%=fastring%></div>
<div id="Layer18" style="position:absolute; left:95px; top:175px; width:100px; height:14px; font-size:14 z-index:11"><%=Sys_CarNo%></div>
<div id="Layer19" style="position:absolute; left:240px; top:175px; width:117px; height:20px; font-size:14 z-index:12"><%=Sys_DCIRETURNCARTYPE%></div>
<div id="Layer20" style="position:absolute; left:440px; top:175px; width:251px; height:17px; font-size:14 z-index:13"><%=funcCheckFont(Sys_Owner,20,1)%></div>
<div id="Layer21" style="position:absolute; left:70px; top:205px; width:507px; height:13px; font-size:14 z-index:14"><%=Sys_OwnerZip&" "&Sys_OwnerZipName&funcCheckFont(Sys_OwnerAddress,20,1)%></div>

<div id="Layer22" style="position:absolute; left:80px; top:230px; width:40px; height:13px; font-size:14 z-index:15"><%=Sys_IllegalDate(0)%></div>
<div id="Layer23" style="position:absolute; left:135px; top:230px; width:40px; height:17px; font-size:14 z-index:16"><%=Sys_IllegalDate(1)%></div>
<div id="Layer24" style="position:absolute; left:180px; top:230px; width:40px; height:16px; font-size:14 z-index:17"><%=Sys_IllegalDate(2)%></div>
<div id="Layer25" style="position:absolute; left:230px; top:230px; width:40px; height:16px; font-size:14 z-index:18"><%=right("00"&Sys_IllegalDate_h,2)%></div>
<div id="Layer26" style="position:absolute; left:280px; top:230px; width:40px; height:13px; font-size:14 z-index:19"><%=right("00"&Sys_IllegalDate_m,2)%></div>
<div id="Layer27" style="position:absolute; left:370px; top:230px; width:600px; height:31px; font-size:14 z-index:20"><%
	...
	response.write "</font>"
%></div>
<div id="Layer28" style="position:absolute; left:70px; top:255px; width:217px; height:15px; font-size:14 z-index:21"><%=Sys_ILLEGALADDRESS%></div>
<div id="Layer29" style="position:absolute; left:100px; top:280px; width:34px; height:11px; font-size:14 z-index:22"><B><%=Sys_DealLineDate(0)%></B></div>
<div id="Layer30" style="position:absolute; left:180px; top:280px; width:35px; height:13px; font-size:14 z-index:23"><B><%=Sys_DealLineDate(1)%></B></div>
<div id="Layer31" style="position:absolute; left:260px; top:280px; width:32px; height:15px; font-size:14 z-index:24"><B><%=Sys_DealLineDate(2)%></B></div>
<div id="Layer32" style="position:absolute; left:385px; top:295px; width:400px; height:49px; font-size:14 z-index:29"><%
...
%></div>
						            ...
						            <!--smith修改 讓條碼往左，因為右邊和紅單線隔太近，7-11條碼機會有讀取問題-->
<div id="Layer34" style="position:absolute; left:370px; top:340px; width:80px; height:30px; font-size:14 z-index:28"><font size=2><%=Sys_STATIONNAME&"<br>"&Sys_StationTel%></font></div>
<div id="Layer33" style="position:absolute; left:445px; top:335px; width:400px; height:30px; font-size:14 z-index:28"><%if showBarCode then response.write "<img src=""../BarCodeImage/"&Sys_BillNo&"_5.jpg"">"%></div>

<!--<div id="Layer35" style="position:absolute; left:455px; top:420px; width:100px; height:49px; font-size:14 z-index:29"><%
	...
	'end if%></div>
<div id="Layer36" style="position:absolute; left:580px; top:420px; width:100px; height:43px; font-size:14 z-index:30">主管</div>
<div id="Layer37" style="position:absolute; left:660px; top:410px; width:200px; height:46px; font-size:14 z-index:31"><%
...
%></div>-->
<div id="Layer38" style="position:absolute; left:185px; top:470px; width:60px; height:10px; font-size:14 z-index:32"><%=sys_Date(0)%></div>
<div id="Layer39" style="position:absolute; left:345px; top:470px; width:60px; height:13px; font-size:14 z-index:33"><%=sys_Date(1)%></div>
<div id="Layer40" style="position:absolute; left:495px; top:470px; width:60px; height:11px; font-size:14 z-index:34"><%=sys_Date(2)%></div>
<div id="Layer41" style="position:absolute; left:645px; top:470px; width:80px; height:12px; font-size:14 z-index:36"><%=Sys_BillFillerMemberID%></div>
</div>
<%
