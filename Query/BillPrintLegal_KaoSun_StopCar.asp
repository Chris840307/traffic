...
<title>舉發單列印-高雄縣 使用 Legal Size</title>
<style media=print>
.Noprint{display:none;}
.PageNext{page-break-after: always;}
</style>
...
.style33{font-family:font-size: 14px}
.style4 {font-family:"標楷體"; font-size: 11px}
.style7 {font-family: font-size: 11px}
...
if trim(Sys_UnitLevelID)="3" then
	if trim(Session("Unit_ID"))="TP00" then	'羅東抓所長
		chkJobID="303,304,360"
	else
...
'response.write Sys_BillTypeID
if trim(Sys_BillTypeID)="1" then
	Set DelphiASPObj = Server.CreateObject("GenBarCode.BarCodeASP") 
	DelphiASPObj.GenBillPrintBarCode PBillSN(i),Sys_BillNo,Sys_Rule1,Sys_CarNo,Sys_MailNumber,"220073",Sys_DriverHomeZip,Sys_DealLineDate(0)&Sys_DealLineDate(1)&Sys_DealLineDate(2),Sys_StationID,"台北市交通事件裁決所",0,Sum_Level,0,"8300","36",Sys_MailDate	
	'DelphiASPObj.GenBillPrintBarCode
	'PBillSN(i),Sys_BillNo,Sys_Rule1,Sys_CarNo,Sys_MailNumber,"220073",Sys_DriverHomeZip,Sys_DealLineDate(0)&Sys_DealLineDate(1)&Sys_DealLineDate(2),Sys_StationID,"台北市交通事件裁決所",0,Sum_Level,0,True,False,Sys_MailDate,640,000,36
	'esponse.write "DelphiASPObj.GenBillPrintBarCode"& PBillSN(i)&","&Sys_BillNo&","&Sys_Rule1&","&Sys_CarNo&","&Sys_MailNumber&",220073,"&Sys_DriverHomeZip&","&Sys_DealLineDate(0)&Sys_DealLineDate(1)&Sys_DealLineDate(2)&","&Sys_StationID&",台北市交通事件裁決所,0,"&Sum_Level&",0,True,False,"&Sys_MailDate
	'response.end
else
	Set DelphiASPObj = Server.CreateObject("GenBarCode.BarCodeASP") 

	DelphiASPObj.GenBillPrintBarCode PBillSN(i),Sys_BillNo,Sys_Rule1,Sys_CarNo,Sys_MailNumber,"220073",Sys_OwnerZip,Sys_DealLineDate(0)&Sys_DealLineDate(1)&Sys_DealLineDate(2),Sys_StationID,"台北市交通事件裁決所",0,Sum_Level,0,"8300","36",Sys_MailDate


	'DelphiASPObj.GenBillPrintBarCode	
	'BillSN(i),Sys_BillNo,Sys_Rule1,Sys_CarNo,Sys_MailNumber,"220073",Sys_DriverHomeZip,Sys_DealLineDate(0)&Sys_DealLineDate(1)&Sys_DealLineDate(2),Sys_StationID,"台北市交通事件裁決所",0,Sum_Level,0,True,False,Sys_MailDate,640,000,36
	'DelphiASPObj.GenBillPrintBarCode PBillSN(i),Sys_BillNo,Sys_Rule1,Sys_CarNo,Sys_MailNumber,"220073",Sys_OwnerZip,Sys_DealLineDate(0)&Sys_DealLineDate(1)&Sys_DealLineDate(2),Sys_StationID,"台北市交通事件裁決所",0,Sum_Level,0,True,False,Sys_MailDate,640,000,36
end if
...
<div id="Layer42" style="position:absolute; left:20px; top:30px;">
<table width="710" height="160" border="0" cellspacing=0 cellpadding=0>
	<!---------------------------------------- start  縣市抬頭, 地址, 電話. --------------------------------------------->
	<tr>
		<td>&nbsp;</td>
		<td class="style15"><%=SysUnitAddress&"<br>高雄縣政府警察局"&SysUnit& "  " & SysUnitTel%></td>
		<td>&nbsp;</td>
	</tr>
	<!---------------------------------------- 放大宗掛號    --------------------------------------------->
	<!--
 <tr >
    ...
    <td  width="530" align="center"><br>
      <p class="style4">大宗郵資已付掛號函件<br>  第<%=right("00000000" & trim(Sys_MailNumber),6)%>號  </p>    </td>
    <td >&nbsp;</td>
...
<tr height="50">
  ...
  <tr>
    ...
    <td >&nbsp;</td>
    <td width="530" align="center"><div align="center"><img src=<%="""../BarCodeImage/"&Sys_BillNo&"_2.jpg"""%>></img>
    <br><%=Sys_MAILCHKNUMBER%></div></td>	
	-->
     <!---------------------------------------- 放 許可證    --------------------------------------------->
     <!--
    		<td class="style2" >
	...
	<tr>
		...
		<td width="510" valign="TOP" class="style2"><br><%
			if trim(Sys_BillTypeID)="1" then
...
<!---------------------------------------- start 列印紅單紅色區域內容 --------------------------------------------->
<%if showBarCode then%>
<div id="Layer1" style="position:absolute; left:60px; top:<%=380+pagepx%>px; width:202px; height:36px; z-index:5">Ｖ</div>
<%else%>
<div id="Layer2" style="position:absolute; left:60px; top:<%=415+pagepx%>px; width:202px; height:36px; z-index:5">Ｖ</div>
<%end if%>
<%if trim(Sys_BillTypeID)="1" then%>
<div id="Layer3" style="position:absolute; left:170px; top:<%=385+pagepx%>px; width:202px; height:36px; z-index:5">v</div>
<%else%>
<div id="Layer4" style="position:absolute; left:170px; top:<%=400+pagepx%>px; width:202px; height:36px; z-index:5">v</div>
<%end if%>
<div id="Layer9" style="position:absolute; left:20px; top:<%=435+pagepx%>px; width:202px; height:36px; z-index:5"><%
	if showBarCode then
...
<!--smith 要蓋掉有誤的X縣警-->
<div id="Layer42" style="position:absolute; left:480px; top:<%=465+pagepx%>px; width:233px; height:12px; z-index:100" class="style4"><font color="#FFDEFF">■■■■</font></div>
<div id="Layer42" style="position:absolute; left:485px; top:<%=465+pagepx%>px; width:233px; height:12px; z-index:100" class="style4"><font color="#FFDEFF">■■■■</font></div>
<!-- 'smith 舉發單號  字型太小改最上方的sytle4 的字型pix-->
<div id="Layer42" style="position:absolute; left:533px; top:<%=465+pagepx%>px; width:233px; height:12px; z-index:36" class="style4">高縣警交字第</div>

<div id="Layer12" style="position:absolute; left:115px; top:<%=493+pagepx%>px; width:300px; height:11px; z-index:3"><span class="style7">逕行舉發　<%=Sys_A_Name%><br><%if int(Sys_Rule1)<>4340003 and int(Sys_Rule1)<>5620001 then response.write "附採證照片"%>　<%=Sys_CarColor%></span></div>
<div id="Layer13" style="position:absolute; left:270px; top:<%=490+pagepx%>px; width:28px; height:11px; z-index:3"><%=Sys_Sex%></div>
<div id="Layer14" style="position:absolute; left:380px; top:<%=490+pagepx%>px; width:324px; height:10px; z-index:4"><%'=Sys_DriverHomeZip&" "&Sys_DriverZipName&Sys_DriverHomeAddress%><%'if showBarCode then response.write "*本單可至郵局或委託代收之超商繳納"%></div>
...
<div id="Layer22" style="position:absolute; left:120px; top:<%=575+pagepx%>px; width:40px; height:13px; z-index:15"><%=Sys_IllegalDate(0)%></div>
<div id="Layer23" style="position:absolute; left:170px; top:<%=575+pagepx%>px; width:40px; height:17px; z-index:16"><%=Sys_IllegalDate(1)%></div>
<div id="Layer24" style="position:absolute; left:220px; top:<%=575+pagepx%>px; width:40px; height:16px; z-index:17"><%=Sys_IllegalDate(2)%></div>
<div id="Layer25" style="position:absolute; left:260px; top:<%=575+pagepx%>px; width:40px; height:16px; z-index:18"><%=right("00"&Sys_IllegalDate_h,2)%></div>
<div id="Layer26" style="position:absolute; left:310px; top:<%=575+pagepx%>px; width:40px; height:13px; z-index:19"><%=right("00"&Sys_IllegalDate_m,2)%></div>
<div id="Layer27" style="position:absolute; left:400px; top:<%=580+pagepx%>px; width:610px; height:31px; z-index:20"><span class="style33"><%
...
%></span></div>
<div id="Layer28" style="position:absolute; left:125px; top:<%=600+pagepx%>px; width:220px; height:15px; z-index:21"><span class="style33"><%=Sys_ILLEGALADDRESS%></span></div>
...
<!-----------------------------------------法條編號 --------------------------------------------->
<div id="Layer32" style="position:absolute; left:418px; top:<%=640+pagepx%>px; width:400px; height:49px; z-index:29"><%
...
<div id="Layer34" style="position:absolute; left:390px; top:<%=680+pagepx%>px; width:400px; height:30px; z-index:28"><%
	if showBarCode then	response.write "<img src=""../BarCodeImage/"&Sys_BillNo&"_5.jpg"">"
%></div>
<div id="Layer33" style="position:absolute; left:630px; top:<%=655+pagepx%>px; width:100px; height:40px; z-index:28"><span class="style4"><%=Sys_STATIONNAME&"<br>"&Sys_StationTel%></span></font></div>
<div id="Layer35" style="position:absolute; left:410px; top:<%=750+pagepx%>px; width:100px; height:49px; z-index:29"><%
	...
	''	response.write "<tr><td style=""border-color:#ff0000;border-style:solid;"" class=""style1"" nowrap>雲林縣警察局<br>"&SysUnit&"</td></tr>"
...
<div id="Layer36" style="position:absolute; left:580px; top:<%=775+pagepx%>px; width:100px; height:43px; z-index:30"><%
	...
	'end if
	%></div>
<div id="Layer37" style="position:absolute; left:580px; top:<%=735+pagepx%>px; width:200px; height:46px; z-index:31"><%
...
<div id="Layer45" class="style3" style="position:absolute; left:48px; top:<%=973+pagepx%>px; z-index:1"><%
	response.write "高雄縣政府警察局"&SysUnit& "送達證書"& "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" & "高雄縣政府警察局"&SysUnit& "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"  & SysUnitAddress

%>
</div>
<!-----------------------------------送達證書 收件人資料---------------------------------->
<div id="Layer45" class="style3" style="position:absolute; left:128px; top:<%=995+pagepx%>px; z-index:1"><%
	response.write Sys_Owner&"<br>"
	response.write InstrAdd(Sys_OwnerZip&Sys_OwnerZipName&Sys_OwnerAddress,14)
%>
</div>
<!-----------------------------------送達證書 文 號 ---------------------------------->
<div id="Layer46" class="style3" style="position:absolute; left:220px; top:<%=1040+pagepx%>px; z-index:1"><%
...
<div id="Layer47" class="style3" style="position:absolute; left:520px; top:<%=1050+pagepx%>px; z-index:1"><img src=<%="""../BarCodeImage/"&Sys_BillNo&"_4.jpg"""%>></div>
</div>
</div><%
