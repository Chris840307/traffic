...
if trim(Sys_BillTypeID)="1" then
	...
	PBillSN(i),Sys_BillNo,Sys_Rule1,Sys_CarNo,Sys_MailNumber,"220073",Sys_DriverHomeZip,right(Sys_DealLineDate(0),2)&Sys_DealLineDate(1)&Sys_DealLineDate(2),Sys_StationID,"台北市交通事件裁決所",0,Sum_Level,0,True,False,Sys_MailDate,400,451,36
...
<table width="645" height="393" border="0">
  <tr>
    <td width="280" height="69" valign="top"></td>
    <td colspan="3" align="left" valign="top">
		<br>　　　　　　　　　　　　　　　　　　<span class="style6"><%'回執%><%
		If instr(Request("Sys_Batchnumber"),"WT")>0 then
			Response.Write "<br><font color=""red"">　　　　　　　　拖吊已繳費</font>"
		end if
	%></span></td>
  </tr>
  <tr>
	...
	<%if trim(Sys_BillTypeID)="1" then%>
    <td width="280" align="left" valign="top" nowrap><span class="style3"><%=Sys_DriverHomeZip%><br>
	...
	<td width="280" align="left" valign="top" nowrap><span class="style3"><%=Sys_OwnerZip%><br>
  ...
  <tr>
    <td>&nbsp;</td>
	<%if trim(Sys_BillTypeID)="1" then%>
    <td width="280"><span class="style3"><%=funcCheckFont(Sys_Driver,16,1)%>　台啟</span></td>
	<%elseif trim(Sys_BillTypeID)="2" then%>
	<td width="280"><span class="style3"><%=funcCheckFont(Sys_Owner,16,1)%>　台啟</span></td>
	<%end if%>
    <td>&nbsp;</td>
	<td>&nbsp;</td>
  </tr>
  ...
  <tr>
    <td height="98" colspan="2" valign="top" nowrap><p>　　<span class="style5">應到案處所：<%=Sys_STATIONNAME%></span><br>　　<span class="style5">應到案處所電話：<%=Sys_StationTel%></span><br>
...
<%end if%>
<%If instr(Request("Sys_Batchnumber"),"WT")>0 then%>
<div id="Layer49" style="position:absolute; left:<%=(60+leftpx)%>px; top:615px; width:800px; height:36px; z-index:5">
======================================
<br>
==================
<br>
...
<%if showBarCode or ((Sys_UnitID="046A" Or Sys_UnitID="0464") and instr(Sys_BillNo,"G0H")=0) then%>
<div id="Layer1" style="position:absolute; left:<%=(55+leftpx)%>px; top:<%=795+toptpx%>px; width:202px; height:36px; z-index:5">Ｖ</div>
<%else%>
<div id="Layer2" style="position:absolute; left:<%=(55+leftpx)%>px; top:<%=825+toptpx%>px; width:202px; height:36px; z-index:5">Ｖ</div>
<%end if%>
<!--
<%if trim(Sys_BillTypeID)="1" then%>
...
<%if trim(Sys_BillTypeID)="1" then%>
	...
	<%end if%>
<%end if%>
<div id="Layer9" style="position:absolute; left:30px; top:<%=850+toptpx%>px; width:202px; height:36px; z-index:5"><%
	if showBarCode then
...
<div id="Layer144" class="style4" style="position:absolute; left:<%=(120+leftpx)%>px; top:<%=910+toptpx%>px; width:250px; height:11px; z-index:3"><%
	...
	elseif instr(Sys_BillNo,"G5H")>0 then
...
<%if trim(Sys_BillTypeID)="1" then%>
<div id="Layer12" style="position:absolute; left:<%=(115+leftpx)%>px; top:<%=920+toptpx%>px; width:150px; height:11px; z-index:3"><span class="style3"><%=Sys_Driver%></span></div>
<%end if
if trim(Sys_BillTypeID)="1" then%>
<div id="Layer13" style="position:absolute; left:<%=(265+leftpx)%>px; top:<%=905+toptpx%>px; width:28px; height:11px; z-index:3"><span class="style3"><%=Sys_Sex%></span></div>
<div id="Layer14" style="position:absolute; left:<%=(360+leftpx)%>px; top:<%=900+toptpx%>px; width:324px; height:10px; z-index:4"><span class="style3"><%=Sys_DriverZipName&Sys_DriverHomeAddress%></Span></div>
<div id="Layer15" style="position:absolute; left:<%=(265+leftpx)%>px; top:<%=930+toptpx%>px; width:100px; height:10px; z-index:8"><span class="style3"><%=Sys_DriverBirth(0)%></span></div>
<div id="Layer142" style="position:absolute; left:<%=(295+leftpx)%>px; top:<%=930+toptpx%>px; width:100px; height:10px; z-index:8"><span class="style3"><%=Sys_DriverBirth(1)%></span></div>
<div id="Layer143" style="position:absolute; left:<%=(325+leftpx)%>px; top:<%=930+toptpx%>px; width:100px; height:10px; z-index:8"><span class="style3"><%=Sys_DriverBirth(2)%></span></div>
<div id="Layer16" style="position:absolute; left:<%=(430+leftpx)%>px; top:<%=930+toptpx%>px; width:106px; height:13px; z-index:9"><span class="style3"><%=Sys_DriverID%></span></div>
<div id="Layer17" style="position:absolute; left:<%=(630+leftpx)%>px; top:<%=930+toptpx%>px; width:99px; height:12px; z-index:10"><span class="style3"><%=fastring%></span></div>
<%end if%>

<div id="Layer18" style="position:absolute; left:<%=(130+leftpx)%>px; top:<%=955+toptpx%>px; width:100px; height:14px; z-index:11"><span class="style3"><%=Sys_CarNo%></span></div>
<div id="Layer19" style="position:absolute; left:<%=(280+leftpx)%>px; top:<%=955+toptpx%>px; width:200px; height:20px; z-index:12"><span class="style3"><%=Sys_DCIRETURNCARTYPE%></span></div>
<div id="Layer20" style="position:absolute; left:<%=(515+leftpx)%>px; top:<%=955+toptpx%>px; width:201px; height:17px; z-index:13"><span class="style3"><%=funcCheckFont(Sys_Owner,14,1)%></span></div>
<div id="Layer21" style="position:absolute; left:<%=(130+leftpx)%>px; top:<%=980+toptpx%>px; width:750px; height:13px; z-index:14"><span class="style3"><%
...
<div id="Layer22" style="position:absolute; left:<%=(123+leftpx)%>px; top:<%=1000+toptpx%>px; width:40px; height:13px; z-index:15"><span class="style5"><%=Sys_IllegalDate(0)%></span></div>
<div id="Layer23" style="position:absolute; left:<%=(176+leftpx)%>px; top:<%=1000+toptpx%>px; width:40px; height:17px; z-index:16"><span class="style5"><%=Sys_IllegalDate(1)%></span></div>
<div id="Layer24" style="position:absolute; left:<%=(216+leftpx)%>px; top:<%=1000+toptpx%>px; width:40px; height:16px; z-index:17"><span class="style5"><%=Sys_IllegalDate(2)%></div>
<div id="Layer25" style="position:absolute; left:<%=(266+leftpx)%>px; top:<%=1000+toptpx%>px; width:40px; height:16px; z-index:18"><span class="style5"><%=right("00"&Sys_IllegalDate_h,2)%></span></div>
<div id="Layer26" style="position:absolute; left:<%=(316+leftpx)%>px; top:<%=1000+toptpx%>px; width:40px; height:13px; z-index:19"><span class="style5"><%=right("00"&Sys_IllegalDate_m,2)%></span></div>
<div id="Layer27" class="style5" style="position:absolute; left:<%=(400+leftpx)%>px; top:<%=1005+toptpx%>px; width:340px; height:31px; z-index:20"><%
...
%></div>
<div id="Layer28" style="position:absolute; left:<%=(120+leftpx)%>px; top:<%=1030+toptpx%>px; width:240px; height:15px; z-index:21"><span class="style5"><%=Sys_ILLEGALADDRESS%></span></div>
<div id="Layer29" style="position:absolute; left:<%=(135+leftpx)%>px; top:<%=1055+toptpx%>px; width:34px; height:11px; z-index:22"><span class="style3"><%=Sys_DealLineDate(0)%></span></div>
<div id="Layer30" style="position:absolute; left:<%=(205+leftpx)%>px; top:<%=1055+toptpx%>px; width:35px; height:13px; z-index:23"><span class="style3"><%=Sys_DealLineDate(1)%></span></div>
<div id="Layer31" style="position:absolute; left:<%=(275+leftpx)%>px; top:<%=1055+toptpx%>px; width:32px; height:15px; z-index:24"><span class="style3"><%=Sys_DealLineDate(2)%></span></div>

<div id="Layer46" style="position:absolute; left:<%=(450+leftpx)%>px; top:<%=1055+toptpx%>px; width:32px; height:15px; z-index:24"><span class="style2"><%
	if len(trim(Sys_Rule1))>7 then response.write "之"&right(trim(Sys_Rule1),1)
%></span></div>

<div id="Layer47" style="position:absolute; left:<%=(450+leftpx)%>px; top:<%=1070+toptpx%>px; width:32px; height:15px; z-index:24"><span class="style2"><%
	if len(trim(Sys_Rule2))>7 then response.write "之"&right(trim(Sys_Rule2),1)
%></span></div>

<div id="Layer32" style="position:absolute; left:<%=(415+leftpx)%>px; top:<%=1065+toptpx%>px; width:400px; height:49px; z-index:29"><span class="style3"><%
	...
	response.write "</font>"
%></span></div>
<div id="Layer33" style="position:absolute; left:<%=(395+leftpx)%>px; top:<%=1095+toptpx%>px; width:400px; height:30px; z-index:1"><%
...
%></div>
<div id="Layer34" style="position:absolute; left:<%=(640+leftpx)%>px; top:<%=1108+toptpx%>px; width:120px; height:30px; z-index:28"><span class="style3"><%=Sys_STATIONNAME%></span></font></div>


<div id="Layer35" style="position:absolute; left:<%=(405+leftpx)%>px; top:<%=1155+toptpx%>px; width:150px; height:49px; z-index:29"><%
...
%></div>
<div id="Layer36" style="position:absolute; left:<%=(580+leftpx)%>px; top:<%=1190+toptpx%>px; width:200px; height:43px; z-index:30"><%
'	if instr("041S",Sys_UnitID)>0 Then
'		Sys_JobName="警務員":Sys_MainChName="楊乾聖"
'	end if

	response.write "<table style=""border-color:#ff0000;border-style:solid;"" border=""1"" cellspacing=0 cellpadding=1>"
	response.write "<tr><td style=""border-color:#ff0000;border-style:solid;"" align=""center"" width=""120""><span class=""style9"">&nbsp;"&Sys_JobName&"&nbsp;</span><br><span class=""style10"">"&Sys_MainChName&"</span></td></tr>"
	response.write "</table>"
%></div>
<div id="Layer37" style="position:absolute; left:<%=(580+leftpx)%>px; top:<%=1140+toptpx%>px; width:200px; height:46px; z-index:31"><%
...
%></div>
<div id="Layer38" style="position:absolute; left:265px; top:<%=1240+toptpx%>px; width:60px; height:10px; z-index:32"><span class="style3"><%=sys_Date(0)%></span></div>
<div id="Layer39" style="position:absolute; left:400px; top:<%=1240+toptpx%>px; width:60px; height:13px; z-index:33"><span class="style3"><%=sys_Date(1)%></span></div>
<div id="Layer40" style="position:absolute; left:530px; top:<%=1240+toptpx%>px; width:60px; height:11px; z-index:34"><span class="style3"><%=sys_Date(2)%></span></div>
<div id="Layer41" style="position:absolute; left:660px; top:<%=1240+toptpx%>px; width:80px; height:12px; z-index:36"><span class="style3"></span></div>
<div id="Layer145" style="position:absolute; left:400px; top:<%=1260+toptpx%>px; width:150px; height:11px; z-index:37"><font size=2><%=Sys_CarColor%></font></div>
</div>
<%
