...
<%	
	...
	If Not rsOld1.eof Then
		if Trim(rsOld1("CityFlag"))="1" Or Trim(rsOld1("CityFlag"))="2" then
			DBUser="traffic"
		elseif Trim(rsOld1("CityFlag"))="3" Or Trim(rsOld1("CityFlag"))="4" then
			DBUser="traffic2"
		elseif trim(request("CityFlag"))="5" then
			DBUser="traffic3"
		elseif trim(request("CityFlag"))="6" then
			DBUser="traffic4"
		end if
	End If
	rsOld1.close
	...
	if ((StoreAndSendFlag=1 or sys_City="彰化縣") and sys_City<>"基隆市") or (ShowSecondAddress=1 and sys_City="基隆市") then
		if trim(rs1("BillTypeID"))="1" then	
			...
			if sys_City="台東縣" then
				...
				rsD.close
				set rsD=nothing
			elseif sys_City="台中市" or (sys_City="高雄市" or sys_City=ApconfigureCityName) Or sys_City="南投縣" then
