...
fname=year(now)&fMnoth&fDay&"違規日慢車_舉發單資料.txt"
...
<%
	...
	strSQL="select a.rule1,a.billstatus,a.sn,a.billno,a.Carno,a.BillTypeID,a.IllegalDate,a.IllegalAddress,a.MemberStation,a.BillFillDate,a.BillUnitiD,a.DeallineDate from passerbase a where a.IllegalDate between to_date('2013/01/01 0:0:0"&"','YYYY/MM/DD/HH24/MI/SS') and to_date('2013/12/31 23:59:59"&"','YYYY/MM/DD/HH24/MI/SS')" &_
...
%>
違規日(IllegalDate);舉發單號(billno);舉發單位(UnitName);裁決日(JudeDate);結案狀態(Billstatus);應到案日期(DealLineDate);車號(CarNo);入案日期(CaseInDate);應到案處所(MemberStation);舉發類別(BillTypeID);填單日期(BillFillDate);法條一(Rule1)
<%  
					...
					if trim(rsfound("IllegalDate"))<>"" and not isnull(rsfound("IllegalDate")) then
						response.write trim(Year(rsfound("IllegalDate"))-1911)&Right("00"&Month(rsfound("IllegalDate")),2)&Right("00"&day(rsfound("IllegalDate")),2) & " " &Right("00"&hour(rsfound("IllegalDate")),2)&Right("00"&minute(rsfound("IllegalDate")),2)
					End If
					response.write ";"
					'舉發單號
