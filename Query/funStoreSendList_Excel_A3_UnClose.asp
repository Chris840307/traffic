...
<%if sys_City<>"嘉義縣" then%>
	<center><font size="3">舉發違反道路交通事件通知單寄存送達(未結案)移送清冊</font></center>
...
%>
		...
		strCntStop="select count(*) as cnt from DCILog a,MemberData b" &_
			...
			" and f.MemberStation in ('20','21','22','23','24','29')" &_
			" and f.RecordStateID=0" &_
			" and a.ExchangeTypeID='N' and a.DciReturnStatusID in ('S','N') and a.DciReturnStatusID='n' and a.BillTypeID<>'2'" &_
			" and a.ReturnMarkType='4'" &_
			" and a.ExchangeTypeID=d.DCIActionID(+) and a.DCIReturnStatusID=d.DCIReturn(+)" &_
			" and a.RecordMemberID=b.MemberID(+) "&strwhere
		set rsCntStop=conn.execute(strCntStop)
...
%>		<tr>
			...
			<td nowrap><%
			'單退狀態
			strStatus1="select DciReturnStatusID from Dcilog where ExchangeTypeID='N' and ReturnMarkType='4' and BillSn="&trim(rs1("BillSn"))
