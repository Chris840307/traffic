...
<table width="710" height="160" border="0" cellspacing=0 cellpadding=0>
	<tr>
		...
		<td>行政文書<br><%=BillPageUnit%>交字第<%=Sys_BillNo%>號
			<br><img src=<%="""../BarCodeImage/"&Sys_BillNo&"_4.jpg"""%>>
		</td>
	</tr>
	<tr>
		...
		<td><%=Sys_STATIONNAME%><%
			if showBarCode then	response.write "<br><img src=""../BarCodeImage/"&Sys_BillNo&"_5.jpg"">"
		%><br>　　　大宗掛號碼</td>
	</tr>
</table>
</div>
...
<!--smith 要蓋掉有誤的X縣警-->
<div id="Layer42" style="position:absolute; left:484px; top:<%=464+pagepx%>px; width:229px; height:12px; z-index:100" class="style4"><font color="#000000">■■■■</font></div>
<div id="Layer42" style="position:absolute; left:489px; top:<%=464+pagepx%>px; width:229px; height:12px; z-index:100" class="style4"><font color="#000000">■■■■</font></div>

<div id="Layer42" style="position:absolute; left:535px; top:<%=463+pagepx%>px; width:233px; height:12px; z-index:36"><font size=1><b>高縣警交字第</b></font></div>
...
<div id="Layer27" style="position:absolute; left:400px; top:<%=580+pagepx%>px; width:610px; height:31px; z-index:20"><span class="style3"><%
...
<div id="Layer32" style="position:absolute; left:418px; top:<%=640+pagepx%>px; width:400px; height:49px; z-index:29"><%
	...
	response.write "</font>"
%></div>
<div id="Layer33" style="position:absolute; left:410px; top:<%=695+pagepx%>px; width:100px; height:40px; z-index:28"><span class="style7"><%=Sys_STATIONNAME&"<br>"&Sys_StationTel%></span></font></div>
<div id="Layer34" style="position:absolute; left:500px; top:<%=680+pagepx%>px; height:30px; z-index:28"><%
...
%></div>
<div id="Layer36" style="position:absolute; left:580px; top:<%=735+pagepx%>px; width:100px; height:43px; z-index:30"><%
	...
	elseif trim(Session("Unit_ID"))="8F00" then
		response.write "<table style=""border-color:#ff0000;border-style:solid;"" border=""1"" cellspacing=0 cellpadding=0>"
			response.write "<tr><td style=""border-color:#ff0000;border-style:solid;"" width=100 height=25 align=""center""><span class=""style1"">警員&nbsp;林享寶</span></td></tr>"
			response.write "</table>"
	end if
%></div>
<div id="Layer37" style="position:absolute; left:580px; top:<%=775+pagepx%>px; width:200px; height:46px; z-index:31"><%
	If trim(Sys_billprintuseimage)="1" Then
...
<!-----------------------------------檢查用車號---------------------------------->
<div id="Layer41" style="position:absolute; left:355px; top:<%=940+pagepx%>px; width:100px; height:11px; z-index:34"><%=Sys_CarNo%></div>
<!-----------------------------------送達證書 繳回單位資料---------------------------------->
<div id="Layer42" class="style3" style="position:absolute; left:48px; top:<%=973+pagepx%>px; z-index:1"><%="高雄縣政府警察局"&SysUnit& "送達證書"%>
</div>
<div id="Layer43" class="style3" style="position:absolute; left:355px; top:<%=973+pagepx%>px; z-index:1"><%="高雄縣政府警察局"&SysUnit%>
</div>
<div id="Layer44" class="style3" style="position:absolute; left:580px; top:<%=973+pagepx%>px; z-index:1"><%=replace(SysUnitAddress,"高雄縣","")%>
</div>
<!-----------------------------------送達證書 收件人資料---------------------------------->
<div id="Layer46" class="style3" style="position:absolute; left:128px; top:<%=995+pagepx%>px; width:230px; z-index:1"><%
	response.write funcCheckFont(Sys_Owner,22,1)&"<br>"
	response.write funcCheckFont(Sys_OwnerZip&Sys_OwnerZipName&Sys_OwnerAddress,18,1)
%>
</div>
<!-----------------------------------送達證書 文 號 ---------------------------------->
<div id="Layer47" class="style3" style="position:absolute; left:220px; top:<%=1040+pagepx%>px; z-index:1"><%
	response.write Sys_BillNo&"　"&Sys_CarNo
%>
</div>
<!--
