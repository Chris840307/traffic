...
If Not unit.eof Then sysunitname=trim(unit("UnitName"))
unit.close
for i=0 to Ubound(PBillSN)
	If instr(sysunitname,"霧峰")>0 Then
		if cint(i)<>0 then response.write "<div class=""PageNext""></div>"%>
		<!--#include virtual="traffic/Query/BillBaseTaiChungWuFeng_Deliver.asp"--><%
	else
		if cint(i)<>0 then response.write "<div class=""PageNext""></div>"%>
		<!--#include virtual="traffic/Query/BillBaseTaiChung_Deliver.asp"--><%
	End if
Next
%>
