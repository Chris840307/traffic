...
'	end if
	...
	strSQLA="Select a.imagefilenameb" &_
		...
		" or a.BillStatus<>'0') and a.ImagePathName is not null  and a.BillNo is null "&strSQLTemp&" Group by imagefilenameb"
...
<%
	...
	strSQL="Select a.imagefilenameb,a.BillNo,a.Sn,a.CarNo,a.BillTypeID,a.Rule1,a.Rule2,a.Rule3,a.Rule4,a.MemberStation,a.EquipMentID" &_
		...
		" from BillBase a" &_
		" where ((a.RecordStateID<>-1 and a.BillStatus='0')" &_
		" or a.BillStatus<>'0') and a.ImagePathName is not null  and a.BillNo is null "&strSQLTemp&STRimagefilenameb
...
%>
	<table width='100%' border='0' cellpadding="2">
		...
		<tr>
			<td><span class="style2">製表時間：</span><span class="style3"><%=now%></span>
			</td>
		</tr>
	</table>
	<hr>
...
<%end if%>
		...
		<tr>
			...
			%></span></td>
			<td colspan="2"><span class="style2">違規路段：</span><span class="style1"><%
...
%>
	...
	<%
		'送達証書
		strScan="select * from BillAttatchImage where BillNo='"&trim(rs1("ImageFileNameB"))&"' and TypeID=0 and Recordstateid=0 order by RecordDate"
		set rsScan=conn.execute(strScan)
		while Not rsScan.eof
		%>
			<div class='PageNext'>&nbsp;</div> <strong>掃瞄檔&nbsp;<%
			'掃描日期
			...
			<input type="button" value="刪除掃瞄檔" onclick="deleteImage('<%=trim(rsScan("Sn"))%>')">
		<%
			rsScan.movenext
		wend
		rsScan.close
