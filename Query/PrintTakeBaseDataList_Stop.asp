...
<%
	...
	If not ifnull(Request("RecordDate1")) Then
		...
		BasSQL="(select sn from TakeBase where RecordDate between TO_DATE('"&ArgueDate1&"','YYYY/MM/DD/HH24/MI/SS') and TO_DATE('"&ArgueDate2&"','YYYY/MM/DD/HH24/MI/SS') and recordstateid=0) tmpPasser"
	...
	If BasSQL = "" Then
		...
		If Ubound(tmp_billsn) >= 100 Then
			...
			For i = 0 to Ubound(sys_billsn)
				...
				tmpSQL=tmpSQL&"select sn from TakeBase where sn in("&sys_billsn(i)&")"
			Next

		else

			tmpSQL="select sn from TakeBase where sn in("&sys_billsn&")"
	...
	strCnt="select count(*) as cnt from TakeBase where Exists(select 'Y' from "&BasSQL&" where SN=TakeBase.SN)"
