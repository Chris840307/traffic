...
fname=year(now)&fMnoth&fDay&"建檔日_逕舉_舉發單資料.txt"
...
<%
	...
	" and a.billtypeid = '2' and BillStatus>'1' and billno is not null "
...
'					end if
				...
				if ogReturnReason<>"" then
					...
					%>,<%'備註
					If not ifnull(rsfound("note")) Then response.write replace(rsfound("note"),",",".")
					%>,<%'法條二
