...
BillNo="":CarNo="":fileCmt=150
strSQL="select distinct a.ImageFileNameB,a.CarNo from (select * from BillBase where ImagePathName is not null and BillStatus>1 and RecordStateId <> -1 and ImageFileNameB is not null and DeallineDate is not null) a,(Select * from DCILog where ExchangeTypeID='A' and DCIReturnStatusID='S') b where a.SN=b.BillSN "&request("SQLstr")&" order by a.CarNo"
...
for cmtI=PrintCnt to Ubound(PBillNo)
	SendAddrFlag="":HomeAddress="":BillCnt1=0:BillCnt2=0
	strSQL="select SendAddrFlag,HomeAddress from StopCaseSendAddr where CarNo=(select Distinct CarNo from BillBase where CarNo='"&PCarNo(cmtI)&"')"
	set rsSend=conn.execute(strSQL)
	If Not rsSend.eof Then SendAddrFlag=trim(rsSend("SendAddrFlag"))
	rsSend.Close

	If ifnull(SendAddrFlag) Then SendAddrFlag=1
	...
	For j=BillCnt1 to BillCnt2
		if PageCnt<>0 then response.write "<div class=""PageNext""></div>"
		PageCnt=PageCnt+1
		Sys_Address=""
		If j=1 Then
			strSQL="select CarNo,Owner,OwnerZip,OwnerAddress from BillbaseDCIReturn where CarNo='"&trim(PCarNo(cmtI))&"' and ExchangetypeID='A'"
			...
			rsmail.close
		elseIf j=2 then
			strSQL="select a.CarNo,a.Owner,b.HomeAddress from (select distinct CarNo,Owner from BillbaseDCIReturn where CarNo='"&trim(PCarNo(cmtI))&"' and ExchangetypeID='A') a,(select distinct CarNo,HomeAddress from StopCaseSendAddr where CarNo='"&trim(PCarNo(cmtI))&"') b where a.CarNo=b.CarNo"
			set rsDci=conn.execute(strSQL)
			if Not rsDci.eof then
				Sys_CarNo=trim(rsDci("CarNo"))
				Sys_Owner=trim(rsDci("Owner"))
				Sys_OwnerAddress=trim(rsDci("HomeAddress"))
				Sys_Address=Sys_OwnerAddress
			end if
			rsDci.close
			...
			set rsmail=conn.execute(strSQL)
			If Not rsmail.eof Then
				Sys_MailNumber=trim(rsmail("StoreAndSendMailNumber"))&"97000717"
				Sys_MailChkNumber=trim(rsmail("StoreAndSendMailNumber"))&"970007 17"
			end if
			rsmail.close
		End if
...
<div id="Layer1" class="style4" style="position:absolute; left:60px; top:110px; z-index:1"><%=funcCheckFont(InstrAdd(Sys_Address,25),25,1)%></div>

<div id="Layer2" class="style4" style="position:absolute; left:60px; top:190px; z-index:1"><%="收件人："&funcCheckFont(Sys_Owner,25,1)%></div>
...
<div id="Layer5" class="style2" style="position:absolute; left:50px; top:380px; z-index:1"><%="車號："&Sys_CarNo&"　車主地址："&funcCheckFont(Sys_Address,25,1)%></div>
...
繳費方式：請持本催繳通知單至統一7-11或全家便利超商全省各門市店繳納或受委託台灣國際開發事業有限公司<br>
　　　　　(花蓮市中福路230號，08：00至22：00停車收費時間繳納)，並保留本收據聯6個月，如已繳交停車欠<br>
　　　　　費請勿重覆繳費，以維護您的權益。<br>
查詢停車紀錄：請電(03)8312920或網址：www.hl.gov.tw或www.tidch.tw上網查詢；台端任何疑問請電<br>
　　　　　　　(03)8221019(花蓮縣警察局交通隊)洽詢。
</div>
<div id="Layer10" class="style2" style="position:absolute; left:80px; top:770px; z-index:1"><%
	response.write "繳費期限及單位代碼："&Sys_DeallIneDate(0)&Sys_DeallIneDate(1)&Sys_DeallIneDate(2)&"241"
	response.write "　　&nbsp;"
	DelphiASPObj.CreateBarCode Sys_DeallIneDate(0)&Sys_DeallIneDate(1)&Sys_DeallIneDate(2)&"241",0,35,160
	response.write "<img src=""../BarCodeImage/"&Sys_DeallIneDate(0)&Sys_DeallIneDate(1)&Sys_DeallIneDate(2)&"241.jpg"">"
...
<div id="Layer11" class="style2" style="position:absolute; left:80px; top:820px; z-index:1"><%
...
<div id="Layer12" class="style2" style="position:absolute; left:80px; top:870px; z-index:1"><%
	tmpDeallIneDate=Sys_DeallIneDate(0)&Sys_DeallIneDate(1)&Sys_DeallIneDate(2)
	tempChkMemony=right("0000000000"&(ForFeitSum+34),9)
	SysChkNumber=CreateChkCode(tmpDeallIneDate,"241",Sys_ImageFileNameB,SysDate,tempChkMemony)
...
<div id="Layer15" class="style3" style="position:absolute; left:130px; top:980px; z-index:1"><%
	response.write funcCheckFont(Sys_Owner,25,1)&"<br>"
	response.write funcCheckFont(InstrAdd(Sys_Address,14),25,1)
%>
</div>
<div id="Layer16" class="style3" style="position:absolute; left:130px; top:1045px; z-index:1"><%
...
<%
next
if (cmtI>0 and (cmtI mod fileCmt)=0) or cmtI>=Ubound(PBillNo) then
	response.write "<form name=myForm method=""post"">"
	response.write "<input type=""Hidden"" name=""SQLstr"" value="""&request("SQLstr")&""">"
	response.write "<input type=""Hidden"" name=""PrintCnt"" value="""&cmtI&""">"
	response.write "</from>"%>
</body>
</html>
<script type="text/javascript" src="../js/Print.js"></script>
<script language="javascript">
	<%if Ubound(PBillNo)<=fileCmt then%>
		window.focus();
		AutoprintWindow(true,5.08,5.08,5.08,5.08);
	<%elseif cmtI<=fileCmt then%>
		window.focus();
		AutoprintWindow(true,5.08,5.08,5.08,5.08);
		delay(90000);
		myForm.submit();
	<%elseif cmtI<Ubound(PBillNo) then%>
		window.focus();
		AutoprintWindow(true,5.08,5.08,5.08,5.08);
		delay(90000);
		myForm.submit();
