...
.style6 {font-family:"標楷體";font-size: 16px; line-height:2;}
...
strSql="select a.*,b.OwnerZip OwnerHomeZip,b.OwnerAddress OwnerHomeAddress,b.Owner OwnerHome,b.DriverHomeZip DriverZip,b.DriverHomeAddress DriverAddress,b.DCIRETURNCARTYPE DciCarType from (select * from BillbaseDCIReturn where BillNo='"&trim(rsbil("BillNo"))&"' and CarNo='"&trim(rsbil("CarNo"))&"' and ExchangetypeID='W') a,(select OwnerZip,OwnerAddress,Owner,CarNo,DriverHomeZip,DriverHomeAddress,DCIRETURNCARTYPE from BillbaseDCIReturn where Carno in(select carno from dcilog where BillSN="&trim(rsbil("BillSN"))&" and CarNo='"&trim(rsbil("CarNo"))&"' and ExchangetypeID='A' and dcireturnstatusid='S') and ExchangetypeID='A') b where a.carno=b.carno(+)"
...
If ifnull(Sys_OwnerAddress) Then	
	...
	If ifnull(Sys_OwnerAddress) Then
		Sys_Owner=trim(rsfound("OwnerHome"))
		Sys_OwnerAddress=trim(rsfound("OwnerHomeAddress"))
		Sys_OwnerZip=trim(rsfound("OwnerHomeZip"))	
	End If 
	...
	If not ifnull(Sys_OwnerAddress) Then
		...
		strSQL="Update BillbaseDCIReturn set Owner='"&Sys_Owner&"',OwnerZip='"&Sys_OwnerZip&"',OwnerAddress='"&Sys_OwnerAddress&"',DCIRETURNCARTYPE='"&trim(rsfound("DciCarType"))&"' where BillNo='"&trim(rsbil("BillNo"))&"' and CarNo='"&trim(rsbil("CarNo"))&"' and ExchangetypeID='W' and OwnerAddress is null"
...
<div id="Layer42" class="style3" style="position:absolute; left:<%=(280+leftpx)%>px; top:<%=75+toptpx%>px; width:320px; height:36px; z-index:5">
	<%if trim(Sys_BillTypeID)="1" then
...
<%if trim(Sys_ImageFileNameA)<>"" then%>
	<div id="Layer44" class="style4" style="position:absolute; left:<%=(30+leftpx)%>px; top:<%=340+toptpx%>px; width:250px; height:36px; z-index:5">
		O
	</div>
<%end if%>

<div id="Layer45" class="style4" style="position:absolute; left:<%=(40+leftpx)%>px; top:<%=75+toptpx%>px; width:250px; height:36px; z-index:5">
	<img src=<%="""../BarCodeImage/"&Sys_BillNo&"_1.jpg"""%>><br><span class="style2">　　<%=Sys_FirstBarCode%></span>
</div>


<div id="Layer44" class="style3" style="position:absolute; left:<%=(280+leftpx)%>px; top:<%=150+toptpx%>px; width:250px; height:36px; z-index:5">
	大宗郵資已付掛號函件<br>
	&nbsp;&nbsp;&nbsp;
	第<%=Sys_MailNumber%>號
</div>
...
<div id="Layer44" class="style5" style="position:absolute; left:<%=(250+leftpx)%>px; top:<%=200+toptpx%>px; width:350px; height:36px; z-index:5">
	&nbsp;&nbsp;&nbsp;&nbsp;
...
<%If instr(Request("Sys_Batchnumber"),"WT")>0 then %>
	<div id="Layer41" style="position:absolute; left:<%=(190+leftpx)%>px; top:<%=270+toptpx%>px; width:600px; height:36px; z-index:5">
...
<%if trim(Sys_ImageFileNameA)<>"" then%>
<div style="position:absolute; left:30px; top:440px;"><img src="<%=Sys_IisImagePath&Sys_ImageFileNameA%>" width="329" height="239"></DIV>
<%end if%>
<%if trim(Sys_ImageFileNameB)<>"" then%>
<div style="position:absolute; left:395px; top:440px;"><img src="<%=Sys_IisImagePath&Sys_ImageFileNameB%>" width="330" height="240"></DIV>
<%end if%>

<% if showBarCode then %>
	<div id="Layer1" style="position:absolute; left:<%=(50+leftpx)%>px; top:<%=720+toptpx%>px; width:202px; height:36px; z-index:5">Ｖ</div>
<% else %>
	<div id="Layer2" style="position:absolute; left:<%=(50+leftpx)%>px; top:<%=750+toptpx%>px; width:202px; height:36px; z-index:5">Ｖ</div>
<% end If %>

<!--
<%if trim(Sys_BillTypeID)="1" then%>
<div id="Layer3" style="position:absolute; left:170px; top:720px; width:202px; height:36px; z-index:5">Ｖ</div>
<%else%>
<div id="Layer4" style="position:absolute; left:170px; top:735px; width:202px; height:36px; z-index:5">Ｖ</div>
<%end if%>
-->

<%if trim(Sys_BillTypeID)="1" then%>
	<%if trim(Sys_INSURANCE)="0" then%>
		<div id="Layer6" style="position:absolute; left:<%=(635+leftpx)%>px; top:<%=725+toptpx%>px; width:202px; height:36px; z-index:5">Ｖ</div>
	<%elseif trim(Sys_INSURANCE)="1" then%>
		<div id="Layer7" style="position:absolute; left:<%=(635+leftpx)%>px; top:<%=740+toptpx%>px; width:202px; height:36px; z-index:5">Ｖ</div>
	<%else%>
		<div id="Layer8" style="position:absolute; left:<%=(635+leftpx)%>px; top:<%=755+toptpx%>px; width:202px; height:36px; z-index:5">Ｖ</div>
	<%end if%>
<%end if%>
<div id="Layer9" style="position:absolute; left:50px; top:<%=770+toptpx%>px; width:202px; height:36px; z-index:5"><%
	if showBarCode then
...
<div id="Layer10" style="position:absolute; left:500px; top:<%=770+toptpx%>px; width:233px; height:32px; z-index:6"><img src=<%="""../BarCodeImage/"&Sys_BillNo&"_4.jpg"""%>></div>
<!--<div id="Layer11" style="position:absolute; left:590px; top:<%=815+toptpx%>px; width:230px; height:12px; z-index:7"><font size=1><%=Sys_BillNo%></font></div>-->

<div id="Layer144" class="style4" style="position:absolute; left:<%=(120+leftpx)%>px; top:<%=840+toptpx%>px; width:250px; height:11px; z-index:3"><%
...
<% uppx=4 %>
<div id="Layer17" style="position:absolute; left:<%=(630+leftpx)%>px; top:<%=865+toptpx-uppx%>px; width:99px; height:12px; z-index:10"><span class="style3"><%=fastring%></span></div>

<div id="Layer18" style="position:absolute; left:<%=(120+leftpx)%>px; top:<%=885+toptpx-uppx%>px; width:100px; height:14px; z-index:11"><span class="style3"><%=Sys_CarNo%></span></div>
<div id="Layer19" style="position:absolute; left:<%=(305+leftpx)%>px; top:<%=885+toptpx-uppx%>px; width:200px; height:20px; z-index:12"><span class="style3"><%=Sys_DCIRETURNCARTYPE%></span></div>
<div id="Layer20" style="position:absolute; left:<%=(555+leftpx)%>px; top:<%=885+toptpx-uppx%>px; width:201px; height:17px; z-index:13"><span class="style3"><%=funcCheckFont(Sys_Owner,14,1)%></span></div>
<div id="Layer21" style="position:absolute; left:<%=(185+leftpx)%>px; top:<%=910+toptpx-uppx%>px; width:750px; height:13px; z-index:14"><span class="style3"><%
...
<div id="Layer22" style="position:absolute; left:<%=(130+leftpx)%>px; top:<%=937+toptpx-uppx%>px; width:40px; height:13px; z-index:15"><span class="style5"><%=Sys_IllegalDate(0)%></span></div>

<div id="Layer23" style="position:absolute; left:<%=(180+leftpx)%>px; top:<%=937+toptpx-uppx%>px; width:40px; height:17px; z-index:16"><span class="style5"><%=Sys_IllegalDate(1)%></span></div>

<div id="Layer24" style="position:absolute; left:<%=(230+leftpx)%>px; top:<%=937+toptpx-uppx%>px; width:40px; height:16px; z-index:17"><span class="style5"><%=Sys_IllegalDate(2)%></div>

<div id="Layer25" style="position:absolute; left:<%=(280+leftpx)%>px; top:<%=937+toptpx-uppx%>px; width:40px; height:16px; z-index:18"><span class="style5"><%=right("00"&Sys_IllegalDate_h,2)%></span></div>

<div id="Layer26" style="position:absolute; left:<%=(330+leftpx)%>px; top:<%=937+toptpx-uppx%>px; width:40px; height:13px; z-index:19"><span class="style5"><%=right("00"&Sys_IllegalDate_m,2)%></span></div>

<div id="Layer27" class="style5" style="position:absolute; left:<%=(410+leftpx)%>px; top:<%=940+toptpx-uppx%>px; width:315px; height:31px; z-index:20"><%
...
%></div>
<div id="Layer28" style="position:absolute; left:<%=(110+leftpx)%>px; top:<%=960+toptpx-uppx%>px; width:240px; height:15px; z-index:21"><span class="style5"><%=Sys_ILLEGALADDRESS%></span></div>
<div id="Layer29" style="position:absolute; left:<%=(140+leftpx)%>px; top:<%=987+toptpx-uppx%>px; width:34px; height:11px; z-index:22"><span class="style3"><%=Sys_DealLineDate(0)%></span></div>
<div id="Layer30" style="position:absolute; left:<%=(210+leftpx)%>px; top:<%=987+toptpx-uppx%>px; width:35px; height:13px; z-index:23"><span class="style3"><%=Sys_DealLineDate(1)%></span></div>
<div id="Layer31" style="position:absolute; left:<%=(285+leftpx)%>px; top:<%=987+toptpx-uppx%>px; width:32px; height:15px; z-index:24"><span class="style3"><%=Sys_DealLineDate(2)%></span></div>

<div id="Layer46" style="position:absolute; left:<%=(430+leftpx)%>px; top:<%=995+toptpx-uppx%>px; width:32px; height:15px; z-index:24"><span class="style2"><%
	'if len(trim(Sys_Rule1))>7 then response.write "之"&right(trim(Sys_Rule1),1)
%></span></div>

<div id="Layer47" style="position:absolute; left:<%=(430+leftpx)%>px; top:<%=1010+toptpx-uppx%>px; width:32px; height:15px; z-index:24"><span class="style2"><%
	'if len(trim(Sys_Rule2))>7 then response.write "之"&right(trim(Sys_Rule2),1)
%></span></div>

<div id="Layer32" style="position:absolute; left:<%=(420+leftpx)%>px; top:<%=1030+toptpx-uppx%>px; width:400px; height:49px; z-index:29"><span class="style5"><%
...
%></span></div>
<div id="Layer33" style="position:absolute; left:<%=(400+leftpx)%>px; top:<%=1065+toptpx-uppx%>px; width:400px; height:30px; z-index:1"><%
...
%></div>
<div id="Layer34" style="position:absolute; left:<%=(645+leftpx)%>px; top:<%=1075+toptpx-uppx%>px; width:90px; height:30px; z-index:28"><span class="style2"><%=Sys_STATIONNAME%></span></div>


<div id="Layer35" style="position:absolute; left:<%=(420+leftpx)%>px; top:<%=1125+toptpx-uppx%>px; width:150px; height:49px; z-index:29"><%
...
<div id="Layer37" style="position:absolute; left:<%=(585+leftpx)%>px; top:<%=1130+toptpx-uppx%>px; width:200px; height:46px; z-index:31"><%
...
%></div>
<div id="Layer38" style="position:absolute; left:170px; top:<%=1172+toptpx-uppx%>px; width:60px; height:10px; z-index:32"><span class="style3"><%=sys_Date(0)%></span></div>
<div id="Layer39" style="position:absolute; left:220px; top:<%=1172+toptpx-uppx%>px; width:60px; height:13px; z-index:33"><span class="style3"><%=sys_Date(1)%></span></div>
<div id="Layer40" style="position:absolute; left:270px; top:<%=1172+toptpx-uppx%>px; width:60px; height:11px; z-index:34"><span class="style3"><%=sys_Date(2)%></span></div>
<div id="Layer145" style="position:absolute; left:400px; top:<%=1190+toptpx-uppx%>px; width:150px; height:11px; z-index:37"><font size=2><%=Sys_CarColor&"&nbsp;&nbsp;&nbsp;"&Sys_A_Name%></font></div>
</div>
<%
