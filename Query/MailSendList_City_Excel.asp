...
'fname=year(now)&fMnoth&fDay&"_大宗掛號清冊.xls"
...
If  sys_City="台南市" Then
	...
	strI="insert into Log values((select nvl(max(Sn),0)+1 from Log),360,"&Trim(Session("User_ID"))&",'"&Trim(Session("Ch_Name"))&"','"&userip&"',sysdate,'大宗掛號清冊(區分縣市),"&Replace(strwhere,"'","""")&"')"
	'response.write strI
	Conn.execute strI
End If 
...
if sys_City="基隆市" then 
	if ExchangeTypeFlag="N" then
		strUpdCounty="update billbasedcireturn set drivercounty=OwnerCounty where billno in " &_
			" (select a.billno from dcilog a,BillBase f where a.Billsn=f.Sn "&strwhere&") and ExchangeTypeID='W' and drivercounty is null"
		conn.execute strUpdCounty
		strCounty="select distinct(e.DriverCounty) as OwnerCounty,e.OwnerZip from DCILog a,MemberData b" &_
		",DCIReturnStatus d,BillBase f,BillBaseDciReturn e where a.BillSN=f.SN and f.RecordStateID=0" &_
		" and e.ExchangeTypeID=d.DCIActionID(+) and e.Status=d.DCIReturn(+)" &_
		" and e.ExchangeTypeID='W' and (d.DCIreturnStatus=1)" &_
		" and a.RecordMemberID=b.MemberID(+) and a.BillNo=e.BillNo and a.CarNo=e.CarNo" &_

		" and f.BillTypeID='2' "&strwhere&" order by DriverCounty"
	else
		strCounty="select distinct(e.OwnerCounty),e.OwnerZip from DCILog a,MemberData b" &_
		",DCIReturnStatus d,BillBase f,BillBaseDciReturn e where a.BillSN=f.SN and f.RecordStateID=0" &_
		" and a.ExchangeTypeID=d.DCIActionID(+) and a.DCIReturnStatusID=d.DCIReturn(+)" &_
		" and a.ExchangeTypeID='W' and ((d.DCIreturnStatus=1 and ((e.DciErrorCarData not in ('1','3','9','a','j','A','H','K','L','T'))) and f.UseTool<>8) or (d.DCIreturnStatus=1 and f.UseTool=8 and (f.EquipmentID<>'-1' or f.EquipmentID is null)))" &_
		" and a.RecordMemberID=b.MemberID(+) and a.BillNo=e.BillNo and a.CarNo=e.CarNo" &_
		" and a.ExchangeTypeID=e.ExchangeTypeID and a.DciReturnStatusID=e.Status" &_
		" and f.BillTypeID='2' "&strwhere&" order by OwnerCounty"
	end if
	...
	if ExchangeTypeFlag="N" then
		...
		",DCIReturnStatus d,BillBase f,BillBaseDciReturn e where a.BillSN=f.SN and f.RecordStateID=0" &_
		" and e.ExchangeTypeID=d.DCIActionID(+) and e.Status=d.DCIReturn(+)" &_
		" and e.ExchangeTypeID='W' and ((d.DCIreturnStatus=1 and f.UseTool<>8) or (d.DCIreturnStatus=1 and f.UseTool=8))" &_
		" and a.RecordMemberID=b.MemberID(+) and a.BillNo=e.BillNo and a.CarNo=e.CarNo" &_
		...
		",DCIReturnStatus d,BillBase f,BillBaseDciReturn e where a.BillSN=f.SN and f.RecordStateID=0" &_
		" and a.ExchangeTypeID=d.DCIActionID(+) and a.DCIReturnStatusID=d.DCIReturn(+)" &_
		" and a.ExchangeTypeID='W' and ((d.DCIreturnStatus=1 and f.UseTool<>8) or (d.DCIreturnStatus=1 and f.UseTool=8 and (f.EquipmentID<>'-1' or f.EquipmentID is null)))" &_
		" and a.RecordMemberID=b.MemberID(+) and a.BillNo=e.BillNo and a.CarNo=e.CarNo" &_
		" and a.ExchangeTypeID=e.ExchangeTypeID and a.DciReturnStatusID=e.Status" &_
		" and f.BillTypeID='2' "&strwhere&" order by OwnerCounty"
	end if
	...
	If Not rsCounty.Bof Then rsCounty.MoveFirst 
	While Not rsCounty.Eof
		if trim(rsCounty("OwnerCounty"))<>"" and not isnull(rsCounty("OwnerCounty")) then
			if RCountyTmp="" then
				RCountyTmp=trim(rsCounty("OwnerCounty"))
			else
				getCounty=split(RCountyTmp,",")
				k=0
				for j=0 to ubound(getCounty)
					if trim(rsCounty("OwnerCounty"))=trim(getCounty(j)) then
						k=1
						exit for
					end if
				next
				if k=0 then
					RCountyTmp=RCountyTmp&","&trim(rsCounty("OwnerCounty"))
				end if
			end if
		end if
	rsCounty.MoveNext
	...
	if sys_City="南投縣" then 
		if ExchangeTypeFlag="N" then
			strCounty="select distinct(e.DriverCounty),e.OwnerZip from DCILog a,MemberData b" &_
			",DCIReturnStatus d,BillBase f,BillBaseDciReturn e where a.BillSN=f.SN and f.RecordStateID=0" &_
			" and e.ExchangeTypeID=d.DCIActionID(+) and e.Status=d.DCIReturn(+)" &_
			" and e.ExchangeTypeID='W' and (a.DCIReturnStatusID in ('Y','S','n','h','L'))" &_
			" and a.RecordMemberID=b.MemberID(+) and a.BillNo=e.BillNo and a.CarNo=e.CarNo" &_

			" and f.BillTypeID='1' "&strwhere&" order by DriverCounty"
		else
			strUpdCounty="update billbasedcireturn set drivercounty=OwnerCounty where billno in " &_
				" (select a.billno from dcilog a,BillBase f where a.Billsn=f.Sn "&strwhere&") and ExchangeTypeID='W' and drivercounty is null"
			conn.execute strUpdCounty
			...
			",DCIReturnStatus d,BillBase f,BillBaseDciReturn e where a.BillSN=f.SN and f.RecordStateID=0" &_
			" and a.ExchangeTypeID=d.DCIActionID(+) and a.DCIReturnStatusID=d.DCIReturn(+)" &_
			" and e.ExchangeTypeID='W' and (a.DCIReturnStatusID in ('Y','S','n','L') and (f.EquipmentID<>'-1' or f.EquipmentID is null))" &_
			" and a.RecordMemberID=b.MemberID(+) and a.BillNo=e.BillNo and a.CarNo=e.CarNo" &_
		...
		if ExchangeTypeFlag="N" then
			...
			",DCIReturnStatus d,BillBase f,BillBaseDciReturn e where a.BillSN=f.SN and f.RecordStateID=0" &_
			" and e.ExchangeTypeID=d.DCIActionID(+) and e.Status=d.DCIReturn(+)" &_
			" and e.ExchangeTypeID='W' and (a.DCIReturnStatusID in ('Y','S','n','L'))" &_
			" and a.RecordMemberID=b.MemberID(+) and a.BillNo=e.BillNo and a.CarNo=e.CarNo" &_
	...
	While Not rsCounty.Eof
		if SCountyTmp="" then
			SCountyTmp=trim(rsCounty("DriverCounty"))
		else
			getCounty=split(SCountyTmp,",")
			k=0
			for j=0 to ubound(getCounty)
				if trim(rsCounty("DriverCounty"))=trim(getCounty(j)) then
					k=1
					exit for
				end if
			next
			if k=0 then
				SCountyTmp=SCountyTmp&","&trim(rsCounty("DriverCounty"))
			end if
		end if
	rsCounty.MoveNext
	Wend
	...
	if isnull(RCountyTmp) then
		RCountyTmp=""
	end if
	if isnull(SCountyTmp) then
		SCountyTmp=""
	end if

	CountyTmp=""
	ReportStationArray=split(RCountyTmp,",")
	StopStationArray=split(SCountyTmp,",")
	for RSA=0 to ubound(ReportStationArray)
		if instr(CountyTmp,ReportStationArray(RSA))=0 then
			if CountyTmp="" then
				CountyTmp=ReportStationArray(RSA)
			else
				CountyTmp=CountyTmp&","&ReportStationArray(RSA)
			end if
		end if
	next
	for SSA=0 to ubound(StopStationArray)
		if instr(CountyTmp,StopStationArray(SSA))=0 then
			if CountyTmp="" then
				CountyTmp=StopStationArray(SSA)
			else
				CountyTmp=CountyTmp&","&StopStationArray(SSA)
			end if
		end if
	next
%>
...
<%	
	PrintSNTotal=0	'編號
	CountyArray=split(CountyTmp,",")
	for CA=0 to ubound(CountyArray)
	if trim(CountyArray(CA))<>"" then
%>
<%		'逕舉
		mailSN=0
if sys_City="基隆市" then 
	if ExchangeTypeFlag="N" then
		strSQL="select a.BatchNumber,a.BillSN,a.BillNo,a.BillTypeID,a.CarNo,a.ExchangeTypeID,f.UseTool,f.Note" &_
		",f.driveraddress,f.driverzip,f.owner,f.ownerzip,f.owneraddress" &_
		" from DCILog a,MemberData b,DCIReturnStatus d,BillBase f,BillBaseDciReturn e,BillMailHistory g" &_
		" where a.BillSN=f.SN and f.RecordStateID=0 and e.ExchangeTypeID=d.DCIActionID(+)" &_
		" and e.Status=d.DCIReturn(+) and e.ExchangeTypeID='W'" &_
		" and ((d.DCIreturnStatus=1 and f.BillTypeID='2' and e.DriverCounty='"&trim(CountyArray(CA))&"')" &_
		" or (a.DCIReturnStatusID in ('Y','S','n','L') and f.BillTypeID='1' and e.DriverCounty='"&trim(CountyArray(CA))&"'))" &_
		" and a.RecordMemberID=b.MemberID(+)" &_
		" and f.SN=g.BillSN" &_
		" and a.BillNo=e.BillNo and a.CarNo=e.CarNo" &_

		" "&strwhere&" order by g.StoreAndSendMailNumber"
	else
		strSQL="select a.BatchNumber,a.BillSN,a.BillNo,a.BillTypeID,a.CarNo,a.ExchangeTypeID,f.UseTool,f.Note" &_
		",f.driveraddress,f.driverzip,f.owner,f.ownerzip,f.owneraddress" &_
		" from DCILog a,MemberData b,DCIReturnStatus d,BillBase f,BillBaseDciReturn e" &_
		" where a.BillSN=f.SN and f.RecordStateID=0 and a.ExchangeTypeID=d.DCIActionID(+)" &_
		" and a.DCIReturnStatusID=d.DCIReturn(+) and a.ExchangeTypeID='W'" &_
		" and ((((d.DCIreturnStatus=1 and ((e.DciErrorCarData not in ('1','3','9','a','j','A','H','K','L','T'))) and f.UseTool<>8) or (d.DCIreturnStatus=1 and f.UseTool=8 and (f.EquipmentID<>'-1' or f.EquipmentID is null))) and f.BillTypeID='2' and e.OwnerCounty='"&trim(CountyArray(CA))&"')" &_
		" or (a.DCIReturnStatusID in ('Y','S','n','L') and (f.EquipmentID<>'-1' or f.EquipmentID is null) and f.BillTypeID='1' and e.DriverCounty='"&trim(CountyArray(CA))&"'))" &_
		" and a.RecordMemberID=b.MemberID(+)" &_
		" and a.BillNo=e.BillNo" &_
		" and a.CarNo=e.CarNo and a.ExchangeTypeID=e.ExchangeTypeID" &_
		" and a.DciReturnStatusID=e.Status" &_
		" "&strwhere&" order by f.RecordMemberID,f.RecordDate"
	end if
elseif sys_City="南投縣" then 
	...
	if ExchangeTypeFlag="N" then
		...
		" where a.BillSN=f.SN and f.RecordStateID=0 and e.ExchangeTypeID=d.DCIActionID(+)" &_
		" and e.Status=d.DCIReturn(+) and e.ExchangeTypeID='W'" &_
		" and ((((d.DCIreturnStatus=1 and f.UseTool<>8) or (d.DCIreturnStatus=1 and f.UseTool=8)) and f.BillTypeID='2' and e.OwnerCounty='"&trim(CountyArray(CA))&"')" &_
		" or (a.DCIReturnStatusID in ('Y','S','n','h','L') and f.BillTypeID='1' and e.DriverCounty='"&trim(CountyArray(CA))&"'))" &_
		" and a.RecordMemberID=b.MemberID(+)" &_
		" and a.BillNo=e.BillNo and a.CarNo=e.CarNo" &_
		...
		" where a.BillSN=f.SN and f.RecordStateID=0 and a.ExchangeTypeID=d.DCIActionID(+)" &_
		" and a.DCIReturnStatusID=d.DCIReturn(+) and a.ExchangeTypeID='W'" &_
		" and ((((d.DCIreturnStatus=1 and f.UseTool<>8) or (d.DCIreturnStatus=1 and f.UseTool=8 and (f.EquipmentID<>'-1' or f.EquipmentID is null))) and f.BillTypeID='2' and e.OwnerCounty='"&trim(CountyArray(CA))&"')" &_
		" or (a.DCIReturnStatusID in ('Y','S','n','L') and (f.EquipmentID<>'-1' or f.EquipmentID is null) and f.BillTypeID='1' and e.DriverCounty='"&trim(CountyArray(CA))&"'))" &_
		" and a.RecordMemberID=b.MemberID(+)" &_
		" and a.BillNo=e.BillNo" &_
		" and a.CarNo=e.CarNo and a.ExchangeTypeID=e.ExchangeTypeID" &_
...
<%end if%>
		<tr>
			...
			<td align="left" width="65%">交寄大宗 &nbsp; &nbsp; &nbsp; &nbsp; 掛 &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; 號 &nbsp; &nbsp; &nbsp; &nbsp; 函件寄存
											<%if sys_City="基隆市" then %>		
															&nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp; &nbsp; &nbsp; &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;&nbsp;   批號 <%=trim(rs1("BatchNumber"))%> 
											<%end if%>
												
			</td>
		</tr>
		<tr>
			<td align="left" colspan="2">地區：<%=CountyArray(CA)%></td>
		</tr>
	</table>
	<table width="710" border="1" cellpadding="3" cellspacing="0">
		<tr>
			<td width="6%" height="28" align="center">號碼</td>
			<td width="9%" align="center">掛號碼</td>
			<td width="22%" align="center">收件人姓名</td>
			<td width="53%" align="center">寄達地名(或地址)</td>
			<td width="10%" align="center">備註</td>
		</tr>
<%		MailPageCount=0
		for i=1 to 30
			if rs1.eof then exit for
			MailPageCount=MailPageCount+1
			mailSN=mailSN+1
			PrintSNTotal=PrintSNTotal+1
%>		<tr>
			<td align="right"><%=PrintSNTotal%></td>	
			<td align="left"><%
		...
		if trim(rs1("BillTypeID"))="2" then
			...
			elseif sys_City="花蓮縣" then
				if ExchangeTypeFlag="N" then	'單退先抓A的driver，沒有的話再抓W的Driver,再沒有就抓W的owner
					...
					rsD.close
					set rsD=nothing
				else	'XXXXX入案先抓A的OwnerNotifyAddress 2.W driver 3.W ownerXXXX
...
%>
	...
	<table width="710" border="0" cellpadding="0" cellspacing="0">
		<tr>
			<td height="26" width="60%" align="right"><%="上開  掛號函件  共  "&MailPageCount&"  件照收無誤"%>
			</td>
...
if sys_City="基隆市" then 
	if ExchangeTypeFlag="N" then
		strSQL="select a.BatchNumber,a.BillSN,a.BillNo,a.BillTypeID,a.CarNo,a.ExchangeTypeID,f.Note" &_
		",f.driveraddress,f.driverzip,f.owner,f.ownerzip,f.owneraddress" &_
		" from DCILog a,MemberData b,DCIReturnStatus d,BillBase f,BillBaseDciReturn e,BillMailHistory g" &_
		" where a.BillSN=f.SN and f.RecordStateID=0 and e.ExchangeTypeID=d.DCIActionID(+)" &_
		" and e.Status=d.DCIReturn(+) and e.ExchangeTypeID='W'" &_
		" and ((d.DCIreturnStatus=1 and f.BillTypeID='2' and e.DriverCounty is null)" &_
		" or (a.DCIReturnStatusID in ('Y','S','n','L') and f.BillTypeID='1' and e.DriverCounty is null))" &_
		" and a.RecordMemberID=b.MemberID(+)" &_
		" and f.SN=g.BillSN" &_
		" and a.BillNo=e.BillNo and a.CarNo=e.CarNo" &_
		...
		" where a.BillSN=f.SN and f.RecordStateID=0 and a.ExchangeTypeID=d.DCIActionID(+)" &_
		" and a.DCIReturnStatusID=d.DCIReturn(+) and a.ExchangeTypeID='W'" &_
		" and ((((d.DCIreturnStatus=1 and ((e.DciErrorCarData not in ('1','3','9','a','j','A','H','K','L','T'))) and f.UseTool<>8) or (d.DCIreturnStatus=1 and f.UseTool=8 and (f.EquipmentID<>'-1' or f.EquipmentID is null))) and f.BillTypeID='2' and e.OwnerCounty is null)" &_
		" or (a.DCIReturnStatusID in ('Y','S','n','L') and (f.EquipmentID<>'-1' or f.EquipmentID is null) and f.BillTypeID='1' and e.DriverCounty is null))" &_
		" and a.RecordMemberID=b.MemberID(+)" &_
		" and a.BillNo=e.BillNo" &_
		" and a.CarNo=e.CarNo and a.ExchangeTypeID=e.ExchangeTypeID" &_
		" and a.DciReturnStatusID=e.Status" &_
...
elseif sys_City="南投縣" then 
	if ExchangeTypeFlag="N" then
		...
		" where a.BillSN=f.SN and f.RecordStateID=0 and e.ExchangeTypeID=d.DCIActionID(+)" &_
		" and e.Status=d.DCIReturn(+) and e.ExchangeTypeID='W'" &_
		" and ((((d.DCIreturnStatus=1 and f.UseTool<>8) or (d.DCIreturnStatus=1 and f.UseTool=8)) and f.BillTypeID='2' and e.OwnerCounty is null)" &_
		" or (a.DCIReturnStatusID in ('Y','S','n','h','L') and f.BillTypeID='1' and e.DriverCounty is null))" &_
		" and a.RecordMemberID=b.MemberID(+)" &_
		" and a.BillNo=e.BillNo and a.CarNo=e.CarNo" &_
		...
		" where a.BillSN=f.SN and f.RecordStateID=0 and a.ExchangeTypeID=d.DCIActionID(+)" &_
		" and a.DCIReturnStatusID=d.DCIReturn(+) and a.ExchangeTypeID='W'" &_
		" and ((((d.DCIreturnStatus=1 and f.UseTool<>8) or (d.DCIreturnStatus=1 and f.UseTool=8 and (f.EquipmentID<>'-1' or f.EquipmentID is null))) and f.BillTypeID='2' and e.OwnerCounty is null)" &_
		" or (a.DCIReturnStatusID in ('Y','S','n','L') and (f.EquipmentID<>'-1' or f.EquipmentID is null) and f.BillTypeID='1' and e.DriverCounty is null))" &_
		" and a.RecordMemberID=b.MemberID(+)" &_
		" and a.BillNo=e.BillNo" &_
		" and a.CarNo=e.CarNo and a.ExchangeTypeID=e.ExchangeTypeID" &_
...
<%end if%>
		...
		<tr>
			<td align="left" colspan="2">地區：</td>
		</tr>
	</table>
	<table width="710" border="1" cellpadding="4" cellspacing="0">
		<tr>
			<td width="6%" height="28" align="center">號碼</td>
			<td width="9%" align="center">掛號碼</td>
			<td width="22%" align="center">收件人姓名</td>
...
'							response.write left(trim(rsH("opengovreportnumber")),6)&"&nbsp;"
'						else
'							response.write left(trim(rsH("firstbarcode")),6)&"&nbsp;"
'						end if
'					else
'						if trim(rs1("BillTypeID"))="2" and trim(rs1("UseTool"))<>"8" then
'							if trim(rs1("ExchangeTypeID"))="W" then
'								if trim(rsH("MailNumber"))<>"" and not isnull(rsH("MailNumber")) then
'									response.write trim(rsH("MailNumber"))&"&nbsp;"
'								else
...
'				set rsH=nothing
			...
			ZipName=""
		if trim(rs1("BillTypeID"))="2" then
			if sys_City="台中市" or sys_City="高雄市" or sys_City="高雄縣" then
				if ExchangeTypeFlag="N" then	'單退先抓A的driver，沒有的話再抓W的Driver,再沒有就抓W的owner
					strSqlD="select DriverHomeZip,DriverHomeAddress,Owner,OwnerZip,OwnerAddress from BIllBaseDCIReturn where ((BillNo='"&trim(rs1("BillNo"))&"' and CarNo='"&trim(rs1("CarNo"))&"') or (BillNo is null and CarNo='"&trim(rs1("CarNo"))&"')) and ExchangeTypeID='A' and Status='S'"
					set rsD=conn.execute(strSqlD)
					if not rsD.eof then
