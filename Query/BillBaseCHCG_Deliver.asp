...
rsfound.close
Sys_Sex=""
strSql="select distinct BillFillerMemberID,DriverSex,DriverBirth,IllegalDate,DealLineDate,IMAGEFILENAME,IMAGEPATHNAME,IMAGEFILENAMEB,RECORDMEMBERID,BILLMEMID1 from BillBase where BillNo='"&trim(rsbil("BillNo"))&"' and CarNo='"&trim(rsbil("CarNo"))&"'"
...
rsbil.close
%>
<!--<div id="Layer1" style="position:absolute; left:70px; top:<%=0+477*(i)%>px; width:150px; height:36px; z-index:5"><img src=<%="""../BarCodeImage/"&Sys_BillNo&".jpg"""%>>
</div>-->

<div id="Layer2" class="style2" style="position:absolute; left:600px; top:<%=35+477*(i)%>px; width:170px; height:36px; z-index:5"><%
reUnitName=""
Sys_UnitName="彰化縣警察局交通隊"
...
<div id="Layer3" class="style4" style="position:absolute; left:330px; top:<%=35+477*(i)%>px; width:250px; height:36px; z-index:5"><%=Sys_UnitAddress%>
</div>

<div id="Layer4" class="style4" style="position:absolute; left:45px; top:<%=75+477*(i)%>px; width:320px; height:36px; z-index:5"><%
		response.write funcCheckFont(Sys_Owner,20,4)&"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;車號："&Sys_CarNo
		response.write "<br>"&Sys_OwnerZip&"&nbsp;&nbsp;"&Sys_OwnerZipName&funcCheckFont(Sys_OwnerAddress,20,4)%>
</div>

<div id="Layer5" class="style4" style="position:absolute; left:45px; top:<%=175+477*(i)%>px; width:250px; height:36px; z-index:5"><%="彰警交第"&Sys_BillNo&"號"%>
</div>
<!--<div id="Layer6" class="style4" style="position:absolute; left:225px; top:<%=150+477*(i)%>px; width:60px; height:36px; z-index:5"><%
	response.write left(trim(Sys_Rule1),2)'條
	if len(trim(Sys_Rule1))>7 then response.write "之"&right(trim(Sys_Rule1),1)%>
</div>
<div id="Layer7" class="style4" style="position:absolute; left:275px; top:<%=150+477*(i)%>px; width:20px; height:36px; z-index:5"><%response.write Mid(trim(Sys_Rule1),3,1)'項%>
</div>
<div id="Layer8" class="style4" style="position:absolute; left:320px; top:<%=150+477*(i)%>px; width:20px; height:36px; z-index:5"><%response.write Mid(trim(Sys_Rule1),4,2)'款%>
</div>
<div id="Layer9" class="style4" style="position:absolute; left:360px; top:<%=150+477*(i)%>px; width:20px; height:36px; z-index:5">testa<%response.write Mid(trim(Sys_Rule2),6,2)%>
</div><%
if trim(Sys_Rule2)<>"0" then
	response.write "<div id=""Layer10"" class=""style4"" style=""position:absolute; left:225px; top:"& 170+477*(i) &"px; width:60px; height:36px; z-index:5"">"&left(trim(Sys_Rule2),2)'條
	if len(trim(Sys_Rule2))>7 then response.write "之"&right(trim(Sys_Rule2),1)
	response.write "a</div>"

	response.write "<div id=""Layer11"" class=""style4"" style=""position:absolute; left:275px; top:"& 170+477*(i) &"px; width:20px; height:36px; z-index:5"">"&Mid(trim(Sys_Rule2),3,1)&"b</div>"'項"

	response.write "<div id=""Layer12"" class=""style4"" style=""position:absolute; left:320px; top:"& 170+477*(i) &"px; width:20px; height:36px; z-index:5"">"&Mid(trim(Sys_Rule2),4,2)&"c</div>"'款"

	response.write "<div id=""Layer13"" class=""style4"" style=""position:absolute; left:360px; top:"& 170+477*(i) &"px; width:20px; height:36px; z-index:5"">"&Mid(trim(Sys_Rule2),6,2)&"d</div>"
end if%>-->
<div id="Layer14" class="style2" style="position:absolute; left:0px; top:<%=230+477*(i)%>px; width:20px; height:36px; z-index:5"><%
reUnitName="交通隊"
Sys_UnitName="交通隊"
