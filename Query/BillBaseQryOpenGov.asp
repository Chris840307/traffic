...
if request("DB_Selt")="Selt" then
		...
		If sys_City="高雄市" Then
			if request("Sys_BatchNo")<>"" then
				...
				strwhere=strwhere&" and c.UserMarkMemberID in (select MemberID from MemberData where UnitID in (" &_
				"select UnitID from UnitInfo where UnitTypeID in (select UnitTypeID from UnitInfo where UnitID in ('"&trim(request("Sys_RecordUnit"))&"')) ))"
				End If 
			else
				strBatch=""
		...
		if request("DCIstatus")="0" then
			if sys_City="台中市" or sys_City="高雄縣" or sys_City="高雄市" Or sys_City=ApconfigureCityName or sys_City="花蓮縣" then
				if request("Sys_BatchNo")<>"" then
					strBatch2="  and a.SN in (select distinct(BillSN) from DciLog where ExchangeTypeID='N' and ReturnMarkType='3' and DCIRETURNSTATUSID is not null "&strBatch&")"
				else
					strBatch2=""
				end if
			...
			ElseIf sys_City="基隆市" or sys_City="苗栗縣" Then 
				if request("Sys_BatchNo")<>"" then
					strwhere=strwhere&" and a.BillStatus in ('3','9') and a.SN in (select distinct(BillSN) from DciLog where ExchangeTypeID='N' and ReturnMarkType='3' and DCIRETURNSTATUSID is not null "&strBatch&")"
				else
					strwhere=strwhere&" and a.BillStatus='3' "
				end If
			elseif sys_City="台南市" Then
				if request("Sys_BatchNo")<>"" then
					strwhere=strwhere&" and a.BillStatus in ('3','9') and a.SN in (select distinct(BillSN) from DciLog where ExchangeTypeID='N' and ReturnMarkType='3' and DCIRETURNSTATUSID is not null "&strBatch&")"
				else
					strwhere=strwhere&" and a.BillStatus='3' and a.SN in (select distinct(BillSN) from DciLog where ExchangeTypeID='N' and ReturnMarkType='3' and DCIRETURNSTATUSID is not null "&strBatch&")"
				end If				
			elseif sys_City="嘉義市" then
				strwhere=strwhere&" and a.BillStatus='3' and a.SN in (select distinct(BillSN) from DciLog where ExchangeTypeID='N' and ReturnMarkType='3' and DCIRETURNSTATUSID<>'n' and DCIRETURNSTATUSID is not null "&strBatch&")"
			elseif sys_City="南投縣" then
				if request("Sys_BatchNo")<>"" Then
					'102/4/30交通隊劉小姐說結案的也要再公示
					strwhere=strwhere&" and a.BillStatus in ('3','9') and a.SN in (select distinct(BillSN) from DciLog where ExchangeTypeID='N' and ReturnMarkType='3' and DCIRETURNSTATUSID is not null "&strBatch&")"
				Else
					'3/24陳淑雲確認要修改
					strwhere=strwhere&" and a.BillStatus='3' and a.SN in (select distinct(BillSN) from DciLog where ExchangeTypeID='N' and ReturnMarkType='3' and (DciReturnStatusID='S' or DciReturnStatusID='h' or DCIRETURNSTATUSID is null) "&strBatch&")"
				End If 
		...
		elseif request("DCIstatus")="1" then
			if strwhere<>"" then
				strwhere=strwhere&" and a.BillStatus='5' and a.Sn in (select distinct(BillSN) from DciLog where BillSN not in (select Billsn from DciLog where exchangeTypeID='N' and (DciReturnStatusID='S' or DciReturnStatusID='n' or DciReturnStatusID='h' or DciReturnStatusID is null) and ReturnMarkType='5') "&strBatch&")"
			else
				strwhere=" and a.BillStatus='5' and a.Sn in (select distinct(BillSN) from DciLog where BillSN not in (select Billsn from DciLog where exchangeTypeID='N' and (DciReturnStatusID='S' or DciReturnStatusID='n' or DciReturnStatusID='h' or DciReturnStatusID is null) and ReturnMarkType='5') "&strBatch&")"
			end if
		end if
		...
		'第一次或第二次做寄存送達
		if StoreAndSendMode=2 then
			if strwhere<>"" then
				strwhere=strwhere&" and (c.MailTypeID<>2 or c.MailTypeID is null)" 
			else
				strwhere=" and (c.MailTypeID<>2 or c.MailTypeID is null)"
			end if
		end if

		if OpenAndGovMode=2 then
			if strwhere<>"" then
				strwhere=strwhere&" and c.MailTypeID=6"
			else
				strwhere=" and c.MailTypeID=6"
			end if
		end if

		CancelBillNo=""
		strchk="select a.SN,a.IllegalDate,a.CarSimpleID,a.BillMem1,a.BillMem2,a.BillMem3,b.ChName,a.BillTypeID,a.BillNo,a.CarNo,a.Driver,a.DriverID,a.Rule1,a.Rule2,a.Rule3,a.Rule4,a.BillUnitID,a.BillStatus,a.RecordStateID,a.RecordDate,a.RecordMemberID,c.UserMarkResonID,c.StoreAndSendReturnResonID,c.UserMarkDate,c.StoreAndSendMailReturnDate,c.OpenGovMailReturnDate from BillBase a,MemberData b,BillMailHistory c where a.RecordMemberID=b.MemberID and c.BillSN=a.SN and c.UserMarkResonID in ('1','2','3','4','8','M','K','L','O','P','Q')"&strwhere&" and a.SN in (select Billsn from dcilog where ExchangeTypeID='N' and ReturnMarkType='Y' and DciReturnStatusID is null) order by c.UserMarkDate"
...
%>
		<script language="JavaScript">
			alert("<%=CancelBillNo%> 撤銷送達監理站尚未處理，請等撤銷送達回傳後，再進行公示送達，避免資料發生錯誤!!");
		</script>
<%		end if
...
'公示(遇到RecordStateID=-1不做)
'if trim(request("kinds"))="toPublic" then
'	strPublic="select a.SN,a.IllegalDate,a.BillTypeID,a.BillNo,a.CarNo,a.BillUnitID,a.BillStatus,a.RecordDate,a.RecordMemberID,c.ReturnResonID,c.StoreAndSendReturnResonID,c.MailReturnDate,c.StoreAndSendMailReturnDate from BillBase a,MemberData b,BillMailHistory c where a.RecordMemberID=b.MemberID and c.BillSN=a.SN and c.UserMarkResonID in (1,2,3,4,8)"&strwhere
'	set rsPublic=conn.execute(strPublic)
'	If Not rsPublic.Bof Then
'		rsPublic.MoveFirst 
'	else
%>
<script language="JavaScript">
...
<%
'	end if
'	While Not rsPublic.Eof
'		funcPublic conn,trim(rsPublic("SN")),trim(rsPublic("BillNo")),trim(rsPublic("BillTypeID")),trim(rsPublic("CarNo")),trim(rsPublic("BillUnitID")),trim(rsPublic("RecordDate")),trim(rsPublic("RecordMemberID"))
'	rsPublic.MoveNext
'	Wend
'	If Not rsPublic.Bof Then 
%>
...
'response.end
		...
		strCnt="select count(*) as cnt from BillBase a,MemberData b,BillMailHistory c where a.RecordMemberID=b.MemberID and c.BillSN=a.SN and c.UserMarkResonID in ('1','2','3','4','8','M','K','L','O','P','Q')"&strwhere
		set Dbrs=conn.execute(strCnt)
		DBsum=Dbrs("cnt")
		Dbrs.close
		tmpSQL=strwhere
		Session.Contents.Remove("BillSQLforOpenGovUpload")
		Session("BillSQLforOpenGovUpload")=strSQL
		Session.Contents.Remove("DciOpenGovToExcelSession")
		Session("DciOpenGovToExcelSession")=strwhere
end if
			...
			<table width="100%" border="0" cellpadding="5" cellspacing="1" bgcolor="#FFFFFF">
				<tr>
					<td>
						...
						if CheckPermission(254,1)=false then
							response.write "disabled"
						end if
						%>>
						<input type="button" name="cancel" value="清除" onClick="location='BillBaseQryOpenGov.asp'"> 
					</td>
				</tr>
			</table>
		</td>
...
'					end if
					...
					set rsBTypeVal=nothing
						...
						response.write "</td>"
						response.write "<td>"
						if not isnull(rsfound("OpenGovMailReturnDate")) and rsfound("OpenGovMailReturnDate")<>"" then
							response.write gInitDT(rsfound("OpenGovMailReturnDate"))&","
						end if
	...
	<tr>
		...
		<%if sys_City="高雄市" Or sys_City=ApconfigureCityName then%>
			<input type="button" name="b1" value="進行公示送達" onclick='window.open("../BillReturn/BillBaseOpenGovDCI.asp?gov_BatchNumber=<%=trim(request("Sys_BatchNo"))%>","WebPage_Del_Bill","left=0,top=0,location=0,width=600,height=400,resizable=yes,scrollbars=yes")'>
		<%else%>
			<input type="button" name="b1" value="進行公示送達" onclick='window.open("../BillReturn/BillBaseOpenGovDCI.asp","WebPage_Del_Bill","left=0,top=0,location=0,width=600,height=400,resizable=yes,scrollbars=yes")'>
		<%end if%>
			<span class="style3"><img src="space.gif" width="5" height="9"></span>
		<%if sys_City="高雄縣" Or sys_City="南投縣" Or sys_City="花蓮縣" Or sys_City="高雄市" Or sys_City=ApconfigureCityName then%>
			<input type="button" name="btnExecel" value="列印單退清冊(公示)" onclick="funPrintList();">
		<%end if%>
...
<script language="javascript">
	...
	function funchgExecel(){
		UrlStr="BillBaseQry_Execel.asp?WorkType=4";
	...
	function toPublic(){
		if (myForm.DB_Selt.value==""){
			...
			myForm.kinds.value="toPublic";
			myForm.submit();
		}
	}

	function funPrintList(){
		UrlStr="ReturnSendList_Excel_Gov_All_Ka.asp?WorkType=4";
		newWin(UrlStr,"inputWin2",790,550,50,10,"yes","yes","yes","no");
	}
</script>
<%
conn.close
