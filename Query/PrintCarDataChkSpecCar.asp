...
if trim(request("kinds"))="Del_NoDci" then
	...
	strSQL="select distinct a.SN,a.CarSimpleID,a.IllegalDate,a.Rule1,a.Rule2,a.IllegalAddress,a.RuleSpeed,a.IllegalSpeed,a.RecordStateID,a.RecordDate,a.RecordMemberID,a.BillNo,a.RuleVer,e.CarNo,e.DCIReturnCarType,e.A_Name,e.DCIReturnCarColor,e.DriverHomeZip,e.DriverHomeAddress,e.Owner,e.OwnerAddress,e.OwnerZip,e.Nwner,e.NwnerID,e.NwnerAddress,e.NwnerZip,e.DCIReturnCarStatus,f.Note from DCILog c,MemberData b,BillBase a,DCIReturnStatus d,BillBaseDCIReturn e,SpecCar f where c.BillSN=a.SN and e.ExchangeTypeID='A' and e.Status='S' and a.CarNo=e.CarNo (+) and c.ExchangeTypeID=d.DCIActionID(+) and c.DCIReturnStatusID=d.DCIReturn(+) and c.RecordMemberID=b.MemberID(+) and a.RecordStateID=0 and a.CarNo=f.CarNo "&strwhere&" order by a.RecordDate"
