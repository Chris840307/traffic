...
Class FreeASPUpload
	...
	Public Sub Save(path)
		...
		For Each fileItem In UploadedFiles.Items
			...
			streamFile.SaveToFile path & left(fileItem.FileName,Len(fileItem.FileName)-4) & Right("00"&Hour(now),2) & Right("00"&minute(now),2) & Right("00"&second(now),2) & Right(fileItem.FileName,4), 2
			streamFile.close
			Set streamFile = Nothing
			fileItem.Path = path & fileItem.FileName
    	  response.write "<script language=""javascript"">"
    	  response.write "alert(""上傳完成"")"
	...
	Private Sub Upload()
		...
		if Err.Number <> 0 then 
			response.write "<br><br>超過上傳的限制<p>"
			Exit Sub
		end if
