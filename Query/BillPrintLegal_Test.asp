...
if trim(Sys_BillTypeID)="1" then
	...
	Sys_MailNumber=0
	DelphiASPObj.GenBillPrintBarCode PBillSN(i),Sys_BillNo,Sys_Rule1,Sys_CarNo,Sys_MailNumber,"220073",Sys_DriverHomeZip,Sys_DealLineDate(0)&Sys_DealLineDate(1)&Sys_DealLineDate(2),Sys_StationID,"台北市交通事件裁決所",0,Sum_Level,0,True,False,Sys_MailDate,400,451,36
...
<table width="710" height="160" border="1" cellspacing=0 cellpadding=0>
	<tr height="12">
		<td>&nbsp;</td>
	</tr>
	<tr>
...
<div id="Layer20" style="position:absolute; left:490px; top:<%=530+pagepx%>px; width:201px; height:17px; z-index:13"><%=funcCheckFont(Sys_Owner,22,1)%></div>
<div id="Layer21" style="position:absolute; left:115px; top:<%=550+pagepx%>px; width:507px; height:13px; z-index:14"><%=Sys_OwnerZip&" "&funcCheckFont(Sys_OwnerZipName&Sys_OwnerAddress,22,1)%></div>
...
<div id="Layer35" style="position:absolute; left:390px; top:<%=740+pagepx%>px; width:100px; height:49px; z-index:29"><%
	if billprintuseimage=1 then
		...
		response.write "<tr><td style=""border-color:#ff0000;border-style:solid;"" class=""style1"">宜蘭縣警察局<br>交通警察隊</td></tr>"
...
<div id="Layer37" style="position:absolute; left:570px; top:<%=725+pagepx%>px; width:200px; height:46px; z-index:31"><%
			response.write "<table style=""border-color:#ff0000;border-style:solid;"" border=""1"" cellspacing=0 cellpadding=0>"
			response.write "<tr><td style=""border-color:#ff0000;border-style:solid;"" width=100 height=25 align=""center""><span class=""style1"">員警&nbsp;張良相</span></td></tr>"
			response.write "</table>"%></div>
...
<div id="Layer15" class="style3" style="position:absolute; left:110px; top:<%=950+pagepx%>px; z-index:1"><%
	response.write Sys_Owner&"<br>"
	response.write InstrAdd(Sys_OwnerZip&Sys_OwnerZipName&Sys_OwnerAddress,14)
%>
</div>
<div id="Layer16" class="style3" style="position:absolute; left:110px; top:<%=1010+pagepx%>px; z-index:1"><%
	response.write Sys_BillNo
%>
</div>
<div id="Layer17" class="style3" style="position:absolute; left:450px; top:<%=1035+pagepx%>px; z-index:1"><img src=<%="""..\BarCodeImage\"&Sys_BillNo&"_4.jpg"""%>></div>
</div>
</div><%
