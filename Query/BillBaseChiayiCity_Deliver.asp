...
Sys_MailNumber=0
DelphiASPObj.GenSendStoreBillno Sys_BillNo,0,50,160
...
<table border="0">
	<tr>
		<td class="style2" rowspan=2 valign="top" align="right" height=320 width=10>
			<div id=idDiv style="position:absolute;left:-15px;">
				<img src=<%="""../BarCodeImage/"&Sys_BillNo&".jpg"""%>>
			</div>
			<br><br><br><br><br><br><br><br><br><br>
			<B><%=wordporss(chstr(thenPasserCity&replace(SysUnit,"","")&"送達證書"))%></B>
		</td>
		...
		if len(strtmp)>19 and instr(strtmp,"@")=0 then
			response.write wordporss(chstr(mid(strtmp,1,18)))
		else
			response.write wordporss(chstr(funcCheckFont(strtmp,20,4)))
		end if%>
		</td>
		<td class="style4" valign="bottom" align="right" height=150 width=15><%
			if len(strtmp)>19 and instr(strtmp,"<")=0 then response.write wordporss(chstr(mid(strtmp,19,len(strtmp))))%>
		</td>
		...
		<td class="style3" valign="bottom" align="right" height=320 width=30><%
			...
			response.write wordporss(chstr(funcCheckFont(tmpstr,20,4)))
			%>
		</td>
		...
		<td class="style3" valign="bottom" align="right" height=320 width=10><%
			if trim(Sys_Rule2)<>"0" then
				...
				response.write "<font size=2>"&wordporss(chstr(funcCheckFont(tmpstr,20,4)))&"</font>"
			end if
			%>
		</td>
		<td class="style4" valign="bottom" align="right" width="600" width=0><%=wordporss(chstr(sysunit&"  請繳回:"&SysUnitAddress))%></td>
	</tr>
	<tr>
