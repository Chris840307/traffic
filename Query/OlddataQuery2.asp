...
<%
'by Jafe
'2011/01/25 加入該函數，原因是丟給Th那邊的時候，他那邊會做 gArrDT 這個函數的轉換，但是這個函數需要西元年，例2001/01/01，但是在舊系統裡只有 950101這樣子的型態
'故增加這個函數把950101轉成 2006/01/01，讓補印的時候不至於在填單日期(BillFillDate)那邊出現異常的日期資料。
...
function QueryLawContent(value,RealSpeed,LimitSpeed) 
    Rule_c = ""
		A_DESC = ""
		Rule_D = ""
    LawSql="Select a.rule_c as a_rule,b.rule_c b_rule,a.A_DESC as aA_DESC,b.A_DESC as bA_DESC,a.Rule_D as aRule_D,b.Rule_D as bRule_D" &_
		" from rule_n a,rule_c b where a.rule_c(+)=b.rule_c and (a.rule_c="&QuotedStr(trim(value))&" or b.rule_c="&QuotedStr(trim(value))&")"

    'LawSql="Select * from rule_n where Rule_C=" & QuotedStr(trim(value))
    set LawRs=conn.execute(LawSql)

    if not LawRs.Eof Then
				if not isnull(LawRs("a_rule")) then 
					Rule_c = LawRs("a_rule")
					A_DESC = LawRs("aA_DESC")
					Rule_D = LawRs("aRule_D")
				else
					Rule_c = LawRs("b_rule")
					A_DESC= LawRs("bA_DESC")
					Rule_D = LawRs("bRule_D")
				End If
				
        if (mid(Rule_c,1,3)="293") and (A_DESC="1") then
            QueryLawContent = replace(Rule_D,"重量 噸","重量" & LimitSpeed & "噸")
        ...
        elseif (mid(Rule_c,1,4)="4010") and (A_DESC="1") then
            QueryLawContent = replace(Rule_D,"限速 公里","限速" & LimitSpeed & "公里")
...
if request("DB_Selt")="Selt" then   
  ...
  BillCloseSQL=""
	if trim(request("sBillClose")) <> "" and trim(request("sBillClose"))="1"  then 
			...
			BillCloseSQL=" and (a.cls_dt='      ' or a.cls_dt='' or a.cls_dt=' ')"
	end If
	...
	<tr>
		<td bgcolor="#CCCCCC">
            ...
            <br /><br />
						...
						<input type="button" name="btnUnit" value="選擇單位" onclick="opened=openWindow('Olddata_Unit2.asp','myWin','scrollbars=yes');">
						<img src="space.gif" width="20" height="2">
				...
				<tr>
					<td>
						...
						<img src="space.gif" width="20" height="2">
						<input type="button" name="cancel" value="清除" onClick="location='OlddataQuery2.asp'"> 
					</td>
				</tr>
			</table>
		...
		<td bgcolor="#E0E0E0">
            ...
            response.write "<td width='6%'><font size='2'>" & trim(rsfound("vil_dt")) & "</font></td>"						
						...
						if trim(request("billclosechk"))="on" then
							...
							response.write "<td width='6%'><font size='2'>"& trim(SetDate(QueryCol("out_dt","peo_rec","tkt_no=" & QuotedStr(trim(rsfound("tkt_no")))))) &"</font>&nbsp;</td>"
            ...
            %>	
			    ...
			    <input type="button" name="b1" value="詳細" onclick='window.open("OlddataDetail2.asp?BillNo=<%=trim(rsfound("tkt_no"))%>","OldBaseDetail","left=0,top=0,location=0,width=980,height=620,resizable=yes,scrollbars=yes,menubar=yes")' style="font-size: 10pt; width: 40px; height:26px;">
			   <% UserID=trim(Session("User_ID")) %>
			    <input type="button" name="b2" value="備註"  onclick='window.open("olddataNote2.asp?<%="UserID="&UserID & "&BillNO=" & trim(rsfound("tkt_no"))%>","OldBaseDetail","left=0,top=0,location=0,width=500,height=275,resizable=yes,scrollbars=yes,menubar=yes")' style="font-size: 10pt; width: 40px; height:26px;"> 
	            ...
	            <input type="button" name="b6" value="催告書"  onclick='window.open("../PasserBase/PasserUrge_Word_old.asp?<%=PrtUrge%>","OldBaseDetail","left=0,top=0,location=0,width=500,height=275,resizable=yes,scrollbars=yes,menubar=yes")' style="font-size: 10pt; width: 42px; height:26px;" >  
					...
					<input type="button" name="b9" value="債權"  onclick='window.open("PasserCreditor_Old2.asp?BillNo=<%=trim(rsfound("tkt_no"))%>","OldBaseDetail","left=0,top=0,location=0,width=800,height=575,resizable=yes,scrollbars=yes,menubar=yes")' style="font-size: 10pt; width: 42px; height:26px;">
			    <%
...
function CloseSend(Billno)
{
    ...
    window.open("OlddataCloseCase2.asp?BillNo=" +Billno);
    myForm.submit();   
}
  ...
  function funchgExecel()
    {
		UrlStr="Olddata_BillList2.asp?WorkType=1";
		newWin(UrlStr,"inputWin",790,550,50,10,"yes","yes","yes","no");
	}
	
	function funchgExecel1()
    {
		UrlStr="Olddata_detailList2.asp?WorkType=1";
		newWin(UrlStr,"inputWin",790,550,50,10,"yes","yes","yes","no");
	}
