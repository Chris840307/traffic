...
'AuthorityCheck(234)
	...
	strStReport="select distinct(DCIReturnStation) from (select f.SN,a.BillNo,a.CarNo from DCILog a,MemberData b," &_
		...
		" ,BillBaseDCIReturn e,BillMailHistory g where a.SN=g.BillSn and a.BillNo=e.BillNO and a.CarNo=e.CarNo and e.ExchangeTypeID='W' and e.Status in ('Y','S','n','L') and g.UserMarkResonID in ('5','6','7','T')"
	...
	strStStop="select distinct(f.MemberStation) from DCILog a,MemberData b" &_
		...
		" and a.DCIReturnStatusID=d.DCIReturn(+) and a.RecordMemberID=b.MemberID(+) and a.ReturnMarkType='3' and g.UserMarkResonID in ('5','6','7','T')"&strwhere
...
<%if (sys_City<>"台南市" and sys_City<>"南投縣") or (sys_City="南投縣" and trim(Session("Unit_ID"))="05CB") then %>
...
%>
		...
		strCntReport="select count(*) as cnt from (select f.SN,a.BillNo,a.CarNo from DCILog a,MemberData b," &_
			...
			" and e.DCIReturnStation in ('20','21','22','23','24','29') and g.UserMarkResonID in ('5','6','7','T')"
		...
		" and a.BillTypeID<>'2'" &_
		" and a.ExchangeTypeID=d.DCIActionID(+) and a.DCIReturnStatusID=d.DCIReturn(+)" &_
		" and a.RecordMemberID=b.MemberID(+) and a.ReturnMarkType='3' and g.UserMarkResonID in ('5','6','7','T') "&strwhere
...
%>
		...
		strCntReport="select count(*) as cnt from (select f.SN,a.BillNo,a.CarNo from DCILog a,MemberData b," &_
			...
			" and a.RecordMemberID=b.MemberID(+) and a.ReturnMarkType='3' "&strwhere&") a,BillBaseDCIReturn e,BillMailHistory g" &_
			" where a.BillNo=e.BillNO and a.SN=g.BillSn and a.CarNo=e.CarNo and e.ExchangeTypeID='W' and e.Status in ('Y','S','n','L')" &_
			" and e.DCIReturnStation in ('30','31','32') and g.UserMarkResonID in ('5','6','7','T')"
...
%>
		...
		strCntReport="select count(*) as cnt from (select f.SN,a.BillNo,a.CarNo from DCILog a,MemberData b," &_
			...
			" and a.RecordMemberID=b.MemberID(+) and a.ReturnMarkType='3' "&strwhere&") a,BillBaseDCIReturn e,BillMailHistory g" &_
			" where a.BillNo=e.BillNO and a.SN=g.BillSn and a.CarNo=e.CarNo and e.ExchangeTypeID='W' and e.Status in ('Y','S','n','L')" &_
			" and e.DCIReturnStation='"&trim(StationArray(SA))&"' and g.UserMarkResonID in ('5','6','7','T')"
...
%>
<%		'逕舉and攔停
		strSQL="select a.BillSN,a.BillNO,a.CarNO,a.BillTypeID,a.BatchNumber,e.Driver,e.Owner,e.OwnerID,f.CarSimpleID,f.IllegalDate" &_
		",f.Rule1,f.Rule2,f.Rule3,f.Rule4,f.BillUnitID,f.BillMem1,f.BillMem2" &_
		...
		" or (a.BillTypeID<>'2' and f.MemberStation in ('20','21','22','23','24','29') and e.ExchangeTypeID='W' and e.Status in ('Y','S','n','L'))) and g.UserMarkResonID in ('5','6','7','T')" &_
...
%>		
	...
	<td align="center" height="28" colspan="2"><span class="style4"><%
		...
		response.write TitleValue&"&nbsp退件(寄存)資料"
...
%>
<%		'逕舉and攔停
		strSQL="select a.BillSN,a.BillNO,a.CarNO,a.BillTypeID,a.BatchNumber,e.Driver,e.Owner,f.CarSimpleID,f.IllegalDate" &_
		",f.Rule1,f.Rule2,f.Rule3,f.Rule4,f.BillUnitID,f.BillMem1,f.BillMem2" &_
		" from (select a.BillSN,a.BillNo,a.CarNo,a.BillTypeID,a.BatchNumber,a.ExchangeTypeID,a.DciReturnStatusID from DciLog a where a.BillSN is not null "&strwhere&") a" &_
		" ,BillBaseDCIReturn e,BillBase f,BillMailHistory g" &_
		" where a.BillSN=f.SN and f.RecordStateID=0" &_
		...
		" or (a.BillTypeID<>'2' and f.MemberStation in ('30','31','32') and e.ExchangeTypeID='W' and e.Status in ('Y','S','n','L'))) and g.UserMarkResonID in ('5','6','7','T')" &_
		" order by g.UserMarkDate"
...
<%		'逕舉and攔停
		...
		" or (a.BillTypeID<>'2' and f.MemberStation='"&trim(StationArray(SA2))&"' and e.ExchangeTypeID='W' and e.Status in ('Y','S','n','L'))) and g.UserMarkResonID in ('5','6','7','T')" &_
		" order by g.UserMarkDate"
