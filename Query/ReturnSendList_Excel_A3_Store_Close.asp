...
<%
	...
	if sys_City="花蓮縣" then
		CloseDciReturnStatusID="DciReturnStatusID not in ('S','N','k')"
	else
	...
	strStReport="select distinct(DCIReturnStation) from (select f.SN,a.BillNo,a.CarNo from DCILog a,MemberData b," &_
		...
		" ,BillBaseDCIReturn e,BillMailHistory g where a.BillNo=e.BillNO and a.SN=g.BillSn and a.CarNo=e.CarNo and e.ExchangeTypeID='W' and e.Status in ('Y','S','n','L') and g.UserMarkResonID in ('5','6','7','T') order by DCIReturnStation"
	...
	strStStop="select distinct(f.MemberStation) from DCILog a,MemberData b" &_
		...
		" and a.DCIReturnStatusID=d.DCIReturn(+) and a.RecordMemberID=b.MemberID(+) and a.ReturnMarkType='3' and g.UserMarkResonID in ('5','6','7','T') "&strwhere&" order by MemberStation"
...
%>		
	...
	<td align="center" height="28" colspan="2"><span class="style4"><%
		...
		response.write TitleValue&"&nbsp(寄存已結案)資料"
