...
<div id="Layer1" class="style2" style="position:absolute; left:220px; top:0px; width:500px; height:36px; z-index:5"><b>違反道路交通管理事件通知書</b></div>
<div id="Layer42" style="position:absolute; left:40px; top:<%=5+pagepx%>px;">
<table width="710" height="160" border="0" cellspacing=0 cellpadding=0>
	<tr>
		<td nowrap><%if int(Sys_Rule1)<>4340003 and int(Sys_Rule1)<>5620001 then Response.Write "『內有採證相片』　"%></td>
		<td>&nbsp;</td>
		<td>&nbsp;</td>
	...
	<tr height="20">
		<td>&nbsp;</td>
		<td class="style15"><%=SysUnitAddress&"<br>"&sys_title&SysUnit&"<br>"&SysUnitTel%></td>
		<td>&nbsp;</td>
	</tr>
	<tr>
		<td width="80" height="2">&nbsp;</td>
		<td width="460" valign="TOP" class="style2"><br><%
			if trim(Sys_BillTypeID)="1" then
		...
		<td class="style2" valign="bottom">
			...
			<%=Sys_DealLineDate(0)&Sys_DealLineDate(1)&Sys_DealLineDate(2)%></td>
	</tr>
</table>
</div>
<%if showBarCode then%>
...
<%end if%>
<div id="Layer9" style="position:absolute; left:10px; top:<%=435+pagepx%>px; width:202px; height:36px; z-index:5"><%
	if showBarCode then
...
%></div>
<div id="Layer10" style="position:absolute; left:505px; top:<%=425+pagepx%>px; width:233px; height:32px; z-index:6"><img src=<%="""../BarCodeImage/"&Sys_BillNo&"_4.jpg"""%>></div>

<div id="Layer42" class="style3" style="position:absolute; left:515px; top:<%=465+pagepx%>px; width:233px; height:12px; z-index:36">宜警交</div>

<div id="Layer12" style="position:absolute; left:95px; top:<%=490+pagepx%>px; width:300px; height:11px; z-index:3"><span class="style7">逕行舉發　<%=Sys_A_Name%><br><%if int(Sys_Rule1)<>4340003 and int(Sys_Rule1)<>5620001 then response.write "附採證照片"%>　<%=Sys_CarColor%></span></div>
...
<div id="Layer15" class="style3" style="position:absolute; left:250px; top:<%=500+pagepx%>px; width:100px; height:10px; z-index:8"><%if trim(Sys_DriverBirth(0))<>"" then response.write Sys_DriverBirth(0)&"年"&right("0"&Sys_DriverBirth(1),2)&"月"&right("0"&Sys_DriverBirth(2),2)&"日"%></div>
...
<div id="Layer21" style="position:absolute; left:115px; top:<%=555+pagepx%>px; width:507px; height:13px; z-index:14"><%=Sys_OwnerZip&" "&funcCheckFont(Sys_OwnerZipName&Sys_OwnerAddress,16,1)%></div>

<div id="Layer22" style="position:absolute; left:100px; top:<%=575+pagepx%>px; width:40px; height:13px; z-index:15"><%=Sys_IllegalDate(0)%></div>
<div id="Layer23" style="position:absolute; left:150px; top:<%=575+pagepx%>px; width:40px; height:17px; z-index:16"><%=Sys_IllegalDate(1)%></div>
<div id="Layer24" style="position:absolute; left:200px; top:<%=575+pagepx%>px; width:40px; height:16px; z-index:17"><%=Sys_IllegalDate(2)%></div>
<div id="Layer25" style="position:absolute; left:240px; top:<%=575+pagepx%>px; width:40px; height:16px; z-index:18"><%=right("00"&Sys_IllegalDate_h,2)%></div>
<div id="Layer26" style="position:absolute; left:290px; top:<%=575+pagepx%>px; width:40px; height:13px; z-index:19"><%=right("00"&Sys_IllegalDate_m,2)%></div>
...
<div id="Layer29" style="position:absolute; left:120px; top:<%=630+pagepx%>px; width:34px; height:11px; z-index:22"><%=Sys_DealLineDate(0)%></div>
<div id="Layer30" style="position:absolute; left:200px; top:<%=630+pagepx%>px; width:35px; height:13px; z-index:23"><%=Sys_DealLineDate(1)%></div>
<div id="Layer31" style="position:absolute; left:270px; top:<%=630+pagepx%>px; width:32px; height:15px; z-index:24"><%=Sys_DealLineDate(2)%></div>
<div id="Layer32" class="style3" style="position:absolute; left:380px; top:<%=650+pagepx%>px; width:400px; height:49px; z-index:29"><%
...
<div id="Layer34" style="position:absolute; left:360px; top:<%=695+pagepx%>px; width:400px; height:30px; z-index:28"><%
	if showBarCode then	response.write "<img src=""../BarCodeImage/"&Sys_BillNo&"_5.jpg"">"
%></div>
<div id="Layer33" class="style3" style="position:absolute; left:610px; top:<%=695+pagepx%>px; width:100px; height:40px; z-index:28"><span class="style7"><%=Sys_STATIONNAME&"<br>"&Sys_StationTel%></span></div>
<div id="Layer35" style="position:absolute; left:390px; top:<%=750+pagepx%>px; width:100px; height:49px; z-index:29"><%
...
<div id="Layer36" style="position:absolute; left:570px; top:<%=775+pagepx%>px; width:100px; height:43px; z-index:30"><%
	if instr(Sys_BillNo,"QZ")>0 then
			response.write "<table style=""border-color:#ff0000;border-style:solid;"" border=""1"" cellspacing=0 cellpadding=0>"
			response.write "<tr><td style=""border-color:#ff0000;border-style:solid;"" width=100 height=25 align=""center""><span class=""style1"">交通違規專用章<br>分隊長&nbsp;廖文良</span></td></tr>"
			response.write "</table>"
	...
	'elseif trim(Session("Unit_ID"))="TQ00" then
		'蘇澳分局不要出現主管章
	elseif trim(Session("Unit_ID"))="TN00" then
...
<div id="Layer37" style="position:absolute; left:570px; top:<%=735+pagepx%>px; width:200px; height:46px; z-index:31"><%
		if instr(Sys_BillNo,"QZ")>0 then
			response.write "<table style=""border-color:#ff0000;border-style:solid;"" border=""1"" cellspacing=0 cellpadding=0>"
...
<div id="Layer38" style="position:absolute; left:200px; top:<%=815+pagepx%>px; width:60px; height:10px; z-index:32"><%=sys_Date(0)%></div>
<div id="Layer39" style="position:absolute; left:355px; top:<%=815+pagepx%>px; width:60px; height:13px; z-index:33"><%=sys_Date(1)%></div>
<div id="Layer40" style="position:absolute; left:505px; top:<%=815+pagepx%>px; width:60px; height:11px; z-index:34"><%=sys_Date(2)%></div>
<div id="Layer41" class="style11" style="position:absolute; left:30px; top:<%=915+pagepx%>px; width:200px; height:12px; z-index:36"><%=sys_title&"<br>"&SysUnit%></div>
<div id="Layer43" class="style11" style="position:absolute; left:320px; top:<%=925+pagepx%>px; width:200px; height:12px; z-index:36"><%=sys_title&SysUnit%></div>
<div id="Layer44" class="style11" style="position:absolute; left:540px; top:<%=915+pagepx%>px; width:200px; height:12px; z-index:36"><%=InstrAdd(SysUnitAddress,12)%></div>
<div id="Layer45" class="style3" style="position:absolute; left:110px; top:<%=955+pagepx%>px; width:200px; z-index:1"><%
...
<div id="Layer46" class="style3" style="position:absolute; left:110px; top:<%=1020+pagepx%>px; z-index:1"><%
...
<div id="Layer47" class="style3" style="position:absolute; left:450px; top:<%=1040+pagepx%>px; z-index:1"><img src=<%="""../BarCodeImage/"&Sys_BillNo&"_4.jpg"""%>></div>
</div>
</div><%
