...
<%	
	...
	If sys_City="台中市" Then
		if trim(request("ReportNo"))<>"" Then
			...
			Else
				strQry=strQry&",ReportNo="&Trim(request("ReportNo"))
			End if
			strSQLTemp=strSQLTemp&" and a.sn=(select Billsn from BillReportNo where ReportNo='"&Trim(request("ReportNo"))&"')"
		end If
	End If 
	...
	strSQL="Select a.BillNo,a.Sn,a.CarNo,a.BillTypeID,a.Rule1,a.Rule2,a.Rule3,a.Rule4,a.MemberStation,a.EquipMentID" &_
		...
		",a.DealLineDate,a.Note,a.CarSimpleID,a.CarAddID,a.ImageFileName from BillBase a,BillBaseDciReturn b" &_
		...
		" and b.Status in ('Y','S','n','L') "&strSQLTemp
	...
	strSQL="Select a.BillNo,a.Sn,a.CarNo,a.BillTypeID,a.Rule1,a.Rule2,a.Rule3,a.Rule4,a.MemberStation,a.EquipMentID" &_
		...
		",a.BillMemID4,a.BillMem4,a.RuleVer,a.IllegalAddressID,a.IllegalAddress,a.BillFillDate,a.BillUnitID,a.SignType" &_
		",a.driveraddress,a.driverzip,a.owner,a.ownerzip,a.owneraddress" &_
		",a.DealLineDate,a.Note,a.CarSimpleID,a.CarAddID,a.ImageFileName from BillBase a " &_
		" where ((a.RecordStateID<>-1 and a.BillStatus='0')" &_
...
<%End If %>
		...
		<tr>
			...
			<%if (sys_City="高雄市" or sys_City=ApconfigureCityName) or sys_City="高雄縣" then%>	
				<tr>
...
<%
	...
	<%if sys_City="台中市" then%>
		<tr>
			<td colspan="4">告示單號：</span><span class="style1"><%
	...
	<hr>
<%if (sys_City<>"高雄市" and sys_City<>ApconfigureCityName) or trim(request("IsShow"))="1" then%>
	<table width='100%' border='0' cellpadding="2">
		<tr>
...
<%else
		...
		%>
			...
			rsSMem.close
			set rsSMem=nothing
			%></strong><br><img src='<%=replace(trim(rsScan("FileName")),"/img/","/scanimg/")%>' name='imgB1' width='800' onclick="OpenPic('<%=replace(trim(rsScan("FileName")),"/img/","/scanimg/")%>')">
		<%
		rsScan.movenext
		wend
		rsScan.close
...
<%	If sys_City="花蓮縣" Then
	...
	If sys_City="台南市" or sys_City="花蓮縣" Then
		...
		%>
			...
			rsSMem.close
			set rsSMem=nothing
			%></strong><br><img src='<%=replace(trim(rsScan("FileName")),"/img/","/img/")%>' name='imgB1' width='800' onclick="OpenPic('<%=replace(trim(rsScan("FileName")),"/img/","/img/")%>')">
		<%
			rsScan.movenext
		wend
		rsScan.close
...
<Div id="Layer112" style="width:1041px; height:24px;">
 ...
 <%	If (sys_City="高雄縣" And (trim(rs1("IllegalAddress"))="鳳山市鳳頂路與田中央路口" Or trim(rs1("IllegalAddress"))="大寮鄉鳳屏路高屏大橋下橋處(往高雄)")) Or (sys_City<>"高雄縣" And sys_City<>"台東縣") or (sys_City="台東縣" And Trim(rs1("Rule1"))<>"5620001") Or sys_City="南投縣" Then
 '違規影像資料
...
<%
 ...
 <%
		...
		If sys_City="屏東縣" Then
		    dim fp2
			fp2="F:\\image\\finish\scanimage\\" & Trim(rs1("BillNo")) & " 001.jpg"
...
'NoCase=1 
if NoCase=1 and strSQLTemp2<>"" and (sys_City="高雄市" or sys_City=ApconfigureCityName) then 
	strSQL2="Select * from BillBase " &_
		" where BillStatus='9' and RecordMemberID in (select Memberid from Memberdata where UnitID in " &_
...
<table width='100%' border='0' cellpadding="2">
	...
	<table width='100%' border='0' cellpadding="2">
		<tr>
			...
			if trim(rs2("RecordStateID"))="-1" and (sys_City="高雄市" or sys_City=ApconfigureCityName) then
...
<%end if%>
		<tr>
			...
			response.write trim(rs2("IllegalAddressID"))&" "&trim(rs2("IllegalAddress"))
			%></span></td>
			<td><span class="style2">是否郵寄：</span><span class="style1"><%
...
<%if NoCase=0 then%>
  <%if (sys_City="高雄市" or sys_City=ApconfigureCityName) and trim(request("IsShow"))<>"1" then%>
  <input type="button" value="顯示郵寄歷程" onclick="showMailHistory();">
 <%elseif (sys_City="高雄市" or sys_City=ApconfigureCityName) and trim(request("IsShow"))="1" then%>
