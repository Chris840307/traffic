...
If not ifnull(Sys_OwnerAddress) Then
	Sys_OwnerAddress=replace(replace(Sys_OwnerAddress,"臺","台"),"台灣省","")

	Sys_OwnerAddress=replace(replace(Sys_OwnerAddress,"福建省",""),Sys_OwnerZipName,"")
end if
...
strSQL="select a.UnitID,a.UnitName,a.UnitTypeID,a.UnitLevelID,b.chName,c.Content from Unitinfo a,Memberdata b,(select ID,Content from Code where TypeID=4 ) c where a.UnitID=b.UnitID and b.jobid=c.id(+) and b.MemberID="&Sys_BillMemID2
...
if trim(Sys_BillTypeID)="1" then
	...
	DelphiASPObj.CreateBarCode Sys_MailNumber&"60105017",128,60,150
...
Sys_MAILCHKNUMBER=Sys_MailNumber&" 601050 17"
rs.close
rsbil.close
...
<div id="Layer44" class="style2" style="position:absolute; left:50px; top:0px; height:12px; z-index:36"><%="嘉義縣警察局"&SysUnit&"送達證書"%></div>

<div id="Layer42" class="style2" style="position:absolute; left:230px; top:0px; width:500px; height:12px; z-index:36"><%="請繳回："&SysAddress&"　電話:"&SysUnitTel%></div>

<div id="Layer41" class="style3" style="position:absolute; left:570px; top:0px; width:200px; height:12px; z-index:36"><%=Sys_BatchNumber&"("&CNum&")_"&Sys_MailNumber%></div>

<div id="Layer01" class="style3" style="position:absolute; left:130px; top:20px; z-index:3"><%
...
<div id="Layer03" class="style3" style="position:absolute; left:85px; top:50px; z-index:1"><%
...
<div id="Layer04" class="style3" style="position:absolute; left:130px; top:285px; z-index:1"><b><%
	if trim(Sys_BillTypeID)="1" then
...
<div id="Layer06" class="style3" style="position:absolute; left:110px; top:305px; width:370px; z-index:5"><b><%
	'--------------------------------------如果是抓 戶籍補正的資料-----------------------------------------------------------------------------------------------------------
			if trim(Sys_BillTypeID)="1" then
...
<div id="Layer45" class="style3" style="position:absolute; left:125px; top:325px; height:12px; z-index:1"><b><%="嘉義縣警察局"&SysUnit%></b></div>

<div id="Layer55" class="style3" style="position:absolute; left:125px; top:345px; height:12px; z-index:1"><b><%="(地址:"&SysAddress&") 電話:"&SysUnitTel%></b></div>

<div id="Layer05" class="style3" style="position:absolute; left:300px; top:287px; z-index:4">
	　<b>第<%=Sys_MailNumber&" 601050 17"%>號</b>
</div>
<div id="Layer05" class="style3" style="position:absolute; left:450px; top:280px; z-index:3">
	　<img src=<%="""../BarCodeImage/"&Sys_MailNumber&"60105017"&".jpg"""%>><br>
</div>
<!--
<div id="Layer07" class="style3" style="position:absolute; left:60px; top:370px; z-index:5"><%=Sys_BatchNumber&"　"&SysUnit&"　"&"("&cdbl(i+1)&"/"&cdbl(Ubound(PBillSN)+1)&")"%>
</div>
...
if trim(Sys_ImageFileNameB)<>"" and (Sys_Rule1 <>"6020303" and Sys_Rule2<>"6020303") then
	setWidth="300":setHeight="238"
end if
if trim(Sys_ImageFileNameA)<>"" then
%>
	<div id="Layer09" style="position:absolute; left:40px; top:460px; z-index:5"><%
		response.write "<img src="""&Sys_IisImagePath&Sys_ImageFileNameA&""" width="""&setWidth&""" height="""&setHeight&""">"
	%></DIV>
<%
elseIf trim(Sys_Rule1)="5620001" and trim(Sys_ImageFileName)<>"" Then%>
	<div id="Layer09" style="position:absolute; left:40px; top:460px; z-index:5"><%
		response.write "<img src=""../StopCarPicture/"&Sys_ImageFileName&""" width="""&setWidth&""" height="""&setHeight&""">"
	%></DIV>
<%End if
...
%>
<%if trim(Sys_ImageFileNameB)<>"" and (Sys_Rule1 <>"6020303" and Sys_Rule2<>"6020303") then%>
	<div id="Layer10" style="position:absolute; left:340px; top:460px; z-index:1"><%
		response.write "<img src="""&Sys_IisImagePath&Sys_ImageFileNameB&""" width=""390"" height=""280"">"
	%></DIV>
<%end if%>

<%if showBarCode then%>
<div id="Layer1" style="position:absolute; left:50px; top:795px; width:202px; height:36px; z-index:5">Ｖ</div>
<%else%>
<div id="Layer2" style="position:absolute; left:50px; top:830px; width:202px; height:36px; z-index:5">Ｖ</div>
<%end if%>
<%if trim(Sys_BillTypeID)="1" then%>
<div id="Layer3" style="position:absolute; left:175px; top:790px; width:202px; height:36px; z-index:5">v</div>
<%else%>
<div id="Layer4" style="position:absolute; left:175px; top:810px; width:202px; height:36px; z-index:5">v</div>
<%end if%>

<div id="Layer9" style="position:absolute; left:40px; top:850px; width:202px; height:36px; z-index:5"><%
	if showBarCode then
...
<div id="Layer12" style="position:absolute; left:110px; top:910px; width:250px; height:11px; z-index:6"><span class="style7">逕行舉發　<%=Sys_A_Name%><br><%if int(Sys_Rule1)<>5620001 then response.write "附採證照片"%>　<%=Sys_CarColor%></span></div>

<div id="Layer14" style="position:absolute; left:380px; top:900px; width:324px; height:10px; z-index:4"><%if showBarCode then response.write "*本單可至郵局或委託代收之超商繳納"%></div>

<div id="Layer17" style="position:absolute; left:630px; top:925px; width:100px; height:12px; z-index:10"><%=fastring%></div>
<div id="Layer18" style="position:absolute; left:125px; top:960px; width:100px; height:14px; z-index:11"><%=Sys_CarNo%></div>
<div id="Layer19" style="position:absolute; left:320px; top:960px; width:250px; height:20px; z-index:12"><%=Sys_DCIRETURNCARTYPE%></div>
<div id="Layer20" style="position:absolute; left:545px; top:960px; width:300px; height:17px; z-index:13"><%=funcCheckFont(Sys_Owner,16,1)%></div>
<div id="Layer21" style="position:absolute; left:180px; top:990px; width:610px; height:13px; z-index:14"><%=Sys_OwnerZip&" "&funcCheckFont(Sys_OwnerZipName&Sys_OwnerAddress,16,1)&chkaddress%></div>

<div id="Layer22" style="position:absolute; left:145px; top:1010px; width:40px; height:13px; z-index:15"><%=Sys_IllegalDate(0)%></div>
<div id="Layer23" style="position:absolute; left:195px; top:1010px; width:40px; height:17px; z-index:16"><%=Sys_IllegalDate(1)%></div>
<div id="Layer24" style="position:absolute; left:245px; top:1010px; width:40px; height:16px; z-index:17"><%=Sys_IllegalDate(2)%></div>
<div id="Layer25" style="position:absolute; left:295px; top:1010px; width:40px; height:16px; z-index:18"><%=right("00"&Sys_IllegalDate_h,2)%></div>
<div id="Layer26" style="position:absolute; left:345px; top:1010px; width:40px; height:13px; z-index:19"><%=right("00"&Sys_IllegalDate_m,2)%></div>
<div id="Layer27" style="position:absolute; left:435px; top:1015px; width:320px; z-index:20"><span class="style3"><%
...
%></span></div>
<div id="Layer28" style="position:absolute; left:115px; top:1035px; width:260px; height:15px; z-index:21"><span class="style3"><%=Sys_ILLEGALADDRESS%></span></div>
<div id="Layer29" style="position:absolute; left:150px; top:1055px; width:50px; height:11px; z-index:22"><%=Sys_DealLineDate(0)%></div>
<div id="Layer30" style="position:absolute; left:230px; top:1055px; width:35px; height:13px; z-index:23"><%=Sys_DealLineDate(1)%></div>
<div id="Layer31" style="position:absolute; left:330px; top:1055px; width:32px; height:15px; z-index:24"><%=Sys_DealLineDate(2)%></div>
<div id="Layer32" class="style3" style="position:absolute; left:520px; top:1100px; width:400px; height:49px; z-index:29"><%
...
<div id="Layer34" style="position:absolute; left:430px; top:1140px; width:400px; height:30px; z-index:28"><%
	if showBarCode then	response.write "<img src=""../BarCodeImage/"&Sys_BillNo&"_5.jpg"">"
%></div>
<div id="Layer33" class="style2" style="position:absolute; left:675px; top:1140px; width:55px; height:40px; z-index:28"><%=Sys_STATIONNAME&"<br>"&Sys_StationTel%></div>
...
%></div>
<div id="Layer35" class="style10" style="position:absolute; left:450px; top:1190px; width:130px; height:49px; z-index:29"><%="嘉義縣警察局<br>交通隊<br>05-3620229"%></div>

<div id="Layer37" style="position:absolute; left:630px; top:1200px; width:200px; height:46px; z-index:31"><%
	response.write "<table style=""border-color:#ff0000;border-style:solid;"" border=""1"" cellspacing=0 cellpadding=0>"
	response.write "<tr><td style=""border-color:#ff0000;border-style:solid;"" height=25 align=""center""><span class=""style13"">"&Sys_BillJobName&"&nbsp;</span><span class=""style13"">"&Sys_ChName&"</span></td></tr>"
	response.write "</table>　　"&Sys_BillFillerMemberID
	%></div>

<div id="Layer38" style="position:absolute; left:160px; top:1245px; width:60px; height:10px; z-index:32"><%=sys_Date(0)%></div>
<div id="Layer39" style="position:absolute; left:220px; top:1245px; width:60px; height:13px; z-index:33"><%=sys_Date(1)%></div>
<div id="Layer40" style="position:absolute; left:280px; top:1245px; width:60px; height:11px; z-index:34"><%=sys_Date(2)%></div>
