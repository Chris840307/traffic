...
Sys_OwnerZip="":Orz_DriverHomeZip=""
...
rsfound.close
Sys_Sex=""
strSql="select distinct BillFillerMemberID,BillMemID2,DriverSex,DriverBirth,IllegalDate,DealLineDate,IMAGEFILENAME,RECORDMEMBERID,imagefilename from BillBase where SN="&trim(rsbil("BillSN"))
...
strSQL="update billmailhistory set mailchknumber='"&Sys_MailNumber&" 900018 17' where BILLSN="&replace(trim(rsbil("BillSN")),"","0")
conn.execute(strSQL)

if trim(Sys_BillTypeID)="1" then

	DelphiASPObj.GenBillPrintBarCode PBillSN(i),Sys_BillNo,Sys_Rule1,Sys_CarNo,Sys_MailNumber&"90001817","220073",Sys_DriverHomeZip,right(Sys_DealLineDate(0),2)&Sys_DealLineDate(1)&Sys_DealLineDate(2),Sys_StationID,"台北市交通事件裁決所",0,Sum_Level,0,True,False,Sys_MailDate
	...
	DelphiASPObj.GenBillPrintBarCode PBillSN(i),Sys_BillNo,Sys_Rule1,Sys_CarNo,Sys_MailNumber&"95000017","220073",Sys_OwnerZip,right(Sys_DealLineDate(0),2)&Sys_DealLineDate(1)&Sys_DealLineDate(2),Sys_StationID,"台北市交通事件裁決所",0,Sum_Level,0,True,False,Sys_MailDate
end if
...
<div id="Layer01" class="style3" style="position:absolute; left:125px; top:21px; z-index:3"><%
...
<div id="Layer03" class="style3" style="position:absolute; left:165px; top:50px; z-index:1"><%
	Response.Write "<img src=""../BarCodeImage/"&Sys_BillNo&"_4.jpg"">"%>
</div>

<div id="Layer05" class="style3" style="position:absolute; left:560px; top:287px; z-index:1">
	　　　<b>第<%=Sys_MailNumber%>號</b><br>
	<img src=<%="""../BarCodeImage/"&Sys_BillNo&"_2.jpg"""%>><br>

</div>

<div id="Layer03" class="style3" style="position:absolute; left:405px; top:270px; z-index:1"><%
	Response.Write "<img src=""../image/cutter.jpg""></img>"%>
</div>

<!---------------------------------- 第一段送達證書到這邊------------------------------------->

<div id="Layer04" class="style3" style="position:absolute; left:130px; top:285px; z-index:1"><%
	if trim(Sys_BillTypeID)="1" then
...
<div id="Layer05" class="style3" style="position:absolute; left:350px; top:330px; z-index:1"><%
	Response.Write "<img  src=""..\BarCodeImage\"&Sys_BillNo&"_1.jpg"">"%>
</div>

<div id="Layer06" class="style3" style="position:absolute; left:110px; top:309px; width:330px; z-index:1"><%
	'--------------------------------------如果是抓 戶籍補正的資料-----------------------------------------------------------------------------------------------------------
			if trim(Sys_BillTypeID)="1" then
...
<div id="Layer07" class="style3" style="position:absolute; left:110px; top:360px; z-index:1"><%
	'Response.Write Sys_CarNo&"　"&Sys_A_Name&"　"&Sys_CarColor&"　"&Sys_STATIONNAME
	Response.Write Sys_CarNo&"　"&Sys_STATIONNAME
%>
</div>

<div id="Layer08" class="style3" style="position:absolute; left:450px; top:360px; z-index:1"><%
...
<%if trim(Sys_ImageFileNameA)<>"" then%>
	...
	%></DIV>
<%
elseIf trim(Sys_Rule1)="5620001" and trim(Billbaseimagefilename)<>"" Then%>
	<div id="Layer09" style="position:absolute; left:38px; top:480px; z-index:5"><%
...
<%end if%>
<%if trim(Sys_BillTypeID)="1" then%>
<div id="Layer3" style="position:absolute; left:175px; top:815px; width:202px; height:36px; z-index:5">v</div>
<%else%>
<div id="Layer4" style="position:absolute; left:175px; top:830px; width:202px; height:36px; z-index:5">v</div>
<%end if%>

<div id="Layer9" style="position:absolute; left:10px; top:865px; width:202px; height:36px; z-index:5"><%
	if showBarCode then
...
%></div>
<div id="Layer10" style="position:absolute; left:490px; top:860px; width:233px; height:32px; z-index:6"><img src=<%="""../BarCodeImage/"&Sys_BillNo&"_4.jpg"""%>></div>

<div id="Layer12" style="position:absolute; left:120px; top:920px; width:150px; height:11px; z-index:3"><span class="style7">逕行舉發　<%=Sys_A_Name%><br><%if int(Sys_Rule1)<>5620001 then response.write "附採證照片"%>　<%=Sys_CarColor%></span></div>
...
<div id="Layer19" style="position:absolute; left:260px; top:965px; width:117px; height:20px; z-index:12"><%=Sys_DCIRETURNCARTYPE%></div>
<div id="Layer20" style="position:absolute; left:500px; top:965px; width:300px; height:17px; z-index:13"><%=funcCheckFont(Sys_Owner,16,1)%></div>
<div id="Layer21" style="position:absolute; left:125px; top:990px; width:507px; height:13px; z-index:14"><%=Orz_DriverHomeZip&" "&funcCheckFont(Orz_DriverHomeZipName&Orz_DriverHomeAddress,16,1)%></div>

<div id="Layer22" style="position:absolute; left:110px; top:1010px; width:40px; height:13px; z-index:15"><%=Sys_IllegalDate(0)%></div>
...
<div id="Layer25" style="position:absolute; left:260px; top:1010px; width:40px; height:16px; z-index:18"><%=right("00"&Sys_IllegalDate_h,2)%></div>
<div id="Layer26" style="position:absolute; left:310px; top:1010px; width:40px; height:13px; z-index:19"><%=right("00"&Sys_IllegalDate_m,2)%></div>
...
<div id="Layer29" style="position:absolute; left:120px; top:1060px; width:50px; height:11px; z-index:22"><%=Sys_DealLineDate(0)%></div>
<div id="Layer30" style="position:absolute; left:210px; top:1060px; width:35px; height:13px; z-index:23"><%=Sys_DealLineDate(1)%></div>
<div id="Layer31" style="position:absolute; left:280px; top:1060px; width:32px; height:15px; z-index:24"><%=Sys_DealLineDate(2)%></div>
<div id="Layer32" class="style3" style="position:absolute; left:405px; top:1075px; width:400px; height:49px; z-index:29"><%
...
<div id="Layer34" style="position:absolute; left:385px; top:1105px; width:400px; height:30px; z-index:28"><%
	if showBarCode then	response.write "<img src=""../BarCodeImage/"&Sys_BillNo&"_5.jpg"">"
%></div>
<div id="Layer33" style="position:absolute; left:635px; top:1110px; width:100px; height:40px; z-index:28"><span class="style7"><%=Sys_STATIONNAME&"<br>"&Sys_StationTel%></span></font></div>
<div id="Layer35" style="position:absolute; left:400px; top:1170px; width:100px; height:49px; z-index:29"><%
	if billprintuseimage=1 then
		response.write "<table border=""1"" cellspacing=0 cellpadding=0>"
		response.write "<tr><td class=""style3"">屏東縣政府警察局交通隊</td></tr>"
		response.write "<tr><td class=""style3"">TEL(08)7533112</td></tr>"
		response.write "</table>"
	end if%></div>
<div id="Layer36" style="position:absolute; left:510px; top:1145px; width:100px; height:43px; z-index:30"><%'=主管%></div>
<div id="Layer37" style="position:absolute; left:590px; top:1170px; width:200px; height:46px; z-index:31"><%
	...
	response.Write "<font size=2>"&Sys_ChName2&"</font>"%></div>
<div id="Layer38" style="position:absolute; left:210px; top:1245px; width:60px; height:10px; z-index:32"><%=sys_Date(0)%></div>
...
<div id="Layer41" style="position:absolute; left:690px; top:1245px; width:80px; height:12px; z-index:36"><%=Sys_BillFillerMemberID%></div>
<div id="Layer43" style="position:absolute; left:300px; top:1285px; width:250px; height:12px; z-index:36"><%=Sys_DCIRETURNCARTYPE%></div>
</div>
