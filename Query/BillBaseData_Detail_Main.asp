...
<!--#include virtual="/traffic/Common/Reportdb.ini"-->
...
<%	
	...
	If sys_City = "台中市" Then
		RDate1=Year(now) & "/" & Month(now) & "/" & Day(now) 
		RDate2=DateAdd("yyyy",-5,RDate1)
		strSQLTemp=strSQLTemp & " and a.Recorddate between TO_DATE('"&RDate2&" 0:0:0','YYYY/MM/DD/HH24/MI/SS') and TO_DATE('"&RDate1&" 23:59:59','YYYY/MM/DD/HH24/MI/SS') "
	elseif sys_City = "高雄市" Then
		RDate1=Year(now) & "/" & Month(now) & "/" & Day(now) 
		RDate2=DateAdd("yyyy",-3,RDate1)
		strSQLTemp=strSQLTemp & " and a.Recorddate between TO_DATE('"&RDate2&" 0:0:0','YYYY/MM/DD/HH24/MI/SS') and TO_DATE('"&RDate1&" 23:59:59','YYYY/MM/DD/HH24/MI/SS') "
	End If  
	...
	if trim(request("IllegalID"))<>"" Then
		...
		Else
			strQry=strQry&",IllegalID="&Trim(request("IllegalID"))
		End if
		strSQLTempPlus=strSQLTempPlus&" and (b.OwnerID='"&trim(request("IllegalID"))&"' or b.DriverID='"&trim(request("IllegalID"))&"' or a.DriverID='"&trim(request("IllegalID"))&"')"
	end If
	If sys_City="台中市" Then
		if trim(request("ReportNo"))<>"" Then
			...
			Else
				strQry=strQry&",ReportNo="&Trim(request("ReportNo"))
			End if
			strSQLTempPlus=strSQLTempPlus&" and a.sn=(select Billsn from BillReportNo where ReportNo='"&Trim(request("ReportNo"))&"')"
		end If
	End If 
	if trim(request("BillSn"))<>"" then
		strSQLTemp=strSQLTemp&" and a.SN='"&trim(request("BillSn"))&"'"
	end If
	'if sys_City="台南市" Or sys_City="台中市" Or sys_City="嘉義縣" Or sys_City="嘉義市" Or sys_City="基隆市" Or sys_City="苗栗縣" Or sys_City="高雄市" Or sys_City="台東縣" Or sys_City="花蓮縣" Or sys_City="澎湖縣" Or sys_City="彰化縣" Or sys_City="屏東縣" Or sys_City="雲林縣" Or sys_City="保二總隊四大隊二中隊" Or sys_City="保二總隊三大隊一中隊" Then
	...
	strSQL="Select a.BillNo,a.Sn,a.CarNo,a.BillTypeID,a.Rule1,a.Rule2,a.Rule3,a.Rule4,a.MemberStation,a.EquipMentID" &_
		...
		",a.DealLineDate,a.Note,a.CarSimpleID,a.CarAddID,a.ImageFileName"&strSQLAdd&" from BillBase a,BillBaseDciReturn b" &_
		...
		" and b.Status in ('Y','S','n','L') "& strSQLTemp & strSQLTempPlus
'--------2013/04/18-by jafe------------------------------------------------------------
	Set rs=conn.execute(strsql)
	If rs.eof Then 

	strSQLTempPlus=""
		if trim(request("IllegalName"))="" Then	'要用違規人姓名查必須要入案才能查
			if trim(request("IllegalID"))<>"" Then
				strSQLTempPlus=strSQLTempPlus&" and (a.DriverID='"&trim(request("IllegalID"))&"')"
			...
			strSQL="Select a.BillNo,a.Sn,a.CarNo,a.BillTypeID,a.Rule1,a.Rule2,a.Rule3,a.Rule4,a.MemberStation,a.EquipMentID" &_
				...
				",a.BillMemID4,a.BillMem4,a.RuleVer,a.IllegalAddressID,a.IllegalAddress,a.BillFillDate,a.BillUnitID,a.SignType" &_
				",a.driveraddress,a.driverzip,a.owner,a.ownerzip,a.owneraddress" &_
				",a.DealLineDate,a.Note,a.CarSimpleID,a.CarAddID,a.ImageFileName"&strSQLAdd&" from BillBase a " &_
				" where ((a.RecordStateID<>-1 and a.BillStatus='0')" &_
				" or a.BillStatus<>'0') " & strSQLTemp & strSQLTempPlus
			End If
		end If
	End If 

	strSQL=strSQL&" order by a.Recorddate"
...
<%	Cnt=0
	...
	tdstopflag=0
	'on error resume next  
	ConnExecute strQry&",原因:"&trim(request("QryReason")),356
...
<%	end if
	...
	if Trim(rs1("Rule1"))="5620001" Then
	 tdstopflag=1
	End If 
	'--------------------------------------BILLBASEDCIRETURN------------------------------------
...
'	set rsDciA=nothing
	...
	strCaseIn="select a.*,c.* from BillBaseDciReturn a,DciLog c" &_
			" where " &_
			" a.ExchangeTypeID=c.ExchangeTypeID and a.Status=c.DciReturnStatusID" &_
...
<%if trim(rs1("Rule3"))<>"" and not isnull(rs1("Rule3")) then%>
		<tr>
			...
			if (left(trim(rs1("Rule3")),2)="40" or (int(rs1("Rule3"))>4310200 and int(rs1("Rule3"))<4310209) or (int(rs1("Rule3"))>3310100 and int(rs1("Rule3"))<3310111)) and sys_City="基隆市" then
				...
				if trim(rs1("Rule3"))<>"" and not isnull(rs1("Rule3")) then
					if left(trim(rs1("Rule3")),4)="2110" or left(trim(rs1("Rule1")),4)="2210" or trim(rs1("Rule3"))="4310102" or trim(rs1("Rule3"))="4310103" or trim(rs1("Rule3"))="4310104" or trim(rs1("Rule3"))="4200001" then
						if trim(rs1("CarSimpleID"))=1 or trim(rs1("CarSimpleID"))=2 then
							strCarImple2=" and CarSimpleID in ('5','0')"
						elseif trim(rs1("CarSimpleID"))=3 or trim(rs1("CarSimpleID"))=4 then
							strCarImple2=" and CarSimpleID in ('3','0')"
...
<%end if%>
		<tr>
			...
			%></span></td>
<%	If sys_City="台東縣" Then
		If (trim(rs1("Rule1"))="5620001" Or trim(rs1("Rule1"))="5320001") And not isnull(rs1("imagefilename")) Then 
%>
...
%>
		...
		<% if sys_City="台東縣" then %>
			<td>
				車主證號(查車) : 
				<%
					strReturn="select OWNERID from BillBaseDCIReturn where BillNo is null and CarNo='"&trim(rs1("CarNo"))&"' order by DCICASEINDATE desc"	
					set rsReturn=conn.execute(strReturn)
					If Not rsReturn.eof Then 
...
<%
	...
	<%if sys_City="高雄縣" or (sys_City="高雄市" or sys_City=ApconfigureCityName Or sys_City="苗栗縣") then%>
		<tr>
			<td colspan="2">
	...
	<%End If %>
	<%If sys_City="高雄市" or (sys_City="台中市" and rs1("Illegaldate")>"2023/10/31") or sys_City="基隆市" or sys_City="彰化縣" or sys_City="澎湖縣" or sys_City="金門縣" or sys_City="南投縣" or sys_City="宜蘭縣" or sys_City="嘉義市" then%>
	<%
	...
	if ((StoreAndSendFlag=1 or sys_City="彰化縣") and sys_City<>"基隆市") or (ShowSecondAddress=1 and sys_City="基隆市") Or (ReturnMailNumber<>"" And sys_City="台南市") then
		if trim(rs1("BillTypeID"))="1" then	
			...
			if sys_City="台東縣" then
				...
				rsD.close
				set rsD=nothing
			elseif sys_City="台中市" or (sys_City="高雄市" Or sys_City="苗栗縣" or sys_City=ApconfigureCityName) then
				strSqlD="select DriverHomeZip,DriverHomeAddress,Owner,OwnerZip,OwnerAddress from BIllBaseDCIReturn where ((BillNo='"&trim(rs1("BillNo"))&"' and CarNo='"&trim(rs1("CarNo"))&"') or (BillNo is null and CarNo='"&trim(rs1("CarNo"))&"')) and ExchangeTypeID='A' and Status='S'"
				set rsD=conn.execute(strSqlD)
				if not rsD.eof then
			...
			elseif sys_City="基隆市" or sys_City="宜蘭縣" then
					...
					'單退先抓W看有沒有做戶籍補正，沒有就抓owner
						if trim(rsD2("DriverHomeAddress"))<>"" and not isnull(rsD2("DriverHomeAddress"))  then
							if sys_City="宜蘭縣" then
								...
								if not rsZip.eof Then
									If CDbl(Year(rs1("IllegalDate")))<2011 then
										ZipName=ChangeOldCity(trim(rsD2("DriverHomeZip")),ZipName=trim(rsZip("ZipName")))
									Else
										ZipName=trim(rsZip("ZipName"))
									End If								
								end if
				...
				rsD2.close
				set rsD2=Nothing
			elseif sys_City="南投縣" Then	'比照台中市
		...
		<tr>
			<td colspan="2"><span class="style2"><% if sys_City="台東縣" then %>
				...
				<%else%>
					寄存送達生效(完成)
				<% end if%>	日：</span><span class="style1"><%
				if sys_City<>"台中市" then
...
<%else
		...
		%>
				<img src="<%=trim(rsImgKS("IISImagePath"))&trim(rsImgKS("ImageFileNameA"))%>" name="imgB1" width="500" alt="" onclick="OpenPic('<%=trim(rsImgKS("IISImagePath"))&trim(rsImgKS("ImageFileNameA"))%>')">
		<%
			end if
			if trim(rsImgKS("ImageFileNameB"))<>"" then
		%>
				<img src="<%=trim(rsImgKS("IISImagePath"))&trim(rsImgKS("ImageFileNameB"))%>" name="imgB2" width="400" alt="" onclick="OpenPic('<%=trim(rsImgKS("IISImagePath"))&trim(rsImgKS("ImageFileNameB"))%>')">
		<%
			end if
	...
	End If 
		...
		while Not rsOpenGov2.eof
			...
			%>
				<div class='PageNext'>&nbsp;</div> <strong>公告公文</strong><br><img src='<%=replace(trim(rsScan2("FileName")),"/img/","/scanimg/")%>' name='imgB1' width='800' onclick="OpenPic('<%=replace(trim(rsScan2("FileName")),"/img/","/scanimg/")%>')">
			<%
			rsScan2.movenext
			wend
			rsScan2.close
...
<%	If sys_City="花蓮縣" Or sys_City="屏東縣" Or sys_City="嘉義縣" Or sys_City="南投縣" Or sys_City="彰化縣" Or sys_City="基隆市" Or sys_City="台中市" Or sys_City="保二總隊三大隊一中隊" Or sys_City="金門縣" Or sys_City="雲林縣" Or sys_City="台東縣" Or sys_City="台南市" Or sys_City="保二總隊四大隊二中隊" Or sys_City="保二總隊三大隊二中隊" Or sys_City="嘉義市" Then
		...
		<%
				...
				if trim(rsImgKS("ImageFileNameE"))<>"" then
		%>
				<img src="<%=trim(rsImgKS("IISImagePath"))&trim(rsImgKS("ImageFileNameE"))%>" name="imgB3" width="400" onclick="OpenPic('<%=trim(rsImgKS("IISImagePath"))&trim(rsImgKS("ImageFileNameE"))%>')">
		<%
				end If

				if trim(rsImgKS("ImageFileNameF"))<>"" then
		%>
				<img src="<%=trim(rsImgKS("IISImagePath"))&trim(rsImgKS("ImageFileNameF"))%>" name="imgB3" width="400" onclick="OpenPic('<%=trim(rsImgKS("IISImagePath"))&trim(rsImgKS("ImageFileNameF"))%>')">
		<%
				end If

				if trim(rsImgKS("ImageFileNameG"))<>"" then
		%>
				<img src="<%=trim(rsImgKS("IISImagePath"))&trim(rsImgKS("ImageFileNameG"))%>" name="imgB3" width="400" onclick="OpenPic('<%=trim(rsImgKS("IISImagePath"))&trim(rsImgKS("ImageFileNameG"))%>')">
		<%
				end If

				if trim(rsImgKS("ImageFileNameH"))<>"" then
		%>
				<img src="<%=trim(rsImgKS("IISImagePath"))&trim(rsImgKS("ImageFileNameH"))%>" name="imgB3" width="400" onclick="OpenPic('<%=trim(rsImgKS("IISImagePath"))&trim(rsImgKS("ImageFileNameH"))%>')">
		<%
				end If

				if trim(rsImgKS("ImageFileNameI"))<>"" then
		%>
				<img src="<%=trim(rsImgKS("IISImagePath"))&trim(rsImgKS("ImageFileNameI"))%>" name="imgB3" width="400" onclick="OpenPic('<%=trim(rsImgKS("IISImagePath"))&trim(rsImgKS("ImageFileNameI"))%>')">
		<%
				end If

				if trim(rsImgKS("ImageFileNameJ"))<>"" then
		%>
				<img src="<%=trim(rsImgKS("IISImagePath"))&trim(rsImgKS("ImageFileNameJ"))%>" name="imgB3" width="400" onclick="OpenPic('<%=trim(rsImgKS("IISImagePath"))&trim(rsImgKS("ImageFileNameJ"))%>')">
		<%
				end If
			End If 
		end if
		...
		set rsImgKS=Nothing
	End If 
	If sys_City="台南市" or sys_City="花蓮縣" Or sys_City="屏東縣" Or sys_City="嘉義縣" Or sys_City="澎湖縣" Or sys_City="彰化縣" Or sys_City="基隆市" Or sys_City="保二總隊四大隊二中隊" Or sys_City="保二總隊三大隊二中隊" Or sys_City="雲林縣" Then
		' Or sys_City="台中市"
		'送達証書
...
<%
		...
		If sys_City="澎湖縣"  Then
			fp5="E:\\Image\\Finish\\BillBaseDetail\\"&rs1("Sn") 
		elseIf sys_City="金門縣" Then
			fp5="F:\\Image\\Finish\\BillBaseDetail\\"&Request("BillSn") 
		End If 
		...
		if (fso5.FolderExists(fp5))=true then 
			...
			For Each fil In fic5
				If fil.Name<>"Thumbs.db" then
				 %>
				<a href="../../billimage/<%=rs1("Sn")&"/"&fil.Name%>" target="_blank">影像檔(請按滑鼠右鍵另存目標)</a>&nbsp; &nbsp;
				<%
				End If 
			Next
	   else
...
<Div id="Layer112" style="width:1041px; height:24px;">
  <div align="center">
  <%if (sys_City<>"高雄市" and sys_City<>"苗栗縣" and sys_City<>ApconfigureCityName) or trim(Session("Credit_ID"))="A000000000" then%>
  <input type="button" value="完整詳細資料" onclick='window.open("ViewBillBaseData_Car.asp?BillSN=<%=trim(rs1("SN"))%>&BillType=0","WebPage_Detail2","left=0,top=0,location=0,width=980,height=575,resizable=yes,scrollbars=yes,menubar=yes")'>
		
  <%end if%>
 <%if (sys_City<>"高雄市" and sys_City<>"苗栗縣" and sys_City<>ApconfigureCityName) then%>
 <%	If (sys_City="高雄縣" And (trim(rs1("IllegalAddress"))="鳳山市鳳頂路與田中央路口" Or trim(rs1("IllegalAddress"))="大寮鄉鳳屏路高屏大橋下橋處(往高雄)")) Or (sys_City<>"高雄縣" And sys_City<>"台東縣") or (sys_City="台東縣" And Trim(rs1("Rule1"))<>"5620001" And Trim(rs1("Rule1"))<>"5630001") Or sys_City="南投縣" Then
 '違規影像資料
...
<%
 ...
 <%
		...
		dim fp
			If sys_City="高雄市" Then
				fp="S:\\Image\\BillBaseDetail\\"&Request("BillSn") 
			elseIf sys_City="苗栗縣" Then
				fp="F:\\Image\\BillBaseDetail\\"&Request("BillSn") 
			
			else
                 fp="d:\\F\\Image\\BillBaseDetail\\"&Request("BillSn") 
			End if
	    ...
	    if (fso.FolderExists(fp))=true then 
            ...
            For Each fil In fic
				If fil.Name<>"Thumbs.db" then
				 ...
				 %>
                    <a title="開啟影像資料.." onclick="OpenImageWinUserUpload('../../billimage/<%=request("BillSn")&"/"&fil.Name%>')" <%lightbarstyle 1%>><u><font color="blue">影像 <%=i%></font></u></a>
			    <%
				End If 
            Next
       else
...
<%
	strSQLTemp2=""
	if trim(request("BillNo"))<>"" then
		strSQLTemp2=strSQLTemp2&" and BillNo='"&trim(request("BillNo"))&"'"
	end if
	if trim(request("CarNo"))<>"" then
		strSQLTemp2=strSQLTemp2&" and CarNo='"&trim(request("CarNo"))&"'"
	end if
	If strSQLTemp2<>"" Then
		RDate1Temp2=Year(now) & "/" & Month(now) & "/" & Day(now) 
		RDate2Temp2=DateAdd("yyyy",-3,RDate1Temp2)
		strSQLTemp2=strSQLTemp2 & " and illegaldate between TO_DATE('"&RDate2Temp2&" 0:0:0','YYYY/MM/DD/HH24/MI/SS') and TO_DATE('"&RDate1Temp2&" 23:59:59','YYYY/MM/DD/HH24/MI/SS') "
	End If 
'NoCase=1	拖吊已結用不到了 不要執行
if NoCase=100 and strSQLTemp2<>"" and (sys_City="高雄市" Or sys_City="苗栗縣" or sys_City=ApconfigureCityName) then 
	strSQL2="Select * from BillBase " &_
		" where BillStatus='9' and billUnitID in " &_
		" ('0861','0862','0863','0864') " &_
		" " & strSQLTemp2
	set rs2=conn.execute(strSQL2)
	If Not rs2.Bof Then
		rs2.MoveFirst 
...
<table width='100%' border='0' cellpadding="2">
	...
	<table width='100%' border='0' cellpadding="2">
		<tr>
			<td width="25%"><span class="style2">告發單號：</span><span class="style1"><%
			if trim(rs2("BillNO"))<>"" and not isnull(rs2("BillNO")) then
				response.write trim(rs2("BillNO"))
			end if
			%></span></td>
			<td width="27%"><span class="style2">到案處所：</span><span class="style1"><%
			StationName2=trim(rs2("MemberStation"))
			strStation="select * from Station where DciStationID='"&StationName2&"'"
			set rsStation=conn.execute(strStation)
			if not rsStation.eof then
				response.write trim(rsStation("DCIStationName"))
			end if
			...
			%></span></td>
			<td width="25%"><span class="style2">舉發單狀態：</span><span class="style1"><%
			if trim(rs2("RecordStateID"))="-1" then
				response.write "<font color=""red"">已刪除</font>"
			else
				response.write "正常"
			end if
			'刪除原因
			strDelRea="select b.Content from BillDeleteReason a,DciCode b where a.BillSn="&trim(rs2("Sn"))&" and b.TypeID=3 and a.DelReason=b.ID"
			set rsDelRea=conn.execute(strDelRea)
			if not rsDelRea.eof then
				response.write "<font color=""red"">." & trim(rsDelRea("Content")) & "</font>"
			else
			...
			if trim(rs2("RecordStateID"))="-1" and (sys_City="高雄市" Or sys_City="苗栗縣" or sys_City=ApconfigureCityName) then
'				strDelTime="select * from log where typeid=352 and ActionContent like '%單號:"&trim(rs1("BillNo"))&"%' and ActionContent like '%車號:"&trim(rs2("CarNo"))&"%' and rownum<=1 order by ActionDate Desc"
'				set rsDelTime=conn.execute(strDelTime)
'				if not rsDelTime.eof then
'					response.write "<font color=""red"">."&year(rsDelTime("ActionDate"))-1911&"/"&month(rsDelTime("ActionDate"))&"/"&day(rsDelTime("ActionDate"))&" "&hour(rsDelTime("ActionDate"))&":"&minute(rsDelTime("ActionDate"))&"</font>"
'				end if
...
'				set rsDelTime=nothing
		...
		<tr>
			...
			if trim(rs2("BillMem1"))<>"" and not isnull(rs2("BillMem1")) then
				response.write trim(rs2("BillMem1"))
				strMem1="select LoginID from MemberData where memberId="&trim(rs2("BillMemID1"))
				set rsMem1=conn.execute(strMem1)
				if not rsMem1.eof then
					response.write "("&trim(rsMem1("LoginID"))&")"
				end if
			...
			if trim(rs2("BillMem2"))<>"" and not isnull(rs2("BillMem2")) then
				response.write "/&nbsp;"&trim(rs2("BillMem2"))
				strMem2="select LoginID from MemberData where memberId="&trim(rs2("BillMemID2"))
				set rsMem2=conn.execute(strMem2)
				if not rsMem2.eof then
					response.write "("&trim(rsMem2("LoginID"))&")"
				end if
			...
			if trim(rs2("BillMem3"))<>"" and not isnull(rs2("BillMem3")) then
				response.write "/&nbsp;"&trim(rs2("BillMem3"))
				strMem3="select LoginID from MemberData where memberId="&trim(rs2("BillMemID3"))
				set rsMem3=conn.execute(strMem3)
				if not rsMem3.eof then
					response.write "("&trim(rsMem3("LoginID"))&")"
				end if
			...
			if trim(rs2("BillMem4"))<>"" and not isnull(rs2("BillMem4")) then
				response.write "/&nbsp;"&trim(rs2("BillMem4"))
				strMem4="select LoginID from MemberData where memberId="&trim(rs2("BillMemID4"))
				set rsMem4=conn.execute(strMem4)
				if not rsMem4.eof then
					response.write "("&trim(rsMem4("LoginID"))&")"
				end if
		...
		<tr>
			<td colspan="5"><span class="style2">違反法條：</span><span class="style1"><%
			strCarImple=""
			if (left(trim(rs2("Rule1")),2)="40" or (int(rs2("Rule1"))>4310200 and int(rs2("Rule1"))<4310209) or (int(rs2("Rule1"))>3310100 and int(rs2("Rule1"))<3310111)) and sys_City="基隆市" then
				response.write trim(rs2("Rule1"))&" "&"該路段限速"&trim(rs2("RuleSpeed"))&"公里、經雷達測速為"&trim(rs2("IllegalSpeed"))&"公里、超速"&cint(rs2("IllegalSpeed"))-cint(rs2("RuleSpeed"))&"公里"
			else
				if trim(rs2("Rule1"))<>"" and not isnull(rs2("Rule1")) then
					if left(trim(rs2("Rule1")),4)="2110" or trim(rs2("Rule1"))="4310102" or trim(rs2("Rule1"))="4310103" or trim(rs2("Rule1"))="4310104" then
						if trim(rs2("CarSimpleID"))=1 or trim(rs2("CarSimpleID"))=2 then
							strCarImple=" and CarSimpleID in ('5','0')"
						elseif trim(rs2("CarSimpleID"))=3 or trim(rs2("CarSimpleID"))=4 then
							strCarImple=" and CarSimpleID in ('3','0')"
						else
							strCarImple=""
						end if
					end if
					strR1="select IllegalRule,Level1 from Law where ItemID='"&trim(rs2("Rule1"))&"' and Version='"&trim(rs2("RuleVer"))&"'"&strCarImple&" order by CarSimpleID Desc"
					set rsR1=conn.execute(strR1)
					if not rsR1.eof then 
						response.write trim(rs2("Rule1"))&" "&trim(rsR1("IllegalRule"))
					end if
					rsR1.close
					set rsR1=nothing

					if trim(rs2("BillTypeID"))="2" and trim(rs2("Rule4"))<>"" and not isnull(rs2("Rule4")) then
						response.write "&nbsp;"&trim(rs2("Rule4"))
					end if
				end if	
			end if
			%></span></td>
...
<%if trim(rs2("Rule2"))<>"" and not isnull(rs2("Rule2")) then%>
		<tr>
			<td colspan="5"><span class="style2">違反法條：</span><span class="style1"><%
			if (left(trim(rs2("Rule2")),2)="40" or (int(rs2("Rule2"))>4310200 and int(rs2("Rule2"))<4310209) or (int(rs2("Rule2"))>3310100 and int(rs2("Rule2"))<3310111)) and sys_City="基隆市" then
				response.write trim(rs2("Rule2"))&" "&"該路段限速"&trim(rs2("RuleSpeed"))&"公里、經雷達測速為"&trim(rs2("IllegalSpeed"))&"公里、超速"&cint(rs2("IllegalSpeed"))-cint(rs2("RuleSpeed"))&"公里"
			else
				if trim(rs2("Rule2"))<>"" and not isnull(rs2("Rule2")) then
					if left(trim(rs2("Rule2")),4)="2110" or trim(rs2("Rule2"))="4310102" or trim(rs2("Rule2"))="4310103" or trim(rs2("Rule2"))="4310104" then
						if trim(rs2("CarSimpleID"))=1 or trim(rs2("CarSimpleID"))=2 then
							strCarImple2=" and CarSimpleID in ('5','0')"
						elseif trim(rs2("CarSimpleID"))=3 or trim(rs2("CarSimpleID"))=4 then
							strCarImple2=" and CarSimpleID in ('3','0')"
					...
					strR2="select IllegalRule,Level1 from Law where ItemID='"&trim(rs2("Rule2"))&"' and Version='"&trim(rs2("RuleVer"))&"'"&strCarImple2&" order by CarSimpleID Desc"
					set rsR2=conn.execute(strR2)
					if not rsR2.eof then 
						response.write trim(rs2("Rule2"))&" "&trim(rsR2("IllegalRule"))
					end if
					rsR2.close
					set rsR2=nothing
				end if
...
<%if trim(rs2("Rule3"))<>"" and not isnull(rs2("Rule3")) then%>
		<tr>
			<td colspan="5"><span class="style2">違反法條：</span><span class="style1"><%
			if (left(trim(rs2("Rule3")),2)="40" or (int(rs2("Rule3"))>4310200 and int(rs2("Rule3"))<4310209) or (int(rs2("Rule3"))>3310100 and int(rs2("Rule3"))<3310111)) and sys_City="基隆市" then
				response.write trim(rs2("Rule3"))&" "&"該路段限速"&trim(rs2("RuleSpeed"))&"公里、經雷達測速為"&trim(rs2("IllegalSpeed"))&"公里、超速"&cint(rs2("IllegalSpeed"))-cint(rs2("RuleSpeed"))&"公里"
			else
				if trim(rs2("Rule3"))<>"" and not isnull(rs2("Rule3")) then
					if left(trim(rs2("Rule3")),4)="2110" or trim(rs2("Rule3"))="4310102" or trim(rs2("Rule3"))="4310103" or trim(rs2("Rule3"))="4310104" then
						if trim(rs2("CarSimpleID"))=1 or trim(rs2("CarSimpleID"))=2 then
							strCarImple2=" and CarSimpleID in ('5','0')"
						elseif trim(rs2("CarSimpleID"))=3 or trim(rs2("CarSimpleID"))=4 then
							strCarImple2=" and CarSimpleID in ('3','0')"
					...
					strR2="select IllegalRule,Level1 from Law where ItemID='"&trim(rs2("Rule3"))&"' and Version='"&trim(rs2("RuleVer"))&"'"&strCarImple2&" order by CarSimpleID Desc"
					set rsR2=conn.execute(strR2)
					if not rsR2.eof then 
						response.write trim(rs2("Rule3"))&" "&trim(rsR2("IllegalRule"))
					end if
					rsR2.close
					set rsR2=nothing
				end if	
...
<%end if%>
<%if trim(rs2("Rule4"))<>"" and not isnull(rs2("Rule4")) and trim(rs2("BillTypeID"))<>"2" then%>
		<tr>
			<td colspan="5"><span class="style2">違反法條：</span><span class="style1"><%
			if (left(trim(rs2("Rule4")),2)="40" or (int(rs2("Rule4"))>4310200 and int(rs2("Rule4"))<4310209) or (int(rs2("Rule4"))>3310100 and int(rs2("Rule4"))<3310111)) and sys_City="基隆市" then
				response.write trim(rs2("Rule4"))&" "&"該路段限速"&trim(rs2("RuleSpeed"))&"公里、經雷達測速為"&trim(rs2("IllegalSpeed"))&"公里、超速"&cint(rs2("IllegalSpeed"))-cint(rs2("RuleSpeed"))&"公里"
			else
				if trim(rs2("Rule4"))<>"" and not isnull(rs2("Rule4")) then
					if left(trim(rs2("Rule4")),4)="2110" or trim(rs2("Rule4"))="4310102" or trim(rs2("Rule4"))="4310103" or trim(rs2("Rule4"))="4310104" then
						if trim(rs2("CarSimpleID"))=1 or trim(rs2("CarSimpleID"))=2 then
							strCarImple2=" and CarSimpleID in ('5','0')"
						elseif trim(rs2("CarSimpleID"))=3 or trim(rs2("CarSimpleID"))=4 then
							strCarImple2=" and CarSimpleID in ('3','0')"
					...
					strR2="select IllegalRule,Level1 from Law where ItemID='"&trim(rs2("Rule4"))&"' and Version='"&trim(rs2("RuleVer"))&"'"&strCarImple2&" order by CarSimpleID Desc"
					set rsR2=conn.execute(strR2)
					if not rsR2.eof then 
						response.write trim(rs2("Rule4"))&" "&trim(rsR2("IllegalRule"))
					end if
					rsR2.close
					set rsR2=nothing
				end if	
...
<%end if%>
		<tr>
			...
			If sys_City = "台中市" then 
				response.write rs2("IllegalZip")
			End if
			response.write trim(rs2("IllegalAddress"))
			%></span></td>
			<td><span class="style2">是否郵寄：</span><span class="style1"><%
			if trim(rs2("EquipMentID"))<>"" and not isnull(rs2("EquipMentID")) then
				if trim(rs2("EquipMentID"))="1" then
					response.write "是"
				else
					response.write "否"
				end if
		...
		<tr>
			...
			<%
				'可參考google doc "攔停 簽收 狀況 "
				if trim(rs2("SignType"))<>"" and not isnull(rs2("SignType")) then
					if rs2("SignType")="A" then response.write "簽收"
					if rs2("SignType")="U" then 
						strR2="select SignStateID from BillUserSignDate where billsn=" & trim(rs2("sn"))
						set rsR2=conn.execute(strR2)
						if not rsR2.eof then 
							if rsR2("SignStateID")="2" then response.write "拒簽已收"
							if rsR2("SignStateID")="3" then response.write "已簽拒收"							
		...
		<% if sys_City="台東縣" then %>
			<td>
				車主證號(查車) : 
				<%
					strReturn="select OWNERID from BillBaseDCIReturn where BillNo is null and CarNo='"&trim(rs2("CarNo"))&"' order by DCICASEINDATE desc"	
					set rsReturn=conn.execute(strReturn)
					If Not rsReturn.eof Then 
						response.write "<span class='style1'>" & rsReturn("OWNERID") & "</span>"
					else
	  ...
	  <% end if %>			
		...
		<tr>
			<td><span class="style2">車號：</span><span class="style1"><font size="4"><%=trim(rs2("CarNo"))%></font></span></td>
			<td><span class="style2">車主姓名：</span><span class="style1"><%=funcCheckFont(OwnerName,20,1)%></span></td>
			<td colspan="3"><span class="style2">車主住址：</span><span class="style1"><%=funcCheckFont(OwnerAddress,20,1)%></span></td>
		</tr>
		<tr>
			...
			if trim(rs2("BillUnitID"))<>"" and not isnull(rs2("BillUnitID")) then
				response.write trim(rs2("BillUnitID"))&"&nbsp;"
				strBillUnit="select UnitName from UnitInfo where UnitID='"&trim(rs2("BillUnitID"))&"'"
				set rsBillUnit=conn.execute(strBillUnit)
				if not rsBillUnit.eof then
					response.write trim(rsBillUnit("UnitName"))
				end if
		...
		<tr>
			...
			if trim(rs2("CarSimpleID"))<>"" and not isnull(rs2("CarSimpleID")) then
				...
				elseif trim(rs2("CarSimpleID"))="4" then
					response.write "輕機"
				elseif trim(rs2("CarSimpleID"))="6" then
					response.write "臨時車牌"
				end if
			end if	
			%></span></td>
			<td><span class="style2">建檔日期：</span><span class="style1"><%
			if trim(rs2("RecordDate"))<>"" and not isnull(rs2("RecordDate")) then
				response.write gArrDT(trim(rs2("RecordDate")))
			end if	
			%></span></td>
			<td><span class="style2">操作人員：</span><span class="style1"><%
			strRecMem="select ChName from MemberData where MemberID='"&trim(rs2("RecordMemberID"))&"'"
			set rsRecMem=conn.execute(strRecMem)
			if not rsRecMem.eof then
				response.write trim(rsRecMem("ChName"))
			end if
			...
			<%if (sys_City="高雄市" Or sys_City="苗栗縣" or sys_City=ApconfigureCityName) then%>			
				<tr>
				<td>
					<span class="style2">備註：</span><span class="style1"><%=trim(rs2("Note"))%></span>
				</td>
				</tr>
				<%if trim(rs2("BillTypeID"))="2" then

				strOther="select b.BillNO,c.DelReason from OtherBill a,BillBase b,BillDeleteReason c " &_
					" where a.NewBillSn="&trim(rs2("Sn")) &_
					" and a.OldBillSN=b.Sn and a.OldBillSN=c.Billsn" 
				set rsOther=conn.execute(strOther)
				if not rsOther.eof then
				%>
	...
	<%if sys_City="高雄縣" or (sys_City="高雄市" or sys_City=ApconfigureCityName Or sys_City="苗栗縣") then%>
		<tr>
			...
			<span class="style2">代保管物：</span><span class="style1"><%
			strFast="select a.FASTENERTYPEID,b.Content from BILLFASTENERDETAIL a,DciCode b" &_
				" where a.FASTENERTYPEID=b.ID and b.TypeID=6 and a.BillSn="&trim(rs2("SN"))
			set rsFast=conn.execute(strFast)
			If Not rsFast.Bof Then rsFast.MoveFirst 
			While Not rsFast.Eof
					response.write trim(rsFast("FASTENERTYPEID"))&trim(rsFast("Content"))&" "
...
<%end if%>
  ...
  <%if sys_City="台東縣" and tdstopflag=1 then%>
		<input type="button" value="停管對照表" onclick='window.open("StopCarIllegalTime.asp?BillSN=<%=trim(BillSnTmp)%>&BillType=0","StopCarIllegalTime","left=0,top=0,location=0,width=980,height=735,resizable=yes,scrollbars=yes,menubar=yes")'>
  <%end if %>
  <br>
...
function DP(){
<%if (sys_City="高雄市" or sys_City=ApconfigureCityName Or sys_City="苗栗縣") and NoCase=0 then%>
