...
<%		
		...
		" and a.RecordStateID=0 and g.UserMarkResonID in ('1','2','3','4','8','K','L','M','O','P','Q')" &strwhere
		...
		strSQL="select distinct a.ImageFileNameB,a.CarNo,e.Owner,g.UserMarkResonID " &_
		" from BillBase a,(select billsn from DciLog where exchangetypeid='A'"&dciStr&") b,(select CarNo,Owner from billbasedcireturn where exchangetypeid='A') e,StopBillMailHistory g" &_
		" where a.Sn=b.BillSn and a.CarNo=e.CarNo and a.Sn=g.BillSn" &_
		" and a.RecordStateID=0 and g.UserMarkResonID in ('1','2','3','4','8','K','L','M','O','P','Q')" &strwhere &_
		" order by a.ImageFileNameB"
...
%>		
	...
	<tr>
		<td align="center" height="75" colspan="6"><span class="style4"><%
	...
	<tr>
		<td align="center" height="40" width="6%"><span class="style4">編號</span></td>
		<!-- <td align="center" width="12%"><span class="style4">違規日期</span></td> -->
		<td align="center" width="19%"><span class="style4">催繳單號</span></td>
		<td align="center" width="11%"><span class="style4">車號</span></td>
		<td align="center" width="19%"><span class="style4">車主姓名</span></td>
		<% If sys_City<>"台東縣" then %>
		<td align="center" width="28%"><span class="style4">車主地址</span></td>
		<% end If %>
		<td align="center" width="15%"><span class="style4">退件原因</span></td>
	</tr>
<%		for i=1 to 20
...
%>
	<tr>
		...
		'車主姓名
		OwnerPrint=""
		if trim(rs1("Owner"))<>"" or not isnull(rs1("Owner")) then
			OwnerPrint = funcCheckFont(trim(rs1("Owner")),18,1)
		else
			OwnerPrint = "&nbsp;"
		end if	
		strO1="select * from BillBase where ImageFileNameB='"&trim(rs1("ImageFileNameB"))&"'"
		Set rsO1=conn.execute(strO1)
		If Not rsO1.eof Then 
			If Trim(rsO1("Owner"))<>"" Then
				OwnerPrint=Trim(rsO1("Owner"))
			End If 
		End If 
		rsO1.close
		Set rsO1=Nothing 
		response.write OwnerPrint
		%></span></td>
		<% If sys_City<>"台東縣" then %>
		<td align="center"><span class="style4"><%
		'車主地址
