...
%>
		...
		strCntReport="select count(*) as cnt from (select f.BillNo,f.CarNo from mailstationreturn a" &_
                    ",BillBase f where " & strwhere &_
                    " and a.billno=f.billno and f.RecordStateID=0" & ") a " &_
                    " ,BillBaseDCIReturn e where a.BillNo=e.BillNO and a.CarNo=e.CarNo and e.ExchangeTypeID='W' and e.Status='Y'" &_
	...
	'其他監理所列表
            ...
            strSQL="select f.BillTypeID,e.DCIReturnStation as DCIReturnStation,f.MemberStation as MemberStation,f.SN,f.BillNO,f.CarNO,e.Owner,e.OwnerZip,f.Rule1,f.Rule2,f.Rule3" &_
                ...
                " and ((f.BillTypeID='2' and e.ExchangeTypeID='W' and e.Status='Y')" &_
                " or (f.BillTypeID<>'2' and e.ExchangeTypeID='W' and e.Status in ('Y','S','n')))" &_
            ...
            While Not rs1.Eof
                    ...
                    %>
                        ...
                        <tr>
                                ...
                                <td align="left"><%
                                '應到案處所
                                'f.BillTypeID,e.DCIReturnStation ,f.MemberStation
                                
                                if trim(rs1("BillTypeID"))=1 then sStationID= trim(rs1("MemberStation"))
                                if trim(rs1("BillTypeID"))=2 then sStationID= trim(rs1("DCIReturnStation"))

                                strSqlStationName="select DCIstationName from Station where DCIstationID='"& sStationID &"'"
                                set rsSN=conn.execute(strSqlStationName)
                                if not rsSN.eof then
                                    response.write trim(rsSN("DCIstationName"))
                                else
