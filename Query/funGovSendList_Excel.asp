...
<%
	...
	if sys_City="台中市" then
		CloseDciReturnStatusID=" and a.DciReturnStatusID in ('n','h','S','N')"
	else
		CloseDciReturnStatusID=""
	end if

	'逕舉的到案處所用BillBaseDCIReturn
	ReportStationArrayTemp=""
	strStReport="select distinct(e.DCIReturnStation) from (select a.BillNo,a.CarNo from DCILog a" &_
		",BillBase f where a.BillSN=f.SN" &_
		" and f.RecordStateID=0" &_
		" and a.ExchangeTypeID='N' and a.BillTypeID='2'"&CloseDciReturnStatusID &_
	...
	strStStop="select distinct(f.MemberStation) from DCILog a,MemberData b,DCIReturnStatus d" &_
		",BillBase f where a.BillSN=f.SN" &_
		" and f.RecordStateID=0" &_
		" and a.ExchangeTypeID='N' and a.BillTypeID<>'2'"&CloseDciReturnStatusID &_
		" and a.ReturnMarkType='5'" &_
...
<%if sys_City<>"台南市" then %>
	<center><font size="3">舉發違反道路交通事件通知單公示送達移送清冊</font></center>
...
%>
		...
		",BillBase f where a.BillSN=f.SN" &_
		" and f.RecordStateID=0" &_
		" and a.ExchangeTypeID='N' and a.DciReturnStatusID in ('n','h') and a.BillTypeID='2'" &_
		" and a.ReturnMarkType='5'" &_
		" and a.ExchangeTypeID=d.DCIActionID(+) and a.DCIReturnStatusID=d.DCIReturn(+)" &_
		" and a.RecordMemberID=b.MemberID(+) "&strwhere&") a,BillBaseDCIReturn e" &_
		" where a.BillNo=e.BillNO and a.CarNo=e.CarNo and e.ExchangeTypeID='W' and e.Status in ('Y','S','n','L')" &_
...
<%
	...
	'新北市交通事件裁決所數量==============================================
	if instr(StationArrayTemp,"40")>0 or instr(StationArrayTemp,"41")>0 or instr(StationArrayTemp,"46")>0 or instr(StationArrayTemp,"48")>0 then
%>
		<tr>
			<td><span class="style3"><%
			'受文單位
		...
		" and a.RecordMemberID=b.MemberID(+) "&strwhere&") a,BillBaseDCIReturn e" &_
		" where a.BillNo=e.BillNO and a.CarNo=e.CarNo and e.ExchangeTypeID='W' and e.Status in ('Y','S','n','L')" &_
		" and e.DCIReturnStation in ('40','41','46','48')"
		set rsCntReport=conn.execute(strCntReport)
		if not rsCntReport.eof then
			StationCnt=StationCnt+trim(rsCntReport("cnt"))
		end if
		...
		",BillBase f where a.BillSN=f.SN" &_
		" and f.RecordStateID=0" &_
		" and f.MemberStation in ('40','41','46','48')" &_
		" and a.ExchangeTypeID='N' and a.BillTypeID<>'2'"&CloseDciReturnStatusID &_
		" and a.ReturnMarkType='5'" &_
		" and a.ExchangeTypeID=d.DCIActionID(+) and a.DCIReturnStatusID=d.DCIReturn(+)" &_
		" and a.RecordMemberID=b.MemberID(+) "&strwhere
...
%>
		...
		" and a.RecordMemberID=b.MemberID(+) "&strwhere&") a,BillBaseDCIReturn e" &_
		" where a.BillNo=e.BillNO and a.CarNo=e.CarNo and e.ExchangeTypeID='W' and e.Status in ('Y','S','n','L')" &_
		" and e.DCIReturnStation in ('60','61','63','68')"
		set rsCntReport=conn.execute(strCntReport)
		if not rsCntReport.eof then
			StationCnt=StationCnt+trim(rsCntReport("cnt"))
		end if
...
<%
	...
	for SA=0 to ubound(StationArray)
		if instr("20,21,22,23,24,29,30,31,32,40,41,46,48,60,61,63,68",trim(StationArray(SA)))<=0 then
%>
		<tr>
			<td><span class="style3"><%
			'受文單位
...
<%	
	...
	'逕舉 and 攔停
	PrintSN=0
	strSQL="select a.BillSN,a.BillNO,a.CarNO,e.Driver,e.Owner,e.OwnerZip,f.BillTypeID,f.Rule1,f.Rule2,f.Rule3" &_
		...
		" and a.ExchangeTypeID='N' "&CloseDciReturnStatusID&"" &_
...
%>
	...
	<br>
	&nbsp; &nbsp; 到案處所：<%="台北市交通事件裁決所"%>
	<table width="<%=TableWidth%>" align="center" border="1" cellpadding="1" cellspacing="0">
		<tr>
...
<%if sys_City<>"嘉義縣" then %>
	<%if sys_City="基隆市" then %>
			<td align="center">單退狀態<br>/送達狀態</td>
	<%else%>
...
%>		<tr>
			...
			%></td>
			<td nowrap><%
			if ((sys_City="基隆市" or sys_City="澎湖縣") and trim(rs1("BillTypeID"))="1") then
				'駕駛人姓名
			...
			'車主姓名
			if ((sys_City="南投縣") and trim(rs1("BillTypeID"))="1") Then	'2015/4/28南投陳淑雲:攔停要出駕駛人
				if trim(rs1("Driver"))<>"" and not isnull(rs1("Driver")) then
					response.write funcCheckFont(trim(rs1("Driver")),18,1)
				else
					response.write "&nbsp;"
				...
				if trim(rs1("Owner"))<>"" and not isnull(rs1("Owner")) Then
					If sys_City="高雄市" then 
						response.write funcCheckFont(getNameHidden(Trim(rs1("Owner"))),18,1)
					Else
...
<%end if%>
			...
			ReturnReason=""
			MailNumberTmp=""
			strMail="select MailNumber,StoreAndSendMailNumber,OpenGovNumber,ReturnResonID,StoreAndSendReturnResonID,UserMarkResonID from BillMailHistory where BillSN='"&trim(rs1("BillSN"))&"'"
			set rsMail=conn.execute(strMail)
			if not rsMail.eof then
				if (trim(rsMail("OpenGovNumber"))="" or isnull(rsMail("OpenGovNumber"))) then
					response.write "&nbsp;"
				else
					response.write trim(rsMail("OpenGovNumber"))
				end if
				if (trim(rsMail("StoreAndSendMailNumber"))="" or isnull(rsMail("StoreAndSendMailNumber"))) then
					'貼條號碼
...
<%if sys_City<>"嘉義縣" then %>
			<td nowrap><%
			'單退狀態
			strStatus1="select DciReturnStatusID from Dcilog a where a.ExchangeTypeID='N' and a.ReturnMarkType='5' and a.BillSn="&trim(rs1("BillSn"))&strwhere
...
<%end if
	...
	if sys_City="宜蘭縣" Or sys_City="南投縣" then 
		CaseSn=0
	end if
...
'新北市交通事件裁決所列表==============================================
	...
	strSQL="select a.BillSN,a.BillNO,a.CarNO,e.Driver,e.Owner,e.OwnerZip,f.BillTypeID,f.Rule1,f.Rule2,f.Rule3" &_
		...
		" and a.ExchangeTypeID='N' "&CloseDciReturnStatusID&"" &_
		" and a.BillNo=e.BillNO and a.CarNo=e.CarNo" &_
		" and ((a.BillTypeID='2' and e.DCIReturnStation in ('40','41','46','48') and e.ExchangeTypeID='W' and e.Status in ('Y','S','n','L'))" &_
		" or (a.BillTypeID<>'2' and f.MemberStation in ('40','41','46','48') and e.ExchangeTypeID='W' and e.Status in ('Y','S','n','L')))" &_
		" order by g.UserMarkDate"
...
<%end if
	...
	strSQL="select a.BillSN,a.BillNO,a.CarNO,e.Driver,e.Owner,e.OwnerZip,f.BillTypeID,f.Rule1,f.Rule2,f.Rule3" &_
		...
		" or (a.BillTypeID<>'2' and f.MemberStation in ('60','61','63','68') and e.ExchangeTypeID='W' and e.Status in ('Y','S','n','L')))" &_
		" order by g.UserMarkDate"
...
<%end if
	...
	for SA2=0 to ubound(StationArray)
		if instr("20,21,22,23,24,29,30,31,32,40,41,46,48,60,61,63,68",trim(StationArray(SA2)))<=0 Or trim(StationArray(SA2))="" then
			if sys_City="宜蘭縣" Or sys_City="南投縣" then 
				CaseSn=0
			end if
	'逕舉and 攔停
