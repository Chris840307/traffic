...
Sys_Owner=""
'================
strSql="select Rule4,BillTypeID,Driver,DriverID,DriverAddress,DriverZip,ILLEGALADDRESS,IllegalSpeed,RuleSpeed,INSURANCE,RuleVer,Note,BillFillDate from BillBase where SN="&trim(rsbil("BillSN"))
...
if trim(Sys_BillTypeID)="1" then
	...
	DelphiASPObj.GenBillPrintBarCode_PT PBillSN(i),Sys_BillNo,Sys_Rule1,Sys_CarNo,Sys_MailNumber,"220073",Sys_DriverHomeZip,right(Sys_DealLineDate(0),2)&Sys_DealLineDate(1)&Sys_DealLineDate(2),Sys_StationID,"台北市交通事件裁決所",0,Sum_Level,0,True,False,Sys_MailDate,"900","018","17"
else
	Set DelphiASPObj = Server.CreateObject("GenBarCode.BarCodeASP") 
	delphiASPObj.GenBillPrintBarCode_PT PBillSN(i),Sys_BillNo,Sys_Rule1,Sys_CarNo,Sys_MailNumber,"220073",Sys_OwnerZip,right(Sys_DealLineDate(0),2)&Sys_DealLineDate(1)&Sys_DealLineDate(2),Sys_StationID,"台北市交通事件裁決所",0,Sum_Level,0,True,False,Sys_MailDate,"900","018","17"
end if
...
<!---------------------------------------- start 列印紅單紅色區域內容 --------------------------------------------->
<%if showBarCode then%>
<div id="Layer1" style="position:absolute; left:50px; top:<%=375+pagepx%>px; width:202px; height:36px; z-index:5">Ｖ</div>
<%else%>
<div id="Layer2" style="position:absolute; left:50px; top:<%=410+pagepx%>px; width:202px; height:36px; z-index:5">Ｖ</div>
<%end if%>
<%if trim(Sys_BillTypeID)="1" then%>
<div id="Layer3" style="position:absolute; left:165px; top:<%=380+pagepx%>px; width:202px; height:36px; z-index:5">v</div>
<%else%>
<div id="Layer4" style="position:absolute; left:165px; top:<%=395+pagepx%>px; width:202px; height:36px; z-index:5">v</div>
<%end if%>
<div id="Layer9" style="position:absolute; left:20px; top:<%=430+pagepx%>px; width:210px; height:36px; z-index:5"><%
	if showBarCode then
...
%></div>
<div id="Layer10" style="position:absolute; left:500px; top:<%=423+pagepx%>px; width:233px; height:32px; z-index:6"><img src=<%="""../BarCodeImage/"&Sys_BillNo&"_4.jpg"""%>></div>
...
<div id="Layer12" style="position:absolute; left:105px; top:<%=488+pagepx%>px; width:300px; height:11px; z-index:20"><span class="style7">逕行舉發　<%=Sys_A_Name%><br><%if int(Sys_Rule1)<>4340003 and int(Sys_Rule1)<>5620001 then response.write "附採證照片"%>　<%=Sys_CarColor%></span></div>
<div id="Layer13" style="position:absolute; left:270px; top:<%=490+pagepx%>px; width:28px; height:11px; z-index:3"><%=Sys_Sex%></div>
<div id="Layer14" style="position:absolute; left:435px; top:<%=480+pagepx%>px; width:324px; height:10px; z-index:4"><%if showBarCode then response.write "*本單可至郵局或委託代收之超商繳納"%></div><%'=Sys_DriverHomeZip&" "&Sys_DriverZipName&Sys_DriverHomeAddress%>
...
<div id="Layer19" style="position:absolute; left:260px; top:<%=530+pagepx%>px; width:117px; height:20px; z-index:12"><%=Sys_DCIRETURNCARTYPE%></div>
<div id="Layer20" style="position:absolute; left:510px; top:<%=530+pagepx%>px; width:201px; height:17px; z-index:13"><%=funcCheckFont(Sys_Owner,20,1)%></div>
<div id="Layer21" style="position:absolute; left:105px; top:<%=555+pagepx%>px; width:520px; height:13px; z-index:14"><%=Sys_OwnerZip&" "&funcCheckFont(Sys_OwnerZipName&Sys_OwnerAddress,20,1)%></div>

<div id="Layer22" style="position:absolute; left:110px; top:<%=575+pagepx%>px; width:40px; height:13px; z-index:15"><%=Sys_IllegalDate(0)%></div>
<div id="Layer23" style="position:absolute; left:160px; top:<%=575+pagepx%>px; width:40px; height:17px; z-index:16"><%=Sys_IllegalDate(1)%></div>
<div id="Layer24" style="position:absolute; left:210px; top:<%=575+pagepx%>px; width:40px; height:16px; z-index:17"><%=Sys_IllegalDate(2)%></div>
...
<div id="Layer27" style="position:absolute; left:400px; top:<%=580+pagepx%>px; width:340px; height:31px; z-index:20"><span class="style33"><%
...
%></span></div>
<div id="Layer28" style="position:absolute; left:110px; top:<%=600+pagepx%>px; width:220px; height:15px; z-index:21"><span class="style33"><%=Sys_ILLEGALADDRESS%></span></div>
...
<!-----------------------------------------法條編號 --------------------------------------------->
<div id="Layer32" style="position:absolute; left:405px; top:<%=640+pagepx%>px; width:400px; height:49px; z-index:29"><%
...
<div id="Layer33" style="position:absolute; left:400px; top:<%=685+pagepx%>px; width:90px; height:40px; z-index:28"><span class="style3"><%=Sys_STATIONNAME&"<br>"&Sys_StationTel%></span></font></div>

<div id="Layer34" style="position:absolute; left:480px; top:<%=680+pagepx%>px; width:400px; height:30px; z-index:2"><%
	if showBarCode then	response.write "<img src=""../BarCodeImage/"&Sys_BillNo&"_5.jpg"">"
%></div>
<!----'smith                     舉發單位章	--->
<div id="Layer35" style="position:absolute; left:410px; top:<%=750+pagepx%>px; width:100px; height:89px; z-index:29"><%
...
<!-----------------------------------送達證書 繳回單位資料---------------------------------->
<div id="Layer42" class="style3" style="position:absolute; left:50px; top:<%=950+pagepx%>px; z-index:1"><%=InstrAdd(sys_CityUnit&SysUnit,8)%></div>

<div id="Layer45" class="style3" style="position:absolute; left:340px; top:<%=950+pagepx%>px; z-index:1"><%=InstrAdd(sys_CityUnit&SysUnit,8)%></div>
...
<!-----------------------------------送達證書 收件人資料---------------------------------->
<div id="Layer47" class="style3" style="position:absolute; left:125px; top:<%=985+pagepx%>px; z-index:1"><%
...
<!-----------------------------------送達證書 文 號 ---------------------------------->
<div id="Layer48" class="style3" style="position:absolute; left:225px; top:<%=1030+pagepx%>px; z-index:1"><%=Sys_BillNo%>
</div>
...
<div id="Layer49" class="style3" style="position:absolute; left:510px; top:<%=1045+pagepx%>px; z-index:1"><img src=<%="""../BarCodeImage/"&Sys_BillNo&"_4.jpg"""%>></div>
</div>
</div><%
