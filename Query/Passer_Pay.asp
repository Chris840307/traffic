...
if trim(request("kinds"))="db_insert" then
	...
	if trim(request("PayAmount"))<>"" and trim(request("PayNo"))<>"" then
		...
		strIns="insert into PasserPay(BillSN,BillNo,PayNo,PayTimes,PayTypeID,PayDate,Payer,ForFeit" &_
			...
			","&trim(request("ForFeit"))&","&trim(request("PayAmount"))&","&theCaseClose &_
		...
		if theCaseClose=1 then
			...
			strIns="Update PasserPay set CaseClose=null,CaseCloseDate=0 where BillSN="&trim(request("BillSN"))
			conn.execute strIns
		end if
		%>
		<script language="JavaScript">
...
<table width="100%" height="100%" border="0">
    ...
    <td height="26"><table width="100%" height="100%"  border="0" cellpadding="5" cellspacing="1" bgcolor="#FFFFFF">
      ...
      <tr>
        ...
        <td width="19%"><%
			...
			<td><%
			if trim(rsSql("ForFeit"))<>"" then
				theForFeit=trim(rsSql("ForFeit"))
			else
				L1ForFeit=0
				L2ForFeit=0
				L3ForFeit=0
      ...
      <tr>
        ...
        <td width="19%">
			<select name="IsLate">
				<option value="0">如期繳納</option>
				<option value="1">逾期繳納</option>
			</select>
		</td>
        <td width="9%" nowrap bgcolor="#FFFF99"><div align="right" class="style3">繳費金額</div></td>
        <td>
			<input type="text" name="PayAmount" value="<%=theForFeit%>" maxlength="6" onkeyup="value=value.replace(/[^\d]/g,'')">
		</td>
      </tr>
      <tr>
        <td nowrap bgcolor="#FFFF99"><div align="right"><span class="style3">繳費日期</span></div></td>
        <td width="19%">
		<input type="text" name="PayDate" size="6" maxlength="8" value="<%=gInitDT(date)%>" onkeyup="value=value.replace(/[^\d]/g,'')">
      ...
      <tr>
        ...
        <td nowrap bgcolor="#FFFFff">
			<input type="text" name="CaseCloseDate" size="6" maxlength="8" value="<%=gInitDT(date)%>" onkeyup="value=value.replace(/[^\d]/g,'')">
  ...
  <tr>
    <td height="35" bgcolor="#FFDD77" colspan="4"><p align="center" class="style1">
        ...
        <input name="Submit43322" type="button" class="style3" value="儲存" onclick="db_insert();">
