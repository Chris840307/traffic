				...
				<%
					while not rsfound.eof 
						...
						if sys_City="台中市" then
							...
							clspay = QueryCol("clspay","(Select clspay from peo_ALL where tkt_no=" & QuotedStr(trim(rsfound("tkt_no"))) & "union Select clspay from peo_New where tkt_no=" & QuotedStr(trim(rsfound("tkt_no"))) & ")","") 
						else
						...
						response.write "<td width='8%'>" & QueryUnitName(trim(rsfound("Acc_no"))) & "&nbsp;</td>"
						response.write "<td width='8%'>" & trim(rsfound("Rule_1")) &  "&nbsp;</td>"
						Moneyn = QueryCol("Mey_01","Rule_n"," rule_c=" & QuotedStr(trim(rsfound("Rule_1")))) 'QueryCol("Moneyn","Peo_Rec"," tkt_no=" & QuotedStr(trim(rsfound("tkt_no"))))
						if trim(Moneyn) = "" then Moneyn = trim(rsfound("moneyn"))
						response.write "<td width='8%'>" & Moneyn & "&nbsp;</td>"

						'response.write "<td width='13%'>"& trim(rsfound("cls_dt")) & "&nbsp;</td>"
						response.write "<td width='13%'>"& CloseDate & "&nbsp;</td>"

						'response.write "<td width='8%'>" & trim(rsfound("cls_no")) &  "&nbsp;</td>"
						response.write "<td width='8%'>" & clsno &  "&nbsp;</td>"	

						'response.write "<td width='8%'>" & trim(rsfound("clspay")) &  "&nbsp;</td>"				
						response.write "<td width='8%'>" & clspay &  "&nbsp;</td>"

						'if trim(Moneyn) <= trim(rsfound("clspay")) then
						if trim(Moneyn) <= trim(clspay) then
							response.write "<td width='8%'>結案" & "&nbsp;</td>"
						else
							response.write "<td width='8%'>未結案" & "&nbsp;</td>"
						end if
						response.write "</tr>"
						rsfound.movenext
				    wend
