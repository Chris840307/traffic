...
<%
	...
	if trim(request("WorkType"))="4" then
		CaseSN=""
		strSQLsn="select a.SN from BillBase a,MemberData b,BillMailHistory c where a.RecordMemberID=b.MemberID and c.BillSN=a.SN and c.UserMarkResonID in ('1','2','3','4','8','M','K','L','O','P','Q') "&Session("DciOpenGovToExcelSession")&" order by c.UserMarkDate"
		set rsSqlSn=conn.execute(strSQLsn)
		if not rsSqlSn.eof then
			rsSqlSn.MoveFirst 
			While Not rsSqlSn.Eof
				if CaseSN="" then
					CaseSN=trim(rsSqlSn("Sn"))
				else
					CaseSN=CaseSN&","&trim(rsSqlSn("Sn"))
				end if
				rsSqlSn.MoveNext
			Wend
		end if
		rsSqlSn.close
		set rsSqlSn=nothing
		'response.write strSQLsn
		strwhere=" and a.BillSn in ("&CaseSN&") "
	else
	...
	strStReport="select distinct(DCIReturnStation) from (select f.SN,a.BillNo,a.CarNo from DCILog a,MemberData b," &_
		...
		" and a.ExchangeTypeID='W' and a."&CloseDciReturnStatusID&" and a.BillTypeID='2'" &_
		" and a.ExchangeTypeID=d.DCIActionID(+)" &_
		" and a.DCIReturnStatusID=d.DCIReturn(+) and a.RecordMemberID=b.MemberID(+)  "&strwhere&") a" &_
		" ,BillBaseDCIReturn e,BillMailHistory g where a.SN=g.BillSn and a.BillNo=e.BillNO and a.CarNo=e.CarNo and e.ExchangeTypeID='W' and e.Status in ('Y','S','n','L') and g.UserMarkResonID in ('1','2','3','4','8','M','K','L','O','P','Q')"
	...
	strStStop="select distinct(f.MemberStation) from DCILog a,MemberData b" &_
		...
		" and a.ExchangeTypeID='W' and a."&CloseDciReturnStatusID&"" &_
		" and a.BillTypeID<>'2' and a.ExchangeTypeID=d.DCIActionID(+)"&_
		" and a.DCIReturnStatusID=d.DCIReturn(+) and a.RecordMemberID=b.MemberID(+) and g.UserMarkResonID in ('1','2','3','4','8','M','K','L','O','P','Q')"&strwhere
...
%>
		...
		strCntReport="select count(*) as cnt from (select f.SN,a.BillNo,a.CarNo from DCILog a,MemberData b," &_
			...
			" and a.RecordMemberID=b.MemberID(+) "&strwhere&") a,BillBaseDCIReturn e,BillMailHistory g" &_
		...
		" and a.BillTypeID<>'2'" &_
		" and a.ExchangeTypeID=d.DCIActionID(+) and a.DCIReturnStatusID=d.DCIReturn(+)" &_
		" and a.RecordMemberID=b.MemberID(+) and g.UserMarkResonID in ('1','2','3','4','8','M','K','L','O','P','Q') "&strwhere
...
%>
		...
		'逕舉
		StationCnt=0
		strCntReport="select count(*) as cnt " &_
		" from (select distinct a.BillSN,a.BillNo,a.CarNo,a.BillTypeID from DciLog a where a.BillSN is not null "&strwhere&_
		" and a.ExchangeTypeID='W' and a."&CloseDciReturnStatusID&"" &_
		") a" &_
		...
		" and ((a.BillTypeID='2' and e.DCIReturnStation='"&trim(StationArray(SA))&"' and e.ExchangeTypeID='W' and e.Status in ('Y','S','n','L'))" &_
		" or (a.BillTypeID<>'2' and f.MemberStation='"&trim(StationArray(SA))&"' and e.ExchangeTypeID='W' and e.Status in ('Y','S','n','L'))) and g.UserMarkResonID in ('1','2','3','4','8','M','K','L','O','P','Q')"
...
<%
	...
	if instr(StationArrayTemp,"20")>0 or instr(StationArrayTemp,"21")>0 or instr(StationArrayTemp,"22")>0 or instr(StationArrayTemp,"23")>0 or instr(StationArrayTemp,"24")>0 or instr(StationArrayTemp,"29")>0 then
		StationCnt=0
		strCntReport="select count(*) as cnt " &_
		" from (select distinct a.BillSN,a.BillNo,a.CarNo,a.BillTypeID from DciLog a " &_
		" where a.BillSN is not null "&strwhere &_
		" and a.ExchangeTypeID='W' and a."&CloseDciReturnStatusID&"" &_
		") a" &_
		" ,BillBaseDCIReturn e,BillBase f,BillMailHistory g" &_
		" where a.BillSN=f.SN and f.RecordStateID=0" &_
		...
		" or (a.BillTypeID<>'2' and f.MemberStation in ('20','21','22','23','24','29') and e.ExchangeTypeID='W' and e.Status in ('Y','S','n','L'))) and g.UserMarkResonID in ('1','2','3','4','8','M','K','L','O','P','Q')" 
		set rsCntReport=conn.execute(strCntReport)
		if not rsCntReport.eof then
			StationCnt=StationCnt+trim(rsCntReport("cnt"))
		end if
	...
	if instr(StationArrayTemp,"30")>0 or instr(StationArrayTemp,"31")>0 or instr(StationArrayTemp,"32")>0 then
		StationCnt=0
		strCntReport="select acount(*) as cnt " &_
		" from (select distinct a.BillSN,a.BillNo,a.CarNo,a.BillTypeID from DciLog a " &_
		" where a.BillSN is not null "&strwhere &_
		" and a.ExchangeTypeID='W' and a."&CloseDciReturnStatusID&"" &_
		") a" &_
		...
		" or (a.BillTypeID<>'2' and f.MemberStation in ('30','31','32') and e.ExchangeTypeID='W' and e.Status in ('Y','S','n','L'))) and g.UserMarkResonID in ('1','2','3','4','8','M','K','L','O','P','Q')" 
...
%>		
	...
	<td align="center" height="28" colspan="2"><span class="style4"><%
		...
		if trim(StationCnt(SA2))="0" then
			...
			pagecnt=fix(Cint(trim(StationCntTaipei))/23+0.9999999)
		end if
	%></span></td>
	</tr>
	<tr>
...
%>
		<tr>
			...
			if trim(rs1("CarSimpleID"))<>"" and not isnull(rs1("CarSimpleID")) then
				...
				%></td>
					<td><%
			if sys_City="台東縣" or (sys_City="基隆市" and trim(rs1("BillTypeID"))="1") then
				'駕駛人姓名
...
%>		
	...
	<td align="center" height="28" colspan="2"><span class="style4"><%
		...
		if trim(StationCnt(SA2))="0" then
			...
			pagecnt=fix(Cint(trim(StationCntKS))/23+0.9999999)
		end if
	%></span></td>
	</tr>
	<tr>
	<td width="85%">到案處所：<%="高雄市交通事件裁決所"%>
	&nbsp; &nbsp; &nbsp;
	列印日期：<%=now%>&nbsp; &nbsp; &nbsp;<%
	%>
	</td>
