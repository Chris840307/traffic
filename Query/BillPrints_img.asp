...
<table width="645" height="393" border="0">
  ...
  <tr>
    ...
    <%=funcCheckFont(Sys_OwnerZipName&Sys_OwnerAddress,25,1)%></span></td>
	<%end if%>
  </tr>
  <tr>
    <td>&nbsp;</td>
	<%if trim(Sys_BillTypeID)="1" then%>
    <td colspan="2"><span class="style3"><%=Sys_Driver%>　台啟</span></td>
	<%elseif trim(Sys_BillTypeID)="2" then%>
...
<%if trim(Sys_ImageFileNameA)<>"" then%>
	<div style="position:absolute; left:20px; top:460px; z-index:5"><%
		response.write "<img src="""&Sys_IisImagePath&Sys_ImageFileNameA&""" width=""420"" height=""310"">"
	%></DIV>
<%end if%>

<%if trim(Sys_ImageFileNameB)<>"" then%>
	<div style="position:absolute; left:440px; top:460px; z-index:1"><%
		response.write "<img src="""&Sys_IisImagePath&Sys_ImageFileNameB&""" width=""330"" height=""265"">"
	%></DIV>
<%end if%>

<div id="Layer6" style="position:absolute; left:30px; top:775px; width:400px; height:36px; z-index:5"><span class="style7">查詢電話：<%=DB_UnitTel%><%'=Sys_UnitTel%>（<%=DB_UnitName%><%'=Sys_UnitName%>）</span></div>
...
<%if trim(Sys_BillTypeID)="1" then%>
	...
	<%end if%>
<%end if%>
<div id="Layer9" style="position:absolute; left:10px; top:855px; width:202px; height:36px; z-index:5"><%
	if showBarCode then
...
%></div>
<div id="Layer10" style="position:absolute; left:500px; top:850px; width:233px; height:32px; z-index:6"><img src=<%="""../BarCodeImage/"&Sys_BillNo&"_4.jpg"""%>></div>
...
<%end if%>
<div id="Layer18" style="position:absolute; left:115px; top:960px; width:100px; height:14px; z-index:11"><span class="style7"><%=Sys_CarNo%></span></div>
<div id="Layer19" style="position:absolute; left:265px; top:960px; width:117px; height:20px; z-index:12"><span class="style7"><%=Sys_DCIRETURNCARTYPE%></span></div>
<div id="Layer20" style="position:absolute; left:500px; top:960px; width:201px; height:17px; z-index:13"><span class="style3"><%=funcCheckFont((Sys_Owner,22,1)%></span></div>
<div id="Layer21" style="position:absolute; left:115px; top:985px; width:507px; height:13px; z-index:14"><span class="style7"><%=Sys_OwnerZip&" "&funcCheckFont(Sys_OwnerZipName&Sys_OwnerAddress,22,1)%></span></div>

<div id="Layer22" style="position:absolute; left:120px; top:1003px; width:40px; height:13px; z-index:15"><span class="style7"><%=Sys_IllegalDate(0)%></span></div>
<div id="Layer23" style="position:absolute; left:170px; top:1003px; width:40px; height:17px; z-index:16"><span class="style7"><%=Sys_IllegalDate(1)%></span></div>
<div id="Layer24" style="position:absolute; left:220px; top:1003px; width:40px; height:16px; z-index:17"><span class="style7"><%=Sys_IllegalDate(2)%></div>
<div id="Layer25" style="position:absolute; left:265px; top:1003px; width:40px; height:16px; z-index:18"><span class="style7"><%=right("00"&Sys_IllegalDate_h,2)%></span></div>
<div id="Layer26" style="position:absolute; left:315px; top:1003px; width:40px; height:13px; z-index:19"><span class="style7"><%=right("00"&Sys_IllegalDate_m,2)%></span></div>
<div id="Layer27" style="position:absolute; left:395px; top:1015px; width:600px; height:31px; z-index:20"><span class="style7"><%
	if left(trim(Sys_Rule1),2)="40" or (int(Sys_Rule1)>4310200 and int(Sys_Rule1)<4310209) or (int(Sys_Rule1)>3310101 and int(Sys_Rule1)<3310111) then
		if trim(Sys_IllegalSpeed)<>"" and trim(Sys_RuleSpeed)<>"" then
			response.write "限速"&Sys_RuleSpeed&"公里、經雷達測速時速"&Sys_IllegalSpeed&"公里、超速"&Sys_IllegalSpeed-Sys_RuleSpeed&"公里"
...
%></span></div>
<div id="Layer28" style="position:absolute; left:115px; top:1030px; width:217px; height:15px; z-index:21"><span class="style3"><%=Sys_ILLEGALADDRESS%></span></div>
<div id="Layer29" style="position:absolute; left:120px; top:1060px; width:34px; height:11px; z-index:22"><span class="style7"><%=Sys_DealLineDate(0)%></span></div>
<div id="Layer30" style="position:absolute; left:210px; top:1060px; width:35px; height:13px; z-index:23"><span class="style7"><%=Sys_DealLineDate(1)%></span></div>
<div id="Layer31" style="position:absolute; left:290px; top:1060px; width:32px; height:15px; z-index:24"><span class="style7"><%=Sys_DealLineDate(2)%></span></div>
...
<div id="Layer36" style="position:absolute; left:500px; top:1180px; width:100px; height:43px; z-index:30"><%
	response.write "<table style=""border-color:#ff0000;border-style:solid;"" border=""1"" cellspacing=0 cellpadding=0>"
	if Session("Unit_ID") <>"0207" then 
		response.write "<tr><td nowrap style=""border-color:#ff0000;border-style:solid;"" align=""center""><span class=""style9"">違規舉發專用章</span><br><span class=""style10"">"&Sys_JobName&"&nbsp;"&Sys_MainChName&"</span></td></tr>"
	end if
	response.write "</table>"
%></div>
...
%></div>
<div id="Layer38" style="position:absolute; left:200px; top:1242px; width:60px; height:10px; z-index:32"><span class="style7"><%=sys_Date(0)%></span></div>
<div id="Layer39" style="position:absolute; left:355px; top:1242px; width:60px; height:13px; z-index:33"><span class="style7"><%=sys_Date(1)%></span></div>
<div id="Layer40" style="position:absolute; left:505px; top:1242px; width:60px; height:11px; z-index:34"><span class="style7"><%=sys_Date(2)%></span></div>
