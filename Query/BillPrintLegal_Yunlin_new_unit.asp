...
Sys_Driver="":Sys_DriverID="":Sys_DriverHomeZip="":Sys_DriverHomeAddress=""
Sys_Rule4=""
...
'smith 雲林的大宗掛號barcode 圖為14碼
'掛號號碼 + 局號 + 郵件種類
sMailNumberBarCode=right("00000000" & trim(Sys_MailNumber),6) &" 640000 "&"17"
sMailNumberImageFileName=right("00000000" & trim(Sys_MailNumber),6)&"640000"&"17"
DelphiASPObj.GenSendstorebillno right("00000000" & trim(Sys_MailNumber),6)&"640000"&"17","128",101,606
if trim(Sys_BillTypeID)="1" then

	DelphiASPObj.GenBillPrintBarCode
	PBillSN(i),Sys_BillNo,Sys_Rule1,Sys_CarNo,Sys_MailNumber,"220073",Sys_DriverHomeZip,right(Sys_DealLineDate(0),2)&Sys_DealLineDate(1)&Sys_DealLineDate(2),Sys_StationID,"台北市交通事件裁決所",0,Sum_Level,0,True,False,Sys_MailDate,640,000,36
...
<%end if%>
<%if trim(Sys_BillTypeID)="1" then%>
<div id="Layer3" style="position:absolute; left:180px; top:<%=385+pagepx%>px; width:202px; height:36px; z-index:5">v</div>
<%else%>
...
<div id="Layer9" style="position:absolute; left:40px; top:<%=435+pagepx%>px; width:202px; height:36px; z-index:5"><%
	if showBarCode then
		...
		response.write "<span class=""style6""><B>　　"&SysUnit&"<br>　　本件須應到案，不得郵繳</B></span>"
	end if
%></div>
...
<div id="Layer12" style="position:absolute; left:125px; top:<%=490+pagepx%>px; width:300px; height:11px; z-index:3"><span class="style7">逕行舉發　<%=Sys_A_Name%><br><%if int(Sys_Rule1)<>4340003 and int(Sys_Rule1)<>5620001 then response.write "附採證照片"%>　<%=Sys_CarColor%></span></div>
<div id="Layer13" style="position:absolute; left:270px; top:<%=490+pagepx%>px; width:28px; height:11px; z-index:3"><%=Sys_Sex%></div>
<div id="Layer14" style="position:absolute; left:380px; top:<%=485+pagepx%>px; width:324px; height:10px; z-index:4"><%
	if showBarCode then
		response.write "*本單可至郵局或委託代收之超商繳納"
	else
		response.write "*本單須至監理機關繳納"
	end if
%></div><%'=Sys_DriverHomeZip&" "&Sys_DriverZipName&Sys_DriverHomeAddress%>
...
<div id="Layer27" style="position:absolute; left:400px; top:<%=580+pagepx%>px; width:610px; height:31px; z-index:20"><span class="style3"><%
	...
	if left(trim(Sys_Rule1),2)="40" or (int(Sys_Rule1)>4310200 and int(Sys_Rule1)<4310209) or (int(Sys_Rule1)>3310101 and int(Sys_Rule1)<3310111) then
		...
		if left(trim(Sys_Rule1),2)="40" then Response.Write "(前方設有警52告示牌)"

		if trim(Sys_Rule1)="3310102" then Response.Write "(前方設有警52告示牌)"
		if trim(Sys_Rule1)="3310104" then Response.Write "(前方設有警52告示牌)"
		if trim(Sys_Rule1)="3310106" then Response.Write "(前方設有警52告示牌)"

		if trim(Sys_Rule1)="4310201" then Response.Write "(前方設有警52告示牌)"
		if trim(Sys_Rule1)="4310202" then Response.Write "(前方設有警52告示牌)"
		if trim(Sys_Rule1)="4310203" then Response.Write "(前方設有警52告示牌)"
	else
...
<div id="Layer34" style="position:absolute; left:485px; top:<%=680+pagepx%>px; width:400px; height:30px; z-index:28"><%
	if showBarCode then	response.write "<img src=""../BarCodeImage/"&Sys_BillNo&"_5.jpg"">"
%></div>
<div id="Layer33" style="position:absolute; left:410px; top:<%=685+pagepx%>px; width:90px; z-index:28"><span class="style3"><%=Sys_STATIONNAME&"<br>"&Sys_StationTel%></span></font></div>
<div id="Layer35" style="position:absolute; left:407px; top:<%=743+pagepx%>px; width:100px; height:49px; z-index:29"><%
	...
	if  billprintuseimage=1 then
		...
		response.write "<tr><td style=""border-color:#ff0000;border-style:solid;"" width=""90"" align=""center"" nowrap><span class=""style1"">雲林縣警察局<br>"&SysUnit&"<br>"&SysUnitTel&"</span></td></tr>"
		
		response.write "</table>"
	end if

	%></div>
<div id="Layer36" style="position:absolute; left:609px; top:<%=773+pagepx%>px; width:200px; height:43px; z-index:30"><%
	...
	'	else
'			response.write "<table style=""border-color:#ff0000;border-style:solid;"" border=""1"" cellspacing=0 cellpadding=0>"
'			response.write "<tr><td style=""border-color:#ff0000;border-style:solid;"" width=100 height=25 align=""center""><span class=""style4"">"&Sys_JobName&"&nbsp;"&Sys_MainChName&"</span></td></tr>"
'			response.write "</table>" 
	'	end if
	''else
	'	response.write "<table style=""border-color:#ff0000;border-style:solid;"" border=""1"" cellspacing=0 cellpadding=0>"
...
<%
	response.write funcCheckFont(Sys_Owner,18,1) & " &nbsp;&nbsp;&nbsp;&nbsp; <span  class=""style5"">( " & right("00000000" & trim(Sys_MailNumber),6) & " )</span><br>"
