...
<div id="Layer45" class="style3" style="position:absolute; left:110px; top:800px; height:12px; z-index:1"><b><%="寄件人："&SysUnit&"<br>地址："&SysAddress%></b></div>

<div id="Layer04" class="style3" style="position:absolute; left:110px; top:840px; z-index:1"><b><%
	Response.Write "收件人："
	if trim(Sys_BillTypeID)="1" then
...
<div id="Layer06" class="style3" style="position:absolute; left:110px; top:870px; width:560px; z-index:5"><b><%
	'--------------------------------------如果是抓 戶籍補正的資料-----------------------------------------------------------------------------------------------------------
			Response.Write "地　址："
			if trim(Sys_BillTypeID)="1" then
...
<div id="Layer07" class="style3" style="position:absolute; left:420px; top:990px; z-index:5"><%=Sys_BatchNumber&"　"&"("&cdbl(i+1)&"/"&cdbl(Ubound(PBillSN)+1)&")&nbsp;"&Sys_DCIRETURNCARTYPE%>
</div>

<div id="Layer05" class="style3" style="position:absolute; left:420px; top:930px; z-index:1"><%
...
if showBarCode then
%>
<div id="Layer1" style="position:absolute; left:45px; top:135px; width:202px; height:36px; z-index:5">Ｖ</div>
<%else%>
<div id="Layer2" style="position:absolute; left:45px; top:170px; width:202px; height:36px; z-index:5">Ｖ</div>
<%end if%>
<%if trim(Sys_BillTypeID)="1" then%>
	<div id="Layer3" style="position:absolute; left:165px; top:140px; width:202px; height:36px; z-index:5">V</div>
<%else%>
<div id="Layer4" style="position:absolute; left:165px; top:155px; width:202px; height:36px; z-index:5">V</div>
<%end if%>
...
<%end if%>>-->
<div id="Layer9" style="position:absolute; left:45px; top:185px; width:233px; height:36px; z-index:5"><%
	if showBarCode then
...
%></div>
<div id="Layer10" style="position:absolute; left:480px; top:185px; width:233px; height:32px; z-index:6"><img src=<%="""../BarCodeImage/"&Sys_BillNo&"_4.jpg"""%>></div>
<div id="Layer12" style="position:absolute; left:100px; top:255px; width:150px; height:11px; z-index:3"><font size=2>逕行舉發　<%=Sys_A_Name%><br>附採證照片　<%=Sys_CarColor%></font></div>
<div id="Layer13" style="position:absolute; left:225px; top:250px; width:28px; height:11px; z-index:3"><%=Sys_Sex%></div>
<div id="Layer14" style="position:absolute; left:355px; top:250px; width:370px; height:10px; z-index:4"><font size=2><%if showBarCode then response.write "*本單可至郵局或委託代收之超商繳納(7-11,全家,萊爾富,OK)"%></font></div>
<div id="Layer15" style="position:absolute; left:250px; top:275px; width:100px; height:10px; z-index:8"><%if trim(Sys_DriverBirth(0))<>"" then response.write Sys_DriverBirth(0)&"年"&Sys_DriverBirth(1)&"月"&Sys_DriverBirth(2)&"日"%></div>
<div id="Layer16" style="position:absolute; left:420px; top:275px; width:106px; height:13px; z-index:9"><%=Sys_DriverID%></div>
<div id="Layer17" style="position:absolute; left:655px; top:275px; width:99px; height:12px; z-index:10"><%=fastring%></div>
<div id="Layer18" style="position:absolute; left:100px; top:305px; width:100px; height:14px; z-index:11"><%=Sys_CarNo%></div>
<div id="Layer19" style="position:absolute; left:260px; top:305px; width:160px; height:20px; z-index:12"><%=Sys_DCIRETURNCARTYPE%></div>
<div id="Layer20" style="position:absolute; left:420px; top:305px; width:200px; height:17px; z-index:13"><%=funcCheckFont(Sys_Owner,20,1)%></div>
<div id="Layer21" style="position:absolute; left:100px; top:335px; width:507px; height:13px; z-index:14"><%=Sys_OwnerZip&" "&Sys_OwnerZipName&funcCheckFont(Sys_OwnerAddress,20,1)%></div>

<div id="Layer22" style="position:absolute; left:100px; top:360px; width:40px; height:13px; z-index:15"><%=Sys_IllegalDate(0)%></div>
<div id="Layer23" style="position:absolute; left:160px; top:360px; width:40px; height:17px; z-index:16"><%=Sys_IllegalDate(1)%></div>
<div id="Layer24" style="position:absolute; left:200px; top:360px; width:40px; height:16px; z-index:17"><%=Sys_IllegalDate(2)%></div>
<div id="Layer25" style="position:absolute; left:250px; top:360px; width:40px; height:16px; z-index:18"><%=right("00"&Sys_IllegalDate_h,2)%></div>
<div id="Layer26" style="position:absolute; left:300px; top:360px; width:40px; height:13px; z-index:19"><%=right("00"&Sys_IllegalDate_m,2)%></div>
<div id="Layer27" style="position:absolute; left:390px; top:360px; width:620px; height:31px; z-index:20"><%
	...
	response.write "</font>"
%></div>
<div id="Layer28" style="position:absolute; left:100px; top:385px; width:267px; height:15px; z-index:21"><%=Sys_ILLEGALADDRESS%></div>
<div id="Layer29" style="position:absolute; left:130px; top:410px; width:50px; height:11px; z-index:22"><%=Sys_DealLineDate(0)%></div>
<div id="Layer30" style="position:absolute; left:210px; top:410px; width:50px; height:13px; z-index:23"><%=Sys_DealLineDate(1)%></div>
<div id="Layer31" style="position:absolute; left:270px; top:410px; width:50px; height:15px; z-index:24"><%=Sys_DealLineDate(2)%></div>
<div id="Layer32" style="position:absolute; left:390px; top:410px; width:400px; height:49px; z-index:29"><%
...
<div id="Layer33" style="position:absolute; left:380px; top:440px; width:400px; height:30px; z-index:28"><%
	if showBarCode then	response.write "<img src=""../BarCodeImage/"&Sys_BillNo&"_5.jpg"">"
%></div>

<div id="Layer34" style="position:absolute; left:630px; top:450px; width:100px; height:30px; z-index:29"><font size=2><%=Sys_STATIONNAME&"<br>"&Sys_StationTel%></font></div>


<div id="Layer35" style="position:absolute; left:400px; top:510px; height:49px; z-index:28"><%
	response.write "<table style=""border-color:#ff0000;border-style:solid; height:50px;"" border=""1"" cellspacing=0 cellpadding=0>"
	response.write "<tr><td style=""border-color:#ff0000;border-style:solid;"" align=""center""><span class=""style10"">保二總隊第三大隊<br>第一中隊</span><br><span class=""style10"">"&SysUnitTel&"</span></td></tr>"
	response.write "</table>"
%></div>
<div id="Layer36" style="position:absolute; left:570px; top:550px; width:400px; height:46px; z-index:30"><%
	response.write "<table style=""border-color:#ff0000;border-style:solid;"" border=""1"" cellspacing=0 cellpadding=0>"
	response.write "<tr><td style=""border-color:#ff0000;border-style:solid;"" width=100 height=25 align=""center""><span class=""style10"">隊長&nbsp;</span><span class=""style10"">劉明彰</span></td></tr>"
	response.write "</table>"
%></div>
<div id="Layer37" style="position:absolute; left:570px; top:500px; width:400px; height:46px; z-index:31"><%
	response.write "<table style=""border-color:#ff0000;border-style:solid;"" width=""100"" border=""1"" cellspacing=0 cellpadding=0>"
	response.write "<tr><td style=""border-color:#ff0000;border-style:solid;"" height=25 align=""center""><span class=""style10"">警員</span><span class=""style10"">"&Sys_ChName&"</span></td></tr>"
	response.write "</table><font size=2>　　"&Sys_BillFillerMemberID&"</font>"
	'Response.Write Sys_BillFillerMemberID
%></div>


<div id="Layer38" style="position:absolute; left:220px; top:595px; width:60px; height:10px; z-index:32"><%=sys_Date(0)%></div>
<div id="Layer39" style="position:absolute; left:400px; top:595px; width:60px; height:13px; z-index:33"><%=sys_Date(1)%></div>
<div id="Layer40" style="position:absolute; left:540px; top:595px; width:60px; height:11px; z-index:34"><%=sys_Date(2)%></div>
<div id="Layer41" style="position:absolute; left:620px; top:595px; width:120px; height:12px; z-index:36">　<%'=Sys_BillFillerMemberID%></div>
<%
'Sys_ImageFileNameA="531080926084712140A.jpg"
'Sys_ImageFileNameB="531080926084714093A.jpg"
'Sys_IisImagePath="../"
if trim(Sys_ImageFileNameA)<>"" then%>
	<div id="Layer09" style="position:absolute; left:20px; top:630px; z-index:5"><%
...
<%if trim(Sys_ImageFileNameB)<>"" and (Sys_Rule1 <>"6020303" and Sys_Rule2<>"6020303") then%>
	<div id="Layer10" style="position:absolute; left:410px; top:630px; z-index:1"><%
		response.write "<img src="""&Sys_IisImagePath&Sys_ImageFileNameB&""" width=""320"" height=""238"">"
	%></DIV>
<%end if%>
</div>
<%
