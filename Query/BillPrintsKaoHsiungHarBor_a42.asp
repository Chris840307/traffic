...
if showBarCode then
%>
	<div id="Layer1" style="position:absolute; font-size:16px; left:63px; top:0px; width:202px; height:36px; z-index:5">Ｖ</div>
<%else%>
	<div id="Layer2" style="position:absolute; font-size:16px; left:63px; top:20px; width:202px; height:36px; z-index:5">Ｖ</div>
<%end if%>
<%if trim(Sys_BillTypeID)="1" then%>
	<div id="Layer3" style="position:absolute; font-size:16px; left:178px; top:10px; width:202px; height:36px; z-index:5">Ｖ</div>
<%else%>
	<div id="Layer4" style="position:absolute; font-size:16px; left:178px; top:20px; width:202px; height:36px; z-index:5">Ｖ</div>
<%end if%>
...
%></div>
<div id="Layer10" style="position:absolute; font-size:16px; left:535px; top:45px; width:233px; height:32px; z-index:6"><img src=<%="""../BarCodeImage/"&Sys_BillNo&"_4.jpg"""%>></div>
<div id="Layer11" style="position:absolute; font-size:16px; left:515px; top:85px; width:300px; height:12px; z-index:7"><%="　　　　　第　　　　　　號"%></div>
<div id="Layer12" style="position:absolute; font-size:16px; left:130px; top:120px; width:250px; height:30px; z-index:3">逕行舉發　<%=Sys_A_Name%><br><%if left(Sys_BillNo,2)<>"NR" then response.write "附採證照片"%>　<%=Sys_CarColor%></div>
<div id="Layer13" style="position:absolute; font-size:16px; left:280px; top:110px; width:28px; height:11px; z-index:3"><%=Sys_Sex%></div>
<div id="Layer14" style="position:absolute; font-size:16px; left:395px; top:115px; width:360px; height:10px; z-index:4"><%if showBarCode then response.write "*本單可至郵局或委託代收之超商繳納，惟手續費需另行支付"%></div>
<div id="Layer15" style="position:absolute; font-size:16px; left:285px; top:130px; width:100px; height:10px; z-index:8"><%if trim(Sys_DriverBirth(0))<>"" then response.write Sys_DriverBirth(0)&"  "&Sys_DriverBirth(1)&"  "&Sys_DriverBirth(2)&"日"%></div>
...
<div id="Layer18" style="position:absolute; font-size:16px; left:130px; top:180px; width:100px; height:14px; z-index:11"><%=Sys_CarNo%></div>
<div id="Layer19" style="position:absolute; font-size:16px; left:325px; top:180px; width:117px; height:20px; z-index:12"><%=Sys_DCIRETURNCARTYPE%></div>
<div id="Layer20" style="position:absolute; font-size:16px; left:570px; top:177px; width:251px; height:17px; z-index:13"><%=onderline(funcCheckFont(Sys_Owner,22,1))%></div>
<div id="Layer21" style="position:absolute; font-size:16px; left:200px; top:206px; width:527px; height:13px; z-index:14"><%=Sys_OwnerZip&"　"&funcCheckFont(Sys_OwnerZipName&Sys_OwnerAddress,22,1)%></div>

<div id="Layer22" style="position:absolute; font-size:16px; left:130px; top:235px; width:50px; height:13px; z-index:15"><%=Sys_IllegalDate(0)%>年</div>
<div id="Layer23" style="position:absolute; font-size:16px; left:175px; top:235px; width:50px; height:17px; z-index:16"><%=Sys_IllegalDate(1)%>月</div>
<div id="Layer24" style="position:absolute; font-size:16px; left:215px; top:235px; width:50px; height:16px; z-index:17"><%=Sys_IllegalDate(2)%>日</div>
<div id="Layer25" style="position:absolute; font-size:16px; left:255px; top:235px; width:50px; height:16px; z-index:18"><%=right("00"&Sys_IllegalDate_h,2)%>時</div>
<div id="Layer26" style="position:absolute; font-size:16px; left:300px; top:235px; width:50px; height:13px; z-index:19"><%=right("00"&Sys_IllegalDate_m,2)%>分</div>
<div id="Layer27" style="position:absolute; font-size:15px; left:445px; top:235px; width:620px; height:31px; z-index:20"><%
...
%></div>
<div id="Layer28" style="position:absolute; font-size:16px; left:130px; top:260px; width:267px; height:15px; z-index:21"><%=Sys_ILLEGALADDRESS%></div>
<div id="Layer29" style="position:absolute; font-size:16px; left:180px; top:280px; width:150px; height:11px; z-index:22"><%=Sys_DealLineDate(0)%>年<%=Sys_DealLineDate(1)%>月<%=Sys_DealLineDate(2)%>日前</div>
<div id="Layer32" style="position:absolute; font-size:16px; left:535px; top:325px; width:400px; height:49px; z-index:29"><%
...
<div id="Layer34" style="position:absolute; font-size:12px; left:440px; top:360px; width:75px; height:30px; z-index:28"><%=Sys_STATIONNAME&"<br>"&Sys_StationTel%></div>

<div id="Layer33" style="position:absolute; font-size:16px; left:510px; top:360px; height:30px; z-index:28"><%
	if showBarCode then	response.write "<img src=""../BarCodeImage/"&Sys_BillNo&"_5.jpg"">"
%></div>

<div id="Layer35" style="position:absolute; font-size:16px; left:445px; top:410px; width:150px; height:49px; z-index:29"><%
	'if instr(SysUnit,"仁武")=0 and instr(SysUnit,"林園")=0 then
		response.write Sys_UnitName&"<br>"&Sys_UnitFillerTel
	'end if
%></div>
<div id="Layer37" style="position:absolute; font-size:16px; left:640px; top:420px; width:200px; height:46px; z-index:31"><%
...
%></div>
<div id="Layer38" style="position:absolute; font-size:16px; left:150px; top:460px; width:250px; height:10px; z-index:32"><%=sys_Date(0)%></div>
<div id="Layer39" style="position:absolute; font-size:16px; left:210px; top:460px; width:60px; height:13px; z-index:33"><%=sys_Date(1)%></div>
<div id="Layer40" style="position:absolute; font-size:16px; left:260px; top:460px; width:60px; height:11px; z-index:34"><%=sys_Date(2)%></div>

<div id="Layer10" style="position:absolute; font-size:16px; left:488px; top:560px; width:233px; height:32px; z-index:6"><img src=<%="""../BarCodeImage/"&Sys_BillNo&"_4.jpg"""%>></div>
<div id="Layer11" style="position:absolute; font-size:16px; left:470px; top:595px; width:300px; height:12px; z-index:7"><%=BillPageUnit%>字第<%="　　　　　　"%>號</div>
