...
for cmtI=0 to Ubound(PBillNo)
	...
	Ctrl=0:MailNumberKind="":AddressKind=""
	ONfadd="":owradd="":Drhadd="":ONfaddID="":owraddID="":DrhaddID=""

	strSQL="select a.OwnerAddress,a.OwnerNotifyAddress,a.DriverHomeAddress,b.OwnerHomeAddress,b.DriverAddress from (select CarNo,nvl(OwnerAddress,' ') OwnerAddress,nvl(OwnerNotifyAddress,' ')OwnerNotifyAddress,nvl(DriverHomeAddress,' ') DriverHomeAddress from BillbaseDCIReturn where CarNo='"&trim(PCarNo(cmtI))&"' and ExchangetypeID='A') a,(select CarNo,OwnerAddress OwnerHomeAddress,DriverAddress from BillBase where ImageFileNameB='"&PBillNo(cmtI)&"') b where a.CarNo=b.CarNo"
	...
	arr_MailNumberKind=split(MailNumberKind,",")
	For j=1 to Ctrl
		if PageCnt<>0 then response.write "<div class=""PageNext""></div>"
		PageCnt=PageCnt+1
		Sys_Address="":strOwnerZip="":Sys_OwnerZip=""
		
		strSQL="select a.CarNo,a.Owner,a.OwnerZipDCI,a.OwnerAddress,a.OwnerNotifyAddress,a.DriverHomeZipDCI,a.DriverHomeAddress,b.OwnerZip,b.OwnerHomeAddress,b.DriverZip,b.DriverAddress from (select CarNo,Owner,OwnerZip OwnerZipDCI,OwnerAddress,OwnerNotifyAddress,DriverHomeZip DriverHomeZipDCI,DriverHomeAddress from BillbaseDCIReturn where CarNo='"&trim(PCarNo(cmtI))&"' and ExchangetypeID='A') a,(select CarNo,OwnerZip,OwnerAddress OwnerHomeAddress,DriverZip,DriverAddress from BillBase where ImageFileNameB='"&PBillNo(cmtI)&"') b where a.CarNo=b.CarNo"
		...
		if Not rsDci.eof then
			...
			If trim(arr_AddressKind(j-1)) = "OwnerAddress" Then
				Sys_OwnerAddress=trim(rsDci("OwnerAddress"))
				Sys_OwnerZip=trim(rsDci("OwnerZipDCI"))

			elseIf trim(arr_AddressKind(j-1)) = "DriverHomeAddress" Then
				Sys_OwnerAddress=trim(rsDci("DriverHomeAddress"))
				Sys_OwnerZip=trim(rsDci("DriverHomeZipDCI"))

			elseIf trim(arr_AddressKind(j-1)) = "OwnerHomeAddress" Then
				Sys_OwnerAddress=trim(rsDci("OwnerHomeAddress"))
				Sys_OwnerZip=trim(rsDci("OwnerZip"))

			elseIf trim(arr_AddressKind(j-1)) = "DriverAddress" Then
				Sys_OwnerAddress=trim(rsDci("DriverAddress"))
				Sys_OwnerZip=trim(rsDci("DriverZip"))

			elseIf trim(arr_AddressKind(j-1)) = "OwnerNotifyAddress" Then
		...
		strSQL="select distinct "&arr_MailNumberKind(j-1)&" MailNumber from StopBillMailHistory where BillNo='"&PBillNo(cmtI)&"'"
...
繳費方式：請持本催繳通知單至「四大超商」統一7-11、全家、萊爾富、OK&nbsp;便利超商全省各門市繳納或受委<br>
　　　　　託國雲公寓大廈管理維護股份有限公司(花蓮市中福路230號，08：00至22：00停車收費時間繳納)，<br>
...
<div id="Layer10" class="style2" style="position:absolute; left:80px; top:770px; z-index:1"><%
	response.write "繳費期限及單位代碼："&Sys_DeallIneDate(0)&Sys_DeallIneDate(1)&Sys_DeallIneDate(2)&"24A"
	response.write "　　&nbsp;&nbsp;"
	DelphiASPObj.CreateBarCode Sys_DeallIneDate(0)&Sys_DeallIneDate(1)&Sys_DeallIneDate(2)&"24A",0,35,160
	response.write "<img src=""..\BarCodeImage\"&Sys_DeallIneDate(0)&Sys_DeallIneDate(1)&Sys_DeallIneDate(2)&"24A.jpg"">"
