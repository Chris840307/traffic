...
Function wordporss(word)
	pro_word="":img_word="":img_word2=""
	min=0:max=0:min2=0:max2=0
	...
	If instr(word,"<") >0 Then
		min2=instr(word,"<")
		max2=instr(word,">")-min+1
		img_word2=mid(word,min2,max2)
		word=replace(word,img_word2,"")
	End if
	
	for y=len(word) to 1 step -1
		If (min>0 and len(word)<min and y=len(word)) Then pro_word=img_word&"<br>"
		If (min2>0 and len(word)<min2 and y=len(word)) Then pro_word=img_word2&"<br>"
		pro_word=pro_word&Mid(word,y,1)
		if y<>1 then
			...
			if (min2>0 and y=min2) or (min2>0 and y=1) then
				If not (min2>0 and len(word)<min2 and y=len(word)) Then pro_word=pro_word&img_word2&"<br>"
			end if
		end if
	next
	wordporss=pro_word
...
Dim strtmp
min=0:max=0:min2=0:max2=0:img_word="":img_word2=""
...
If instr(istr,"<") >0 Then
	min2=instr(istr,"<")
	max2=instr(istr,">")-min2+1
	img_word2=mid(istr,min2,max2)
	istr=replace(istr,img_word2,"")
End if
if trim(istr)<>"" then
	strtmp = Replace(istr, "(", "（") 
...
If min2 >0 Then strtmp=mid(strtmp,1,min2-1)&img_word2&mid(strtmp,min2)
...
for i=0 to Ubound(strBillSN)
	...
	<!--#include virtual="traffic/Query/BillBaseTaiChungCity_Deliver.asp"--><%
Next
%>
<div id=idDiv></div>
<div id=oCodeDiv></div>
