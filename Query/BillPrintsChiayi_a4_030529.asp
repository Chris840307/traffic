...
If Not ifnull(trim(rsfound("DriverHomeAddress"))) Then
...
cssStyle=""
If len(trim(Sys_Owner))>8 and instr(Sys_Owner,"@")=0 Then
...
if trim(Sys_BillTypeID)="1" then
	...
	'DelphiASPObj.GenBillPrintBarCode PBillSN(i),Sys_BillNo,Sys_Rule1,Sys_CarNo,Sys_MailNumber,"600050",Sys_DriverHomeZip,right(Sys_DealLineDate(0),2)&Sys_DealLineDate(1)&Sys_DealLineDate(2),Sys_StationID,"台北市交通事件裁決所",0,Sum_Level,0,True,False,Sys_MailDate,800,263,36
else
	Sys_MailNumber=0
	...
	'DelphiASPObj.GenBillPrintBarCode PBillSN(i),Sys_BillNo,Sys_Rule1,Sys_CarNo,Sys_MailNumber,"600050",Sys_OwnerZip,right(Sys_DealLineDate(0),2)&Sys_DealLineDate(1)&Sys_DealLineDate(2),Sys_StationID,"台北市交通事件裁決所",0,Sum_Level,0,True,False,Sys_MailDate,800,263,36
end if
...
<%end if%>
<%if trim(Sys_BillTypeID)="1" then%>
	<div id="Layer3" style="position:absolute; left:165px; top:5px; width:202px; height:36px; z-index:5">Ｖ</div>
<%else%>
<div id="Layer4" style="position:absolute; left:165px; top:20px; width:202px; height:36px; z-index:5">Ｖ</div>
<%end if%>
...
%></div>
<div id="Layer10" style="position:absolute; left:470px; top:50px; width:233px; height:32px; z-index:6"><img src=<%="""../BarCodeImage/"&Sys_BillNo&"_4.jpg"""%>></div>
...
<div id="Layer14" style="position:absolute; left:390px; top:125px; width:324px; height:10px; z-index:4"><%if showBarCode then response.write "*本單可至郵局或委託代收之超商繳納"%></div>
...
<div id="Layer18" style="position:absolute; left:120px; top:185px; width:100px; height:14px; z-index:11"><%=Sys_CarNo%></div>
<div id="Layer19" style="position:absolute; left:325px; top:185px; height:20px; z-index:12"><%=Sys_DCIRETURNCARTYPE%></div>
<div id="Layer20" style="position:absolute; left:560px; top:180px;<%=cssStyle%> width:251px; height:17px; z-index:13"><%=funcCheckFont(Sys_Owner,22,1)%></div>
...
<div id="Layer22" style="position:absolute; left:110px; top:240px; width:40px; height:13px; z-index:15"><%=Sys_IllegalDate(0)%></div>
<div id="Layer23" style="position:absolute; left:170px; top:240px; width:40px; height:17px; z-index:16"><%=Sys_IllegalDate(1)%></div>
<div id="Layer24" style="position:absolute; left:230px; top:240px; width:40px; height:16px; z-index:17"><%=Sys_IllegalDate(2)%></div>
<div id="Layer25" style="position:absolute; left:290px; top:240px; width:40px; height:16px; z-index:18"><%=right("00"&Sys_IllegalDate_h,2)%></div>
<div id="Layer26" style="position:absolute; left:350px; top:240px; width:40px; height:13px; z-index:19"><%=right("00"&Sys_IllegalDate_m,2)%></div>
<div id="Layer27" class="style3" style="position:absolute; left:440px; top:240px; width:620px; height:31px; z-index:20"><%
	...
	response.write "<br>(期限內自動繳納處新臺幣"&Sys_Level1&"元)"
	...
	if trim(Sys_Rule2)<>"0" then
		...
		response.write "<br>(期限內自動繳納處臺台幣"&Sys_Level2&"元)"
	end if				
...
<div id="Layer29" style="position:absolute; left:140px; top:285px; width:50px; height:11px; z-index:22"><%=Sys_DealLineDate(0)%></div>
<div id="Layer30" style="position:absolute; left:230px; top:285px; width:50px; height:13px; z-index:23"><%=Sys_DealLineDate(1)%></div>
<div id="Layer31" style="position:absolute; left:320px; top:285px; width:50px; height:15px; z-index:24"><%=Sys_DealLineDate(2)%></div>
<div id="Layer32" class="style3" style="position:absolute; left:520px; top:325px; width:400px; height:49px; z-index:29"><%
...
<div id="Layer34" class="style3" style="position:absolute; left:435px; top:365px; width:75px; height:30px; z-index:28"><%=Sys_STATIONNAME&"<br>"&Sys_StationTel%></div>

<div id="Layer33" style="position:absolute; left:495px; top:360px; width:400px; height:30px; z-index:26"><%
	if showBarCode then	response.write "<img src=""../BarCodeImage/"&Sys_BillNo&"_5.jpg"">"
%></div>

<div id="Layer35" class="style5" style="position:absolute; left:455px; top:420px; width:110px; height:49px; z-index:29"><%="嘉義縣警察局<br>交通隊<br>05-3620229"%></div>
...
<div id="Layer37" class="style3" style="position:absolute; left:635px; top:420px; width:200px; height:46px; z-index:31"><%
	response.write "<table style=""border-color:#ff0000;border-style:solid;"" border=""1"" cellspacing=0 cellpadding=0>"
...
%></div>
<div id="Layer38" style="position:absolute; left:130px; top:465px; width:60px; height:10px; z-index:32"><%=sys_Date(0)%></div>
<div id="Layer39" style="position:absolute; left:190px; top:465px; width:60px; height:13px; z-index:33"><%=sys_Date(1)%></div>
<div id="Layer40" style="position:absolute; left:250px; top:465px; width:60px; height:11px; z-index:34"><%=sys_Date(2)%></div>
<div id="Layer41" style="position:absolute; left:300px; top:465px; width:120px; height:12px; z-index:36"><%=Sys_BillFillerMemberID%></div>
...
<%end if%>
<%if trim(Sys_BillTypeID)="1" then%>
	<div id="Layer44" style="position:absolute; left:165px; top:525px; width:202px; height:36px; z-index:5">v</div>
<%else%>
	<div id="Layer45" style="position:absolute; left:165px; top:545px; width:202px; height:36px; z-index:5">v</div>
<%end if%>
...
<div id="Layer51" style="position:absolute; left:470px; top:565px; width:233px; height:32px; z-index:6"><img src=<%="""../BarCodeImage/"&Sys_BillNo&"_4.jpg"""%>></div>
<!--
...
<div id="Layer53" class="style3" style="position:absolute; left:120px; top:645px; width:150px; height:11px; z-index:3">逕行舉發　<%=Sys_A_Name%><br><%if int(Sys_Rule1)<>5620001 then response.write "附採證照片"%>　<%=Sys_CarColor%></div>
...
<div id="Layer59" style="position:absolute; left:120px; top:705px; width:100px; height:14px; z-index:11"><%=Sys_CarNo%></div>
<div id="Layer60" style="position:absolute; left:330px; top:705px; height:20px; z-index:12"><%=Sys_DCIRETURNCARTYPE%></div>
<div id="Layer61" style="position:absolute; left:560px; top:705px;<%=cssStyle%> width:251px; height:17px; z-index:13"><%=funcCheckFont(Sys_Owner,22,1)%></div>
<div id="Layer62" style="position:absolute; left:180px; top:735px; width:507px; height:13px; z-index:14"><%=Sys_OwnerZip&" "&Sys_OwnerZipName&funcCheckFont(Sys_OwnerAddress,22,1)%></div>

<div id="Layer63" style="position:absolute; left:110px; top:765px; width:40px; height:13px; z-index:15"><%=Sys_IllegalDate(0)%></div>
<div id="Layer64" style="position:absolute; left:170px; top:765px; width:40px; height:17px; z-index:16"><%=Sys_IllegalDate(1)%></div>
<div id="Layer65" style="position:absolute; left:230px; top:765px; width:40px; height:16px; z-index:17"><%=Sys_IllegalDate(2)%></div>
...
<div id="Layer67" style="position:absolute; left:350px; top:765px; width:40px; height:13px; z-index:19"><%=right("00"&Sys_IllegalDate_m,2)%></div>
<div id="Layer68" class="style3" style="position:absolute; left:440px; top:765px; width:620px; height:31px; z-index:20"><%
	'response.write "<font size=2>"
	if left(trim(Sys_Rule1),2)="40" or (int(Sys_Rule1)>4310200 and int(Sys_Rule1)<4310209) or (int(Sys_Rule1)>3310101 and int(Sys_Rule1)<3310111) then
		if trim(Sys_IllegalSpeed)<>"" and trim(Sys_RuleSpeed)<>"" then
			response.write "限速"&Sys_RuleSpeed&"公里、經測速時速"&Sys_IllegalSpeed&"公里、超速"&Sys_IllegalSpeed-Sys_RuleSpeed&"公里"
	...
	if trim(Sys_Rule2)<>"0" then
		...
		response.write "<br>(期限內自動繳納處新臺幣"&Sys_Level2&"元)"
	end if				
	'	if trim(Sys_Note)<>"" then response.write "<br>("&Sys_Note&")"
	'response.write "</font>"
%></div>
...
<div id="Layer70" style="position:absolute; left:140px; top:810px; width:50px; height:11px; z-index:22"><%=Sys_DealLineDate(0)%></div>
<div id="Layer71" style="position:absolute; left:230px; top:810px; width:50px; height:13px; z-index:23"><%=Sys_DealLineDate(1)%></div>
<div id="Layer72" style="position:absolute; left:320px; top:810px; width:50px; height:15px; z-index:24"><%=Sys_DealLineDate(2)%></div>
<div id="Layer73" class="style3" style="position:absolute; left:520px; top:850px; width:400px; height:49px; z-index:29"><%
	'Response.Write "　　　　　道 路 交 通 管 理 處 罰 條 例<br>"
	response.write "第"&left(trim(Sys_Rule1),2)&"條"
	if len(trim(Sys_Rule1))>7 then response.write "之"&right(trim(Sys_Rule1),1)
	response.write ""
...
<div id="Layer75" class="style3" style="position:absolute; left:435px; top:890px; width:75px; height:30px; z-index:28"><%=Sys_STATIONNAME&"<br>"&Sys_StationTel%></div>

<div id="Layer74" style="position:absolute; left:495px; top:880px; width:400px; height:30px; z-index:26"><img src=<%="""../BarCodeImage/"&Sys_BillNo&"_5.jpg"""%>></div>

<div id="Layer76" class="style3" style="position:absolute; left:550px; top:940px; width:110px; height:49px; z-index:29"><%
...
<div id="Layer77" class="style3" style="position:absolute; left:645px; top:950px; width:200px; height:46px; z-index:31"><%
	response.write "<table style=""border-color:#ff0000;border-style:solid;"" border=""1"" cellspacing=0 cellpadding=0>"
	response.write "<tr><td style=""border-color:#ff0000;border-style:solid;"" height=25 align=""center""><span class=""style2"">"&Sys_BillJobName&"&nbsp;</span><span class=""style2"">"&Sys_ChName&"</span></td></tr>"
	response.write "</table>　　"&Sys_BillFillerMemberID
%></div>
<div id="Layer78" style="position:absolute; left:130px; top:990px; width:60px; height:10px; z-index:32"><%=sys_Date(0)%></div>
<div id="Layer79" style="position:absolute; left:190px; top:990px; width:60px; height:13px; z-index:33"><%=sys_Date(1)%></div>
<div id="Layer80" style="position:absolute; left:250px; top:990px; width:60px; height:11px; z-index:34"><%=sys_Date(2)%></div>
<div id="Layer81" style="position:absolute; left:300px; top:990px; width:120px; height:12px; z-index:36"><%=Sys_BillFillerMemberID%></div>
</div>
<%
