...
<%end if%>
<%if trim(Sys_BillTypeID)="1" then%>
	<div id="Layer3" style="position:absolute; left:200px; top:5px; width:202px; height:36px; z-index:5">Ｖ</div>
<%else%>
<div id="Layer4" style="position:absolute; left:200px; top:25px; width:202px; height:36px; z-index:5">Ｖ</div>
<%end if%>
...
%></div>
<div id="Layer10" style="position:absolute; left:552px; top:55px; width:233px; height:32px; z-index:6"><img src=<%="""..\BarCodeImage\"&Sys_BillNo&"_4.jpg"""%>></div>
<div id="Layer11" style="position:absolute; left:515px; top:95px; width:230px; height:12px; z-index:7"><font size=1>　<%=BillPageUnit%>交字第<%=Sys_BillNo%>號</font></div>
...
<div id="Layer14" style="position:absolute; left:395px; top:115px; width:324px; height:10px; z-index:4"><font size=2><%if showBarCode then response.write "*本單可至郵局或委託代收之超商繳納"%></font></div>
...
<div id="Layer19" style="position:absolute; left:295px; top:160px; width:117px; height:20px; z-index:12"><%=Sys_DCIRETURNCARTYPE%></div>
<div id="Layer20" style="position:absolute; left:475px; top:160px; width:251px; height:17px; z-index:13"><%=Sys_Owner%></div>
<div id="Layer21" style="position:absolute; left:140px; top:180px; width:507px; height:13px; z-index:14"><%=Sys_OwnerZip&" "&Sys_OwnerZipName&Sys_OwnerAddress%></div>
...
<div id="Layer25" style="position:absolute; left:250px; top:205px; width:40px; height:16px; z-index:18"><%=Sys_IllegalDate_h%>時</div>
<div id="Layer26" style="position:absolute; left:290px; top:205px; width:40px; height:13px; z-index:19"><%=Sys_IllegalDate_m%>分</div>
<div id="Layer27" style="position:absolute; left:435px; top:215px; width:620px; height:31px; z-index:20"><%
...
<div id="Layer29" style="position:absolute; left:130px; top:270px; width:34px; height:11px; z-index:22"><%=Sys_DealLineDate(0)%>年</div>
<div id="Layer30" style="position:absolute; left:170px; top:270px; width:35px; height:13px; z-index:23"><%=Sys_DealLineDate(1)%>月</div>
<div id="Layer31" style="position:absolute; left:210px; top:270px; width:50px; height:15px; z-index:24"><%=Sys_DealLineDate(2)%>日前</div>
<div id="Layer32" style="position:absolute; left:435px; top:275px; width:400px; height:49px; z-index:29"><%
...
<div id="Layer33" style="position:absolute; left:425px; top:315px; width:400px; height:30px; z-index:28"><%
	if showBarCode then	response.write "<img src=""..\BarCodeImage\"&Sys_BillNo&"_5.jpg"">"
%></div>

<div id="Layer34" style="position:absolute; left:665px; top:320px; width:100px; height:30px; z-index:28"><font size=2><%=Sys_STATIONNAME%></font></div>
...
<div id="Layer36" style="position:absolute; left:580px; top:390px; width:160px; height:43px; z-index:30"><%
	...
	response.write "<tr><td style=""border-color:#ff0000;border-style:solid;"" width=63 height=25 align=""center""><span class=""style1"">隊長&nbsp;&nbsp;&nbsp;</span><br><span class=""style2"">張清立</span></td></tr>"
...
%></div>
<div id="Layer38" style="position:absolute; left:175px; top:470px; width:60px; height:10px; z-index:32"><%=sys_Date(0)%>年</div>
<div id="Layer39" style="position:absolute; left:330px; top:470px; width:60px; height:13px; z-index:33"><%=sys_Date(1)%>月</div>
<div id="Layer40" style="position:absolute; left:480px; top:470px; width:60px; height:11px; z-index:34"><%=sys_Date(2)%>日</div>
<div id="Layer41" style="position:absolute; left:580px; top:470px; width:120px; height:12px; z-index:36">填單　<%=Sys_BillFillerMemberID%></div>
...
<%else%>
	<div id="Layer45" style="position:absolute; left:200px; top:550px; width:202px; height:36px; z-index:5">Ｖ</div>
<%end if%>
...
<div id="Layer61" style="position:absolute; left:475px; top:685px; width:251px; height:17px; z-index:13"><%=Sys_Owner%></div>
<div id="Layer62" style="position:absolute; left:130px; top:710px; width:507px; height:13px; z-index:14"><%=Sys_OwnerZipName&Sys_OwnerAddress%></div>
...
<div id="Layer66" style="position:absolute; left:250px; top:735px; width:40px; height:16px; z-index:18"><%=Sys_IllegalDate_h%>時</div>
<div id="Layer67" style="position:absolute; left:290px; top:735px; width:40px; height:13px; z-index:19"><%=Sys_IllegalDate_m%>分</div>
<div id="Layer68" style="position:absolute; left:435px; top:750px; width:620px; height:31px; z-index:20"><%
	response.write "<font size=2>"
	if left(trim(Sys_Rule1),2)="40" or (int(Sys_Rule1)>4310200 and int(Sys_Rule1)<4310209) or (int(Sys_Rule1)>3310101 and int(Sys_Rule1)<3310111) then
		if trim(Sys_IllegalSpeed)<>"" and trim(Sys_RuleSpeed)<>"" then
			response.write "限速"&Sys_RuleSpeed&"公里、經測速時速"&Sys_IllegalSpeed&"公里、超速"&Sys_IllegalSpeed-Sys_RuleSpeed&"公里"
...
<div id="Layer70" style="position:absolute; left:130px; top:800px; width:34px; height:11px; z-index:22"><%=Sys_DealLineDate(0)%>年</div>
<div id="Layer71" style="position:absolute; left:170px; top:800px; width:35px; height:13px; z-index:23"><%=Sys_DealLineDate(1)%>月</div>
<div id="Layer72" style="position:absolute; left:210px; top:800px; width:50px; height:15px; z-index:24"><%=Sys_DealLineDate(2)%>日前</div>
<div id="Layer73" style="position:absolute; left:435px; top:805px; width:400px; height:49px; z-index:29"><%
	response.write "<font size='2'>第"&left(trim(Sys_Rule1),2)&"條"
	if len(trim(Sys_Rule1))>7 then response.write "之"&right(trim(Sys_Rule1),1)
	response.write Mid(trim(Sys_Rule1),3,1)&"項"&Mid(trim(Sys_Rule1),4,2)&"款"&Mid(trim(Sys_Rule1),6,2)&"規定"
	response.write "(期限內自動繳納處新台幣"&Sys_Level1&"元)"
...
<div id="Layer74" style="position:absolute; left:425px; top:845px; width:400px; height:30px; z-index:28"><img src=<%="""..\BarCodeImage\"&Sys_BillNo&"_5.jpg"""%>></div>
<div id="Layer75" style="position:absolute; left:665px; top:850px; width:100px; height:30px; z-index:28"><font size=2><%=Sys_STATIONNAME%></font></div>
...
<div id="Layer78" style="position:absolute; left:175px;top:995px; width:60px; height:10px; ">
<%=sys_Date(0)%>年
</div>
<div id="Layer79" style="position:absolute; left:330px;top:995px;width:60px; height:13px; "><%=sys_Date(1)%>月</div>
<div id="Layer80" style="position:absolute; left:480px;top:995px;width:60px; height:11px; "><%=sys_Date(2)%>日</div>
<div id="Layer81" style="position:absolute; left:580px;top:995px;width:120px; height:12px; ">填單　
<%=Sys_BillFillerMemberID%></div>
</div>
<%next
end if%>
