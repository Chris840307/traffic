...
.style4 {font-family:新細明體; color=0044ff; line-height:12px; font-size: 9px}
...
If sys_City="苗栗縣" Then
	strB="select distinct(a.Batchnumber) as Batchnumber from DciLog a,BillBase f where a.BillSN=f.SN "&_
...
if sys_City="基隆市" then 
	strStation="select distinct(e.DCIReturnStation) from DCILog a,MemberData b,DCIReturnStatus d" &_
		...
		" and a.DCIReturnStatusID in ('Y','S','n','L') and ((((e.DciErrorCarData not in ('1','3','9','a','j','A','H','K','L','T'))) and UseTool<>8) or (d.DCIreturnStatus=1 and UseTool=8))" &_
...
elseif sys_City="台南縣" or sys_City="雲林縣" then 
	strStation="select distinct(e.DCIReturnStation) from DCILog a,MemberData b,DCIReturnStatus d" &_
		...
		" and a.CarNo=e.CarNo and a.ExchangeTypeID=e.ExchangeTypeID" &_
		" and a.DCIReturnStatusID=e.Status and a.ExchangeTypeID='W'" &_
		" and a.DCIReturnStatusID in ('Y','S','n','L') and ((d.DCIreturnStatus=1 and UseTool<>8) or (d.DCIreturnStatus=1 and UseTool=8))" &_
		" and a.BillTypeID='2'" &_
		" and a.ExchangeTypeID=d.DCIActionID(+) and a.DCIReturnStatusID=d.DCIReturn(+)" &_
		" and a.RecordMemberID=b.MemberID(+) and f.RecordStateID=0  "&strwhere&" order by DCIReturnStation"
else
	...
	If Not rsStation.Bof Then
		...
		response.write "查無資料，請確認此批舉發單是逕舉或逕舉手開單!"
	end if
...
<%if (sys_City<>"雲林縣" and sys_City<>"嘉義縣" and sys_City<>"台南市" and sys_City<>"台中市" and sys_City<>"南投縣") or (sys_City="南投縣" and trim(Session("Unit_ID"))="05CB") then %>
<center><font size="3">舉發違反道路交通事件通知單逕行舉發移送清冊</font></center>
...
if sys_City="基隆市" then 
		...
		" and a.DCIReturnStatusID=e.Status and a.ExchangeTypeID='W'" &_
		" and a.DCIReturnStatusID in ('Y','S','n','L') and ((((e.DciErrorCarData not in ('1','3','9','a','j','A','H','K','L','T'))) and UseTool<>8) or (d.DCIreturnStatus=1 and UseTool=8))" &_
		" and e.DCIReturnStation in ('40','41','46','48') and a.BillTypeID='2'" &_
		" and a.ExchangeTypeID=d.DCIActionID(+) and a.DCIReturnStatusID=d.DCIReturn(+)" &_
		" and a.RecordMemberID=b.MemberID(+) and f.RecordStateID=0  "&strwhere
elseif sys_City="台南市" then 
		strCntReport="select count(*) as cnt from DCILog a,MemberData b,DCIReturnStatus d" &_
...
if sys_City="基隆市" then 
		...
		" and a.DCIReturnStatusID=e.Status and a.ExchangeTypeID='W'" &_
		" and a.DCIReturnStatusID in ('Y','S','n','L') and ((((e.DciErrorCarData not in ('1','3','9','a','j','A','H','K','L','T'))) and UseTool<>8) or (d.DCIreturnStatus=1 and UseTool=8))" &_
		" and e.DCIReturnStation in ('60','61','63','68') and a.BillTypeID='2'" &_
		" and a.ExchangeTypeID=d.DCIActionID(+) and a.DCIReturnStatusID=d.DCIReturn(+)" &_
		" and a.RecordMemberID=b.MemberID(+) and f.RecordStateID=0  "&strwhere
elseif sys_City="台南市" then 
		strCntReport="select count(*) as cnt from DCILog a,MemberData b,DCIReturnStatus d" &_
...
if sys_City="基隆市" then
		strCnt="select count(*) as cnt from DCILog a,MemberData b,DCIReturnStatus d," &_
			...
			" and ((((e.DciErrorCarData not in ('1','3','9','a','j','A','H','K','L','T'))) and UseTool<>8) or (d.DCIreturnStatus=1 and UseTool=8))" &_
...
if sys_City="基隆市" then
		strSQL="select f.SN,f.BillNo,f.BillTypeID,f.CarNo,f.CarSimpleID,f.IllegalDate,f.RecordDate" &_
			...
			",e.DCIErrorIDData,f.TrafficAccidentType,f.IllegalAddress,f.IsVideo" &_
			",d.DCIReturnStatus,a.FileName,a.BatchNumber" &_
			",e.Owner,f.BillUnitID,f.EquipMentID from DCILog a,MemberData b,DCIReturnStatus d," &_
			"BillBaseDCIReturn e,BillBase f where f.SN=a.BillSN and a.BillNo=e.BillNO" &_
			" and e.DCIReturnStation in ('20','21','22','23','24','29') and a.CarNo=e.CarNo" &_
			" and a.ExchangeTypeID=e.ExchangeTypeID and a.DCIReturnStatusID=e.Status" &_
			" and a.ExchangeTypeID='W' and a.DCIReturnStatusID in ('Y','S','n','L')" &_
...
elseif sys_City="台南縣" or sys_City="雲林縣" then
		strSQL="select f.SN,f.BillNo,f.BillTypeID,f.CarNo,f.CarSimpleID,f.IllegalDate,f.RecordDate" &_
			...
			",e.DriverHomeAddress,f.DriverID,f.BillMem1,e.DCICaseInDate,e.DCIErrorCarData" &_
			",e.DCIErrorIDData,f.TrafficAccidentType,f.IllegalAddress,f.IsVideo" &_
			",d.DCIReturnStatus,a.FileName,a.BatchNumber,f.DealLineDate,f.BillFillDate" &_
			",e.Owner,f.BillUnitID,f.EquipMentID from DCILog a,MemberData b,DCIReturnStatus d," &_
			"BillBaseDCIReturn e,BillBase f where f.SN=a.BillSN and a.BillNo=e.BillNO" &_
			" and e.DCIReturnStation in ('20','21','22','23','24','29') and a.CarNo=e.CarNo" &_
			" and a.ExchangeTypeID=e.ExchangeTypeID and a.DCIReturnStatusID=e.Status" &_
...
%>
	...
	%>：<%=Right("000"&year(now)-1911,3)&Right("00"&month(now),2)&Right("00"&day(now),2)%>&nbsp; (本批案件已透過中華電信數據分公司作入案管制)&nbsp; Page <%=fix(PrintSN/PageCount)+1%> of <%=pagecnt%></td>
		</tr>
	</table>
	...
	<table width="710" border="0" cellpadding="0" cellspacing="0">
		...
		<tr>
			...
			<td>駕駛人/車主</td>
			<td>駕籍資料</td>
			<td><span class="style6">全程錄影</span></td>
			<td>車籍資料</td>
			<%if sys_City="雲林縣" then%>
			<td>填單日期</td>
			<%else%>
			...
			elseif trim(rs1("DCIErrorCarData"))<>"" and not isnull(rs1("DCIErrorCarData")) then
				...
				if not rsCD.eof then
					...
					elseif trim(rs1("DCIErrorCarData"))="$" then
						response.write "<span class=""style5""><strong>"&trim(rs1("DCIErrorCarData"))&" "&trim(rsCD("StatusContent"))&"</strong></span>"
					else
...
if sys_City="基隆市" then
		strCnt="select count(*) as cnt from DCILog a,MemberData b,DCIReturnStatus d," &_
			"BillBaseDCIReturn e,BillBase f where f.SN=a.BillSN and a.BillNo=e.BillNO" &_
			" and e.DCIReturnStation in ('40','41','46','48') and a.CarNo=e.CarNo" &_
			" and a.ExchangeTypeID=e.ExchangeTypeID and a.DCIReturnStatusID=e.Status" &_
			" and a.ExchangeTypeID='W' and a.DCIReturnStatusID in ('Y','S','n','L')" &_
			" and ((((e.DciErrorCarData not in ('1','3','9','a','j','A','H','K','L','T'))) and UseTool<>8) or (d.DCIreturnStatus=1 and UseTool=8))" &_
			" and a.BillTypeID='2' and a.ExchangeTypeID=d.DCIActionID(+)" &_
...
if sys_City="基隆市" then
		strCnt="select count(*) as cnt from DCILog a,MemberData b,DCIReturnStatus d," &_
			"BillBaseDCIReturn e,BillBase f where f.SN=a.BillSN and a.BillNo=e.BillNO" &_
			" and e.DCIReturnStation in ('60','61','63','68') and a.CarNo=e.CarNo" &_
			" and a.ExchangeTypeID=e.ExchangeTypeID and a.DCIReturnStatusID=e.Status" &_
			" and a.ExchangeTypeID='W' and a.DCIReturnStatusID in ('Y','S','n','L')" &_
			" and ((((e.DciErrorCarData not in ('1','3','9','a','j','A','H','K','L','T'))) and UseTool<>8) or (d.DCIreturnStatus=1 and UseTool=8))" &_
			" and a.BillTypeID='2' and a.ExchangeTypeID=d.DCIActionID(+)" &_
