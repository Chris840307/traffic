...
End function
	...
	strSQL="select BillTypeID,Billunitid,DealLineDate,BillfillDate,CarNo,BillMem1,Rule1,Billno,Illegaldate from BillBase where RecordStateID=0 and IllegalDate between to_date('"&date1&" 0:0:0"&"','YYYY/MM/DD/HH24/MI/SS') and to_date('"&date2&" 23:59:59"&"','YYYY/MM/DD/HH24/MI/SS')	"
...
<%  
					...
					%>,<%
					'到案地點 DciReturnStation
					response.write GetDataName("DciStationName","Station","DciStationID",GetDataName2(rsfound("Billno"),rsfound("Carno")))
					%>,<%
					'應到案日期 
					...
					'證號
					If trim(rsfound("BillTypeID"))="1" Then 
					    response.write Getdriver(rsfound("Billno"),rsfound("Carno"))
					Else
					    response.write GetOwner(rsfound("Billno"),rsfound("Carno"))
					End If
					%>,<%
					'資料建檔日期 
					...
					%>,<%
					'入案日期 
				    response.write GetCaseInDate(rsfound("Billno"),rsfound("Carno"))
					%>,<%
					'違規法條一 
	...
	strSQL="select Memberstation,BillTypeID,Billunitid,DealLineDate,driverid,BillfillDate,CarNo,BillMem1,Rule1,Billno,Illegaldate,recorddate from passerbase where RecordStateID=0 and IllegalDate between to_date('"&date1&" 0:0:0"&"','YYYY/MM/DD/HH24/MI/SS') and to_date('"&date2&" 23:59:59"&"','YYYY/MM/DD/HH24/MI/SS')	"
	set rsfound=conn.execute(strSQL)
