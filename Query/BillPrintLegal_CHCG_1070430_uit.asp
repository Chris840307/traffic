...
strSql="select distinct BillFillerMemberID,BillMemID1,BillMemID2,DriverSex,DriverBirth,IllegalDate,DealLineDate,IMAGEFILENAME,IMAGEPATHNAME,IMAGEFILENAMEB from BillBase where SN="&PBillSN(i)
...
strSQL="select UnitName,Tel,Address from UnitInfo where UnitID=(select UnitTypeID from UnitInfo where UnitID=(Select UnitID from MemberData where MemberID='"&trim(Sys_RecordMemberID)&"'))"
set Unit=conn.execute(strSQL)
ReSysUnit=Unit("UnitName")
ReSysUnitTel=Unit("Tel")
ReSysAddress=Unit("Address")
Unit.close
...
<div id="Layer45" class="style3" style="position:absolute; left:60px; top:60px; height:12px; z-index:1"><b><%="寄件人：彰化縣警察局"&ReSysUnit%></b></div>
...
<div id="Layer07" class="style3" style="position:absolute; left:60px; top:260px; z-index:5"><%=Sys_BatchNumber&"　"&"("&cdbl(i+1)&"/"&cdbl(Ubound(PBillSN)+1)&")&nbsp;"&Sys_DCIRETURNCARTYPE%>
</div>
...
<%if trim(Sys_ImageFileNameA)<>"" then%>
	<div id="Layer09" style="position:absolute; left:40px; top:70px; z-index:5"><%
...
<%if trim(Sys_ImageFileNameB)<>"" then%>
	<div id="Layer10" style="position:absolute; left:430px; top:70px; z-index:1"><%
		response.write "<img src="""&Sys_IisImagePath&Sys_ImageFileNameB&""" width=""300"" height=""238"">"
	%></DIV>
<%end if%>

<%if showBarCode then%>
<div id="Layer1" style="position:absolute; left:50px; top:380px; width:202px; height:36px; z-index:5">Ｖ</div>
<%else%>
<div id="Layer2" style="position:absolute; left:50px; top:415px; width:202px; height:36px; z-index:5">Ｖ</div>
<%end if%>
<%if trim(Sys_BillTypeID)="1" then%>
<div id="Layer3" style="position:absolute; left:175px; top:375px; width:202px; height:36px; z-index:5">v</div>
<%else%>
<div id="Layer4" style="position:absolute; left:175px; top:395px; width:202px; height:36px; z-index:5">v</div>
<%end if%>

<div id="Layer9" style="position:absolute; left:40px; top:435px; width:202px; height:36px; z-index:5"><%
	if showBarCode then
...
%></div>
<div id="Layer10" style="position:absolute; left:490px; top:435px; width:233px; height:32px; z-index:3"><img src=<%="""../BarCodeImage/"&Sys_BillNo&"_4.jpg"""%>></div>

<div id="Layer12" style="position:absolute; left:120px; top:520px; width:250px; height:11px; z-index:6"><span class="style7">逕行舉發　<%=Sys_A_Name%><br><%if int(Sys_Rule1)<>5620001 then response.write "附採證照片"%>　<%=Sys_CarColor%></span></div>

<div id="Layer13" style="position:absolute; left:260px; top:515px; width:28px; height:11px; z-index:3"><%=Sys_Sex%></div>
<div id="Layer14" style="position:absolute; left:370px; top:520px; width:324px; height:10px; z-index:4"><%if showBarCode then response.write "*本單可至郵局或委託代收之超商繳納"%></div>

<div id="Layer15" style="position:absolute; left:260px; top:525px; width:100px; height:10px; z-index:8"><font size=2><%if trim(Sys_DriverBirth(0))<>"" then response.write Sys_DriverBirth(0)&"年"&right("0"&Sys_DriverBirth(1),2)&"月"&right("0"&Sys_DriverBirth(2),2)&"日"%></font></div>
<div id="Layer16" style="position:absolute; left:425px; top:525px; width:106px; height:13px; z-index:9"><%=Sys_DriverID%></div>
<div id="Layer17" style="position:absolute; left:620px; top:525px; width:99px; height:12px; z-index:10"><%=fastring%></div>
<div id="Layer18" style="position:absolute; left:120px; top:565px; width:100px; height:14px; z-index:11"><%=Sys_CarNo%></div>
<div id="Layer19" style="position:absolute; left:270px; top:565px; width:250px; height:20px; z-index:12"><%=Sys_DCIRETURNCARTYPE%></div>
<div id="Layer20" style="position:absolute; left:510px; top:565px; width:300px; height:17px; z-index:13"><%=funcCheckFont(Sys_Owner,16,1)%></div>
<div id="Layer21" style="position:absolute; left:125px; top:590px; width:610px; height:13px; z-index:14"><%=Sys_OwnerZip&" "&funcCheckFont(Sys_OwnerZipName&Sys_OwnerAddress,16,1)&chkaddress%></div>

<div id="Layer22" style="position:absolute; left:115px; top:615px; width:40px; height:13px; z-index:15"><%=Sys_IllegalDate(0)&"年"%></div>
<div id="Layer23" style="position:absolute; left:165px; top:615px; width:40px; height:17px; z-index:16"><%=Sys_IllegalDate(1)&"月"%></div>
<div id="Layer24" style="position:absolute; left:215px; top:615px; width:40px; height:16px; z-index:17"><%=Sys_IllegalDate(2)&"日"%></div>
<div id="Layer25" style="position:absolute; left:265px; top:615px; width:40px; height:16px; z-index:18"><%=right("00"&Sys_IllegalDate_h,2)&"時"%></div>
<div id="Layer26" style="position:absolute; left:315px; top:615px; width:40px; height:13px; z-index:19"><%=right("00"&Sys_IllegalDate_m,2)&"分"%></div>
<div id="Layer27" style="position:absolute; left:400px; top:620px; width:350px; height:31px; z-index:20"><span class="style3"><%
...
%></span></div>
<div id="Layer28" style="position:absolute; left:120px; top:645px; width:220px; height:15px; z-index:21"><span class="style3"><%=Sys_ILLEGALADDRESS%></span></div>
<div id="Layer29" style="position:absolute; left:120px; top:670px; width:50px; height:11px; z-index:22"><%=Sys_DealLineDate(0)&"年"%></div>
<div id="Layer30" style="position:absolute; left:200px; top:670px; width:35px; height:13px; z-index:23"><%=Sys_DealLineDate(1)&"月"%></div>
<div id="Layer31" style="position:absolute; left:280px; top:670px; width:32px; height:15px; z-index:24"><%=Sys_DealLineDate(2)&"日"%></div>
<div id="Layer32" style="position:absolute; left:405px; top:680px; width:400px; height:49px; z-index:29"><%
...
<div id="Layer34" style="position:absolute; left:390px; top:725px; width:400px; height:30px; z-index:28"><%
	if showBarCode then	response.write "<img src=""../BarCodeImage/"&Sys_BillNo&"_5.jpg"">"
%></div>
<div id="Layer33" style="position:absolute; left:635px; top:730px; width:100px; height:40px; z-index:28"><span class="style7"><%=Sys_STATIONNAME&"<br>"&Sys_StationTel%></span></font></div>
<div id="Layer35" style="position:absolute; left:400px; top:780px; width:130px; height:49px; z-index:29"><%

	response.write "<table style=""border-color:#ff0000;border-style:solid;"" border=""1"" cellspacing=0 cellpadding=0>"
	response.write "<tr><td align=""center"" style=""border-color:#ff0000;border-style:solid;"" class=""style14"" nowrap>彰化縣警察局<br>"&SysUnit&"</td></tr>"
	response.write "<tr><td style=""border-color:#ff0000;border-style:solid;"" class=""style14"">TEL &nbsp;"&SysUnitTel&"</td></tr>"
	response.write "</table>"

%></div>

<div id="Layer37" style="position:absolute; left:610px; top:790px; width:200px; height:46px; z-index:31"><%
	if trim(Sys_MemberFilename)<>"" then
		...
		response.write "<tr><td style=""border-color:#ff0000;border-style:solid;"" height=25 align=""center""><span class=""style13"">"&Sys_BillFillerJobName&"</span><span class=""style13"">"&Sys_ChName&"</span></td></tr>"
...
<div id="Layer38" style="position:absolute; left:220px; top:855px; width:60px; height:10px; z-index:32"><%=sys_Date(0)%></div>
<div id="Layer39" style="position:absolute; left:365px; top:855px; width:60px; height:13px; z-index:33"><%=sys_Date(1)%></div> 
<div id="Layer40" style="position:absolute; left:515px; top:855px; width:60px; height:11px; z-index:34"><%=sys_Date(2)%></div>

<div id="Layer44" class="style2" style="position:absolute; left:30px; top:930px; height:12px; z-index:36"><%="請繳回："&ReSysAddress%></div>

<div id="Layer41" class="style4" style="position:absolute; left:160px; top:950px; width:200px; height:12px; z-index:36"><%
	If Sys_MailNumber <> "" and Sys_MailNumber<>0 Then
		Sys_MAILCHKNUMBER
	end if
%></div>

<div id="Layer01" class="style9" style="position:absolute; left:140px; top:980px; width:390px; z-index:3"><%
...
<div id="Layer02" class="style3" style="position:absolute; left:450px; top:1030px; z-index:2"><%
	response.write Sys_BillNo%>
</div>

<div id="Layer03" class="style3" style="position:absolute; left:490px; top:905px; z-index:1"><%
	Response.Write "<img src=""../BarCodeImage/"&Sys_BillNo&"_4.jpg"">"%>
</div>
