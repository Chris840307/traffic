...
if request("DB_Del")="Del" then
	strSQL="Delete from TDDT_RAREWORD where TD_SN="&request("TD_SN")&" and TD_Process=0"
	conn.execute strSQL
end if
...
if trim(request("DB_State"))="Selt" then
	...
	strSQL="select TD_SN,TD_RecordDate,TD_RecordCity,TD_CARNO,TD_OwnerName,Decode(TD_PROCESS,'0','回報','1','DCI處理中','處理完成') TD_ProcessName,TD_PROCESS from TDDT_RAREWORD"&strWhere&" order by TD_RecordCity,TD_RecordDate DESC"
...
function funAdd(){
	newWin("","inputWin",800,150,50,10,"yes","yes","yes","no");
