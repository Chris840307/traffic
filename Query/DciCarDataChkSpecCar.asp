...
if trim(request("kinds"))="Del_NoDci" then
	...
	strToDel="select distinct c.SN,c.CarSimpleID,c.IllegalDate,c.Rule1,c.Rule2,c.RuleVer,c.IllegalAddress,c.RuleSpeed,c.IllegalSpeed,c.RecordStateID,c.RecordDate,e.BillNo,e.CarNo,e.A_Name,e.DCIReturnCarType,e.DCIReturnCarColor,e.DriverHomeZip,e.DriverHomeAddress,e.Owner,e.OwnerAddress,e.OwnerZip,e.Nwner,e.NwnerID,e.NwnerAddress,e.NwnerZip,e.DCIReturnCarStatus,f.Note from DCILog a,MemberData b,BillBase c,DCIReturnStatus d,BillBaseDCIReturn e,SpecCar f where a.BillSN=c.SN and e.ExchangeTypeID='A' and e.Status='S' and c.CarNo=e.CarNo (+) and a.ExchangeTypeID=d.DCIActionID(+) and a.DCIReturnStatusID=d.DCIReturn(+) and a.RecordMemberID=b.MemberID(+) and c.RecordStateID=0 and c.CarNo=f.CarNo and f.RecordStateID=0 "&strwhere&" order by c.RecordDate"
