...
if sys_City="基隆市" then 
	...
	" and a.DciReturnStatusID=e.Status and (((((d.DCIreturnStatus=1 and ((e.DciErrorCarData not in ('1','3','9','a','j','A','H','K','L','T')) or (e.DciErrorCarData='F' and e.rule4='2607')) and f.UseTool<>8) or (d.DCIreturnStatus=1 and f.UseTool=8 and (f.EquipmentID<>'-1' or f.EquipmentID is null))) and f.BillTypeID='2')" &_
...
elseif sys_City="南投縣" then
	if ExchangeTypeFlag="N" then
		...
		" and ((e.ExchangeTypeID='W')) and a.ExchangeTypeID='N' and d.DCIreturnStatus=1" &_
		" and a.RecordMemberID=b.MemberID(+) "&strwhere&" order by e.OwnerZip,g.UserMarkDate"
	else
		strSQL="select a.BillSN,a.BillNo,a.BillTypeID,a.CarNo,a.ExchangeTypeID,f.BillFillDate,f.RecordDate,a.BatchNumber" &_
		" from DCILog a" &_
		",MemberData b,DCIReturnStatus d,BillBaseDciReturn e,BillBase f where a.BillSN=f.SN" &_
...
elseif sys_City="台南縣" or sys_City="高雄縣" or sys_City="高雄市" or sys_City="嘉義市" then
	if ExchangeTypeFlag="N" then
		strSQL="select a.BillSN,a.BillNo,a.BillTypeID,a.CarNo,a.ExchangeTypeID,f.BillFillDate,f.RecordDate,a.BatchNumber" &_
		" from DCILog a" &_
		",MemberData b,DCIReturnStatus d,BillBaseDciReturn e,BillBase f,BillMailHistory g where a.BillSN=f.SN" &_
...
if mailSN>0 then response.write "<div class=""PageNext"">&nbsp;</div>"
	...
	'							set rsZip=nothing
		...
		'收件人姓名
			...
			strList=strList&"<td align=""left"" width=""100""class=""style35"">"&funcCheckFont(GetMailMem,15,1)&"</td>"
		'收件地址
			strList=strList&"<td align=""left"" class=""style35"" width=""300"">"&funcCheckFont(GetMailAddress,15,1)&"</td>"
...
<td>
   ...
   <tr>
    <%if sys_City<>"花蓮縣" and sys_City<>"嘉義市" then%>
