...
<!--#include virtual="traffic/Common/BarCode.asp"-->
<html>
<head>
...
'	strSQL="select SendAddrFlag,HomeAddress from StopCaseSendAddr where CarNo=(select Distinct CarNo from BillBase where CarNo='"&PCarNo(i)&"')"
'	set rsSend=conn.execute(strSQL)
'	If Not rsSend.eof Then SendAddrFlag=trim(rsSend("SendAddrFlag"))
'	rsSend.Close
'
'	If ifnull(SendAddrFlag) or SendAddrFlag=1 Then
	...
	For j=1 to 2
		...
		If j=1 Then
			...
			if Not rsDci.eof then
				...
				chk_homeaddr=trim(rsDci("DriverHomeAddress"))
			...
			rsmail.close
			If ifnull(chk_homeaddr) or chk_homeaddr=Sys_OwnerAddress Then j=3
		elseIf j=2 then
		...
		strSQL="select distinct CarNo,BillUnitID,DeallIneDate,ImageFileNameB from BillBase where ImageFileNameB='"&PBillNo(i)&"'"
		set rsbill=conn.execute(strSQL)
...
<div id="Layer18" class="style4" style="position:absolute; font-size:14px; left:200px; top:230px; z-index:1"><%
	DelphiASPObj.CreateBarCode Sys_MailNumber,128,35,260
...
　　　　　託<%=Sys_Company%>(<%=Sys_CompanyAddress%>，08：00至22：00停車收費時間繳納)，<br>
