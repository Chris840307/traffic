...
If Not ifnull(trim(rsfound("Driver"))) Then
	...
	if Not rsfound.eof then Sys_OwnerAddress=rsfound("DriverHomeAddress")
...
Sys_OwnerAddress=replace(Sys_OwnerAddress," ","　")
...
If Sys_Rule1=5620001 Then
SysUnitLevel3="屏東縣政府公有收費停車管理組"
SysAddressLevel3="屏東市自由路527號"

Sys_UnitName="屏東縣政府<br>公有收費停車管理組"
Sys_UnitTel="08-7329767"
End if
...
if showBarCode then
%>
<div id="Layer1" style="position:absolute; left:0px; top:<%=pagesum+15%>px; width:10px; height:20px; z-index:5">Ｖ</div>
<%else%>
<div id="Layer2" style="position:absolute; left:0px; top:<%=pagesum+40%>px; width:10px; height:20px; z-index:5">Ｖ</div>
<%end if%>
<%if trim(Sys_BillTypeID)="1" then%>
	<div id="Layer3" style="position:absolute; left:120px; top:<%=pagesum+25%>px; width:202px; height:36px; z-index:5">Ｖ</div>
<%else%>
	<div id="Layer4" style="position:absolute; left:120px; top:<%=pagesum+35%>px; width:202px; height:36px; z-index:5">Ｖ</div>
<%end if%>
...
<%end if%>-->
<div id="Layer9" style="position:absolute; left:0px; top:<%=pagesum+70%>px; width:202px; height:36px; z-index:5"><%
	if showBarCode then
...
<div id="Layer10" style="position:absolute; left:450px; top:<%=pagesum+70%>px; width:233px; height:32px; z-index:6"><img src=<%="""../BarCodeImage/"&Sys_BillNo&"_4.jpg"""%>></div>
...
<div id="Layer12" style="position:absolute; left:50px; top:<%=pagesum+130%>px; width:150px; height:11px; z-index:3"><%
	response.write "逕行舉發　"&Sys_A_Name&"<br>"
	if left(trim(Sys_Rule1),2)<>"562" then response.write "附採證照片"
	response.write "　"&Sys_CarColor
%></div>
<div id="Layer13" style="position:absolute; left:210px; top:<%=pagesum+130%>px; width:28px; height:11px; z-index:3"><%=Sys_Sex%></div>
<div id="Layer14" style="position:absolute; left:320px; top:<%=pagesum+130%>px; width:500px; height:10px; z-index:4"><%if showBarCode then response.write "<font size=2>*本單可至郵局或期限內至統一、全家、ok、萊爾富等超商繳納</font>"%></div>
<div id="Layer15" style="position:absolute; left:200px; top:<%=pagesum+160%>px; width:100px; height:10px; z-index:8"><font size=2><%if trim(Sys_DriverBirth(0))<>"" then response.write Sys_DriverBirth(0)&"年"&right("0"&Sys_DriverBirth(1),2)&"月"&right("0"&Sys_DriverBirth(2),2)&"日"%></font></div>
<div id="Layer16" style="position:absolute; left:355px; top:<%=pagesum+160%>px; width:106px; height:13px; z-index:9"><%=Sys_DriverID%></div>
<div id="Layer17" style="position:absolute; left:550px; top:<%=pagesum+160%>px; width:99px; height:12px; z-index:10"><%=fastring%></div>
<div id="Layer18" style="position:absolute; left:50px; top:<%=pagesum+175%>px; width:100px; height:14px; z-index:11"><B><%=Sys_CarNo%></B></div>
<div id="Layer19" style="position:absolute; left:215px; top:<%=pagesum+175%>px; width:117px; height:20px; z-index:12"><%=Sys_DCIRETURNCARTYPE%></div>
<div id="Layer20" style="position:absolute; left:385px; top:<%=pagesum+175%>px; width:251px; height:17px; z-index:13"><%=funcCheckFont(Sys_Owner,20,1)%></div>
<div id="Layer21" style="position:absolute; left:50px; top:<%=pagesum+195%>px; width:507px; height:13px; z-index:14"><%=Sys_OwnerZip&" "&funcCheckFont(Sys_OwnerZipName&Sys_OwnerAddress,20,1)%></div>

<div id="Layer22" style="position:absolute; left:60px; top:<%=pagesum+220%>px; width:40px; height:13px; z-index:15"><%=Sys_IllegalDate(0)%></div>
<div id="Layer23" style="position:absolute; left:110px; top:<%=pagesum+220%>px; width:40px; height:17px; z-index:16"><%=Sys_IllegalDate(1)%></div>
<div id="Layer24" style="position:absolute; left:160px; top:<%=pagesum+220%>px; width:40px; height:16px; z-index:17"><%=Sys_IllegalDate(2)%></div>
<div id="Layer25" style="position:absolute; left:210px; top:<%=pagesum+220%>px; width:40px; height:16px; z-index:18"><%=right("00"&Sys_IllegalDate_h,2)%></div>
<div id="Layer26" style="position:absolute; left:260px; top:<%=pagesum+220%>px; width:40px; height:13px; z-index:19"><%=right("00"&Sys_IllegalDate_m,2)%></div>
<div id="Layer27" style="position:absolute; left:345px; top:<%=pagesum+220%>px; width:600px; height:31px; z-index:20"><%
...
'	if trim(Sys_Note)<>"" then response.write "<br>("&Sys_Note&")"
	response.write "</font>"
%></div>
<div id="Layer28" style="position:absolute; left:50px; top:<%=pagesum+260%>px; width:280px; height:15px; z-index:21"><%=Sys_ILLEGALADDRESS%></div>
<div id="Layer29" style="position:absolute; left:90px; top:<%=pagesum+290%>px; width:34px; height:11px; z-index:22"><%=Sys_DealLineDate(0)%></div>
<div id="Layer30" style="position:absolute; left:170px; top:<%=pagesum+290%>px; width:35px; height:13px; z-index:23"><%=Sys_DealLineDate(1)%></div>
<div id="Layer31" style="position:absolute; left:250px; top:<%=pagesum+290%>px; width:32px; height:15px; z-index:24"><%=Sys_DealLineDate(2)%></div>
<div id="Layer32" style="position:absolute; left:360px; top:<%=pagesum+295%>px; width:400px; height:49px; z-index:29"><%
...
<div id="Layer34" style="position:absolute; left:345px; top:<%=pagesum+335%>px; width:95px; height:30px; z-index:28"><font size=2><%=Sys_STATIONNAME&"<br>"&Sys_StationTel%></font></div>
...
<div id="Layer35" style="position:absolute; left:350px; top:<%=pagesum+405%>px; width:100px; height:49px; z-index:29"><%
...
<div id="Layer37" style="position:absolute; left:645px; top:<%=pagesum+410%>px; width:200px; height:46px; z-index:31"><%=Sys_ChName%></div>
<div id="Layer38" style="position:absolute; left:220px; top:<%=pagesum+485%>px; width:60px; height:10px; z-index:32"><%=sys_Date(0)%></div>
<div id="Layer39" style="position:absolute; left:380px; top:<%=pagesum+485%>px; width:60px; height:13px; z-index:33"><%=sys_Date(1)%></div>
<div id="Layer40" style="position:absolute; left:530px; top:<%=pagesum+485%>px; width:60px; height:11px; z-index:34"><%=sys_Date(2)%></div>
<div id="Layer41" style="position:absolute; left:635px; top:<%=pagesum+485%>px; width:80px; height:12px; z-index:36"><%=Sys_BillFillerMemberID%></div>

<div style="position:absolute; left:60px; top:<%=pagesum+600%>px;">
<table width="645" border="0">
  <tr>
	<th align="left">&nbsp;</th>
    <th align="left" valign="top"><span class="style8">900<br><%=SysUnit&replace(SysUnitLevel3,SysUnit,"")%><br>　<%=SysAddressLevel3%></span></th>
    <td align="left" height="130" valign="top"></td>
  </tr>
  <tr>
	<td align="left" valign="top"></td>
	<%if trim(Sys_BillTypeID)="1" then%>
    <th align="left" colspan="2"><span class="style8"><%=funcCheckFont(Sys_Driver,25,1)%>　台啟</span></th>
	<%elseif trim(Sys_BillTypeID)="2" then%>
	<th align="left"><span class="style8"><%=funcCheckFont(Sys_Owner,25,1)%>　台啟</span></th>
	<%end if%>
	<td align="left" valign="top"></td>
  </tr>
  <tr>
    ...
    <%=Sys_DriverZipName&funcCheckFont(Sys_DriverHomeAddress,25,1)%></span></th>
	<%elseif trim(Sys_BillTypeID)="2" then%>
	<th align="left" valign="top" nowrap><span class="style8"><%=Sys_OwnerZip%><br>
    <%=Sys_OwnerZipName&funcCheckFont(Sys_OwnerAddress,25,1)%></span></th>
	<%end if%>
    <td>&nbsp;</td>
  </tr>
</table>
...
<div id="Layer43" style="position:absolute; left:310px; top:<%=pagesum+1050%>px; width:400px; height:12px; z-index:36"><%=SysUnit%></div>
<div id="Layer44" style="position:absolute; left:155px; top:<%=pagesum+1065%>px; width:350px; height:12px; z-index:36"><%
	if Sys_BillTypeID="1" then
		response.write "<font size=2>"&funcCheckFont(Sys_Driver,20,1)&"</font>"
		response.write "<font size=2><br>"&Sys_DriverHomeZip&"&nbsp;&nbsp;"&Sys_DriverZipName&funcCheckFont(Sys_DriverHomeAddress,20,1)&"</font>"
	else
		response.write "<font size=2>"&funcCheckFont(Sys_Owner,20,1)&"</font>"
		response.write "<font size=2><br>"&Sys_OwnerZipDeliver&"&nbsp;&nbsp;"&Sys_OwnerZipNameDeliver&funcCheckFont(Sys_OwnerAddressDeliver,20,1)&"</font>"
	end if%></div>
<div id="Layer45" style="position:absolute; left:210px; top:<%=pagesum+1115%>px; width:280px; height:12px; z-index:36"><%=Sys_BillNo%></div>
<div id="Layer46" style="position:absolute; left:300px; top:<%=pagesum+1105%>px; width:200px; height:32px; z-index:6"><img src=<%="""../BarCodeImage/"&Sys_BillNo&"_4.jpg"""%>></div>
<div id="Layer47" style="position:absolute; left:65px; top:<%=pagesum+1510%>px; width:500px; height:12px; z-index:36"><%=SysAddress%></div>
</div>
<%
