...
Sys_StoreAndSendFinalMailDate=request("StoreAndSendFinalMailDate")
while Not rs.eof
	strSQL="Update BillMailHistory set StoreAndSendFinalMailDate="&funGetDate(gOutDT(Sys_StoreAndSendFinalMailDate),0)&" where BillSn="&trim(rs("BillSN"))
	conn.execute(strSQL)
	rs.movenext
wend
rs.close
