...
<td align="left" width="45%"><%
	If sys_City="台東縣" And trim(Session("Unit_ID"))="9A00" Then
		response.write "特約戶編號：00216"
	Else
		response.write "特約戶編號：147"
	End If 
%>
</td>
</tr>
...
<td align="left" width="45%"><span class="style2">寄件人名稱：<%
	'寄件人
	if sys_City<>"宜蘭縣" And sys_City<>"高雄市" Then
		If sys_City="台東縣" And trim(Session("Unit_ID"))="9A00" Then
...
'總郵費總額
	if MailMoney<>"" then
		response.write (ArrayCount+1)*MailMoney
	else
		response.write "&nbsp;"
	end if
%></td>
...
'總郵費總額
	if MailMoney<>"" then
		response.write (ArrayCount2+1-16)*MailMoney
	else
		response.write "&nbsp;"
	end if
%></td>
