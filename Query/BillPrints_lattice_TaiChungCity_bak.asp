...
strBil="select distinct BillSN,BillNo,CarNo from DCILOG where BillSN="&request("PBillSN")&" and ExchangetypeID='W'"
...
StrBass="select DciReturnCarColor from BillBaseDCIReturn where EXCHANGETYPEID='W' and CarNo='"&trim(rsbil("CarNo"))&"'"
...
if trim(Sys_BillTypeID)="1" then
	...
	Sys_MailNumber=0
	DelphiASPObj.GenBillPrintBarCode rsbil("BillSN"),Sys_BillNo,Sys_Rule1,Sys_CarNo,Sys_MailNumber,"220073",Sys_DriverHomeZip,Sys_DealLineDate(0)&Sys_DealLineDate(1)&Sys_DealLineDate(2),Sys_StationID,"台北市交通事件裁決所",0,Sum_Level,0,True,False,Sys_MailDate
	...
	Set DelphiASPObj = Server.CreateObject("GenBarCode.BarCodeASP")
	Sys_MailNumber=0
	DelphiASPObj.GenBillPrintBarCode rsbil("BillSN"),Sys_BillNo,Sys_Rule1,Sys_CarNo,Sys_MailNumber,"220073",Sys_OwnerZip,Sys_DealLineDate(0)&Sys_DealLineDate(1)&Sys_DealLineDate(2),Sys_StationID,"台北市交通事件裁決所",0,Sum_Level,0,True,False,Sys_MailDate
end if
...
<div id="Layer20" class="style1" style="position:absolute; left:450px; top:180px; width:251px; height:17px; z-index:13"><%=chstr(Sys_Owner)%></div>
<div id="Layer21" class="style1" style="position:absolute; left:70px; top:205px; width:507px; height:13px; z-index:14"><%=Sys_OwnerZip&" "&chstr(Sys_OwnerZipName&Sys_OwnerAddress)%></div>
...
window.focus();
alert("下列車主姓名不足三個字\n<%=errBillNo%>");
top.frames.factory.printing.header = "";
top.frames.factory.printing.footer = "";
top.frames.factory.printing.portrait = true;
top.frames.factory.printing.leftMargin = 0;
top.frames.factory.printing.topMargin = 0;
top.frames.factory.printing.rightMargin = 0;
top.frames.factory.printing.bottomMargin = 0;
top.frames.factory.printing.Print(false);
setTimeout("DP()",60000);
</script>