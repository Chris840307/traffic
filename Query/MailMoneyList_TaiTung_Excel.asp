...
<tr>
<td align="left" width="45%"><span class="style2"><%
	If sys_City="台東縣" And trim(Session("Unit_ID"))="9A00" Then
		response.write "特約戶編號：000216"
	Else
		response.write "特約戶編號：１７４"
	End If 
%></span><br>
<span class="style2">寄件人名稱：<%
	'寄件人
	If sys_City="台東縣" And trim(Session("Unit_ID"))="9A00" Then
		response.write "臺東縣警察局臺東分局"
	else
...
i=0
	strCntReport="select distinct a.batchnumber from DCILog a,DCIReturnStatus d,BillBaseDCIReturn e,BillBase f" &_
		...
		" and NVL(f.EquiPmentID,1)<>-1 and a.ExchangeTypeID=d.DCIActionID(+) and a.DCIReturnStatusID=d.DCIReturn(+)" &strwhere
...
<td colspan="3"><span class="style3"><%
	...
	strCntReport="select distinct a.batchnumber from DCILog a,DCIReturnStatus d,BillBaseDCIReturn e,BillBase f" &_
		" where a.BillSN=f.Sn and f.RecordStateID=0" &_
		" and ((a.BillNo=e.BillNO) or (a.BillNo is null and e.BillNO is null)) and a.CarNo=e.CarNo" &_
		" and NVL(f.EquiPmentID,1)<>-1 and a.ExchangeTypeID=d.DCIActionID(+) and a.DCIReturnStatusID=d.DCIReturn(+) and a.batchnumber='" &rsCntReport("batchnumber")&"'"

	set rsCntReport2=conn.execute(strCntReport)
	  response.write rsCntReport2("batchnumber")
	rsCntReport2.close
	set rsCntReport2=nothing
...
<td align="center"><%
	...
	strCntReport="select count(*) as cnt from DCILog a,DCIReturnStatus d,BillBaseDCIReturn e,BillBase f" &_
		" where a.BillSN=f.Sn and f.RecordStateID=0" &_
		" and ((a.BillNo=e.BillNO) or (a.BillNo is null and e.BillNO is null)) and a.CarNo=e.CarNo" &_
		" and  f.equipmentid='1' and a.ExchangeTypeID=d.DCIActionID(+) and a.DCIReturnStatusID=d.DCIReturn(+)   and (e.DciErrorCarData not in ('1','3','9','a','j','A','H','K','T')) and a.batchnumber='" &rsCntReport("batchnumber")&"'"

	set rsCntReport3=conn.execute(strCntReport)
	if not rsCntReport3.eof then
		MailCount1=MailCount1+cint(rsCntReport3("cnt"))
		MailCountTotal=MailCountTotal+cint(rsCntReport3("cnt"))
	end if
	rsCntReport3.close
	set rsCntReport3=nothing
...
<td colspan="9" height="55" align="right" valign="top"><span class="style4"><br><br><%=GetChinaWord(4,MailCountTotal*MailMoney)%>&nbsp;
