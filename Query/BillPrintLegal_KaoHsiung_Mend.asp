...
If ifnull(Sys_OwnerAddress) Then
	...
	If not ifnull(Sys_OwnerAddress) Then
		strSQL="Update Billbase set Owner='"&rsfound("Owner")&"',OwnerZip='"&Sys_OwnerZip&"',OwnerAddress='"&Sys_OwnerAddress&"',DriverZip='"&trim(rsfound("DriverZip"))&"',DriverAddress='"&trim(rsfound("DriverAddress"))&"' where sn="&trim(rsbil("BillSN"))
		conn.execute(strSQL)
	end if
end if
...
<div id="Layer49" style="position:absolute; left:500px; top:<%=pagesum+660%>px; width:200px; height:32px; z-index:6"><img src=<%="""../BarCodeImage/"&Sys_BillNo&"_4.jpg"""%>></div>
...
<div id="Layer46" style="position:absolute; left:450px; top:<%=pagesum+1130%>px; z-index:6"><img src=<%="""../BarCodeImage/"&Sys_BillNo&"_4.jpg"""%>></div>
