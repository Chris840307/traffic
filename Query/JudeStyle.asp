	...
	if trim(request("printStyle"))="0" then
		...
		for i=0 to Ubound(PBillSN)
			...
			if ISnull(rscnt("StoreAndSendMailNumber")) then
				strSQL="Update BillMailHistory set MailDate="&funGetDate(date,0)&",StoreAndSendMailNumber=MailNumber_Sn.NextVal where BillSN="&PBillSN(i)
				conn.execute(strSQL)
			end if
			rscnt.close
		next
	...
	<tr>
		<td bgcolor="#CCCCCC">
			<table width="100%" border="0" cellpadding="5" cellspacing="1" bgcolor="#FFFFFF">
				<tr>
					<td align="right" nowrap bgcolor="#FFFF99"><font size=4>送達證書格式</font></td>
					<td>
...
function funPrint(){
	if(myForm.DB_Add.value!=''){
		...
		window.opener.myForm.Sys_JudeAgentSex.value=myForm.hd_JudeAgentSex.value;
		window.opener.funJudesubmit();
	}
}
</script>
<%conn.close%>