...
.style10 {font-family: "@標楷體"; font-size: 10px;line-height:12px}
...
<style type="text/css">
.img1 {position:absolute; left:3px;z-index:-3;top:3px;}
</style>
<![endif]-->
<!--[if !IE]> -->
<style type="text/css">
.img1 {position:absolute; left:-40px;z-index:-3;top:60px;}
</style>
<![endif]-->
...
for gyi=0 to Ubound(strBillSN)
  if trim(strBillSN(gyi))<>"" then
	if sys_City="高雄縣" then
		...
		%><!--#include virtual="traffic/Query/BillBaseHuaLienKaoHsiung_DeliverV.asp"--><%

	elseif sys_City="嘉義市" then
		%><!--#include virtual="traffic/Query/BillBase_DeliverV_ChiayiCity.asp"--><%

	else
		%><!--#include virtual="traffic/Query/BillBaseHuaLien_DeliverV.asp"--><%

	end if
	if cint(gyi+1) mod 2 =0 then
...
<script language="javascript">
	...
	if(navigator.userAgent.search("MSIE 9") > -1||navigator.userAgent.search("MSIE 10") > -1){
