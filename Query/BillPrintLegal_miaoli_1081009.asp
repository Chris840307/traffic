...
Sys_IllegalSpeed="":Sys_RuleSpeed="":Sys_Jurgeday="":Sys_CarAddID=""
...
strSql="select distinct BillFillerMemberID,BillMemID1,BillMemID2,BillMemID3,BillMemID4,DriverSex,DriverBirth,IllegalDate,DealLineDate,IMAGEFILENAME,IMAGEPATHNAME,IMAGEFILENAMEB from BillBase where SN="&PBillSN(i)
...
if Not unit.eof then SysAddress=trim(unit("Address"))
...
if trim(Sys_BillTypeID)="1" then
	...
	PBillSN(i),Sys_BillNo,Sys_Rule1,Sys_CarNo,Sys_MailNumber,"220073",Sys_DriverHomeZip,right(Sys_DealLineDate(0),2)&Sys_DealLineDate(1)&Sys_DealLineDate(2),Sys_StationID,"台北市交通事件裁決所",0,Sum_Level,0,True,False,Sys_MailDate,504,023,36
...
Response.Write Sys_BatchNumber&"　"&"("&cdbl(i+1)&"/"&cdbl(Ubound(PBillSN)+1)&")&nbsp;"&Sys_DCIRETURNCARTYPE
...
<%if showBarCode then%>
<div id="Layer1" style="position:absolute; left:55px; top:295px; width:202px; height:36px; z-index:5">Ｖ</div>
<%else%>
<div id="Layer2" style="position:absolute; left:55px; top:325px; width:202px; height:36px; z-index:5">Ｖ</div>
<%end if%>
<%if trim(Sys_BillTypeID)="1" then%>
<div id="Layer3" style="position:absolute; left:175px; top:275px; width:202px; height:36px; z-index:5">v</div>
<%else%>
<div id="Layer4" style="position:absolute; left:175px; top:315px; width:202px; height:36px; z-index:5">v</div>
<%end if%>
...
<div id="Layer12" style="position:absolute; left:115px; top:445px; width:250px; height:11px; z-index:6"><span class="style7"><%
	If Sys_RecordMemberID = 3552 Then

		Response.Write "停車收費　"&Sys_A_Name&"<br>"
		Response.Write "逾期未繳案件　"&Sys_CarColor
	else 

		Response.Write "逕行舉發　"&Sys_A_Name&"<br>"

		if int(Sys_Rule1)<>5620001 then response.write "附採證照片　"
		Response.Write Sys_CarColor
	End If 
%></span></div>

<div id="Layer13" style="position:absolute; left:280px; top:440px; width:28px; height:11px; z-index:3"><%=Sys_Sex%></div>
<div id="Layer14" style="position:absolute; left:375px; top:440px; width:360px; height:10px; z-index:4"><%if showBarCode then response.write "*本單可至郵局或委托代收超商繳納。"%></div>

<div id="Layer15" style="position:absolute; left:280px; top:450px; width:100px; height:10px; z-index:8"><font size=2><%if trim(Sys_DriverBirth(0))<>"" then response.write Sys_DriverBirth(0)&"年"&right("0"&Sys_DriverBirth(1),2)&"月"&right("0"&Sys_DriverBirth(2),2)&"日"%></font></div>
<div id="Layer16" style="position:absolute; left:445px; top:450px; width:106px; height:13px; z-index:9"><%=Sys_DriverID%></div>
<div id="Layer17" style="position:absolute; left:640px; top:450px; width:99px; height:12px; z-index:10"><%=fastring%></div>
<div id="Layer18" style="position:absolute; left:115px; top:490px; width:100px; height:14px; z-index:11"><%=Sys_CarNo%></div>
<div id="Layer19" style="position:absolute; left:270px; top:490px; width:250px; height:20px; z-index:12"><%=Sys_DCIRETURNCARTYPE%></div>
<div id="Layer20" style="position:absolute; left:510px; top:490px; width:300px; height:17px; z-index:13"><%=funcCheckFont(Sys_Owner,16,1)%></div>
...
<div id="Layer22" style="position:absolute; left:125px; top:535px; width:40px; height:13px; z-index:15"><%=Sys_IllegalDate(0)%></div>
<div id="Layer23" style="position:absolute; left:175px; top:535px; width:40px; height:17px; z-index:16"><%=Sys_IllegalDate(1)%></div>
<div id="Layer24" style="position:absolute; left:225px; top:535px; width:40px; height:16px; z-index:17"><%=Sys_IllegalDate(2)%></div>
<div id="Layer25" style="position:absolute; left:275px; top:535px; width:40px; height:16px; z-index:18"><%=right("00"&Sys_IllegalDate_h,2)%></div>
<div id="Layer26" style="position:absolute; left:325px; top:535px; width:40px; height:13px; z-index:19"><%=right("00"&Sys_IllegalDate_m,2)%></div>
<div id="Layer27" style="position:absolute; left:400px; top:540px; width:350px; height:31px; z-index:20"><span class="style3"><%

	If ifnull(Sys_Jurgeday(0)) Then
		Response.Write "民眾檢舉案件，檢舉日期 "&Sys_Jurgeday(0)&"年"&Sys_Jurgeday(1)&"月"&Sys_Jurgeday(2)&"日<br>"
	end if
...
<div id="Layer29" style="position:absolute; left:140px; top:590px; width:50px; height:11px; z-index:22"><%=Sys_DealLineDate(0)%></div>
<div id="Layer30" style="position:absolute; left:220px; top:590px; width:35px; height:13px; z-index:23"><%=Sys_DealLineDate(1)%></div>
<div id="Layer31" style="position:absolute; left:300px; top:590px; width:32px; height:15px; z-index:24"><%=Sys_DealLineDate(2)%></div>
<div id="Layer32" style="position:absolute; left:410px; top:605px; width:400px; height:49px; z-index:29"><%
...
<div id="Layer34" style="position:absolute; left:400px; top:635px; width:400px; height:30px; z-index:28"><%
	if showBarCode then	response.write "<img src=""../BarCodeImage/"&Sys_BillNo&"_5.jpg"">"
%></div>
<div id="Layer33" style="position:absolute; left:655px; top:645px; width:100px; height:40px; z-index:28"><span class="style7"><%=Sys_STATIONNAME&"<br>"&Sys_StationTel%></span></font></div>
...
<div id="Layer37" style="position:absolute; left:610px; top:705px; width:200px; height:46px; z-index:31"><%
	if trim(Sys_MemberFilename)<>"" then
		...
		Response.Write "<td style=""border-color:#ff0000;border-style:solid;border-width:1px;"" height=""25"" align=""center"" nowrap><span class=""style13"">&nbsp;"&Sys_BillJobName&"</span>　<span class=""style13"">"&Sys_ChName&"&nbsp;</span></td>"

		If not ifnull(Sys_BillMemName2) Then
			Response.Write "<td style=""border-color:#ff0000;border-style:solid;border-width:1px;"" height=""25"" align=""center"" nowrap><span class=""style13"">&nbsp;"&Sys_BillJobName2&"</span>&nbsp;<span class=""style13"">"&Sys_BillMemName2&"&nbsp;</span></td>"
		End if
		...
		If not ifnull(Sys_BillMemName3) Then
			...
			Response.Write "<td style=""border-color:#ff0000;border-style:solid;border-width:1px;"" height=""25"" align=""center"" nowrap><span class=""style13"">&nbsp;"&Sys_BillJobName3&"</span>&nbsp;<span class=""style13"">"&Sys_BillMemName3&"&nbsp;</span></td>"

			If not ifnull(Sys_BillMemName4) Then
				Response.Write "<td style=""border-color:#ff0000;border-style:solid;border-width:1px;"" height=""25"" align=""center"" nowrap><span class=""style13"">&nbsp;"&Sys_BillJobName4&"</span>&nbsp;<span class=""style13"">"&Sys_BillMemName4&"&nbsp;</span></td>"
			End if
...
<div id="Layer44" class="style2" style="position:absolute; left:115px; top:840px; height:12px; z-index:36"><%=Sys_MAILCHKNUMBER%></div>

<div id="Layer41" class="style15" style="position:absolute; left:290px; top:850px; width:280px; height:12px; z-index:36"><%="苗栗縣政府警察局送達證書"%></div>

<div id="Layer01" class="style12" style="position:absolute; left:130px; top:890px; width:380px;  z-index:3"><%
...
<div id="Layer03" class="style3" style="position:absolute; left:130px; top:925px; z-index:1"><%
	Response.Write "<img src=""../BarCodeImage/"&Sys_BillNo&"_4.jpg"">"%>
</div>
