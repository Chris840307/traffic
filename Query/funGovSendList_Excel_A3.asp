...
<%
	...
	strStReport="select distinct(e.DCIReturnStation) from (select a.BillNo,a.CarNo from DCILog a,MemberData b,DCIReturnStatus" &_
		...
		" and e.CarNo=a.CarNo and e.ExchangeTypeID=a.ExchangeTypeID" &_
		" and a.RecordMemberID=b.MemberID(+) "&strwhere&") a" &_
		" ,BillBaseDCIReturn e where a.BillNo=e.BillNO and a.CarNo=e.CarNo and e.ExchangeTypeID='W' and e.Status in ('Y','S','n','L')" &_
		" order by DCIReturnStation"
...
<%	
	...
	PrintSN=0
	'逕舉 and 攔停
	strSQL="select a.BillSN,a.BillNO,a.CarNO,e.Owner,f.Rule1,f.Rule2,f.Rule3,e.OwnerZip" &_
...
<%end if%>
			<td align="left">違規單號</td>
			<td align="left">車號</td>
			<td align="left">車主姓名</td>
<%if sys_City="嘉義縣" then %>
			<td align="center">鄉鎮別</td>
<%end if%>
			<td align="left">法條一</td>
<%if sys_City<>"花蓮縣" then %>
			<td align="left">法條二</td>
			<td align="left">法條三</td>
<%else%>
			<td align="left">附件</td>
<%end if%>
			<td align="left">送達書號</td>
			<td align="left">貼條號碼</td>
			<td align="left">退件原因</td>
<%if sys_City<>"嘉義縣" then %>
			<td align="left">單退狀態/送達狀態</td>
<%end if%>
		</tr>
<%
	for i=1 to 35
...
<%end if%>
			...
			if trim(rs1("Owner"))<>"" and not isnull(rs1("Owner")) then
				response.write funcCheckFont(rs1("Owner"),16,1)
			else
				response.write "&nbsp;"
			end if				
			%></td>
...
<%else%>
			<td nowrap><%
			'附件
			if trim(rs1("Rule1"))<>"4340003" and trim(rs1("Rule1"))<>"5620001"  and left(trim(rs1("BillNO")),2) <>"P1"   then
				response.write "舉發單及相片"
			else
				response.write "舉發單"
			end if	
			%></td>
