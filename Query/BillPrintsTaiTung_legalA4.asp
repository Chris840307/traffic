...
if not ifnull(Sys_OwnerZipName) then Sys_OwnerAddress=replace(Sys_OwnerAddress,replace(Sys_OwnerZipName,"台","臺"),"")
...
if Not rsFound.eof then Orz_DriverHomeAddress=trim(rsfound("DriverHomeAddress"))
if Not rsFound.eof then Orz_DriverHomeZip=trim(rsfound("DriverHomeZip"))
chkExchange="" 
If ifnull(Orz_DriverHomeAddress) and trim(Sys_DCIerrorCarData)<>"D" then
	chkExchange="A"
	strSql="select * from BillbaseDCIReturn where CarNo='"&trim(rsbil("CarNo"))&"' and ExchangetypeID='A'"
	set rsdata=conn.execute(strsql)
	If Sys_BillTypeID=1 Then
		if Not rsdata.eof then Orz_Driver=trim(rsdata("Driver"))
	else
		if Not rsdata.eof then Orz_Driver=trim(rsdata("Owner"))
	End if

	if Not rsdata.eof then Orz_DriverID=trim(rsdata("DriverID"))
	if Not rsdata.eof then Orz_DriverHomeAddress=trim(rsdata("DriverHomeAddress"))
	if Not rsdata.eof then Orz_DriverHomeZip=trim(rsdata("DriverHomeZip"))
	rsdata.close
end if

If ifnull(Orz_DriverHomeAddress) or len(Orz_DriverHomeAddress)<4 Then
	chkExchange=""
	if Not rsfound.eof then Orz_Driver=trim(rsfound("Owner"))
	if Not rsfound.eof then Orz_DriverHomeAddress=trim(rsfound("OwnerAddress"))
	if Not rsfound.eof then Orz_DriverHomeZip=trim(rsfound("OwnerZip"))
end if
...
rszip.close
If trim(chkExchange)="A" Then Orz_DriverHomeZipName=""
...
<table width="710" height="160" border="0">
  ...
  <tr height="12">&nbsp;</tr>
  <tr>
    <img src=<%="""../BarCodeImage/"&Sys_BillNo&"_1.jpg"""%> hspace="0" vspace="0" align="left"></img>  
  </tr>
  <tr>
    <%=Sys_FirstBarCode%>
  </tr>
  <tr>
    ...
    <td valign="TOP"><%
			if trim(Sys_BillTypeID)="1" then
				response.write Orz_DriverHomeZip&"<br><br>"
			...
			elseif trim(Sys_BillTypeID)="2" then
				response.write Orz_DriverHomeZip&"<br><br>"
				response.write funcCheckFont(replace(Orz_DriverHomeZipName&Orz_DriverHomeAddress,Orz_DriverHomeZipName&Orz_DriverHomeZipName,Orz_DriverHomeZipName),25,1)
			end if
			response.write "<br><br>"
			if trim(Sys_BillTypeID)="1" then
				response.write Orz_Driver
			elseif trim(Sys_BillTypeID)="2" then
				response.write funcCheckFont(Orz_Driver,25,1)
			end if%>	　台啟
	<br>
	<br>
	<%=Sys_CarNo&"　"&Sys_A_Name&"　"&Sys_CarColor%>
...
<%if showBarCode then%>
<div id="Layer1" style="position:absolute; left:40px; top:585px; width:202px; height:36px; z-index:5">Ｖ</div>
<%else%>
<div id="Layer2" style="position:absolute; left:40px; top:620px; width:202px; height:36px; z-index:5">Ｖ</div>
<%end if%>
<%if trim(Sys_BillTypeID)="1" then%>
<div id="Layer3" style="position:absolute; left:160px; top:590px; width:202px; height:36px; z-index:5">v</div>
<%else%>
<div id="Layer4" style="position:absolute; left:160px; top:605px; width:202px; height:36px; z-index:5">v</div>
<%end if%>
...
<%if trim(Sys_BillTypeID)="1" then%>
	...
	<%end if%>
<%end if%>-->
<div id="Layer9" style="position:absolute; left:10px; top:640px; width:202px; height:36px; z-index:5"><%
	if showBarCode then
...
%></div>
<div id="Layer10" style="position:absolute; left:490px; top:635px; width:233px; height:32px; z-index:6"><img src=<%="""../BarCodeImage/"&Sys_BillNo&"_4.jpg"""%>></div>
<div id="Layer12" style="position:absolute; left:110px; top:695px; width:150px; height:11px; z-index:3"><span class="style7">逕行舉發　<%=Sys_A_Name%><br><%if int(Sys_Rule1)<>5620001 then response.write "附採證照片"%>　<%=Sys_CarColor%></span></div>
...
<div id="Layer18" style="position:absolute; left:125px; top:740px; width:100px; height:14px; z-index:11"><%=Sys_CarNo%></div>
<div id="Layer19" style="position:absolute; left:260px; top:740px; width:117px; height:20px; z-index:12"><%=Sys_DCIRETURNCARTYPE%></div>
<div id="Layer20" style="position:absolute; left:500px; top:740px; width:201px; height:17px; z-index:13"><%=funcCheckFont(Sys_Owner,22,1)%></div>
<div id="Layer21" style="position:absolute; left:125px; top:765px; width:507px; height:13px; z-index:14"><%=Sys_OwnerZip&" "&funcCheckFont(Sys_OwnerZipName&Sys_OwnerAddress,22,1)%></div>

<div id="Layer22" style="position:absolute; left:110px; top:785px; width:40px; height:13px; z-index:15"><%=Sys_IllegalDate(0)%></div>
<div id="Layer23" style="position:absolute; left:160px; top:785px; width:40px; height:17px; z-index:16"><%=Sys_IllegalDate(1)%></div>
<div id="Layer24" style="position:absolute; left:210px; top:785px; width:40px; height:16px; z-index:17"><%=Sys_IllegalDate(2)%></div>
<div id="Layer25" style="position:absolute; left:250px; top:785px; width:40px; height:16px; z-index:18"><%=right("00"&Sys_IllegalDate_h,2)%></div>
<div id="Layer26" style="position:absolute; left:300px; top:785px; width:40px; height:13px; z-index:19"><%=right("00"&Sys_IllegalDate_m,2)%></div>
...
<div id="Layer29" style="position:absolute; left:130px; top:835px; width:34px; height:11px; z-index:22"><%=Sys_DealLineDate(0)%></div>
<div id="Layer30" style="position:absolute; left:210px; top:835px; width:35px; height:13px; z-index:23"><%=Sys_DealLineDate(1)%></div>
<div id="Layer31" style="position:absolute; left:280px; top:835px; width:32px; height:15px; z-index:24"><%=Sys_DealLineDate(2)%></div>
<div id="Layer32" style="position:absolute; left:405px; top:850px; width:400px; height:49px; z-index:29"><%
...
<div id="Layer34" style="position:absolute; left:375px; top:880px; width:400px; height:30px; z-index:28"><%
	if showBarCode then	response.write "<img src=""../BarCodeImage/"&Sys_BillNo&"_5.jpg"">"
%></div>
<div id="Layer33" style="position:absolute; left:625px; top:885px; width:100px; height:40px; z-index:28"><span class="style7"><%=Sys_STATIONNAME&"<br>"&Sys_StationTel%></span></font></div>
<div id="Layer35" style="position:absolute; left:400px; top:940px; width:100px; height:49px; z-index:29"><%
	if billprintuseimage=1 then
		...
		'response.write "<tr><td class=""style1"">申訴電話</td></tr>"
		'response.write "<tr><td class=""style1"">TEL(03)8239164</td></tr>"
...
<div id="Layer37" style="position:absolute; left:590px; top:935px; width:200px; height:46px; z-index:31"><%
...
<div id="Layer41" style="position:absolute; left:670px; top:1020px; width:80px; height:12px; z-index:36"><%=Sys_BillFillerMemberID%></div>
<div id="Layer43" style="position:absolute; left:300px; top:1060px; width:250px; height:12px; z-index:36"><%=Sys_DCIRETURNCARTYPE%></div>
</div>
</div>
