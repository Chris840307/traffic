...
strSQL="select * from "&TrafficType&"UnitInfo where UnitID='"&DB_UnitID&"'"
...
strBil="select distinct BillSN,BillNo,CarNo,DCIerrorCarData,BatchNumber from "&TrafficType&"DCILOG where BillSN="&PBillSN(i)&" and ExchangetypeID='W'"
...
strSql="select * from "&TrafficType&"Billbase where SN="&trim(rsbil("BillSN"))
...
strSQL="select ZipName from "&TrafficType&"Zip where ZipID='"&Sys_DriverHomeZip&"'"
...
strSql="select a.*,b.DriverHomeZip DriverZip,b.DriverHomeAddress DriverAddress from (select * from "&TrafficType&"BillbaseDCIReturn where BillNo='"&trim(rsbil("BillNo"))&"' and CarNo='"&trim(rsbil("CarNo"))&"' and ExchangetypeID='W') a,(select CarNo,DriverHomeZip,DriverHomeAddress from "&TrafficType&"BillbaseDCIReturn where Carno in(select carno from "&TrafficType&"dcilog where BillSN="&trim(rsbil("BillSN"))&" and CarNo='"&trim(rsbil("CarNo"))&"' and ExchangetypeID='A') and ExchangetypeID='A') b where a.carno=b.carno(+)"
...
strSql="select BillNo from "&TrafficType&"BillbaseDCIReturn where BillNo='"&trim(rsbil("BillNo"))&"' and CarNo='"&trim(rsbil("CarNo"))&"' and ExchangetypeID='N'"
...
strSQL="select ZipName from "&TrafficType&"Zip where ZipID='"&Sys_OwnerZip&"'"
set rszip=conn.execute(strSQL)
...
strsql="select * from "&TrafficType&"DCICODE where ID='"&Sys_DCIRETURNCARTYPE&"' and TypeID=5"
Sys_DCIRETURNCARTYPE=""
set cartype=conn.execute(strsql)
if Not cartype.eof then Sys_DCIRETURNCARTYPE=trim(cartype("Content"))
cartype.close

strSQL="select ZipName from "&TrafficType&"Zip where ZipID='"&Sys_OwnerZipDeliver&"'"
set rszip=conn.execute(strSQL)
...
rsfound.close
Sys_Sex=""
strSql="select distinct BillFillerMemberID,DriverSex,DriverBirth,IllegalDate,DealLineDate,IMAGEFILENAME,IMAGEPATHNAME,IMAGEFILENAMEB from "&TrafficType&"BillBase where SN="&PBillSN(i)
...
if Not rssex.eof then
	...
	strSQL="select NVL(count(*),0) cnt from "&TrafficType&"BillUserTake where CarNo='"&Sys_CarNo&"' and IllegalDate between "&funGetDate(tmpIllegalDate1,0)&" and "&funGetDate(tmpIllegalDate2,0)
...
strSql="select a.LoginID,a.ChName,b.UnitName,a.ImageFilename as MemberFileName,b.ImageFilename,b.Tel from "&TrafficType&"MemberData a,"&TrafficType&"UnitInfo b where a.UnitID=b.UnitID and MemberID="&Sys_BillFillerMemberID
...
strSQL="select UnitID,UnitName,UnitTypeID,UnitLevelID from "&TrafficType&"UnitInfo where UnitID=(Select UnitID from "&TrafficType&"MemberData where MemberID='"&trim(Sys_RecordMemberID)&"')"
...
If Sys_UnitLevelID=1 Then
	strSQL="select * from "&TrafficType&"UnitInfo where UnitID='"&Sys_UnitID&"'"
else
	strSQL="select * from "&TrafficType&"UnitInfo where UnitID='"&Sys_UnitTypeID&"'"
end if
...
strSQL="select UnitName,Address,Tel from "&TrafficType&"UnitInfo where UnitID='"&Sys_BillUnitID&"'"
set Unit=conn.execute(strSQL)
...
if trim(Sys_Rule1)<>"0" and not isnull(Sys_Rule1) then
	strRule1="select * from "&TrafficType&"Law where ItemID='"&trim(Sys_Rule1)&"' and VerSion='"&Sys_RuleVer&"'"
...
if trim(Sys_Rule2)<>"0" and not isnull(Sys_Rule2) then
	strRule1="select * from "&TrafficType&"Law where ItemID='"&trim(Sys_Rule2)&"' and VerSion='"&Sys_RuleVer&"'"
	set rsRule1=conn.execute(strRule1)
	if not rsRule1.eof then
...
strSql="select DCISTATIONID,DCISTATIONNAME,StationTel,StationID from "&TrafficType&"Station where DCIStationID='"&Sys_DCIReturnStation&"'"
set rs=conn.execute(strSql)
...
strSql="select LPAD(MailNumber,6,'0') MailNumber,MailTypeID,MailDate from "&TrafficType&"BillMailHistory where BILLSN="&replace(trim(rsbil("BillSN")),"","0")
...
strSql="select b.Content from "&TrafficType&"BILLFASTENERDETAIL a,"&TrafficType&"DCICode b where a.FASTENERTYPEID=b.ID and b.TypeID=6 and a.BIllSN="&replace(trim(rsbil("BillSN")),"","0")&" and a.CarNo='"&trim(rsbil("CarNo"))&"'"
...
StrBass="select a.A_Name from "&TrafficType&"BillBaseDCIReturn a,(select ID,Content from "&TrafficType&"DCICode where TypeID=4) b,(select ID,Content from "&TrafficType&"DCICode where TypeID=10) c,(select ID,Content from "&TrafficType&"DCICode where TypeID=10) d,Station e where a.DciReturnCarColor=b.ID(+) and a.DCIReturnCarStatus=c.ID(+) and a.Rule4=d.ID(+) and a.DCIReturnStation=e.DCIStationID(+) and a.EXCHANGETYPEID='A' and a.CarNo='"&trim(rsbil("CarNo"))&"'"
...
StrBass="select DciReturnCarColor from "&TrafficType&"BillBaseDCIReturn where EXCHANGETYPEID='W' and CarNo='"&trim(rsbil("CarNo"))&"' and BillNo='"&trim(rsbil("BillNo"))&"'"
...
for y=0 to ubound(Sys_CarColorID)
	...
	strColor="select Content from "&TrafficType&"DCICode where TypeID=4 and ID='"&Sys_CarColorID(y)&"'"
...
if trim(Sys_BillTypeID)="1" then
	...
	'response.write "DelphiASPObj.GenBillPrintBarCode"& PBillSN(i)&","&Sys_BillNo&","&Sys_Rule1&","&Sys_CarNo&","&Sys_MailNumber&",220073,"&Sys_DriverHomeZip&","&Sys_DealLineDate(0)&Sys_DealLineDate(1)&Sys_DealLineDate(2)&","&Sys_StationID&",台北市交通事件裁決所,0,"&Sum_Level&",0,True,False,"&Sys_MailDate,800,263,36
	'response.end
else
...
'	response.write "DelphiASPObj.GenBillPrintBarCode"& PBillSN(i)&","&Sys_BillNo&","&Sys_Rule1&","&Sys_CarNo&","&Sys_MailNumber&",220073,"&Sys_OwnerZip&","&Sys_DealLineDate(0)&Sys_DealLineDate(1)&Sys_DealLineDate(2)&","&Sys_StationID&",台北市交通事件裁決所,0,"&Sum_Level&",0,True,False,"&Sys_MailDate&",800,263,36"
'	response.end
end if

strSql="select MAILCHKNUMBER,FirstBarCode from "&TrafficType&"BillMailHistory where BILLSN="&replace(trim(rsbil("BillSN")),"","0")
