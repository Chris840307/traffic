...
for cmtI=0 to Ubound(PBillNo)	
	'smith 20091015 盲目修正
	If cmtI > 9 Then exit for
	if cmtI<>0 then response.write "<div class=""PageNext""></div>"
	Sys_Address=""
	If trim(Request("BillPrintKind"))="1" Then
		strSQL="select CarNo,Owner,DriverHomeAddress,OwnerZip,OwnerAddress,OwnerNotIfyAddress from BillbaseDCIReturn where CarNo='"&trim(PCarNo(cmtI))&"' and ExchangetypeID='A'"
		...
		rsmail.close
	elseIf trim(Request("BillPrintKind"))="2" Then

		strSQL="select CarNo,Owner,DriverHomeZip,DriverHomeAddress from BillbaseDCIReturn where CarNo='"&trim(PCarNo(cmtI))&"' and ExchangetypeID='A'"
		set rsDci=conn.execute(strSQL)
		if Not rsDci.eof then
			Sys_CarNo=trim(rsDci("CarNo"))
			Sys_Owner=trim(rsDci("Owner"))
...
繳費方式：請持本催繳通知單至「四大超商」統一7-11、全家、萊爾富、OK便利超商全省各門市繳納或受委託<br>
　　　　　國雲公寓大廈管理維護股份有限公司(花蓮市中福路230號，08：00至22：00停車收費時間繳納)，並<br>
　　　　　保留本收據聯6個月，如已繳交停車欠費請勿重覆繳費，以維護您的權益。<br>
...
<form method="POST" action="" target="" name="myForm">
	...
	<input type="Hidden" name="BillPrintKind" value="<%=request("BillPrintKind")%>">
</form>
</body>
</html>
...
<script language="javascript">
	function funPrint(){
		window.focus();
		if(confirm("是否要列印催繳單?")){
			UrlStr="StopBillPrints_HuaLien_Main2.asp";
