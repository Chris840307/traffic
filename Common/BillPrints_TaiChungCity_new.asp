...
<table width="645" height="393" border="0">
  ...
  <tr>
    <td height="41" align="left" valign="top">　　　　<font size=0><%=dragcode(haiwaocde(Sys_BarCode1))%></font><br>　　　　<span class="style7"><%=Sys_FirstBarCode%></span>
	</td>
	<%if trim(Sys_BillTypeID)="1" then%>
    ...
    <%if len(Sys_OwnerZipName&Sys_OwnerAddress)<21 then
			response.write Sys_OwnerZipName&Sys_OwnerAddress
		else
			response.write left(Sys_OwnerZipName&Sys_OwnerAddress,20)&"<br>"&mid(Sys_OwnerZipName&Sys_OwnerAddress,21,len(Sys_OwnerZipName&Sys_OwnerAddress))
		end if%></span></td>
	<%end if%>
  </tr>
  <tr>
	...
	<%elseif trim(Sys_BillTypeID)="2" then%>
	<td width="222"><span class="style7"><%=chstr(Sys_Owner)%>　台啟</span></td>
	<%end if%>
...
<div id="Layer9" style="position:absolute; left:30px; top:850px; width:202px; height:36px; z-index:5"><%
	if showBarCode then
		response.write "<center>"&dragcode(haiwaocde(Sys_BarCode3))&"<br>"&Sys_BarCode3&"</center>"
	else
		response.write SysUnit
	end if
%></div>
<div id="Layer10" style="position:absolute; left:500px; top:845px; width:233px; height:32px; z-index:6"><%="<center>"&dragcode(haiwaocde(Sys_BarCode1))&"<br>"&Sys_BarCode1&"</center>"%></div>
...
<div id="Layer20" style="position:absolute; left:515px; top:950px; width:201px; height:17px; z-index:13"><span class="style3"><%=chstr(Sys_Owner)%></span></div>
<div id="Layer21" style="position:absolute; left:130px; top:975px; width:507px; height:13px; z-index:14"><span class="style7"><%=Sys_OwnerZip&" "&chstr(Sys_OwnerZipName&Sys_OwnerAddress)%></span></div>
