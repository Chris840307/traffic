...
if DB_Display="show" then
	if trim(strwhere)<>"" then
		...
		'if sys_City="基隆市" then
			'tempSQL="where (a.BillTypeID='2' and a.ExchangeTypeID=d.DCIActionID(+) and a.DCIReturnStatusID=d.DCIReturn(+) and a.RecordMemberID=b.MemberID(+) and a.BillTypeID=c.ID(+) and a.ExchangeTypeID=e.DCIActionID(+) and a.DCIERRORCARDATA=h.DciReturn(+) and a.DCIERRORIDDATA=g.DciReturn(+) and a.BillSN=f.SN and a.BillNo=f.BillNo and f.RecordStateId <> -1 and d.DCIRETURNSTATUS='1' and a.ExchangeTypeID<>'E' and a.DciErrorCarData Not in ('1','3','9','a','j','A','F','H','K','L','T','V') "&strwhere&" and NVL(f.EquiPmentID,1)<>-1) or (a.BillTypeID='2' and a.ExchangeTypeID=d.DCIActionID(+) and a.DCIReturnStatusID=d.DCIReturn(+) and a.RecordMemberID=b.MemberID(+) and a.BillTypeID=c.ID(+) and a.ExchangeTypeID=e.DCIActionID(+) and a.DCIERRORCARDATA=h.DciReturn(+) and a.DCIERRORIDDATA=g.DciReturn(+) and a.BillSN=f.SN and a.BillNo=f.BillNo and f.RecordStateId <> -1 and d.DCIRETURNSTATUS='1' and a.ExchangeTypeID<>'E' and a.DciErrorCarData in ('1','3','9','a','j','A','F','H','K','L','T','V') and a.BillNo in (select distinct BillNo from BillBaseDCIReturn where EXCHANGETYPEID='W' and Rule4='2607') "&strwhere&" and NVL(f.EquiPmentID,1)<>-1) or (a.BillTypeID='1' and a.ExchangeTypeID=d.DCIActionID(+) and a.DCIReturnStatusID=d.DCIReturn(+) and a.RecordMemberID=b.MemberID(+) and a.BillTypeID=c.ID(+) and a.ExchangeTypeID=e.DCIActionID(+) and a.DCIERRORCARDATA=h.DciReturn(+) and a.DCIERRORIDDATA=g.DciReturn(+) and a.BillSN=f.SN and a.BillNo=f.BillNo and f.RecordStateId <> -1 and d.DCIRETURNSTATUS='1' and a.ExchangeTypeID<>'E' "&strwhere&" and NVL(f.EquiPmentID,1)<>-1)"
		'else
			tempSQL="where (a.BillTypeID='2' and a.ExchangeTypeID=d.DCIActionID(+) and a.DCIReturnStatusID=d.DCIReturn(+) and a.RecordMemberID=b.MemberID(+) and a.BillTypeID=c.ID(+) and a.ExchangeTypeID=e.DCIActionID(+) and a.DCIERRORCARDATA=h.DciReturn(+) and a.DCIERRORIDDATA=g.DciReturn(+) and a.BillSN=f.SN and a.BillNo=f.BillNo and f.RecordStateId <> -1 and d.DCIRETURNSTATUS='1' and a.ExchangeTypeID<>'E' and a.DciErrorCarData Not in ('1','3','9','a','j','A','H','K','L','T','V','n') and a.DciReturnStatusID<>'n' "&strwhere&" and NVL(f.EquiPmentID,1)<>-1) or (a.BillTypeID='1' and a.ExchangeTypeID=d.DCIActionID(+) and a.DCIReturnStatusID=d.DCIReturn(+) and a.RecordMemberID=b.MemberID(+) and a.BillTypeID=c.ID(+) and a.ExchangeTypeID=e.DCIActionID(+) and a.DCIERRORCARDATA=h.DciReturn(+) and a.DCIERRORIDDATA=g.DciReturn(+) and a.BillSN=f.SN and a.BillNo=f.BillNo and f.RecordStateId <> -1 and a.DciReturnStatusID<>'n' and d.DCIRETURNSTATUS='1' and a.ExchangeTypeID<>'E' "&strwhere&" and NVL(f.EquiPmentID,1)<>-1)"
		'end if
		...
		'if sys_City="基隆市" then
			'strCnt="select count(*) as cnt from DCILog a,MemberData b,(select * from DCIcode where TypeID=2) c,DCIReturnStatus d,(select distinct DCIACTIONID,DCIACTIONNAME from DCIReturnStatus) e,BillBase f,(select * from DciReturnStatus where DciActionID='WE') g,(select * from DciReturnStatus where DciActionID='WE') h where a.BillTypeID='2' and a.ExchangeTypeID=d.DCIActionID(+) and a.DCIReturnStatusID=d.DCIReturn(+) and a.RecordMemberID=b.MemberID(+) and a.BillTypeID=c.ID(+) and a.ExchangeTypeID=e.DCIActionID(+) and a.DCIERRORCARDATA=h.DciReturn(+) and a.DCIERRORIDDATA=g.DciReturn(+) and a.BillSN=f.SN and a.BillNo=f.BillNo and a.DciErrorCarData in ('1','3','9','a','j','A','F','H','K','L','T','V') and a.BillNo not in (select distinct BillNo from BillBaseDCIReturn where EXCHANGETYPEID='W' and Rule4='2607') and d.DCIRETURNSTATUS='1'"&strwhere
		'else
			strCnt="select count(*) as cnt from DCILog a,MemberData b,(select * from DCIcode where TypeID=2) c,DCIReturnStatus d,(select distinct DCIACTIONID,DCIACTIONNAME from DCIReturnStatus) e,BillBase f,(select * from DciReturnStatus where DciActionID='WE') g,(select * from DciReturnStatus where DciActionID='WE') h where a.BillTypeID='2' and a.ExchangeTypeID=d.DCIActionID(+) and a.DCIReturnStatusID=d.DCIReturn(+) and a.RecordMemberID=b.MemberID(+) and a.BillTypeID=c.ID(+) and a.ExchangeTypeID=e.DCIActionID(+) and a.DCIERRORCARDATA=h.DciReturn(+) and a.DCIERRORIDDATA=g.DciReturn(+) and a.BillSN=f.SN and a.BillNo=f.BillNo and a.DciErrorCarData in ('1','3','9','a','j','A','H','K','L','T','V') and d.DCIRETURNSTATUS='1'"&strwhere
		'end if
		set Dbrs=conn.execute(strCnt)
...
<table width="100%" border="0">
	<tr height="30">
		<td bgcolor="#FFCC33"><span class="style3">各式清冊/舉發單列印</span><img src="space.gif" width="60" height="1"> <strong>請勿升級 Internet Explorer 7 . 避免套印舉發單出現異常</strong></img></td>
	</tr>
	<tr>
		<td bgcolor="#CCCCCC">
				...
				<tr>
					<td nowrap>
						...
						<Select Name="Selt_BatchNumber" onchange="fnBatchNumber();">
							...
							strSQL="select distinct TO_char(ExchangeDate,'YYYY/MM/DD') ExchangeDate,BatchNumber from DCILog where RecordMemberID="&Session("User_ID")&" and ExchangeDate between TO_DATE('"&DateAdd("d",-1, date)&" 00:00"&"','YYYY/MM/DD/HH24/MI/SS') and TO_DATE('"&date&" 23:59"&"','YYYY/MM/DD/HH24/MI/SS') order by ExchangeDate DESC"
						...
						<img src="space.gif" width="55" height="1"></img><strong>( 查詢 <%=DBsum%> 筆紀錄 , <%=filsuess%>筆成功 , <%=errCatCnt%> 筆無效  ,  <%=fildel%> 筆失敗 , <%=deldata%> 筆刪除  ,  <%=DBsum-filsuess-fildel-deldata-errCatCnt%>筆未處理. )</strong>
						<img src="space.gif" width="6" height="1"><a href="DciCarErrorData.asp" target="_blank">查看逕舉無效原因</a>
						
						<br>
						<img src="space.gif" width="160" height="1"></img><font size="2" >列印舉發單/各式清冊前，請先輸入 批號 或是 舉發單號 進行 查詢</font>
					</td>
				</tr>
			</table>
		</td>
	...
	<tr>
		<td height="35" bgcolor="#FFDD77" align="left">
			...
			<%if sys_City="基隆市" then%>
				...
				<input type="button" name="btnprint" value="列印 違規通知單( Legal 8.5 X 14郵簡式舉發單 )" onclick="funBillNoPrint(0)">
			<%elseif sys_City="台中市" then%>
				<img src="space.gif" width="8" height="1"></img>
				<input type="button" name="btnprint" value="列印 違規通知單( Legal 8.5 X 14郵簡式舉發單 )" onclick="funBillNoPrint(18)">
				<img src="space.gif" width="8" height="1"></img>
				<input type="button" name="btnprint" value="列印 違規通知單(新版測試)" onclick="funBillNoPrint(20)">
			...
			<%if sys_City="澎湖縣" or sys_City="金門縣" or sys_City="嘉義市" then %>
				<img src="space.gif" width="8" height="1"></img>
				<input type="button" name="btnprint" value="列印 違規通知單(  A4 雙色舉發單  ) " onclick="funBillNoPrint(2);">
				<img src="space.gif" width="57" height="1">
			<%elseif sys_City="高雄縣" then%>
				<img src="space.gif" width="8" height="1"></img>
				<input type="button" name="btnprint" value="違規通知單印章套印設定 " onclick="funSealBillNo(17);">
				<img src="space.gif" width="8" height="1"></img>
				<input type="button" name="btnprint" value="列印 違規通知單(  A4 雙色舉發單  ) " onclick="funBillNoPrint(17);">
			...
			<%elseif sys_City="台中市" then%>
				<img src="space.gif" width="8" height="1"></img>
				<input type="button" name="btnprint" value="列印 違規通知單存根聯" onclick="funBillNoPrint(19);">
				<img src="space.gif" width="57" height="1">
			<%end if%>
			...
			<%if sys_City="基隆市" then%>
				<br>
				<img src="space.gif" width="8" height="1">
				<input type="button" name="btnprint" value=" 新版 舉發單封套 保防用( Legal )" onclick="funLabelStyleKeelung();">
			<%elseif sys_City="台中市" then%>
				<br>
				<img src="space.gif" width="8" height="1">
				<input type="button" name="btnprint" value=" 新版 舉發單封套 保防用" onclick="funLabelStyleKeelung();">
			<%else%>
				<br>
				<img src="space.gif" width="8" height="1">
		...
		<%if sys_City="嘉義縣" or sys_City="嘉義市" then%>
			...
			<span class="style3"><img src="space.gif" width="10" height="8"></span>
		<%else%>
			<span class="style3"><img src="space.gif" width="164" height="8"></span>
		<%end if%>
		<%if sys_City="花蓮縣" or sys_City="嘉義縣" or sys_City="嘉義市" then%>
...
function funBillNoPrint(StyleType){
	if (myForm.DB_Display.value==""){
		...
		myForm.printStyle.value=StyleType;
		runServerScript("BillNoPrint.asp?SQLstr=<%=strSQL2%>&printStyle="+StyleType);
		funsubmit();
	}
}
